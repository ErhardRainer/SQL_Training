...
Dieses Skript profiliert eingebaute Beispielwerte gegen mehrere `TRY_CAST`- und `TRY_CONVERT`-Zielprofile. Die Erstversion zeigt, welche Tokens direkt lesbar sind, wo Near Misses entstehen und welche Vorbereinigung fuer Staging- oder Importpfade typischerweise noetig ist.
...
| Feld | Wert |
|---|---|
| Script | [SafeCastProfile.sql](SafeCastProfile.sql) |
| Version | `1.0` |
| Typ | `didactic-lab` |
| Kapitel | `12_DataTypes_Conversion` |
| Sicherheit | `read-only-tempdb` |
| Zweck | Profiliert Demo-Tokens gegen TRY_CAST- und TRY_CONVERT-Zieltypen und markiert Bereinigungsbedarf. |
<!-- SQLDOC:SUMMARY_TABLE:END -->

## Annahmen

- Die Erstversion arbeitet mit Demo-Werten statt mit produktiven Importtabellen.
- `NearMissRows` markieren Werte, die fachlich nahe am Zieltyp liegen, aber vor dem eigentlichen Cast normiert werden sollten.
- Fuer Datumswerte werden ISO (`style 23`) und DACH (`style 104`) bewusst getrennt betrachtet.
- Boolean-Text wie `true` oder `ja` wird als semantisch plausibel, aber nicht als direkt per `TRY_CAST(... AS BIT)` lesbar behandelt.

## Anwendungsfall

Das Skript eignet sich fuer folgende Leitfragen:

- Welche Token koennen ohne Vorverarbeitung direkt in einen Zieltyp uebernommen werden?
- Welche Fehler sind reine Format- oder Kulturthemen und nicht fachlich komplett unlesbar?
- Welche Bereinigungsschritte sollten vor einem produktiven `CAST` oder `CONVERT` standardisiert werden?
...
- `SafeCastProfileSummary` zeigt pro Zielprofil Erfolgs-, Fehler- und Near-Miss-Raten.
- `SafeCastProfileDetail` macht sichtbar, warum ein bestimmter Token fuer einen Zieltyp scheitert.
- `SafeCastActionHints` verdichtet die Failure-Buckets zu konkreten Bereinigungshinweisen.
...
|---|---|---|---|
| `1.0` | `2026-04-18` | `ER` | Erstversion des Safe-Cast-Profils fuer typische Staging-Tokens |
...
```mermaid
flowchart TD
    A[Demo-Werte in #SafeCastSamples anlegen] --> B[RawValue trimmen und Formatindikatoren ableiten]
    B --> C[TRY_CAST und TRY_CONVERT Profile je Zieltyp berechnen]
    C --> D[FailureBucket und RecommendedAction pro Profil ableiten]
    D --> E[Summary mit Erfolgs und Near-Miss-Raten aggregieren]
    D --> F[Fehlerhafte Einzelwerte fuer Detailansicht ausgeben]
    D --> G[Failure-Buckets zu Action Hints verdichten]
```
<!-- SQLDOC:MERMAID:END -->
