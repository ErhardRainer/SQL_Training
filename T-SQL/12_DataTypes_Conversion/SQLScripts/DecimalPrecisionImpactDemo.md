# DecimalPrecisionImpactDemo.sql

Dieses Skript zeigt mit einem kleinen, deterministischen Demo-Datensatz, wie `DECIMAL(p,s)`-Ziele auf dieselben Eingabewerte reagieren. Der Fokus liegt auf drei Fragen: passt der Wert exakt, wird er gerundet oder scheitert die Konvertierung wegen zu kleiner Praezision.
...
| Feld | Wert |
|---|---|
| Script | [DecimalPrecisionImpactDemo.sql](DecimalPrecisionImpactDemo.sql) |
| Version | `1.0` |
| Typ | `didactic-lab` |
| Kapitel | `12_DataTypes_Conversion` |
| Sicherheit | `read-only-tempdb` |
| Zweck | Zeigt die Auswirkungen von DECIMAL-Praezision und -Skala auf Rundung und Overflow. |
<!-- SQLDOC:SUMMARY_TABLE:END -->

## Annahmen

- Das Skript arbeitet bewusst mit Demo-Werten statt mit produktiven Tabellen.
- `TRY_CONVERT()` ist die fachliche Leitfunktion, um Overflow ohne Abbruch sichtbar zu machen.
- Rundung wird hier als Wertveraenderung zwischen Quellwert und erfolgreich konvertiertem Zielwert verstanden.
- Die Zieltypen sind bewusst unterschiedlich eng und breit gewaehlt, damit Integer-Budget und Scale nebeneinander sichtbar werden.
...
|---|---|---|---|
| `@ShowOnlyRiskRows` | `BIT` | Nein | Gibt bei `1` nur Zeilen mit Rundung oder Overflow aus. |
| `@IncludeTargetSummary` | `BIT` | Nein | Gibt bei `1` eine zweite Ergebnismenge mit Zusammenfassung pro Zieltyp aus. |
...
- `IntegerDigitBudget = precision - scale` zeigt, wie viele Stellen vor dem Komma maximal verfuegbar sind.
- `LostScaleDigits` ist eine didaktische Naeherung fuer verlorene Nachkommastellen nach erfolgreicher Konvertierung.
- `DECIMAL(12,4)` dient als Gegenbeispiel zu den kleineren Zielen und zeigt, wie groessere Praezision viele Risiken entschaerft.
- Besonders lehrreich sind Werte wie `12.345`, `1.005` und `12345.67`, weil sie unterschiedliche Problemklassen provozieren.
...
|---|---|---|---|
| `1.0` | `2026-04-18` | `ER` | Erstversion des didaktischen Labs fuer DECIMAL-Praezision, Skala, Rundung und Overflow |
...
```mermaid
flowchart TD
    A[Parameter validieren] --> B[Demo-Werte und Ziel-DECIMAL-Definitionen in Temp-Tabellen laden]
    B --> C[Je Zieltyp dynamisches TRY_CONVERT ueber alle Demo-Werte ausfuehren]
    C --> D[ExactValue und ConvertedValue gegenueberstellen]
    D --> E{Konvertierung erfolgreich?}
    E -->|Nein| F[Status overflow setzen]
    E -->|Ja| G{Rueckkonvertierter Wert weicht ab?}
    G -->|Ja| H[Status rounded und LostScaleDigits markieren]
    G -->|Nein| I[Status exact_fit setzen]
    F --> J[Detailresultset ausgeben]
    H --> J
    I --> J
    J --> K{IncludeTargetSummary = 1?}
    K -->|Ja| L[Zweite Ergebnismenge je Zieltyp aggregieren]
