# DecimalScaleDriftAudit.sql

Dieses Skript betrachtet Dezimalwerte als didaktischen Demo-Datensatz und zeigt, wie unterschiedliche Ziel-Skalen dieselben Eingaben veraendern. Im Mittelpunkt stehen gerundeter Zielwert, numerische Drift, verlorene Nachkommastellen und eine einfache Klassifikation pro Ziel-Skala.
...
| Feld | Wert |
|---|---|
| Script | [DecimalScaleDriftAudit.sql](DecimalScaleDriftAudit.sql) |
| Version | `1.0` |
| Typ | `didactic-lab` |
| Kapitel | `12_DataTypes_Conversion` |
| Sicherheit | `read-only-tempdb` |
| Zweck | Audit fuer Drift bei Dezimalstellen und Skalierung ueber mehrere didaktische Ziel-Skalen. |
<!-- SQLDOC:SUMMARY_TABLE:END -->

## Annahmen

- Das Skript arbeitet mit stabilen Demo-Werten statt mit produktiven Tabellen oder Importschnittstellen.
- Drift wird als Differenz zwischen exaktem Ausgangswert und `ROUND(..., TargetScaleDigits)` definiert.
- `SourceScaleDigits` stammt aus der vorbereiteten Beispieldatenbasis und dient nur der nachvollziehbaren Einordnung.
- Die Ziel-Skalen `0`, `2`, `3` und `4` stehen fuer typische Verdichtungs- und Reporting-Szenarien.
...
|---|---|---|---|
| `@OnlyDriftRows` | `BIT` | Nein | Gibt bei `1` nur Zeilen aus, in denen der Zielwert vom exakten Wert abweicht. |
| `@IncludeScaleSummary` | `BIT` | Nein | Gibt bei `1` eine zweite Ergebnismenge mit Kennzahlen je Ziel-Skala aus. |
...
- `DriftValue` ist positiv bei Aufrundung und negativ bei Abrundung.
- `rounded_to_zero` hebt sehr kleine Werte hervor, die bei grober Ziel-Skala vollstaendig auf `0` kippen.
- `LostScaleDigits` zeigt nur, wie viele Nachkommastellen gegenueber der Ziel-Skala wegfallen koennen; der reale Driftbetrag steht separat.
- Die zweite Ergebnismenge verdichtet das Audit je Ziel-Skala und macht sichtbar, ab welcher Skala die Drift fuer den Demo-Datensatz deutlich sinkt.
...
|---|---|---|---|
| `1.0` | `2026-04-18` | `ER` | Erstversion des didaktischen Audits fuer Drift durch reduzierte DECIMAL-Skalen |
...
```mermaid
flowchart TD
    A[Parameter validieren] --> B[Demo-Dezimalwerte in #Samples laden]
    B --> C[Ziel-Skalen und didaktische Policies in #ScalePolicies laden]
    C --> D[Per CROSS JOIN jede Probe gegen jede Ziel-Skala auditieren]
    D --> E[ROUND auf Ziel-Skala berechnen]
    E --> F[DriftValue, AbsoluteDriftValue und LostScaleDigits ableiten]
    F --> G{Gerundeter Wert veraendert sich?}
    G -->|Nein| H[DriftClass exact_fit setzen]
    G -->|Ja| I{Gerundet auf 0 bei ungleich 0?}
    I -->|Ja| J[DriftClass rounded_to_zero setzen]
    I -->|Nein| K{Absolute Drift < 0.01?}
    K -->|Ja| L[DriftClass minor_drift setzen]
    K -->|Nein| M[DriftClass material_drift setzen]
    H --> N[Detailresultset optional auf Driftzeilen filtern]
    J --> N
    L --> N
    M --> N
    N --> O{IncludeScaleSummary = 1?}
    O -->|Ja| P[Audit je Ziel-Skala aggregieren]
