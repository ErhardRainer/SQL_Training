# UnicodeLengthAudit.sql

Dieses Skript analysiert Textspalten der aktuellen Datenbank und vergleicht pro Spalte Zeichenlaenge, Bytebedarf und Unicode-bezogene Risikoindikatoren. Der Fokus liegt auf `char`/`varchar`/`nchar`/`nvarchar` sowie auf Unterschieden zwischen klassischem non-Unicode, UTF-8-varchar und Unicode-Speicherung in `nchar`/`nvarchar`.
...
| Feld | Wert |
|---|---|
| Script | `UnicodeLengthAudit.sql` |
| Version | `1.0` |
| Typ | `diagnostic-query` |
| Kapitel | `12_DataTypes_Conversion` |
| Sicherheit | `read-only-tempdb` |
| Zweck | Vergleicht Zeichenlaenge, Bytebedarf und Unicode-Risiken fuer Textspalten. |
<!-- SQLDOC:SUMMARY_TABLE:END -->

## Annahmen

Fuer diese Umsetzung gelten folgende fachliche Annahmen:

- Das Skript soll reale Tabellen der aktuellen Datenbank analysieren und nicht nur Demo-Daten zeigen.
- Die Analyse soll optional auf ein Schema oder eine Tabelle eingeschraenkt werden koennen.
- Unter Unicode-Risiken werden hier vor allem drei Dinge verstanden:
  - non-Unicode-Spalten mit Nicht-ASCII-Inhalten
  - UTF-8-varchar mit variablem Bytebedarf
  - `nchar`/`nvarchar` mit Supplementary-Zeichen und erhoehtem Bytebedarf
...
|---|---|---|---|
| `@SchemaName` | `sysname` | Nein | Optionaler Schemafilter; `NULL` analysiert alle Schemas. |
| `@TableName` | `sysname` | Nein | Optionaler Tabellenfilter; `NULL` analysiert alle passenden Tabellen. |
| `@OnlyRiskColumns` | `BIT` | Nein | Gibt bei `1` nur Spalten mit mittleren oder hohen Unicode-Risiken aus. |
...
- `DeclaredLengthArgument` ist der Typ-Parameter `n`; die Einheit steht separat in `DeclaredLengthUnit`.
- `NonAsciiValueCount` ist eine Heuristik auf Basis von Zeichen ausserhalb des ASCII-Bereichs 32 bis 126.
- `varchar` oder `char` mit `_UTF8`-Kollation werden separat bewertet, weil Nicht-ASCII dort nicht automatisch einen klassischen Unicode-Verlust bedeutet.
- Ohne Filter kann das Skript auf grossen Datenbanken spaltenweise Vollscans ausloesen.
...
|---|---|---|---|
| `1.0` | `2026-04-16` | `ER` | Erstversion des Unicode-Length-Audits fuer Textspalten |
...
```mermaid
flowchart TD
    A[Parameter validieren] --> B[Passende Textspalten aus sys.* ermitteln]
    B --> C{Textspalten gefunden?}
    C -->|Nein| D[Fehler ausgeben]
    C -->|Ja| E[Spalten-Metadaten in Temp-Tabelle speichern]
    E --> F[Je Spalte dynamisches Aggregat ueber die Zieltabelle ausfuehren]
    F --> G[LEN-, DATALENGTH- und Nicht-ASCII-Metriken sammeln]
    G --> H[Unicode-Risiko je Spalte klassifizieren]
    H --> I{OnlyRiskColumns = 1?}
    I -->|Ja| J[Nur medium/high ausgeben]
    I -->|Nein| K[Alle analysierten Textspalten ausgeben]
```
<!-- SQLDOC:MERMAID:END -->
