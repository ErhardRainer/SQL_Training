# SearchForeignKeyNames.sql

Dieses Skript durchsucht Fremdschluessel-Metadaten der aktuell verbundenen Datenbank nach Begriffsfragmenten in FK-Namen sowie in Eltern- und Referenztabellen. Es eignet sich fuer Reviews von Beziehungsmodellen, Umbenennungen und schnellen Inventuren vor Refactorings.
...
| Feld | Wert |
|---|---|
| Script | [SearchForeignKeyNames.sql](SearchForeignKeyNames.sql) |
| Version | `1.0` |
| Typ | `diagnostic-query` |
| Kapitel | `15_SearchInTables` |
| Sicherheit | `read-only` |
| Zweck | Findet Fremdschluessel nach Namen, Eltern- oder Referenztabellen und verdichtet die Treffer je Eltern-Tabelle. |
<!-- SQLDOC:SUMMARY_TABLE:END -->

## Einordnung

Das Artefakt ist als diagnostische Erstversion fuer Metadaten-Analysen gedacht. Es arbeitet rein lesend auf Katalogsichten und verbindet Suchbegriffe mit Fremdschluessel-Namen, Elternobjekten, Referenzzielen und dem technischen Status einzelner FK-Beziehungen.

## Annahmen

- Die Erstversion analysiert nur die aktuell verbundene Datenbank ueber `sys.foreign_keys`, `sys.foreign_key_columns`, `sys.tables`, `sys.schemas` und `sys.columns`.
- Die Suche wird auf FK-Namen sowie Eltern- und Referenztabellen angewendet; Spaltennamen innerhalb der Beziehung dienen nur der lesbaren Ausgabe.
- Ohne Eingabe fuer `@SearchTerms` verwendet das Skript ein didaktisches Standardset aus Business-, Parent-/Child- und Governance-Begriffen.
- Treffer auf deaktivierte oder nicht vertrauenswuerdige Fremdschluessel werden bewusst mit ausgegeben, damit Review-Faelle in derselben Sicht erkennbar bleiben.

## Anwendungsfall

Das Skript eignet sich fuer die Suche nach Kunden-, Auftrags-, Adress- oder Mandantenbeziehungen, fuer Aufraeumarbeiten vor Schema-Umbenennungen und fuer Inventuren, bei denen gezielt nach FK-Namenskonventionen oder Referenzzielen gesucht werden soll.
...
|---|---|---|---|
| `@SearchTerms` | `NVARCHAR(MAX)` | Nein | Pipe-separierte Begriffe wie `customer|order|address`; `NULL` verwendet ein didaktisches Standardset. |
| `@SchemaName` | `SYSNAME` | Nein | Optionaler Schemafilter fuer Eltern- und Referenztabellen; `NULL` durchsucht alle Schemas. |
| `@MatchMode` | `NVARCHAR(10)` | Nein | Verwendet `contains` oder `exact` fuer die Suche in FK-Namen und Tabellennamen. |
| `@IncludeDisabled` | `BIT` | Nein | `1` schliesst deaktivierte Fremdschluessel ein, `0` zeigt nur aktive FK-Beziehungen. |
...
- `ForeignKeyMatches` zeigt je Treffer das referenzierte Objekt, die Spaltenabbildung und den Match-Fokus.
- `ForeignKeyCoverageByTable` verdichtet Treffer je Eltern-Tabelle und markiert Tabellen mit deaktivierten oder nicht vertrauenswuerdigen FK-Beziehungen.
- `ForeignKeySearchTermSummary` eignet sich fuer die schnelle Bewertung des Suchsets, weil auch Begriffe ohne Treffer sichtbar bleiben.
- Mit `@MatchMode = 'exact'` laesst sich die Suche auf exakt benannte FK- oder Tabellenelemente verengen.
...
|---|---|---|---|
| `1.0` | `2026-04-18` | `ER` | Erstversion eines diagnostischen Metadaten-Skripts fuer Fremdschluessel-Namen |
...
```mermaid
flowchart TD
    A[Parameter pruefen] --> B[SearchTerms in temp table laden oder aus Pipe-Liste normalisieren]
    B --> C[FK-Inventar aus sys.foreign_keys und Join-Metadaten aufbauen]
    C --> D[FK-, Eltern- und Referenztabellennamen fuer Suche normalisieren]
    D --> E[Suchbegriffe gegen FK-Name sowie Parent- und Referenzobjekte pruefen]
    E --> F[Mehrfachtreffer je FK und Suchbegriff ueber ROW_NUMBER reduzieren]
    F --> G[ForeignKeyMatches als Detailergebnis ausgeben]
    G --> H[ForeignKeyCoverageByTable je Eltern-Tabelle aggregieren]
    H --> I[ForeignKeySearchTermSummary auch fuer Begriffe ohne Treffer erzeugen]
    I --> J[Temporare SearchTerms- und Match-Tabellen entfernen]
```
<!-- SQLDOC:MERMAID:END -->
