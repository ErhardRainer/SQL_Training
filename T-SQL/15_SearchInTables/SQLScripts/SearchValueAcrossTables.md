# SearchValueAcrossTables.sql

Dieses Skript durchsucht viele Tabellen der aktuell verbundenen Datenbank nach einem konkreten Wert oder Suchmuster. Die Erstversion bleibt rein lesend, arbeitet metadatengetrieben ueber `sys.tables`, `sys.columns` und `sys.types` und nutzt kontrolliertes Dynamic SQL, um Treffer pro Spalte, Tabellenabdeckung und Beispieldaten fuer Reviews sichtbar zu machen.
...
| Feld | Wert |
|---|---|
| Script | [SearchValueAcrossTables.sql](SearchValueAcrossTables.sql) |
| Version | `1.0` |
| Typ | `diagnostic-query` |
| Kapitel | `15_SearchInTables` |
| Sicherheit | `read-only` |
| Zweck | Durchsucht viele Tabellen dynamisch nach einem Suchwert und liefert Treffer-, Tabellen- und Spaltensichten. |
<!-- SQLDOC:SUMMARY_TABLE:END -->

## Einordnung

Das Artefakt eignet sich fuer Datenqualitaetspruefungen, Migrationsvorbereitungen, Datenschutz-Reviews und Impact-Analysen, wenn zunaechst offen ist, in welchen Tabellen ein bestimmter Referenzwert, Code oder Freitext auftaucht. Statt Fachtabellen zu aendern, wertet das Skript ausschliesslich Metadaten und lesende Abfragen der aktuellen Datenbank aus.

## Annahmen

- Die Erstversion durchsucht nur Basis-Tabellen der aktuell verbundenen Datenbank und ignoriert Views, Synonyme und Systemtabellen.
- Es werden nur Spaltentypen beruecksichtigt, die sich didaktisch sinnvoll nach `NVARCHAR(4000)` casten lassen.
- Der Suchvergleich wird ueber ein einheitliches Pattern auf dem String-Cast ausgefuehrt; das ist robust fuer Reviews, aber keine vollstaendige Ersatzlogik fuer typspezifische Fachvergleiche.
- `RowLocator` nutzt bevorzugt den Primaerschluessel der Tabelle und faellt sonst auf `NULL` zurueck, wenn kein Primaerschluessel verfuegbar ist.
- Pro Spalte werden hoechstens `@MaxHitsPerColumn` Detailtreffer ausgegeben, waehrend `HitCount` in der Spaltenzusammenfassung weiterhin die gesamte Trefferzahl dieser Spalte repraesentiert.

## Anwendungsfall

Das Skript hilft, wenn vor Refactorings, Datenbereinigungen oder Migrationen geprueft werden soll, in welchen Tabellen ein Kundenname, ein Legacy-Code, eine GUID, ein Datumswert oder ein anderes Suchmuster vorkommt. Mit `@SchemaPattern`, `@TablePattern` und `@ColumnNamePattern` laesst sich der Suchraum gezielt eingrenzen; `@MaxTables` und `@MaxHitsPerColumn` halten grosse Reviews beherrschbar.
...
|---|---|---|---|
| `@SearchValue` | `NVARCHAR(4000)` | Nein | Konkreter Suchwert oder Muster; `NULL` verwendet ein didaktisches Standardmuster. |
| `@MatchMode` | `NVARCHAR(10)` | Nein | Verwendet `contains`, `exact`, `starts_with` oder `ends_with` fuer den Suchvergleich. |
| `@SchemaPattern` | `NVARCHAR(256)` | Nein | Optionales `LIKE`-Muster fuer Schemas wie `dbo` oder `sales%`. |
| `@TablePattern` | `NVARCHAR(256)` | Nein | Optionales `LIKE`-Muster fuer Tabellen wie `Customer%` oder `%Audit`. |
| `@ColumnNamePattern` | `NVARCHAR(256)` | Nein | Optionales `LIKE`-Muster fuer Spaltennamen wie `%Name%` oder `%Code%`. |
| `@MaxTables` | `INT` | Nein | Begrenzt die Anzahl der geprueften Tabellen fuer kontrollierte Reviews. |
| `@MaxHitsPerColumn` | `INT` | Nein | Begrenzt die Zahl der Detailtreffer pro Spalte. |
...
- `ValueHits` liefert die begrenzte Detailsicht pro Spalte mit Suchmodus, Beispielwert und optionalem `RowLocator`.
- `TableCoverage` zeigt pro Tabelle, wie viele Spalten geprueft wurden und in wie vielen davon Treffer vorlagen.
- `ColumnSummary` bleibt auch fuer Spalten ohne Detailtreffer erhalten und eignet sich damit fuer Vollstaendigkeits- und Filter-Reviews.
- Durch die Kombination aus Metadatenfilterung und `sp_executesql` bleibt das Muster generisch, ohne produktive DML-Operationen zu benoetigen.
...
|---|---|---|---|
| `1.0` | `2026-04-18` | `ER` | Erstversion eines diagnostischen Suchskripts fuer Werte ueber viele Tabellen. |
...
```mermaid
flowchart TD
    A[Parameter und Grenzwerte pruefen] --> B[SearchPattern aus SearchValue und MatchMode aufbauen]
    B --> C[CandidateTables aus sys.tables und sys.schemas filtern]
    C --> D[Geeignete Spalten aus sys.columns und sys.types fuer TargetColumns sammeln]
    D --> E[Primaerschluessel-Ausdruck je Tabelle als RowLocator vorbereiten]
    E --> F[TargetColumns Schleife startet]
    F --> G[Dynamic SQL fuer aktuelle Tabelle und Spalte erzeugen]
    G --> H[Spaltenwert per TRY_CONVERT nach NVARCHAR casten]
    H --> I[MatchValue per LIKE gegen SearchPattern pruefen]
    I --> J[Begrenzte Detailtreffer in ValueHits schreiben]
    J --> K[Vollstaendige Trefferzahl je Spalte in ColumnSummary schreiben]
    K --> L{Weitere Spalten offen?}
    L -->|Ja| F
    L -->|Nein| M[ValueHits ausgeben]
    M --> N[TableCoverage aus ColumnSummary verdichten]
    N --> O[ColumnSummary ausgeben]
    O --> P[Temporare Tabellen entfernen]
```
<!-- SQLDOC:MERMAID:END -->
