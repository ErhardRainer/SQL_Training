# SearchConstraintDefinitions.sql

Dieses Skript durchsucht Constraint-Metadaten der aktuell verbundenen Datenbank nach frei definierbaren Suchbegriffen. Es kombiniert CHECK-, DEFAULT- und Fremdschluesseldefinitionen in einer gemeinsamen Suchsicht und eignet sich fuer Reviews von Validierungs- und Referenzregeln.
...
| Feld | Wert |
|---|---|
| Script | [SearchConstraintDefinitions.sql](SearchConstraintDefinitions.sql) |
| Version | `1.0` |
| Typ | `diagnostic-query` |
| Kapitel | `15_SearchInTables` |
| Sicherheit | `read-only` |
| Zweck | Durchsucht CHECK-, DEFAULT- und FK-Definitionen nach passenden Suchbegriffen und verdichtet die Treffer je Tabelle. |
<!-- SQLDOC:SUMMARY_TABLE:END -->

## Einordnung

Das Artefakt ist als diagnostische Erstversion fuer Schema-Reviews, Refactorings und Governance-Pruefungen gedacht. Statt produktive Regeln zu veraendern, arbeitet es rein lesend auf Katalogsichten und macht sichtbar, in welchen Constraints bestimmte Fach-, Lifecycle- oder Audit-Begriffe bereits verwendet werden.

## Annahmen

- Die Erstversion analysiert nur die aktuell verbundene Datenbank ueber `sys.check_constraints`, `sys.default_constraints`, `sys.foreign_keys`, `sys.foreign_key_columns`, `sys.tables`, `sys.schemas` und `sys.columns`.
- Fremdschluessel werden fuer die Suche als kompakte Textbeschreibung mit referenziertem Objekt und Spaltenliste aufbereitet, damit auch FK-Bezuege in einer gemeinsamen Trefferlogik sichtbar werden.
- Fuer die Suche werden Constraint-Namen und Definitionen normalisiert; die Ergebnisliste zeigt aber einen lesbaren, gekuerzten Definitionsauszug.
- Ohne Eingabe fuer `@SearchTerms` verwendet das Skript ein didaktisches Standardset aus Business-, Lifecycle-, Audit- und Referenzbegriffen.

## Anwendungsfall

Das Skript eignet sich fuer Inventuren vor Umbenennungen, fuer die Suche nach Status- oder Archivierungsregeln, fuer Reviews von Default-Ausdruecken und fuer den Einstieg in strukturierte Constraint-Analysen. Mit Suchbegriffen wie `tenant|region|approved` laesst sich der Fokus schnell auf eine fachliche Domane eingrenzen.
...
|---|---|---|---|
| `@SearchTerms` | `NVARCHAR(MAX)` | Nein | Pipe-separierte Suchbegriffe wie `status|isactive|archive`; `NULL` verwendet ein didaktisches Standardset. |
| `@SchemaName` | `SYSNAME` | Nein | Optionaler Schemafilter; `NULL` durchsucht alle Schemas. |
| `@ConstraintType` | `NVARCHAR(10)` | Nein | Verwendet `ALL`, `CHECK`, `DEFAULT` oder `FK` zur Eingrenzung des Constraint-Typs. |
| `@MatchMode` | `NVARCHAR(10)` | Nein | Verwendet `contains` oder `exact` fuer die Suche in Constraint-Namen und Definitionen. |
...
- `ConstraintDefinitionMatches` zeigt je Constraint den Suchbegriff, den Trefferort und einen gekuerzten Definitionsauszug.
- `ConstraintCoverageByObject` verdichtet die Treffer je Tabelle und zeigt, ob eher Validierungs-, Default- oder Referenzregeln betroffen sind.
- `ConstraintPatternSummary` listet auch Suchbegriffe ohne Treffer auf und eignet sich damit fuer die Review des Suchsets.
- Mit `@ConstraintType = 'FK'` laesst sich die Suche gezielt auf Referenzregeln einschranken; mit `@MatchMode = 'exact'` wird die Suche deutlich enger.
...
|---|---|---|---|
| `1.0` | `2026-04-18` | `ER` | Erstversion eines diagnostischen Metadaten-Skripts fuer Constraint-Definitionen |
...
flowchart TD
    ...
    C --> D[Constraint-Inventar aus CHECK, DEFAULT und optional FK-Katalogsichten aufbauen]
    D --> E[Constraint-Namen und Definitionen fuer die Suche normalisieren]
    E --> F[Suchbegriffe gegen Namen und Definitionen pruefen]
    F --> G[Mehrfachtreffer je Constraint und Suchbegriff ueber ROW_NUMBER reduzieren]
    G --> H[ConstraintDefinitionMatches als Detailergebnis ausgeben]
    H --> I[ConstraintCoverageByObject je Tabelle aggregieren]
    I --> J[ConstraintPatternSummary auch fuer Begriffe ohne Treffer erzeugen]
    J --> K[Temporare SearchTerms-Tabelle entfernen]
```
<!-- SQLDOC:MERMAID:END -->
