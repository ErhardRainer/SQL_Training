# SearchColumnNamePattern.sql

Dieses Skript durchsucht die Metadaten der aktuell verbundenen Datenbank nach Spaltennamen, die auf frei definierbare Suchbegriffe passen. Es eignet sich fuer schnelle Bestandsaufnahmen zu ID-Feldern, fachlichen Namensmustern und auditnahen Benennungen.
...
| Feld | Wert |
|---|---|
| Script | [SearchColumnNamePattern.sql](SearchColumnNamePattern.sql) |
| Version | `1.0` |
| Typ | `diagnostic-query` |
| Kapitel | `15_SearchInTables` |
| Sicherheit | `read-only` |
| Zweck | Durchsucht Spaltennamen in Tabellen- und optional View-Metadaten nach frei definierbaren Suchmustern. |
<!-- SQLDOC:SUMMARY_TABLE:END -->

## Einordnung

Das Artefakt ist als diagnostische Erstversion fuer Schema-Reviews, Naming-Konventionen und Migrationsvorbereitung gedacht. Statt produktive Tabellenregeln zu unterstellen, arbeitet es rein lesend auf Katalogsichten und erlaubt didaktisch nachvollziehbare Pattern-Suchen ueber normalisierte Spaltennamen.

## Annahmen

- Die Erstversion analysiert nur die aktuell verbundene Datenbank ueber `sys.objects`, `sys.schemas`, `sys.columns` und `sys.types`.
- Spaltennamen werden fuer die Suche ohne Leerzeichen, Bindestriche und Unterstriche normalisiert, damit Muster wie `Customer_ID` und `CustomerId` gleich behandelt werden.
- Ohne Eingabe fuer `@SearchTerms` verwendet das Skript ein konservatives Standardset aus ID-, Fach- und Auditmustern.
- Pro Spalte bleibt nur der staerkste Treffer erhalten, damit die Detailausgabe fuer Reviews lesbar bleibt.

## Anwendungsfall

Das Skript eignet sich fuer Inventuren vor Refactorings, fuer die Vorbereitung von Such- und Governance-Regeln oder als Einstieg in Metadatenanalysen. Mit eigenen Suchbegriffen wie `tenant|invoice|approved` kann der Fokus schnell auf eine fachliche Domane eingegrenzt werden.
...
|---|---|---|---|
| `@SearchTerms` | `NVARCHAR(MAX)` | Nein | Pipe-separierte Suchbegriffe wie `customer|id|created`; `NULL` verwendet ein didaktisches Standardset. |
...
| `@MatchMode` | `NVARCHAR(10)` | Nein | Verwendet `contains`, `prefix` oder `exact` fuer die Pattern-Pruefung. |
...
- `ColumnPatternMatches` zeigt pro Spalte den besten Treffer inklusive Kategorie und Vertrauensniveau.
- `ObjectPatternCoverage` verdichtet die Treffer je Objekt und markiert, ob eher IDs, fachliche Begriffe oder Auditmuster ueberwiegen.
- `PatternUsageSummary` listet auch Suchterme ohne Treffer auf und eignet sich damit fuer die Review des Suchsets.
- Mit `@MatchMode = 'exact'` laesst sich die Suche auf sehr enge Namenskonventionen begrenzen.
...
|---|---|---|---|
| `1.0` | `2026-04-18` | `ER` | Erstversion eines diagnostischen Metadaten-Skripts fuer Pattern-Suche in Spaltennamen |
...
flowchart TD
    ...
    B --> C{SearchTerms gesetzt?}
    C -->|Nein| D[Didaktisches Standard-Pattern-Set in temp table laden]
    C -->|Ja| E[Pipe-Liste ueber STRING_SPLIT normalisieren und kategorisieren]
    D --> F[Suchbare Tabellen und optional Views aus sys.objects bestimmen]
    E --> F
    F --> G[Spaltenmetadaten aus sys.columns und sys.types lesen und Namen normalisieren]
    G --> H[Pattern je nach MatchMode exact, prefix oder contains pruefen]
    H --> I[Mehrdeutige Treffer ueber ROW_NUMBER auf besten Match reduzieren]
    I --> J[ColumnPatternMatches als Detailergebnis ausgeben]
    J --> K[ObjectPatternCoverage je Objekt aggregieren]
    K --> L[PatternUsageSummary auch fuer Muster ohne Treffer erzeugen]
