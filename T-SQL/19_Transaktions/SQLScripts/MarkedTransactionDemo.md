# MarkedTransactionDemo.sql

Dieses Skript erklaert markierte Transaktionen als didaktisches Lab fuer koordinierte Restore-Szenarien. Statt produktive Backups oder `RESTORE LOG` auszufuehren, modelliert es typische Einsatzfaelle, leitet das passende Restore-Ziel ab und zeigt nur als Text generierte Beispielkommandos.
...
| Feld | Wert |
|---|---|
| Script | [MarkedTransactionDemo.sql](MarkedTransactionDemo.sql) |
| Version | `1.0` |
| Typ | `didactic-lab` |
| Kapitel | `19_Transaktions` |
| Sicherheit | `read-only-tempdb` |
| Zweck | Demonstriert markierte Transaktionen, koordinierte Restore-Ziele und passende Kommando-Vorschau. |
<!-- SQLDOC:SUMMARY_TABLE:END -->

## Einordnung

Markierte Transaktionen sind dann interessant, wenn ein Restore nicht nur technisch bis zu einem Zeitpunkt, sondern fachlich bis zu einem benannten Ereignis zurueckgefuehrt werden soll. Im Kapitel `19_Transaktions` dient dieses Lab als Bruecke zwischen Transaktionsgrenzen, Backup-Ketten und Restore-Planung ueber mehrere Systeme oder Release-Fenster hinweg.

## Annahmen

- Das Skript ist eine didaktische Erstversion mit modellierten Szenarien in `tempdb`.
- Es fuehrt weder `BEGIN TRAN ... WITH MARK` noch `RESTORE LOG` produktiv aus, sondern zeigt nur die Syntax als Vorschau.
- Die Beispielmarken dienen dem Unterricht und ersetzen keine gepruefte Restore-Runbook-Dokumentation.
- Fuer koordinierte Restores wird vorausgesetzt, dass jede beteiligte Datenbank die Marke in der relevanten Backup-Kette enthaelt.

## Anwendungsfall

Das Lab eignet sich fuer Unterricht, Runbook-Reviews und Incident-Nachbesprechungen, in denen geklaert werden soll, wann `STOPATMARK` oder `STOPBEFOREMARK` fachlich sinnvoll ist. Es hilft dabei, Marken nicht als reine Syntax zu sehen, sondern als Abstimmungspunkt zwischen Backup-Team, Fachbereich und Betriebsprozess.
...
|---|---|---|---|
| `@ScenarioFocus` | `VARCHAR(30)` | Nein | Optionaler Fokus auf `ALL`, `CROSS_DB`, `RELEASE` oder `BATCH`. |
| `@IncludeRestoreGuide` | `BIT` | Nein | Gibt bei `1` eine kompakte Restore-Entscheidungshilfe aus. |
| `@IncludeCommandPreview` | `BIT` | Nein | Gibt bei `1` nur als Text generierte Beispielkommandos fuer Marke und Restore aus. |
...
- `MarkedTransactionInventory` beschreibt den fachlichen Anlass der Marke, den Scope der beteiligten Datenbanken und das angepeilte Restore-Ziel.
- `RestoreDecisionGuide` macht sichtbar, dass `STOPATMARK` und `STOPBEFOREMARK` unterschiedliche fachliche Aussagen treffen.
- `CommandPreview` erzeugt ausschliesslich Textvorschauen und ist bewusst kein ausfuehrbares Restore-Runbook.
...
|---|---|---|---|
| `1.0` | `2026-04-22` | `ER` | Erstversion des didaktischen Labs fuer markierte Transaktionen und Restore-Marken |
...
```mermaid
flowchart TD
    A[Parameter validieren] --> B[MarkedTransactionInventory mit modellierten Marken und Restore-Zielen fuellen]
    B --> C[RestoreDecisionGuide fuer STOPATMARK oder STOPBEFOREMARK ableiten]
    C --> D[CommandPreview als reine Textvorschau per CROSS APPLY erzeugen]
    D --> E[Gefiltertes Szenario-Inventar ausgeben]
    E --> F{IncludeRestoreGuide = 1?}
    F -->|Ja| G[RestoreDecisionGuide ausgeben]
    F -->|Nein| H[Guide ueberspringen]
    G --> I{IncludeCommandPreview = 1?}
    H --> I
    I -->|Ja| J[CommandPreview ausgeben]
