# TransactionAgeByDatabase.sql

Dieses Skript verdichtet offene Transaktionen pro Datenbank auf die Frage, wie alt die aelteste aktuell sichtbare Transaktion ist. Im Kapitel `19_Transaktions` dient es als kompakter Einstieg, wenn eine Datenbankpriorisierung benoetigt wird, bevor das Troubleshooting in Session-, Lock- oder Request-Details wechselt.
...
| Feld | Wert |
|---|---|
| Script | [TransactionAgeByDatabase.sql](TransactionAgeByDatabase.sql) |
| Version | `1.0` |
| Typ | `diagnostic-query` |
| Kapitel | `19_Transaktions` |
| Sicherheit | `read-only` |
| Zweck | Zeigt pro Datenbank kompakt das Alter der aeltesten offenen Transaktion und ein erstes Review-Signal. |
<!-- SQLDOC:SUMMARY_TABLE:END -->

## Einordnung

Viele Diagnoseabfragen liefern sofort lange Detailtabellen pro Session oder pro Transaktion. Diese Erstversion reduziert den Einstieg bewusst auf eine kompakte Priorisierung: Welche Datenbank hat die aelteste offene Transaktion, wie viele offene Transaktionen gibt es dort, und welche einzelne Transaktion repraesentiert aktuell den kritischsten Einstiegspunkt fuer den Drilldown?

## Annahmen

- Das Skript liest ausschliesslich DMVs und Katalogsichten und nimmt keine Aenderungen an Datenbanken oder Transaktionen vor.
- Das Alter wird als Differenz zwischen `transaction_begin_time` und `SYSUTCDATETIME()` berechnet.
- Die Verdichtung pro Datenbank ist ein Review-Einstieg und ersetzt keine vollstaendige Analyse von Sperren, Waits oder Recovery-Verhalten.
- Standardmaessig werden Systemdatenbanken ausgeblendet, damit typische User-Datenbanken im Fokus bleiben.

## Anwendungsfall

Das Skript eignet sich fuer Incident-Review, Unterricht und operative Erstpruefungen, wenn offene Transaktionen datenbankweise priorisiert werden sollen. Das erste Resultset zeigt den kompakten Datenbankblick, das zweite Resultset liefert je Datenbank die aelteste einzelne Transaktion mit Session- und Logkontext, und der Guide uebersetzt die Kennzahlen in naechste Schritte.
...
|---|---|---|---|
| `@MinimumAgeMinutes` | `INT` | Nein | Filtert Datenbanken auf offene Transaktionen ab dieser Mindestdauer. |
| `@IncludeSystemDatabases` | `BIT` | Nein | Bezieht bei `1` auch `master`, `model`, `msdb` und `tempdb` ein. |
| `@IncludeGuide` | `BIT` | Nein | Gibt bei `1` zusaetzlich einen kompakten Leitfaden zur Einordnung aus. |
...
- `DatabaseTransactionAgeSummary` ordnet Datenbanken nach Alter der aeltesten offenen Transaktion, Logvolumen und Review-Signal.
- `OldestTransactionSample` zeigt je Datenbank genau die aktuell aelteste offene Transaktion als kompakten Drilldown.
- Der Guide fokussiert auf Priorisierung, Session-Drilldown, Hintergrundkontext und Loginterpretation.
...
|---|---|---|---|
| `1.0` | `2026-04-22` | `ER` | Erstversion des Diagnose-Skripts fuer das Alter offener Transaktionen je Datenbank |
...
```mermaid
flowchart TD
    A[Parameter validieren] --> B[Offene Transaktionen aus DMVs mit Datenbank-, Session- und Logkontext sammeln]
    B --> C[Inventory in TransactionInventory materialisieren]
    C --> D[Je Datenbank zu Alter, Anzahl und Logvolumen verdichten]
    D --> E[Mit ROW_NUMBER die aelteste Transaktion je Datenbank markieren]
    E --> F[OldestTransactionSample fuellen]
    F --> G[ReviewGuide fuer Priorisierung und Drilldown aufbauen]
    G --> H[DatabaseTransactionAgeSummary ausgeben]
    H --> I[OldestTransactionSample ausgeben]
    I --> J{IncludeGuide = 1?}
