# ShrinkFullDatabase.sql

Dieses Skript shrinkt gezielt die Logdatei einer einzelnen Datenbank, die im Recovery-Modell `FULL` oder `BULK_LOGGED` bleibt. Es ist als Gegenstueck zu den SIMPLE-Skripten gedacht und behandelt die Log-Backup-Kette ausdruecklich als Guardrail.
...
| Feld | Wert |
|---|---|
| Script | [ShrinkFullDatabase.sql](ShrinkFullDatabase.sql) |
| Version | `1.0` |
| Typ | `admin-change` |
| Kapitel | `19_Transaktions` |
| Sicherheit | `admin-change` |
| Zweck | Shrinkt die Logdatei einer einzelnen Datenbank im Recovery-Modell FULL oder BULK_LOGGED mit Guardrails fuer Full-Backup, Log-Backup-Kette, Log-Reuse-Wait, Mindestfreiraum und Zielgroesse. Das Skript kann zuerst als Preview laufen und fuehrt DBCC SHRINKFILE erst mit @ExecuteShrink = 1 aus. |
<!-- SQLDOC:SUMMARY_TABLE:END -->

## Einordnung

Bei `FULL` und `BULK_LOGGED` wird freier Logbereich nicht einfach durch `CHECKPOINT` wiederverwendbar. Entscheidend ist die Log-Backup-Kette: Erst ein regulaeres Logbackup kann die bereits gesicherten Logbereiche fuer Wiederverwendung freigeben. Dieses Skript prueft deshalb Full-Backup, Logbackup-Alter, `log_reuse_wait_desc`, VLF-Ende und Mindestfreiraum, bevor es einen Shrink ausfuehrt.

Der Default ist bewusst `@ExecuteShrink = 0`. In diesem Modus werden Messwerte, Zielgroesse und Befehle gezeigt, aber kein `BACKUP LOG` und kein `DBCC SHRINKFILE` ausgefuehrt.

## Annahmen

- Die Ziel-Datenbank bleibt in `FULL` oder `BULK_LOGGED`.
- Das Skript ist fuer genau eine Logdatei gebaut; mehrere Logdateien werden nicht automatisch geshrinkt.
- Ein optionales Logbackup ist ein regulaeres Logbackup und gehoert zur Restore-Kette.
- Shrink ist eine Ausnahmehandlung nach einem Sonderereignis, keine wiederkehrende Wartung.
- Nach dem Shrink sollten Zielgroesse und Autogrowth der Logdatei bewusst gesetzt werden.

## Alternative: kurz auf SIMPLE wechseln

Eine technische Alternative ist, die Datenbank kurz auf `SIMPLE` zu setzen, dann zu shrinken und anschliessend wieder auf `FULL` zu wechseln. Das kann so aussehen:

```sql
ALTER DATABASE [DeineDatenbank] SET RECOVERY SIMPLE;
CHECKPOINT;
DBCC SHRINKFILE (N'DeineLogDatei', 1024);
ALTER DATABASE [DeineDatenbank] SET RECOVERY FULL;
```

Diese Abkuerzung ist riskant und muss bewusst entschieden werden:

- Der Wechsel auf `SIMPLE` unterbricht die bisherige Log-Backup-Kette.
- Nach dem Wechsel zurueck auf `FULL` ist ein neues Full-Backup oder Differential-/Logbackup-Konzept erforderlich, damit Point-in-Time-Recovery wieder sauber moeglich ist.
- Bestehende Restore-Planaussagen wie "wir koennen bis 10:37 Uhr wiederherstellen" gelten fuer die unterbrochene Kette nicht mehr unveraendert.
- In produktiven Systemen mit RPO/RTO-Vorgaben, Always On, Log Shipping, Replikation oder Audit-Anforderungen ist dieser Weg meist nicht akzeptabel.
- Wenn die Logdatei direkt wieder waechst, wurde die Ursache nicht geloest; der SIMPLE-Umweg verschiebt dann nur das Problem.

Der sauberere Weg fuer produktive `FULL`-Datenbanken ist deshalb meistens: Ursache pruefen, regulaeres Logbackup sicherstellen, Log-Reuse-Wait beheben, gezielt shrinken und danach die passende Loggroesse sowie Autogrowth konfigurieren.
...
|---|---|---|---|
| `@DatabaseName` | `SYSNAME` | Ja | Name der zu verarbeitenden Datenbank. |
| `@TakeLogBackupBeforeShrink` | `BIT` | Nein | 1 = vor dem Shrink ein regulaeres Logbackup erstellen. |
| `@LogBackupDirectory` | `NVARCHAR(260)` | Nein | Zielordner fuer das optionale Logbackup; erforderlich wenn @TakeLogBackupBeforeShrink = 1. |
| `@BackupWithCompression` | `BIT` | Nein | 1 = optionales Logbackup mit COMPRESSION ausfuehren. |
| `@MaxLogBackupAgeMinutes` | `INT` | Nein | Maximales Alter des letzten Logbackups, wenn kein neues Logbackup erstellt wird. |
| `@ResultFillPct` | `INT` | Nein | Ziel-Fuellgrad der Logdatei nach Shrink in Prozent. |
| `@MinimumTargetSizeMB` | `INT` | Nein | Absolute Untergrenze fuer die Zielgroesse der Logdatei. |
| `@MinimumFreePct` | `DECIMAL(5,2)` | Nein | Mindestfreiraum in Prozent vor dem Shrink. |
| `@MinShrinkGainMB` | `DECIMAL(18,2)` | Nein | Mindestersparnis in MB; bei weniger Potenzial wird nicht geshrinkt. |
| `@AllowBulkLogged` | `BIT` | Nein | 1 = BULK_LOGGED-Datenbanken duerfen verarbeitet werden. |
| `@AllowSystemDatabase` | `BIT` | Nein | 1 = Systemdatenbanken duerfen verarbeitet werden. |
| `@ExecuteShrink` | `BIT` | Nein | 0 = Preview; 1 = optionales Logbackup und DBCC SHRINKFILE ausfuehren. |
...
- Mit `@TakeLogBackupBeforeShrink = 1` und `@ExecuteShrink = 1` erstellt das Skript ein echtes Logbackup im angegebenen Ordner.
- Dieses Logbackup darf nicht verworfen werden, solange es fuer Restore-Szenarien relevant ist.
- Wenn `log_reuse_wait_desc` nicht `NOTHING` oder `CHECKPOINT` ist, wird der Shrink blockiert.
- Bei `LOG_BACKUP` ist meist ein aktuelles regulaeres Logbackup erforderlich.
- Bei `ACTIVE_TRANSACTION`, Replikation, HADR oder aehnlichen Waits muss zuerst die Ursache geloest werden.
- Wenn mehrere Logdateien existieren, bricht das Skript ab; dateischarfe Mehrdatei-Log-Shrinks brauchen eine bewusstere VLF-Bewertung.
...
|---|---|---|---|
| `1.0` | `2026-06-01` | `ER` | Erstversion fuer gezielten Log-Shrink von FULL- und BULK_LOGGED-Datenbanken |
...
```mermaid
flowchart TD
    A[Parameter validieren] --> B[Datenbankstatus und Recovery Model lesen]
    B --> C{FULL oder BULK_LOGGED?}
    C -->|Nein| X[Abbruch mit Hinweis auf SIMPLE-Skripte oder nicht unterstuetztes Modell]
    C -->|Ja| D[Full- und Logbackup-Historie aus msdb lesen]
    D --> E[Before-Snapshot mit Loggroesse Lognutzung und VLF-Ende lesen]
    E --> F{Genau eine Logdatei?}
    F -->|Nein| X
    F -->|Ja| G{Optionales Logbackup angefordert?}
    G -->|Ja und Execute| H[BACKUP LOG ausfuehren und After-Logbackup-Snapshot lesen]
    G -->|Nein oder Preview| I[Vorhandenes Logbackup-Alter bewerten]
    H --> J[Log-Reuse-Wait und Shrink-Schwellen pruefen]
    I --> J
    J --> K{Guardrails und Groessenschwellen erfuellt?}
    K -->|Nein| Y[Skip oder Abbruch mit Grund]
    K -->|Ja und Preview| L[DBCC-SHRINKFILE-Befehl nur ausgeben]
    K -->|Ja und Execute| M[CHECKPOINT und DBCC SHRINKFILE ausfuehren]
    L --> N[After-Run-Snapshot und Resultsets ausgeben]
    M --> N
    Y --> N
    X --> N
```
