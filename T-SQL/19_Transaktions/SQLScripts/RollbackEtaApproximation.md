# RollbackEtaApproximation.sql

Dieses Skript versucht fuer laufende Rollbacks eine grobe ETA abzuleiten, ohne operative Eingriffe auszufuehren. Im Kapitel `19_Transaktions` verbindet es die von SQL Server gemeldete `estimated_completion_time` mit einer konservativen Fortschrittsheuristik, damit bei Incident-Reviews schneller erkennbar wird, ob ueberhaupt ein belastbares ETA-Signal vorliegt.
...
| Feld | Wert |
|---|---|
| Script | [RollbackEtaApproximation.sql](RollbackEtaApproximation.sql) |
| Version | `1.0` |
| Typ | `diagnostic-query` |
| Kapitel | `19_Transaktions` |
| Sicherheit | `read-only` |
| Zweck | Versucht fuer laufende Rollbacks eine grobe ETA aus DMV-Signalen und einer konservativen Heuristik abzuschaetzen. |
<!-- SQLDOC:SUMMARY_TABLE:END -->

## Einordnung

Eine ETA fuer Rollbacks ist in SQL Server oft nur naeherungsweise moeglich. Manche Requests liefern `percent_complete` und `estimated_completion_time`, andere zeigen nur Waits, Logdruck oder eine rollende Transaktion ohne brauchbare Restdauer. Das Skript behandelt deshalb die ETA ausdruecklich als Diagnose und nicht als verbindliche Zusage.

## Annahmen

- Das Skript bleibt strikt read-only und fuehrt keine `KILL`, `ROLLBACK`, `CHECKPOINT` oder Recovery-Aktionen aus.
- Wenn `estimated_completion_time` fehlt, wird nur eine einfache Laufzeit-zu-Fortschritt-Heuristik verwendet.
- Die Heuristik ist absichtlich konservativ und dient nur zur ersten Orientierung, nicht als SLA oder exakter Fertigstellungszeitpunkt.
- Hoher Logverbrauch und sichtbares Blocking werden als eigene Review-Signale neben der ETA betrachtet.

## Anwendungsfall

Die Ausgaben helfen bei Fragen wie: Gibt es fuer den aktuellen Rollback ueberhaupt eine beobachtbare ETA? Ist die Anzeige eher aus einer DMV-Meldung oder nur heuristisch abgeleitet? Welche Datenbanken oder Sessions zeigen zusaetzlich Blockierung oder hohes Logvolumen und verdienen deshalb mehr Aufmerksamkeit als ein einzelner Prozentwert vermuten laesst?
...
|---|---|---|---|
| `@MinimumPercentComplete` | `DECIMAL(5,2)` | Nein | Filtert auf Rollbacks ab diesem gemeldeten Prozentfortschritt. |
| `@MinimumRollbackAgeMinutes` | `INT` | Nein | Filtert auf Rollbacks, die mindestens so lange laufen. |
| `@IncludeGuidance` | `BIT` | Nein | Gibt bei `1` zusaetzlich einen Leitfaden fuer die Einordnung der ETA aus. |
...
- `RollbackEtaSamples` zeigt pro beobachtetem Rollback die gemeldete und die heuristische Restdauer nebeneinander.
- `EtaSource` trennt klar zwischen DMV-basierter ETA, Fortschrittsheuristik und unzureichendem Signal.
- `EtaConfidence` bleibt bewusst vorsichtig, weil Rollback-Telemetrie schwanken oder ganz fehlen kann.
- `ReviewSignal` und `ReviewHint` helfen, ETA, Blocking und Logdruck nicht miteinander zu verwechseln.
...
|---|---|---|---|
| `1.0` | `2026-04-22` | `ER` | Erstversion fuer konservative ETA-Abschaetzungen bei laufenden Rollbacks |
...
```mermaid
flowchart TD
    A[Parameter validieren] --> B[Rollback-Kandidaten aus dm_exec_requests und Transaktions-DMVs sammeln]
    B --> C[Fortschritt, Laufzeit, Waits, Logvolumen und Statement-Kontext anreichern]
    C --> D{estimated_completion_time vorhanden?}
    D -->|Ja| E[ApproxEtaUtc aus DMV-Restdauer ableiten]
    D -->|Nein| F{percent_complete und Laufzeit vorhanden?}
    F -->|Ja| G[Heuristische Restdauer aus Laufzeit und Fortschritt berechnen]
    F -->|Nein| H[ETA als insufficient-signal markieren]
    E --> I[EtaSource, EtaConfidence und ReviewSignal bestimmen]
    G --> I
    H --> I
    I --> J[Samples nach Datenbank und ETA-Zuverlaessigkeit verdichten]
    J --> K[Leitfaden fuer DMV-ETA, Heuristik, fehlendes Signal, Blocking und Logdruck fuellen]
    K --> L[RollbackEtaSamples ausgeben]
    L --> M[RollbackEtaPrioritySummary ausgeben]
    M --> N{IncludeGuidance = 1?}
    N -->|Ja| O[EtaGuidance ausgeben]
    N -->|Nein| P[Run beenden]
    O --> P
```
<!-- SQLDOC:MERMAID:END -->
