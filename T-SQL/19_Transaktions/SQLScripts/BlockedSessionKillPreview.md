# BlockedSessionKillPreview.sql

Dieses Skript modelliert Blocking-Ketten als didaktisches Inventar und zeigt, welche Folgen ein `KILL` des vermuteten Root-Blockers haben koennte. Statt produktive DMVs oder echte Session-Abbrueche zu verwenden, berechnet die Erstversion eine Vorschau fuer Opferzahl, geschaetzte Rollback-Dauer und Guardrails fuer die Entscheidung.
...
| Feld | Wert |
|---|---|
| Script | [BlockedSessionKillPreview.sql](BlockedSessionKillPreview.sql) |
| Version | `1.0` |
| Typ | `didactic-lab` |
| Kapitel | `19_Transaktions` |
| Sicherheit | `read-only-tempdb` |
| Zweck | Bewertet modellierte Blocking-Ketten und zeigt die vermuteten Folgen eines `KILL` vorab an. |
<!-- SQLDOC:SUMMARY_TABLE:END -->

## Einordnung

Ein `KILL` kann eine blockierte Kette zwar aufloesen, fuehrt aber haeufig in ein neues Problemfenster: Rollback, Kommunikation mit Fachbereichen und moegliche Nacharbeiten. Das Skript behandelt die Entscheidung deshalb nicht als Sofortmassnahme, sondern als abwaegbare Vorschau aus Opferzahl, angenommener Rueckabwicklungsdauer und sichereren Alternativen.

## Annahmen

- Es handelt sich um eine didaktische Erstversion mit modellierten Blocking-Ketten in `tempdb`.
- Das Skript fuehrt kein `KILL` aus und ersetzt keine produktive DMV- oder Incident-Analyse.
- Rollback-Zeit und Freigabe-Effekt sind bewusst einfache Schaetzwerte fuer Review- und Unterrichtszwecke.
- Die gezeigten Alternativen fokussieren auf Root-Cause-Pruefung, Kommunikation und Batch-/Commit-Design.

## Anwendungsfall

Das Skript eignet sich fuer Unterricht, Incident-Reviews und Troubleshooting-Workshops rund um Blocking-Ketten. Es hilft dabei, vor einer Kill-Entscheidung die Opferzahl, das moegliche Rollback-Fenster und die noetigen Guardrails explizit zu diskutieren.
...
|---|---|---|---|
| `@MinimumBlockedSessions` | `INT` | Nein | Filtert auf Blocking-Ketten ab dieser Anzahl direkt oder indirekt blockierter Sessions. |
| `@IncludeGuardrailGuide` | `BIT` | Nein | Gibt bei `1` zusaetzlich einen Guardrail-Guide fuer Kill-Entscheidungen aus. |
...
- `BlockingChainInventory` beschreibt Root-Blocker, Kettenlaenge, Alter und angenommene Rollback-Datenmenge.
- `KillImpactPreview` leitet daraus eine didaktische Vorschau fuer Freigabe-Prioritaet, Kill-Readiness und Alternativen ab.
- Der Guardrail-Guide macht deutlich, dass eine Kill-Entscheidung immer Incident-Kommunikation und Nacharbeit nach sich ziehen kann.
...
|---|---|---|---|
| `1.0` | `2026-04-19` | `ER` | Erstversion des didaktischen Labs fuer Kill-Vorschau bei Blocking-Ketten |
...
```mermaid
flowchart TD
    A[Parameter validieren] --> B[BlockingChainInventory mit modellierten Root-Blockern und Opferketten fuellen]
    B --> C[KillImpactPreview fuer Opferzahl, Rollback-Minuten und Kill-Readiness ableiten]
    C --> D[GuardrailGuide fuer Verifikation, Kommunikation und Nacharbeit aufbauen]
    D --> E[BlockingChainInventory nach Mindestzahl blockierter Sessions ausgeben]
    E --> F[KillImpactPreview ausgeben]
    F --> G{IncludeGuardrailGuide = 1?}
    G -->|Ja| H[GuardrailGuide ausgeben]
