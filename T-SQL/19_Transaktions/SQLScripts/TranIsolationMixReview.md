# TranIsolationMixReview.sql

Dieses Skript reviewt aktuelle Sessions auf gemischte Transaktions-Isolationsstufen innerhalb vergleichbarer Workload-Gruppen. Im Kapitel `19_Transaktions` hilft es dabei, Unterschiede zwischen Datenbank, Programm und Host sichtbar zu machen, ohne daraus vorschnell einen Fehler abzuleiten.
...
| Feld | Wert |
|---|---|
| Script | [TranIsolationMixReview.sql](TranIsolationMixReview.sql) |
| Version | `1.0` |
| Typ | `diagnostic-query` |
| Kapitel | `19_Transaktions` |
| Sicherheit | `read-only` |
| Zweck | Reviewt aktuelle Sessions auf gemischte Isolationsstufen innerhalb vergleichbarer Workload-Gruppen. |
<!-- SQLDOC:SUMMARY_TABLE:END -->

## Einordnung

Gleiche Anwendungen oder Batch-Pfade sollten meist ein nachvollziehbares Isolationsprofil haben. Wenn innerhalb derselben Workload-Gruppe unterschiedliche Stufen wie `READ COMMITTED`, `SERIALIZABLE` oder `SNAPSHOT` auftauchen, ist das nicht automatisch falsch, aber ein guter Anlass fuer ein strukturiertes Review von Konsistenzbedarf, Sperrverhalten und Session-Initialisierung.

## Annahmen

- Das Skript nutzt aktuelle DMVs und bildet deshalb nur eine Momentaufnahme der laufenden Sessions ab.
- Die Gruppierung erfolgt didaktisch ueber Datenbank, Programm und Host, um vergleichbare Workload-Muster kompakt sichtbar zu machen.
- Gemischte Isolationsstufen koennen bewusst eingesetzt sein; die Ausgabe dient als Review- und Abstimmungshilfe statt als Fehlerurteil.
- Fuer produktive Nutzung werden typischerweise Leserechte auf die benoetigten DMVs benoetigt, etwa `VIEW SERVER STATE`.

## Anwendungsfall

Das Skript eignet sich fuer Reviews nach Blocking-Auffaelligkeiten, bei unerwartetem Versionsspeicher-Verhalten oder wenn mehrere Deployments bzw. Jobpfade derselben Anwendung unterschiedliche Konsistenz- und Sperrprofile zeigen. Es verbindet die Detailsicht pro Session mit einer verdichteten Gruppenperspektive.
...
|---|---|---|---|
| `@IncludeSystemSessions` | `BIT` | Nein | `1` nimmt System-Sessions mit auf, `0` betrachtet nur Benutzer-Sessions. |
| `@MinimumSessionCountForAlert` | `INT` | Nein | Mindestanzahl Sessions pro Workload-Gruppe, bevor eine gemischte Isolationslage als Alert erscheint. |
| `@IncludeGuide` | `BIT` | Nein | Gibt bei `1` zusaetzlich einen didaktischen Review-Guide aus. |
...
- `SessionIsolationInventory` zeigt die beobachtete Isolation pro Session zusammen mit Request-, Wait- und Statement-Kontext.
- `IsolationMixSummary` markiert Workload-Gruppen mit mehreren beobachteten Isolationsstufen und priorisiert sie fuer das Review.
- Der Guide trennt zwischen legitimen Architekturunterschieden und ungeplanten Session-Abweichungen.
...
|---|---|---|---|
| `1.0` | `2026-04-22` | `ER` | Erstversion des Diagnose-Skripts fuer gemischte Isolationsstufen in aktuellen Sessions |
...
```mermaid
flowchart TD
    A[Parameter validieren] --> B[Aktuelle Sessions mit Requests, Verbindung und SQL-Text erfassen]
    B --> C[Isolation, WorkloadGroup und ReviewSignal pro Session ableiten]
    C --> D[Workload-Gruppen nach Anzahl Sessions und Distinct Isolation Levels verdichten]
    D --> E[IsolationLevelsObserved per STRING_AGG zusammensetzen]
    E --> F[RiskSignal und ReviewRecommendation pro Gruppe bestimmen]
    F --> G[SessionIsolationInventory ausgeben]
    G --> H[IsolationMixSummary ausgeben]
    H --> I{IncludeGuide = 1?}
