# LongTranHostProgramAudit.sql

Dieses Skript auditiert aktuell lang laufende Benutzertransaktionen ueber Host und Programm und verbindet den DMV-Befund mit offenem Transaktionszaehler, Wait-Kontext und Blocking-Hinweisen. Im Kapitel `19_Transaktions` dient es als lesende Diagnose fuer die Frage, welche Clients wiederholt lange Transaktionen offen halten.
...
| Feld | Wert |
|---|---|
| Script | [LongTranHostProgramAudit.sql](LongTranHostProgramAudit.sql) |
| Version | `1.0` |
| Typ | `diagnostic-query` |
| Kapitel | `19_Transaktions` |
| Sicherheit | `read-only` |
| Zweck | Auditiert lang laufende Transaktionen nach Host und Programm mit Session-, Wait- und Blocking-Kontext. |
<!-- SQLDOC:SUMMARY_TABLE:END -->

## Einordnung

Das Audit liest aktuelle DMV-Daten der Instanz und verdichtet sie pro Host und Programm. Dadurch wird aus einer reinen Session-Liste ein besserer Ueberblick darueber, welche Anwendungen, Batch-Prozesse oder Clients ueberdurchschnittlich lange Transaktionen offen halten und ob dabei bereits Waits oder Blocking sichtbar sind.

## Annahmen

- Das Skript ist als lesende Diagnose fuer eine laufende Instanz gedacht und verwendet keine Demo-Daten.
- Fuer den Zugriff auf die benoetigten DMVs werden ueblicherweise Leserechte wie `VIEW SERVER STATE` benoetigt.
- Host- und Programmnamen koennen je nach Clientbibliothek unvollstaendig, generisch oder leer sein; das Skript kennzeichnet solche Faelle neutral.
- Die Verdichtung pro Host und Programm ersetzt keine historische Trendanalyse, sondern liefert eine Momentaufnahme fuer Review und Troubleshooting.

## Anwendungsfall

Das Skript eignet sich fuer Incident-Analyse, Datenbankbetrieb und Unterricht zu langen Transaktionen. Es hilft dabei, lange offene Transaktionen nicht nur auf Session-Ebene, sondern direkt aus Sicht der beteiligten Anwendungen zu bewerten: Welcher Host ist betroffen, welches Programm haelt die Transaktion, gibt es Blocking, und welche Kombinationen sollten priorisiert mit dem Anwendungsteam besprochen werden?
...
| `@MinimumOpenTransactionCount` | `INT` | Nein | Filtert auf Sessions mit mindestens so vielen offenen Transaktionen. |
| `@IncludeGuide` | `BIT` | Nein | Gibt bei `1` zusaetzlich einen kompakten Review-Guide aus. |
...
- `LongTranAudit` zeigt die einzelnen langen Transaktionen mitsamt Host, Programm, Login, Wait und Statement-Ausschnitt.
- `HostProgramSummary` verdichtet den Befund pro Host-Programm-Paar und eignet sich fuer eine schnelle Priorisierung.
- Der optionale Guide uebersetzt die Momentaufnahme in konkrete naechste Diagnose- oder Review-Schritte.
...
|---|---|---|---|
| `1.0` | `2026-04-22` | `ER` | Erstversion des Host- und Programm-Audits fuer lang laufende Transaktionen |
...
```mermaid
flowchart TD
    A[Parameter validieren] --> B[DMV-Daten zu aktiven Transaktionen, Sessions und Requests laden]
    B --> C[LongTranAudit fuer lange Benutzertransaktionen fuellen]
    C --> D[HostProgramSummary nach Host und Programm verdichten]
    D --> E[ReviewGuide aus langen Laufzeiten, sleeping Sessions und Blocking ableiten]
    E --> F[LongTranAudit ausgeben]
    F --> G[HostProgramSummary ausgeben]
    G --> H{IncludeGuide = 1?}
