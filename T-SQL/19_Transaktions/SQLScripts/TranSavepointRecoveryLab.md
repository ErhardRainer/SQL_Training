# TranSavepointRecoveryLab.sql

Dieses Skript zeigt ein didaktisches Recovery-Muster mit `SAVE TRANSACTION`. Es trennt einen stabilen Vorlauf von einem riskanten Abschnitt, simuliert eine Validierungsstoerung nach dem Savepoint und demonstriert, wie Teilrollback, kontrollierter Wiederanlauf und anschliessender Commit zusammenspielen koennen.
...
| Feld | Wert |
|---|---|
| Script | [TranSavepointRecoveryLab.sql](TranSavepointRecoveryLab.sql) |
| Version | `1.0` |
| Typ | `didactic-lab` |
| Kapitel | `19_Transaktions` |
| Sicherheit | `demo-write-tempdb` |
| Zweck | Demonstriert Savepoint-basierte Recovery mit Teilrollback, Wiederanlauf und bewusstem Abschluss-Commit. |
<!-- SQLDOC:SUMMARY_TABLE:END -->

## Einordnung

Der Kern des Labs ist nicht nur das Setzen eines Savepoints, sondern die Frage, was nach einem Fehler gezielt erhalten, verworfen oder erneut ausgefuehrt werden darf. Das Skript modelliert deshalb eine kleine Verarbeitungskette mit stabilem Vorlauf, riskanter Teilstrecke und spaeterem Abschlussschritt. So wird sichtbar, dass Recovery mit Savepoints eine fachlich vorbereitete Entscheidung und kein reines Syntaxdetail ist.

## Annahmen

- Es handelt sich um eine didaktische Erstversion mit temporaeren Tabellen in `tempdb`.
- Der Validierungsfehler wird absichtlich simuliert, damit Teilrollback und Recovery reproduzierbar beobachtet werden koennen.
- `XACT_ABORT` bleibt deaktiviert, damit der `TRY...CATCH`-Pfad den Ruecksprung zum Savepoint kontrolliert demonstrieren kann.
- Der Fokus liegt auf Recovery-Entscheidungen innerhalb einer Transaktion, nicht auf produktiven Buchungstabellen oder echter Batch-Orchestrierung.

## Anwendungsfall

Das Skript eignet sich fuer Unterricht, Runbook-Diskussionen und Code-Reviews, wenn geklaert werden soll, ob eine Transaktion nach einem Teilfehler komplett verworfen werden muss oder ob ein vorbereiteter Savepoint einen sicheren Wiederanlauf ermoeglicht. Besonders hilfreich ist es bei Fragen zu stabilen Vorstufen, rollback-faehigen Risikoteilen und bewusstem Commit nach dem Recovery-Pfad.
...
|---|---|---|---|
| `@TriggerValidationFailure` | `BIT` | Nein | `1` simuliert einen fachlichen Fehler nach dem Savepoint und aktiviert den Recovery-Pfad. |
| `@ReplayAfterRecovery` | `BIT` | Nein | `1` fuehrt nach dem Teilrollback eine kontrollierte Ersatzverarbeitung aus. |
| `@ShowRecoveryGuide` | `BIT` | Nein | Gibt bei `1` zusaetzlich einen kompakten Leitfaden fuer Savepoint-basierte Recovery aus. |
...
- `RecoveryTimeline` dokumentiert den Ablauf vom Savepoint ueber den Fehlerfang bis zum Wiederanlauf oder manuellen Review-Pfad.
- `UnitState` macht sichtbar, welche Einheiten vor dem Savepoint erhalten bleiben, welche nach Teilrollback erneut laufen und welche bewusst zurueckgestellt werden.
- Der Guide trennt fachliche Recovery-Entscheidungen von der reinen Syntax und eignet sich deshalb gut fuer Runbook- und Review-Gespraeche.
...
|---|---|---|---|
| `1.0` | `2026-04-22` | `ER` | Erstversion des didaktischen Labs fuer Savepoint-basierte Recovery |
...
```mermaid
flowchart TD
    A[Parameter validieren] --> B[RecoveryUnits und Timeline in tempdb vorbereiten]
    B --> C[BEGIN TRANSACTION TranSavepointRecoveryLab]
    C --> D[Stabilen Vorlauf aktualisieren]
    D --> E[SAVE TRANSACTION RecoveryCheckpoint]
    E --> F[Riskanten Abschnitt ausfuehren]
    F --> G{Validierungsfehler?}
    G -->|Nein| H[Riskanten Abschnitt akzeptieren]
    G -->|Ja| I[TRY CATCH faengt Fehler]
    I --> J[ROLLBACK TRANSACTION RecoveryCheckpoint]
    J --> K{ReplayAfterRecovery = 1?}
    K -->|Ja| L[Kontrollierte Ersatzverarbeitung fuer sichere Einheiten]
    K -->|Nein| M[Riskante Einheiten fuer manuellen Review zurueckstellen]
    H --> N[Stabilen Abschlussschritt markieren]
    L --> N
    M --> N
    N --> O[COMMIT TRANSACTION]
    O --> P[RecoveryTimeline und UnitState ausgeben]
    P --> Q{ShowRecoveryGuide = 1?}
    Q -->|Ja| R[RecoveryGuide ausgeben]
    Q -->|Nein| S[Run beenden]
    R --> S
```
<!-- SQLDOC:MERMAID:END -->
