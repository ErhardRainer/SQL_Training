# DeadlockVictimCaptureStarter.sql

Dieses Skript modelliert Deadlock-Ereignisse als didaktisches Inventar und zeigt, wie moegliche Opferauswahl, Rollback-Kosten und erste Capture-Schritte dokumentiert werden koennen. Statt produktive Extended Events oder echte Deadlocks zu benoetigen, bleibt die Erstversion bei nachvollziehbaren Demo-Faellen im Kapitel `19_Transaktions`.
...
| Feld | Wert |
|---|---|
| Script | [DeadlockVictimCaptureStarter.sql](DeadlockVictimCaptureStarter.sql) |
| Version | `1.0` |
| Typ | `didactic-lab` |
| Kapitel | `19_Transaktions` |
| Sicherheit | `read-only-tempdb` |
| Zweck | Bewertet modellierte Deadlock-Opfer und leitet erste Capture- und Review-Schritte ab. |
<!-- SQLDOC:SUMMARY_TABLE:END -->

## Einordnung

In produktiven Umgebungen stammen Deadlock-Nachweise oft aus `system_health`, einer eigenen Extended-Events-Session oder spaeter aus archivierten Graphen. Diese Erstversion konzentriert sich bewusst auf das Review-Muster: Wer wurde Opfer, warum war diese Session plausibel die Verliererin, und welche Nachweise sollten direkt gesichert werden?

## Annahmen

- Es handelt sich um eine didaktische Erstversion mit modellierten Deadlock-Ereignissen in `tempdb`.
- Das Skript liest keine produktiven Deadlock-Graphen und startet keine Extended-Events-Session.
- Opferwahl und Rollback-Kosten werden ueber einfache Heuristiken erklaert: Deadlock-Priority, ungefaehre Rueckabwicklungskosten und Anzahl betroffener Opfer.
- Die gezeigten Capture-Quellen und Follow-up-Schritte dienen als Review-Startpunkt und muessen spaeter an reale Monitoring-Standards angepasst werden.

## Anwendungsfall

Das Skript eignet sich fuer Unterricht, Incident-Nachbesprechungen und Design-Reviews rund um Deadlocks. Es hilft dabei, Capture, Opferanalyse und Folgearbeit nicht zu vermischen: Zuerst Nachweise sichern, dann die Opferwahl verstehen, danach Reihenfolge, Retry-Strategie und Transaktionsdesign verbessern.
...
|---|---|---|---|
| `@MinimumVictimCount` | `INT` | Nein | Filtert auf modellierte Deadlock-Ereignisse ab dieser Anzahl betroffener Opfer-Sessions. |
| `@IncludeActionGuide` | `BIT` | Nein | Gibt bei `1` zusaetzlich einen kompakten Guide fuer Capture und Nacharbeit aus. |
...
- `DeadlockEventInventory` beschreibt modellierte Deadlock-Faelle mit Ressource, Prioritaeten, Opferzahl und empfohlener Capture-Quelle.
- `VictimAssessment` leitet daraus plausible Auswahlgruende, Capture-Dringlichkeit und zu sichernde Nachweise ab.
- Der Guide uebersetzt die Analyse in konkrete Startmassnahmen fuer Event-Sammlung, Kontext und Folgearbeit.
...
|---|---|---|---|
| `1.0` | `2026-04-19` | `ER` | Erstversion des didaktischen Labs fuer Deadlock-Opfer, Capture und Review |
...
```mermaid
flowchart TD
    A[Parameter validieren] --> B[DeadlockEventInventory mit modellierten Deadlock-Faellen fuellen]
    B --> C[LikelySelectionDriver und CaptureUrgency pro Opfer ableiten]
    C --> D[EvidenceToKeep und ReviewSignal fuer die Nachweissicherung formulieren]
    D --> E[CaptureActionGuide fuer Event-Quelle, Kontext und Follow-up aufbauen]
    E --> F[DeadlockEventInventory nach Mindestzahl Opfer ausgeben]
    F --> G[VictimAssessment priorisiert ausgeben]
    G --> H{IncludeActionGuide = 1?}
    H -->|Ja| I[CaptureActionGuide ausgeben]
