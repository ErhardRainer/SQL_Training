# SavepointDemoHarness.sql

Dieses Skript demonstriert `SAVE TRANSACTION` und partielles Rollback an einer kleinen Beispielstrecke in `tempdb`. Es zeigt, wie vorbereitende Schritte vor dem Savepoint erhalten bleiben, waehrend riskante Aenderungen gezielt bis zum Savepoint zurueckgesetzt und danach kontrolliert fortgesetzt werden koennen.
...
| Feld | Wert |
|---|---|
| Script | [SavepointDemoHarness.sql](SavepointDemoHarness.sql) |
| Version | `1.0` |
| Typ | `didactic-lab` |
| Kapitel | `19_Transaktions` |
| Sicherheit | `demo-write-tempdb` |
| Zweck | Demonstriert Savepoints, partielles Rollback und den verbleibenden Commit-Pfad in einer Transaktion. |
<!-- SQLDOC:SUMMARY_TABLE:END -->

## Einordnung

`SAVE TRANSACTION` ist besonders dann hilfreich, wenn innerhalb einer groesseren Transaktion nur ein Teilabschnitt riskant ist, waehrend vorbereitende oder spaetere Schritte erhalten bleiben sollen. Das Lab arbeitet bewusst nur mit temporaeren Tabellen und macht dadurch sichtbar, dass ein Ruecksprung zum Savepoint nicht dieselbe Wirkung wie ein Vollrollback hat.

## Annahmen

- Das Skript ist eine didaktische Erstversion und schreibt nur in temporaere Tabellen.
- Die markierte Stoerung im riskanten Abschnitt ist absichtlich modelliert und kein echter SQL-Fehlerpfad.
- `XACT_ABORT` bleibt deaktiviert, damit der Unterschied zwischen kontrolliertem Teilrollback und Vollabbruch besser beobachtbar ist.
- Der Fokus liegt auf Transaktionsgrenzen, Savepoint-Semantik und Commit-Verhalten, nicht auf produktiven Fachtabellen.

## Anwendungsfall

Das Skript eignet sich fuer Unterricht, Code-Reviews und Runbook-Diskussionen, wenn geklaert werden soll, ob ein Zwischenstand in einer Transaktion bewusst erhalten bleiben darf. Es hilft dabei, Savepoints nicht nur als Syntaxdetail zu sehen, sondern als fachlich und technisch vorbereiteten Ruecksprungpunkt fuer riskante Teilstrecken.
...
|---|---|---|---|
| `@RollbackToSavepoint` | `BIT` | Nein | `1` rollt den riskanten Abschnitt bis zum Savepoint zurueck, `0` committet die Demo-Strecke ohne Teilrollback. |
| `@SimulateIssue` | `BIT` | Nein | `1` markiert eine didaktische Stoerung im riskanten Abschnitt. |
| `@IncludeGuide` | `BIT` | Nein | Gibt bei `1` zusaetzlich einen kompakten Leitfaden fuer Savepoints und Teil-Rollbacks aus. |
...
- `ExecutionTimeline` zeigt, wann die Transaktion beginnt, der Savepoint gesetzt wird, ein Teilrollback erfolgt und anschliessend weitergearbeitet wird.
- `WorkQueueState` macht sichtbar, welche Arbeitspakete vor dem Savepoint unveraendert erhalten bleiben und welche hinter dem Savepoint verworfen oder erneut ausgefuehrt werden.
- Der Leitfaden grenzt Savepoints bewusst gegen ein Vollrollback ab und betont die noetige fachliche Vorbereitung eines solchen Zwischenzustands.
...
|---|---|---|---|
| `1.0` | `2026-04-22` | `ER` | Erstversion des didaktischen Labs fuer Savepoints und partielles Rollback |
...
```mermaid
flowchart TD
    A[Parameter validieren] --> B[WorkQueue und Timeline in tempdb vorbereiten]
    B --> C[BEGIN TRANSACTION SavepointHarness]
    C --> D[Stabile Vorbereitungsschritte vor dem Savepoint aktualisieren]
    D --> E[SAVE TRANSACTION RiskySectionStart]
    E --> F[Riskanten Abschnitt markieren und Demo-Stoerung optional setzen]
    F --> G{RollbackToSavepoint = 1?}
    G -->|Ja| H[ROLLBACK TRANSACTION RiskySectionStart]
    H --> I[Kontrollierten Retry fuer notwendige Schritte ausfuehren]
    G -->|Nein| J[Riskante Aenderungen in der Transaktion belassen]
    I --> K[Stabilen Abschlussschritt markieren]
    J --> K
    K --> L[COMMIT TRANSACTION SavepointHarness]
    L --> M[ExecutionTimeline und WorkQueueState ausgeben]
    M --> N{IncludeGuide = 1?}
    N -->|Ja| O[SavepointGuide ausgeben]
