# KillDecisionChecklist.sql

Dieses Skript baut eine didaktische Entscheidungscheckliste fuer moegliche `KILL`-Situationen bei blockierenden oder lang laufenden Sessions auf. Statt produktive DMVs oder echte Session-Abbrueche zu verwenden, arbeitet die Erstversion mit modellierten Vorfaellen in `tempdb` und leitet daraus Guardrails, Alternativen und Eskalationsstufen ab.
...
| Feld | Wert |
|---|---|
| Script | [KillDecisionChecklist.sql](KillDecisionChecklist.sql) |
| Version | `1.0` |
| Typ | `didactic-lab` |
| Kapitel | `19_Transaktions` |
| Sicherheit | `read-only-tempdb` |
| Zweck | Baut eine priorisierte Checkliste fuer Kill-Entscheidungen bei modellierten Blocking-Vorfaellen auf. |
<!-- SQLDOC:SUMMARY_TABLE:END -->

## Einordnung

Ein `KILL` ist in Transaktionsvorfaellen selten eine isolierte Technikentscheidung. Das Skript macht deshalb sichtbar, dass Root-Blocker-Bestaetigung, Rollback-Erwartung, fachliche Auswirkung, Alternativen und Kommunikation zusammen betrachtet werden muessen, bevor ein Session-Abbruch ueberhaupt vertretbar erscheint.

## Annahmen

- Es handelt sich um eine didaktische Erstversion mit modellierten Incident-Faellen in `tempdb`.
- Das Skript fuehrt kein `KILL` aus und ersetzt keine produktive Analyse auf Basis von DMVs oder Incident-Playbooks.
- Rollback-Datenmenge, Kritikalitaet und Kommunikationspflichten sind bewusst einfache Bewertungsachsen fuer Review- und Unterrichtszwecke.
- Die Alternativen fokussieren auf Owner-Kontakt, Batch-Grenzen, Warten und geordnete Eskalation statt auf aggressive Sofortmassnahmen.

## Anwendungsfall

Das Skript eignet sich fuer Unterricht, Betriebsworkshops und Retrospektiven rund um Blocking- und Transaktionsvorfaelle. Es hilft dabei, vor einer Kill-Entscheidung systematisch dieselben Fragen zu stellen und die Entscheidung gegen Folgen wie Rollback, Kommunikationsaufwand und Nacharbeit abzuwiegen.
...
|---|---|---|---|
| `@MinimumBlockedSessions` | `INT` | Nein | Filtert auf modellierte Vorfaelle ab dieser Anzahl blockierter Sessions. |
| `@IncludeEscalationMatrix` | `BIT` | Nein | Gibt bei `1` zusaetzlich eine Eskalationsmatrix mit Alternativen und Kommunikationspflichten aus. |
...
- `IncidentInventory` beschreibt Dauer, Breite und vermutetes Rollback-Risiko modellierter Vorfaelle.
- `KillDecisionChecklist` erzeugt pro Vorfall dieselben Review-Schritte, damit eine Kill-Entscheidung nachvollziehbar bleibt.
- `EscalationMatrix` verdichtet Kommunikationsbedarf und naechste Schritte fuer unterschiedlich schwere Vorfaelle.
...
|---|---|---|---|
| `1.0` | `2026-04-22` | `ER` | Erstversion des didaktischen Labs fuer Kill-Entscheidungschecklisten |
...
```mermaid
flowchart TD
    A[Parameter validieren] --> B[IncidentInventory mit modellierten Blocking-Vorfaellen fuellen]
    B --> C[KillDecisionChecklist ueber CROSS APPLY fuer jeden Vorfall ableiten]
    C --> D[EscalationMatrix fuer Kommunikation und naechste Schritte berechnen]
    D --> E[IncidentInventory gefiltert nach blockierten Sessions ausgeben]
    E --> F[KillDecisionChecklist geordnet nach Vorfall und ReviewStep ausgeben]
    F --> G{IncludeEscalationMatrix = 1?}
    G -->|Ja| H[EscalationMatrix priorisiert nach Eskalationsstufe ausgeben]
