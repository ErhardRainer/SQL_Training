# OpenTransactionsBySession.sql

Dieses Skript stellt offene Transaktionen auf Session-Ebene dar und verbindet Alter, Login, Host, Programm, Request-Kontext und Logverbrauch in einer kompakten Diagnose-Sicht. Im Kapitel `19_Transaktions` eignet es sich als Einstieg, wenn nicht nur die Datenbank, sondern der konkrete Session-Besitzer und dessen geschaetzter operativer Impact bewertet werden sollen.
...
| Feld | Wert |
|---|---|
| Script | [OpenTransactionsBySession.sql](OpenTransactionsBySession.sql) |
| Version | `1.0` |
| Typ | `diagnostic-query` |
| Kapitel | `19_Transaktions` |
| Sicherheit | `read-only` |
| Zweck | Zeigt offene Transaktionen pro Session inklusive Alter, Login, Host, ProgramName und didaktischer Impact-Einschaetzung. |
<!-- SQLDOC:SUMMARY_TABLE:END -->

## Einordnung

Viele Diagnosen starten mit Datenbank- oder Log-Sichten. Fuer Incident-Reviews reicht das oft nicht aus, weil der naechste sinnvolle Schritt meist auf Session-, Login- oder Programm-Ebene entschieden wird. Diese Erstversion verdichtet deshalb DMV-Informationen pro Session und verbindet sie mit Blocking- und Log-Kontext, ohne aktive Eingriffe vorzunehmen.

## Annahmen

- Das Skript ist als read-only Diagnosewerkzeug gedacht und fuehrt keine COMMIT-, ROLLBACK- oder KILL-Aktionen aus.
- Die Impact-Stufen `observe`, `medium` und `high` sind bewusst didaktische Priorisierungshilfen statt verbindlicher Produktionsschwellen.
- Die Sicht basiert auf aktuellen DMVs; bereits beendete Requests oder unvollstaendige Zuordnungen koennen daher als Hintergrund- oder Restfaelle erscheinen.
- Fuer produktive Nutzung werden in der Regel Leserechte auf die benoetigten DMVs benoetigt, etwa `VIEW SERVER STATE`.

## Anwendungsfall

Das Skript passt zu Reviews bei laenger offenen Sessions, unerwartetem Logwachstum, Blocking-Ketten oder Fragen wie: Welche Session haelt die Transaktion, ueber welchen Host oder welches Programm kommt sie, und welche Faelle verdienen zuerst Aufmerksamkeit?
...
| `@OnlyUserSessions` | `BIT` | Nein | `1` zeigt nur Benutzer-Sessions, `0` bezieht auch System-Sessions ein. |
| `@IncludeGuide` | `BIT` | Nein | Gibt bei `1` zusaetzlich einen kompakten Leitfaden fuer das Review aus. |
...
- `OpenTransactionsBySession` zeigt die einzelnen offenen Transaktionen mit Session-, Wait- und Statement-Kontext.
- `SessionImpactSummary` priorisiert Sessions nach Alter, Logvolumen und Blocking-Treffern.
- Der Guide uebersetzt die Beobachtungen in Review-Schritte, ohne technische Eingriffe vorwegzunehmen.
...
|---|---|---|---|
| `1.0` | `2026-04-22` | `ER` | Erstversion des Diagnose-Skripts fuer offene Transaktionen pro Session |
...
```mermaid
flowchart TD
    A[Parameter validieren] --> B[Offene Transaktionen aus DMVs mit Session und Request verknuepfen]
    B --> C[Alter, Logvolumen, StatementSnippet und Blocking-Kontext berechnen]
    C --> D[ImpactLevel und EstimatedImpact pro Transaktion ableiten]
    D --> E[SessionImpactSummary pro Session verdichten]
    E --> F[ReviewGuide fuer Priorisierung, Blocking, Logdruck und sleeping Sessions fuellen]
    F --> G[Detail-Resultset OpenTransactionsBySession ausgeben]
    G --> H[Summary-Resultset SessionImpactSummary ausgeben]
    H --> I{IncludeGuide = 1?}
