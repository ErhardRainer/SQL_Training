# BlockingChainWithTranCount.sql

Dieses Skript kombiniert aktuelle Blocking-Ketten mit der Anzahl offener Transaktionen pro Session. So werden nicht nur wartende Requests sichtbar, sondern auch Blocker mit schlafender Session, tiefe Ketten und moegliche Ursachen in Transaktionsgrenzen oder Anwendungscode.
...
| Feld | Wert |
|---|---|
| Script | [BlockingChainWithTranCount.sql](BlockingChainWithTranCount.sql) |
| Version | `1.0` |
| Typ | `diagnostic-query` |
| Kapitel | `19_Transaktions` |
| Sicherheit | `read-only` |
| Zweck | Stellt Blocking-Ketten mit Session-Kontext, offener Transaktionsanzahl und Diagnosehinweisen dar. |
<!-- SQLDOC:SUMMARY_TABLE:END -->

## Einordnung

Bei Blocking-Problemen reicht ein isolierter Blick auf `sys.dm_exec_requests` oft nicht aus. Entscheidend ist, welche Session die Kette an der Wurzel ausloest, wie viele offene Transaktionen dort noch haengen und ob der Blocker aktiv arbeitet oder bereits schlaeft. Das Skript fuehrt diese Perspektiven in einer kompakten Kettensicht zusammen.

## Annahmen

- Das Skript ist als konservative Diagnoseabfrage fuer akute Blocking-Situationen gedacht.
- Die verwendeten DMVs muessen auf der Zielinstanz lesbar sein, typischerweise ueber passende Diagnose- oder Admin-Rechte.
- Offene Transaktionen werden ueber `sys.dm_tran_session_transactions` pro Session gezaehlt.
- Die Kettensicht fokussiert user sessions und schliesst die eigene Session bewusst aus.

## Anwendungsfall

Das Skript eignet sich fuer Troubleshooting waehrend akuter Sperren, fuer Review von langlebigen Transaktionen und als Startpunkt fuer Monitoring-Ideen. Besonders nuetzlich ist es, wenn Teams schnell zwischen eigentlicher Root-Session, betroffenen Requests und moeglichen schlafenden Blockern unterscheiden muessen.
...
|---|---|---|---|
| `@IncludeSleepingBlockers` | `BIT` | Nein | Nimmt bei `1` auch Blocker ohne aktiven Request in die Kettensicht auf. |
| `@MinOpenTransactionCount` | `INT` | Nein | Filtert die Kettensicht auf Sessions mit mindestens dieser Anzahl offener Transaktionen. |
...
- `BlockingChain` zeigt jede Blocking-Kante mit Root-Session, Level, Wait-Kontext und Statement-Ausschnitt.
- `BlockingSummary` verdichtet die Lage pro Blocker-Session und hilft bei der Priorisierung.
- `AlertGuide` markiert typische Diagnosemuster wie schlafende Blocker, mehrere offene Transaktionen oder tiefe Ketten.
...
|---|---|---|---|
| `1.0` | `2026-04-19` | `ER` | Erstversion der Blocking-Ketten-Diagnose mit Transaktionsanzahl |
...
```mermaid
flowchart TD
    A[Parameter validieren] --> B[SessionSnapshot aus Sessions, Requests und offenen Transaktionen aufbauen]
    B --> C[BlockingEdges fuer wartende Sessions mit BlockingSessionID extrahieren]
    C --> D[Roots der Blocking-Ketten ermitteln]
    D --> E[Rekursive BlockingChain mit Level und Statement-Snippets aufbauen]
    E --> F[BlockingSummary pro Blocker verdichten]
    F --> G[AlertGuide fuer sleeping blocker, mehrere offene Transaktionen und tiefe Ketten erzeugen]
    G --> H[BlockingChain ausgeben]
    H --> I[BlockingSummary ausgeben]
    I --> J[AlertGuide ausgeben]
```
<!-- SQLDOC:MERMAID:END -->
