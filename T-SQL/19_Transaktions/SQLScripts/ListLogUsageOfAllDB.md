# ListLogUsageOfAllDB.sql

Dieses Skript listet alle Log-Dateien aller Online-Datenbanken mit aktueller Groesse, DB-weitem Verbrauch, VLF-basierter theoretischer Mindestgroesse und maximal moeglichem Shrink-Potenzial. Zusaetzlich generiert es fertige `DBCC SHRINKFILE`- und `DBCC SHRINKDATABASE`-Befehle, die direkt ausgefuehrt oder als Ausgangspunkt verwendet werden koennen.
...
| Feld | Wert |
|---|---|
| Script | [ListLogUsageOfAllDB.sql](ListLogUsageOfAllDB.sql) |
| Version | `1.0` |
| Typ | `diagnostic-query` |
| Kapitel | `19_Transaktions` |
| Sicherheit | `read-only-tempdb` |
| Zweck | Listet alle Log-Dateien mit VLF-basiertem Shrink-Potenzial und generierten Shrink-Befehlen. |
<!-- SQLDOC:SUMMARY_TABLE:END -->

## Einordnung

Dieses Skript kombiniert drei Datenquellen: `sys.master_files` fuer Datei-Metadaten, `DBCC SQLPERF(LOGSPACE)` fuer DB-weite Belegung und `sys.dm_db_log_info` fuer VLF-Informationen. Aus der VLF-Analyse wird `theoretic_min_size_mb` berechnet – das Ende des letzten aktiven VLF. Dieser Wert ist eine untere Schranke fuer den Shrink-Zielwert, kein Garant. Die generierten Befehle sind Arbeitshilfen, keine fertigen Automatisierungen.

## Annahmen

- `sys.dm_db_log_info` erfordert mindestens SQL Server 2016.
- `theoretic_min_size_mb` basiert auf dem Ende des letzten aktiven VLF; der tatsaechliche Shrink-Effekt haengt von `log_reuse_wait_desc` und aktiven Transaktionen ab.
...
Das Skript eignet sich fuer die vorbeugende Log-Pflege auf einer Instanz, fuer die Vorbereitung von Shrink-Massnahmen mit realistischen Zielwerten und fuer den Ueberblick, welche Datenbanken das groesste Shrink-Potenzial haben.
...
- `theoretic_min_size_mb` ist eine untere Schranke, kein garantierter Shrink-Zielwert.
- Die generierten Shrink-Befehle sind nur ein Ausgangspunkt; vor der Ausfuehrung Reuse-Wait und aktive Transaktionen pruefen.
- `sys.dm_db_log_info` erfordert mindestens SQL Server 2016.
- Sortierung nach `max_shrink_mb` stellt die Datenbanken mit dem groessten Shrink-Potenzial an den Anfang.
...
```mermaid
flowchart TD
    A[DBCC SQLPERF in Temp-Tabelle laden] --> B[CTE LogFiles: alle Log-Dateien aus sys.master_files]
    B --> C[CTE ActiveVlfEnd: Ende des letzten aktiven VLF je Datei aus sys.dm_db_log_info]
    C --> D[JOIN: Datei-Metadaten, SQLPERF-Belegung und VLF-Mindestgroesse zusammenfuehren]
    D --> E[max_shrink_mb berechnen]
    E --> F[SHRINKFILE- und SHRINKDATABASE-Befehle generieren]
    F --> G[Ergebnis sortiert nach Shrink-Potenzial absteigend ausgeben]
    G --> H[Temp-Tabelle beraeumen]
```
<!-- SQLDOC:MERMAID:END -->
