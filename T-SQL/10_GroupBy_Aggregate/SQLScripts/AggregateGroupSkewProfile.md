# AggregateGroupSkewProfile.sql

Dieses didaktische Skript zeigt, wie sich stark ungleich verteilte Untergruppen innerhalb einer groesseren Aggregatgruppe sichtbar machen lassen. Die Demo berechnet pro Region die Anteile der einzelnen `CustomerSegment`-Auspraegungen und leitet daraus kompakte Schiefe-Signale ab.
...
| Feld | Wert |
|---|---|
| Script | [AggregateGroupSkewProfile.sql](AggregateGroupSkewProfile.sql) |
| Version | `1.0` |
| Typ | `didactic-lab` |
| Kapitel | `10_GroupBy_Aggregate` |
| Sicherheit | `read-only-tempdb` |
| Zweck | Profiliert Beitragsanteile je Segment und erkennt dominante Untergruppen innerhalb aggregierter Regionen. |
<!-- SQLDOC:SUMMARY_TABLE:END -->

## Einordnung

Das Artefakt verbindet drei typische Fragen rund um `GROUP BY`:

- Welche Untergruppen bilden innerhalb einer Hauptgruppe den groessten Anteil?
- Wie gross ist der Abstand zwischen dem staerksten und dem kleinsten Segment?
- Ab wann wird eine Gruppe eher als ausgeglichen oder bereits als deutlich verzerrt eingestuft?

Dadurch eignet sich das Skript fuer Unterricht, Datenprofiling und erste Last- oder Konzentrationsanalysen auf Gruppenebene.

## Annahmen

- Die Erstversion nutzt eine lokale Temp-Tabelle `#SalesOrders` statt produktiver Vertriebs- oder Faktendaten.
- Die Hauptgruppe ist bewusst `SalesRegion`; die Schiefe wird ueber die Verteilung von `CustomerSegment` innerhalb jeder Region gemessen.
- Die Klassifikation `dominant_segment`, `moderate_skew` und `balanced` ist didaktisch ueber feste Schwellen modelliert.
- `@MinimumGroupVolume` verhindert, dass sehr kleine Gruppen zu stark interpretiert werden.

## Anwendungsfall

Das Muster passt zu Szenarien, in denen Verteilungen innerhalb aggregierter Gruppen auf Konzentration oder Schraeglagen geprueft werden sollen, etwa bei Kundenstrukturen, Produktmix, Kanalnutzung oder Lastverteilung. In realen Umgebungen kann die Demoquelle spaeter durch Fakt- oder Bewegungsdaten ersetzt werden, waehrend die Aggregate- und Anteilsmuster erhalten bleiben.
...
|---|---|---|---|
| `@DominantShareThreshold` | `DECIMAL(5,4)` | Ja | Schwellwert, ab dem der groesste Segmentanteil als dominant gilt. |
| `@MinimumGroupVolume` | `INT` | Ja | Mindestanzahl an Auftragszeilen je Region fuer eine belastbare Bewertung. |
...
- `SegmentContributionProfile` zeigt pro Region und Segment absolute Mengen und relative Anteile.
- `GroupSkewProfile` verdichtet diese Detailanteile zu einer Regionensicht mit dominantem Segment, groesstem Anteil und Spreizung.
- `SkewClassSummary` zaehlt abschliessend, wie viele Regionen in jede Schiefe-Klasse fallen.
...
|---|---|---|---|
| `1.0` | `2026-04-18` | `ER` | Erstversion eines didaktischen Labs fuer Schiefe-Profile aggregierter Gruppen |
...
flowchart TD
    ...
    B --> C[Demo-Auftraege mit Region Segment und Kanal laden]
    C --> D{ShowOrderPreview = 1?}
    D -->|Ja| E[Einzelauftraege sortiert ausgeben]
    D -->|Nein| F[Direkt zu den Aggregaten wechseln]
    E --> F
    F --> G[RegionTotals und SegmentMetrics per GROUP BY berechnen]
    G --> H[ContributionProfile mit OrderShare und RevenueShare ableiten]
    H --> I[SegmentContributionProfile je Region und Segment ausgeben]
    I --> J[Dominantes Segment und ShareSpread je Region verdichten]
    J --> K[GroupSkewProfile mit SkewClass ausgeben]
    K --> L[Regionen nach SkewClass zaehlen]
```
<!-- SQLDOC:MERMAID:END -->
