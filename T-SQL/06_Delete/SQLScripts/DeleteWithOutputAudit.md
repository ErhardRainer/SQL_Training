# DeleteWithOutputAudit.sql

Dieses Skript demonstriert `DELETE ... OUTPUT` an einer Demo-Retention-Queue. Die geloeschten Zeilen werden dabei nicht nur entfernt, sondern zuerst in einer Capture-Tabelle gesammelt, aus der anschliessend eine Audit-Sicht und eine Undo-Vorschau entstehen.
...
| Feld | Wert |
|---|---|
| Script | [DeleteWithOutputAudit.sql](DeleteWithOutputAudit.sql) |
| Version | `1.0` |
| Typ | `didactic-lab` |
| Kapitel | `06_Delete` |
| Sicherheit | `demo-write-tempdb` |
| Zweck | Demonstriert `DELETE ... OUTPUT` fuer Audit-Protokoll und Undo-Vorschau auf Basis derselben geloeschten Demo-Zeilen. |
<!-- SQLDOC:SUMMARY_TABLE:END -->

## Einordnung

Bei kontrollierten Loeschprozessen reicht es oft nicht, Datensaetze nur zu entfernen. Es muss anschliessend nachvollziehbar bleiben, was geloescht wurde, warum der Vorgang stattfand und wie sich eine Ruecknahme vorbereiten liesse. Genau diese drei Perspektiven verbindet das Skript in einer kleinen, nachvollziehbaren Demo.

## Annahmen

- Die Umsetzung ist eine didaktische Erstversion mit Demo-Daten in `tempdb`.
- Nur Datensaetze vor `@DeleteBeforeDate` ohne `ExportPending` und ohne `IsLegalHold` duerfen in den Delete-Batch.
- `@ExecuteDelete = 0` ist der sichere Startmodus; erst bei `1` werden Demo-Zeilen wirklich geloescht.
- Die Undo-Sicht erzeugt nur eine Vorschau auf Reinsert-Statements mit allen benoetigten Pflichtspalten und fuehrt keine automatische Wiederherstellung aus.

## Anwendungsfall

Das Muster eignet sich fuer Unterricht und Reviews, wenn gezeigt werden soll, wie ein einzelnes `DELETE ... OUTPUT` gleichzeitig die Basis fuer ein Audit-Protokoll und fuer spaetere Undo-Schritte liefern kann. Es ist damit ein gutes Zwischenglied zwischen einfacher Loeschung und kontrollierter Betriebsprozedur.
...
|---|---|---|---|
| `@DeleteBeforeDate` | `DATE` | Nein | Demo-Zeilen mit `RetentionUntil` vor diesem Stichtag kommen fuer die Loeschung in Frage. |
| `@ExecuteDelete` | `BIT` | Nein | `1` fuehrt die Demo-Loeschung aus, `0` zeigt nur Kandidaten und geplante Audit-/Undo-Daten. |
| `@MaxBatchSize` | `INT` | Nein | Begrenzt die Anzahl der in einem Lauf geloeschten Demo-Zeilen. |
| `@DeleteReason` | `NVARCHAR(100)` | Nein | Audit-Grund fuer die Loeschung und die Undo-Dokumentation. |
<!-- SQLDOC:PARAMETERS_TABLE:END -->

## Abhaengigkeiten

<!-- SQLDOC:DEPENDENCIES_LIST:BEGIN -->
- `tempdb` fuer die Demo-Tabellen `#RetentionQueue`, `#DeletedCapture` und `#DeleteAudit`
- `DELETE ... OUTPUT INTO` fuer die einmalige Erfassung geloeschter Zeilen
- CTEs und `ROW_NUMBER()` fuer die lesbare Batch-Auswahl innerhalb der delete-bereiten Kandidaten
- `SYSUTCDATETIME()` und `SUSER_SNAME()` fuer Audit-Zeitpunkt und Ausfuehrungskontext
- `CONCAT` und `CONVERT`, um aus dem OUTPUT-Capture eine Undo-Vorschau abzuleiten
<!-- SQLDOC:DEPENDENCIES_LIST:END -->

## Hinweise

- Die erste Ausgabe zeigt alle Kandidaten inklusive Blockern und Batch-Markierung.
- Nach der echten Demo-Loeschung werden die geloeschten Zeilen aus `#DeletedCapture` in eine eigene Audit-Sicht uebernommen.
- Dieselben Capture-Daten erzeugen eine Undo-Vorschau, damit Audit und Wiederherstellung auf derselben Quelle basieren.
...
|---|---|---|---|
| `1.0` | `2026-04-19` | `ER` | Erstversion fuer DELETE OUTPUT mit Audit- und Undo-Ableitung |
...
```mermaid
flowchart TD
    A[Parameter validieren] --> B[#RetentionQueue #DeletedCapture und #DeleteAudit vorbereiten]
    B --> C[Demo-Daten in die Retention-Queue laden]
    C --> D[DeleteCandidates mit Eligibility und CandidateRank berechnen]
    D --> E[Alle Kandidaten inklusive Batch-Markierung ausgeben]
    E --> F{ExecuteDelete = 1?}
    F -->|Nein| G[Audit und Undo bleiben leer]
    F -->|Ja| H[DeleteBatch per CTE auf due ohne ExportPending und ohne LegalHold begrenzen]
    H --> I[DELETE auf Batch ausfuehren]
    I --> J[OUTPUT schreibt geloeschte Zeilen nach #DeletedCapture]
    J --> K[Audit-Eintraege aus #DeletedCapture ableiten]
    K --> L[UndoPlan aus denselben Capture-Daten erzeugen]
    G --> M[RemainingRetentionQueue und ExecutionGuide ausgeben]
    L --> M
```
<!-- SQLDOC:MERMAID:END -->
