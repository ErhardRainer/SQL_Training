# SafeDeletePreview.sql

Dieses Skript zeigt ein sicheres Preview-Muster fuer Delete-Kandidaten. Vor der optionalen Ausfuehrung werden alle Demo-Zeilen bewertet, Schutzregeln sichtbar gemacht und nur freigegebene Kandidaten fuer eine didaktische Delete-Ausfuehrung in `tempdb` zugelassen.
...
| Feld | Wert |
|---|---|
| Script | [SafeDeletePreview.sql](SafeDeletePreview.sql) |
| Version | `1.0` |
| Typ | `didactic-lab` |
| Kapitel | `06_Delete` |
| Sicherheit | `destructive-demo-tempdb` |
| Zweck | Zeigt Delete-Kandidaten zuerst als Preview und fuehrt nur optional einen didaktischen Delete gegen Demo-Daten aus. |
<!-- SQLDOC:SUMMARY_TABLE:END -->

## Einordnung

Ein `DELETE` sollte vor der Ausfuehrung immer dieselbe Filterlogik sichtbar machen, die spaeter auch fuer die eigentliche Operation gilt. Das Skript trennt deshalb klar zwischen Scope, Schutzregeln und Ausfuehrung und eignet sich als Vorlage fuer Reviews, Dry Runs und Checklisten vor echten Loeschungen.

## Annahmen

- Das Skript arbeitet ausschliesslich mit einer temporaeren Demo-Tabelle in `tempdb`.
- Nur `closed`-Faelle vor dem Stichtag gelten ueberhaupt als Kandidaten.
- Aktive Dispute blockieren die Loeschung immer.
- Hohe Restbetraege werden bei `@ProtectHighValue = 1` zusaetzlich geschuetzt.
- Die Preview- und Delete-Logik verwenden dieselben Kriterien, damit kein Drift zwischen Ansicht und Ausfuehrung entsteht.

## Anwendungsfall

Das Skript passt zu Loeschlaeufen fuer erledigte Arbeitslisten, alte Tickets oder staging-nahe Falltabellen, bei denen vor dem eigentlichen `DELETE` eine belastbare Vorschau benoetigt wird. Es zeigt auch, welche Zeilen wegen Schutzregeln bewusst nicht geloescht werden.
...
|---|---|---|---|
| `@CutoffDate` | `DATE` | Nein | Zeilen mit `DueDate` vor diesem Stichtag gelten als Loeschkandidaten. |
| `@RegionFilter` | `VARCHAR(20)` | Nein | Optionaler Filter fuer eine Region; `NULL` zeigt alle Regionen. |
| `@MaxPreviewRows` | `INT` | Nein | Begrenzt die Anzahl angezeigter Preview-Zeilen. |
| `@ExecuteDelete` | `BIT` | Nein | `0` zeigt nur die Vorschau, `1` fuehrt den didaktischen Delete in `tempdb` aus. |
| `@ProtectHighValue` | `BIT` | Nein | `1` schuetzt Faelle mit hohem `OutstandingAmount` vor der Ausfuehrung. |
<!-- SQLDOC:PARAMETERS_TABLE:END -->

## Abhaengigkeiten

<!-- SQLDOC:DEPENDENCIES_LIST:BEGIN -->
- `tempdb` fuer die Demo-Tabellen `#OpenCases` und `#DeletedRows`
- CTEs `CandidateBase`, `PreviewRows` und `ApprovedRows` fuer Scope, Schutzlogik und Delete-Freigabe
- `CASE` fuer die Entscheidungsgruende in der Vorschau
- `DELETE ... OUTPUT` fuer die nachvollziehbare Demo-Ausfuehrung
- Window-Aggregate fuer Gesamtzahlen direkt in der Preview-Ausgabe
<!-- SQLDOC:DEPENDENCIES_LIST:END -->

## Hinweise

- Die erste Ausgabe zeigt alle relevanten Vorschauinformationen samt Entscheidungsgrund.
- Die zweite Ausgabe verdichtet die Preview pro Region und Entscheidungsgrund.
- Die dritte Ausgabe bleibt leer, solange `@ExecuteDelete = 0` gesetzt ist.
- In einer produktiven Variante sollten zusaetzlich Backup, explizite Transaktion, Logging und Freigabeprozess ergaenzt werden.
...
|---|---|---|---|
| `1.0` | `2026-04-19` | `ER` | Erstversion fuer sicheres Preview-vor-Delete-Labor |
...
```mermaid
flowchart TD
    A[Parameter validieren] --> B[#OpenCases und #DeletedRows vorbereiten]
    B --> C[Demo-Faelle in #OpenCases laden]
    C --> D[CandidateBase markiert Scope ueber Status, Cutoff und Region]
    D --> E[PreviewRows bewertet Schutzregeln und Delete-Freigabe]
    E --> F[DeletePreview mit Gesamtzahlen ausgeben]
    F --> G[DeleteSummary nach Region und Entscheidungsgrund ausgeben]
    G --> H{ExecuteDelete = 1?}
    H -->|Nein| I[DeletedRows bleibt leer]
    H -->|Ja| J[ApprovedRows erneut mit derselben Freigabelogik bilden]
    J --> K[DELETE mit OUTPUT schreibt geloeschte Demo-Zeilen nach #DeletedRows]
    K --> L[DeletedRows ausgeben]
    I --> L
    L --> M[ExecutionGuide mit Safety-Hinweis ausgeben]
```
<!-- SQLDOC:MERMAID:END -->
