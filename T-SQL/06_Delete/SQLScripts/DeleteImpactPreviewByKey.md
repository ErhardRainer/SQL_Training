# DeleteImpactPreviewByKey.sql

Dieses Skript zeigt fuer eine ausgewaehlte `CustomerID`, welche Eltern- und Kindzeilen von einer Loeschung betroffen waeren. Die Umsetzung bleibt bewusst bei einer sicheren Vorschau in `tempdb`, damit sich Delete-Folgen und Blocker ohne Risiko diskutieren lassen.
...
| Feld | Wert |
|---|---|
| Script | [DeleteImpactPreviewByKey.sql](DeleteImpactPreviewByKey.sql) |
| Version | `1.0` |
| Typ | `didactic-lab` |
| Kapitel | `06_Delete` |
| Sicherheit | `read-only-tempdb` |
| Zweck | Zeigt fuer einen Schluessel, welche Eltern- und Kindzeilen von einem Delete betroffen waeren. |
<!-- SQLDOC:SUMMARY_TABLE:END -->

## Einordnung

Vor einem `DELETE` auf Stammdaten reicht die Elternzeile allein selten aus. Offene Rechnungen, Bestellungen, archivierte Historie oder Support-Tickets koennen den Vorgang blockieren oder eine Vorstufe wie Archivierung und Freigabe verlangen. Das Skript bildet genau diese Vorpruefung fuer einen einzelnen Schluessel didaktisch nach.

## Annahmen

- Der Geschaeftsschluessel ist in dieser Erstversion `@CustomerID`.
- Kinddaten werden ueber Demo-Tabellen fuer Bestellungen, Positionen, Rechnungen und Tickets modelliert.
- `@IncludeInactiveChildren = 0` blendet bereits archivierte oder geschlossene Folgesaetze aus, um eine fokussierte Arbeitsliste zu zeigen.
- Das Skript fuehrt kein echtes `DELETE` aus und dient nur als fachlich-technische Vorschau.

## Anwendungsfall

Die Vorschau eignet sich fuer Reviews von Datenbereinigung, DSGVO-Loeschkonzepten oder Stammdatenstilllegung. Statt eine Loeschung direkt auszufuehren, laesst sich zuerst erkennen, welche Folgesaetze noch offene Finanz- oder Support-Aspekte enthalten.
...
|---|---|---|---|
| `@CustomerID` | `INT` | Nein | Demo-Kundenschluessel, fuer den die Delete-Auswirkungen betrachtet werden. |
| `@IncludeInactiveChildren` | `BIT` | Nein | `1` zeigt auch inaktive oder archivierte Kindzeilen, `0` blendet sie aus. |
| `@PreviewMode` | `VARCHAR(20)` | Nein | `summary` liefert nur die Verdichtung, `detail` zeigt zusaetzlich Einzelzeilen. |
<!-- SQLDOC:PARAMETERS_TABLE:END -->

## Abhaengigkeiten

<!-- SQLDOC:DEPENDENCIES_LIST:BEGIN -->
- `tempdb` fuer die Demo-Tabellen `#Customer`, `#SalesOrder`, `#OrderLine`, `#Invoice` und `#SupportTicket`
- CTEs fuer Elternzeile, direkte Kinder, indirekte Kinder und die zusammengefasste Impact-Sicht
- `UNION ALL`, um mehrere betroffene Entitaeten in einer konsistenten Vorschau zusammenzufassen
- `CASE`, um Blocker, manuelle Pruefung und sichere Folgeschritte zu kennzeichnen
- reine `SELECT`-Ausgaben ohne persistente Datenveraenderung
<!-- SQLDOC:DEPENDENCIES_LIST:END -->

## Hinweise

- Eine `CustomerID` mit `CreditState = hold` wird als `manual-review` markiert, selbst wenn der Kunde bereits inaktiv ist.
- Offene Rechnungen werden als staerkster fachlicher Blocker behandelt.
- Detailausgaben helfen dabei, die Auswirkungen pro betroffener Entitaet einzeln nachzuvollziehen.
...
|---|---|---|---|
| `1.0` | `2026-04-17` | `ER` | Erstversion fuer eine didaktische Delete-Impact-Vorschau nach Schluessel |
...
flowchart TD
    ...
    B --> C{CustomerID in Demo-Daten vorhanden?}
    C -->|Nein| D[THROW fuer unbekannte CustomerID]
    C -->|Ja| E[DeleteKeyPreview fuer die Elternzeile bilden]
    E --> F[OrderCandidates, InvoiceCandidates, TicketCandidates und LineCandidates ableiten]
    F --> G[DeleteImpactSummary je Entitaet mit Aktion und Zeilenanzahl verdichten]
    G --> H[DeleteKeyPreview und DeleteImpactSummary ausgeben]
    H --> I{PreviewMode = detail?}
    I -->|Ja| J[DeleteImpactDetail mit Einzelzeilen ausgeben]
    I -->|Nein| K[Detailausgabe ueberspringen]
    J --> L[ExecutionGuide mit Empfehlung und SafetyNote ausgeben]
    K --> L
```
<!-- SQLDOC:MERMAID:END -->
