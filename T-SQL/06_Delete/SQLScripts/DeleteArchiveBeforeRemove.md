# DeleteArchiveBeforeRemove.sql

Dieses Skript zeigt ein Muster, bei dem Loeschkandidaten nicht einfach verschwinden, sondern vor dem `DELETE` in eine Archivtabelle geschrieben werden. Die Demo nutzt dafuer `DELETE ... OUTPUT`, damit Archivierung und Entfernung in einem atomaren Schritt zusammenbleiben.
...
| Feld | Wert |
|---|---|
| Script | [DeleteArchiveBeforeRemove.sql](DeleteArchiveBeforeRemove.sql) |
| Version | `1.0` |
| Typ | `didactic-lab` |
| Kapitel | `06_Delete` |
| Sicherheit | `destructive-demo-tempdb` |
| Zweck | Archiviert Loeschkandidaten atomar vor dem endgueltigen Entfernen in eine Sicherungstabelle. |
<!-- SQLDOC:SUMMARY_TABLE:END -->

## Einordnung

Das Muster ist fuer Faelle gedacht, in denen ein `DELETE` fachlich erlaubt ist, die betroffenen Zeilen aber fuer Nachweis, Support oder moegliche Wiederherstellung zunaechst gesichert werden sollen. Statt zwei getrennte Schritte zu bauen, fuehrt die Demo Archivierung und Loeschung ueber denselben DML-Vorgang zusammen.

## Annahmen

- Das Skript arbeitet ausschliesslich mit temporaeren Demo-Tabellen in `tempdb`.
- `DELETE ... OUTPUT` dient hier als didaktischer Kernmechanismus fuer atomisches Archivieren vor dem Entfernen.
- `closed` und `cancelled` sind die Statuswerte, die in der Demo standardmaessig als loeschbar gelten.
- Ein produktiver Restore-Pfad wird nicht umgesetzt, aber die Archivtabelle behaelt alle fuer eine spaetere Ruecksicherung relevanten Bestellfelder.

## Anwendungsfall

Das Skript eignet sich als Startpunkt fuer Retention-Loeschungen, Storno-Archive oder fachliche Bereinigungen, bei denen vor dem eigentlichen `DELETE` ein nachvollziehbarer Sicherungspunkt benoetigt wird. Im Preview-Modus lassen sich Kandidaten und Filterlogik gefahrlos pruefen.
...
|---|---|---|---|
| `@DeleteBeforeDate` | `DATE` | Nein | Bestellungen mit `RequestedShipDate` vor diesem Datum gelten als Loeschkandidaten. |
| `@ArchiveReason` | `NVARCHAR(120)` | Nein | Dokumentiert den fachlichen Grund fuer Archivierung und Delete. |
| `@PreviewOnly` | `BIT` | Nein | `1` zeigt nur Kandidaten und Plan, `0` archiviert und loescht in der Demo. |
| `@IncludeClosedOnly` | `BIT` | Nein | `1` beschraenkt die Loeschung auf `closed` oder `cancelled`. |
<!-- SQLDOC:PARAMETERS_TABLE:END -->

## Abhaengigkeiten

<!-- SQLDOC:DEPENDENCIES_LIST:BEGIN -->
- `tempdb` fuer die Demo-Tabellen `#SalesOrder` und `#OrderDeleteArchive`
- explizite Transaktion fuer einen gemeinsamen Commit von Archivierung und Delete
- `TRY...CATCH` und `XACT_ABORT` fuer sauberes Rollback im Fehlerfall
- `DELETE ... OUTPUT` als Kernmuster fuer das Archivieren der geloeschten Zeilen
- `SYSUTCDATETIME` und `SUSER_SNAME` fuer Archivmetadaten
<!-- SQLDOC:DEPENDENCIES_LIST:END -->

## Hinweise

- Im Preview-Modus bleibt die Archivtabelle leer; die erste Resultset-Sicht markiert nur die Kandidaten.
- Im Ausfuehrungsmodus landen nur die tatsaechlich geloeschten Zeilen im Archiv, wodurch keine Drift zwischen Archiv und Delete-Menge entsteht.
- `ArchiveReason` ist bewusst ein eigener Parameter, damit die fachliche Ursache des Deletes spaeter nicht nur aus Tabellenfiltern rekonstruiert werden muss.
- Die Demo trennt `DeleteCandidates`, `ArchiveResult` und `RemainingOrders`, damit Vorher/Nachher sowie Sicherung klar lesbar bleiben.
...
|---|---|---|---|
| `1.0` | `2026-04-17` | `ER` | Erstversion fuer atomisches Archivieren vor dem DELETE in tempdb |
...
```mermaid
flowchart TD
    A[Parameter validieren] --> B[#SalesOrder und #OrderDeleteArchive aufbauen]
    B --> C[DeleteCandidates mit IsDeleteCandidate ausgeben]
    C --> D{PreviewOnly?}
    D -->|Ja| E[Archiv und Zielbestand unveraendert lassen]
    D -->|Nein| F[Transaktion fuer Archivierung und Delete starten]
    F --> G[DELETE mit OUTPUT schreibt geloeschte Zeilen in #OrderDeleteArchive]
    G --> H{Fehler im TRY-Block?}
    H -->|Nein| I[COMMIT der Transaktion]
    H -->|Ja| J[ROLLBACK und THROW]
    E --> K[ArchiveResult ausgeben]
    I --> K
    K --> L[RemainingOrders nach der optionalen Ausfuehrung ausgeben]
