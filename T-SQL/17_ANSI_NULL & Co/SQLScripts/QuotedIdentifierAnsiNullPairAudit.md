# QuotedIdentifierAnsiNullPairAudit.sql

Dieses Skript auditiert didaktisch die gemeinsam auftretenden Kombinationen aus `ANSI_NULLS` und `QUOTED_IDENTIFIER` in einem kleinen Legacy-Modulinventar. Es verdichtet beobachtete Paar-Profile, markiert riskante Mischungen und leitet eine pragmatische Reihenfolge fuer Review und Modernisierung ab.
...
| Feld | Wert |
|---|---|
| Script | [QuotedIdentifierAnsiNullPairAudit.sql](QuotedIdentifierAnsiNullPairAudit.sql) |
| Version | `1.0` |
| Typ | `didactic-lab` |
| Kapitel | `17_ANSI_NULL & Co` |
| Sicherheit | `read-only-tempdb` |
| Zweck | Auditiert gemeinsam auftretende Paare aus `ANSI_NULLS` und `QUOTED_IDENTIFIER` in Legacy-Modulen. |
<!-- SQLDOC:SUMMARY_TABLE:END -->

## Einordnung

Im Review von Altmodulen ist oft nicht die einzelne SET-Option das Problem, sondern ihre Kombination. `ANSI_NULLS OFF` und `QUOTED_IDENTIFIER OFF` tauchen haeufig aus unterschiedlichen historischen Gruenden auf, bilden zusammen aber ein klares Modernisierungssignal. Das Skript macht diese Paar-Sicht explizit und verdichtet sie fuer Priorisierung und Teamabsprachen.

## Annahmen

- Es handelt sich um eine didaktische Erstversion mit einem modellierten Legacy-Inventar statt produktiver Metadatenabfragen.
- `ON/ON` gilt als bevorzugte Baseline fuer neue oder bereinigte Module.
- `OFF/ON` und `ON/OFF` werden als unterschiedliche Review-Probleme behandelt, weil sie verschiedene technische Altlasten anzeigen.
- `OFF/OFF` gilt als kumuliertes Risiko und wird deshalb separat als Doppel-Legacy-Profil priorisiert.

## Anwendungsfall

Das Skript eignet sich fuer Unterricht, Code-Review und Migrationsplanung, wenn Altmodule schrittweise an eine konsistente SET-Options-Baseline angepasst werden sollen. Besonders hilfreich ist es, um nicht nur einzelne Module, sondern wiederkehrende Paar-Profile ueber ein ganzes Inventar hinweg sichtbar zu machen.
...
|---|---|---|---|
| `@FlagOnlyRiskPairs` | `BIT` | Nein | Zeigt bei `1` nur Module und Paar-Profile mit erhoehtem Risiko. |
| `@IncludePairSummary` | `BIT` | Nein | Gibt bei `1` zusaetzlich die Verdichtung pro Options-Paar aus. |
...
- `ModulePairAudit` bewertet jedes beobachtete Modul anhand seines Options-Paars und leitet einen klaren Review-Fokus ab.
- `PairProfileSummary` zeigt, welche Paarung im Inventar wie oft vorkommt und wo die groessten Modernisierungsschwerpunkte liegen.
- Der Remediation-Guide uebersetzt die Paar-Sicht in eine sinnvolle Reihenfolge fuer Cleanup und Regressionstests.
...
|---|---|---|---|
| `1.0` | `2026-04-19` | `ER` | Erstversion des didaktischen Audits fuer ANSI_NULLS- und QUOTED_IDENTIFIER-Paare |
...
```mermaid
flowchart TD
    A[Parameter validieren] --> B[Soll-Paare fuer ANSI_NULLS und QUOTED_IDENTIFIER definieren]
    B --> C[Didaktisches Legacy-Modulinventar aufbauen]
    C --> D[Per CROSS APPLY PairCode je Modul aus ON/OFF-Kombination ableiten]
    D --> E[ModulePairAudit mit Risiko, ReviewFocus und Action fuellen]
    E --> F[PairProfileSummary per GROUP BY verdichten]
    F --> G[RemediationGuide fuer Baseline, Risk Review und Migration erzeugen]
    G --> H[ModulePairAudit ausgeben]
    H --> I{IncludePairSummary = 1?}
    I -->|Ja| J[PairProfileSummary ausgeben]
    I -->|Nein| K[Direkt zum RemediationGuide weitergehen]
