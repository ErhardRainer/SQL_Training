# AnsiNullDefaultInventory.sql

Dieses Skript inventarisiert didaktisch die Varianten rund um `ANSI_NULL_DFLT_ON` und `ANSI_NULL_DFLT_OFF`. Die SQL-Datei zeigt, wie sich weggelassene `NULL`- oder `NOT NULL`-Angaben in DDL-Szenarien auswirken und welche Guardrails fuer konsistente Deployments sinnvoll sind.
...
| Feld | Wert |
|---|---|
| Script | [AnsiNullDefaultInventory.sql](AnsiNullDefaultInventory.sql) |
| Version | `1.0` |
| Typ | `didactic-lab` |
| Kapitel | `17_ANSI_NULL & Co` |
| Sicherheit | `read-only-tempdb` |
| Zweck | Inventarisiert ANSI_NULL_DFLT-Varianten und erklaert die Auswirkung ausgelassener Nullability in DDL. |
<!-- SQLDOC:SUMMARY_TABLE:END -->

## Einordnung

Das Thema ist didaktisch interessant, weil dieselbe `CREATE TABLE`-Anweisung je nach Session-Default unterschiedlich interpretiert werden kann, sobald `NULL` oder `NOT NULL` in der Spaltendefinition fehlen. Das Skript arbeitet deshalb mit nachvollziehbaren Szenarien statt produktive Objekte anzulegen oder zu veraendern.

## Annahmen

- Es handelt sich um eine didaktische Erstversion ohne produktive Schemaaenderungen.
- Die Auswirkungen von `ANSI_NULL_DFLT_*` werden ueber Beispiel-DDL und Entscheidungshilfen erklaert, nicht ueber echte Tabellenmigrationen.
- Explizite `NULL`- oder `NOT NULL`-Angaben gelten als wichtigste Guardrail gegen abweichende Tool- und Session-Defaults.

## Anwendungsfall

Das Skript eignet sich fuer Unterricht, Reviews und Deployment-Diskussionen rund um DDL-Konventionen. Es hilft dabei, implizite Default-Annahmen sichtbar zu machen und daraus robuste Teamregeln fuer Tabellenentwuerfe abzuleiten.
...
|---|---|---|---|
| `@ShowOptionMatrix` | `BIT` | Nein | Gibt bei `1` das Inventar der relevanten ANSI_NULL_DFLT-Varianten aus. |
| `@ShowExampleScenarios` | `BIT` | Nein | Gibt bei `1` didaktische CREATE-TABLE-Szenarien mit effektiver Nullability aus. |
...
- `ANSI_NULL_DFLT_ON` und `ANSI_NULL_DFLT_OFF` werden hier als Einflussgroessen fuer ausgelassene Nullability behandelt.
- Die Beispielszenarien zeigen bewusst, dass dieselbe Spaltendefinition ohne explizites `NULL` oder `NOT NULL` je Umgebung unterschiedliche Folgen haben kann.
- Der Entscheidungsleitfaden priorisiert lesbare und reviewbare DDL vor impliziten Session-Annahmen.
...
|---|---|---|---|
| `1.0` | `2026-04-19` | `ER` | Erstversion des didaktischen ANSI_NULL_DFLT-Inventars |
...
```mermaid
flowchart TD
    A[Parameter validieren] --> B[Inventar fuer ANSI_NULL_DFLT Varianten aufbauen]
    B --> C[Beispielmatrix fuer CREATE TABLE Szenarien fuellen]
    C --> D[Entscheidungsleitfaden fuer Reviews und Deployments erzeugen]
    D --> E{ShowOptionMatrix = 1?}
    E -->|Ja| F[OptionInventory ausgeben]
    E -->|Nein| G{ShowExampleScenarios = 1?}
    F --> G
    G -->|Ja| H[ExampleScenarioMatrix ausgeben]
    G -->|Nein| I[DecisionGuide vorbereiten]
    H --> I
    I --> J[DecisionGuide ausgeben]
```
<!-- SQLDOC:MERMAID:END -->
