# ArithmeticAbortSettingReview.sql

Dieses Skript reviewt `ARITHABORT` anhand didaktischer Workload-Szenarien und macht sichtbar, welche praktischen Folgen die Einstellung fuer Fehlersignale, Planvergleich und Feature-Voraussetzungen haben kann. Die Erstversion arbeitet bewusst mit modellierten Review-Situationen statt mit produktiven Sessions oder echten Objektdefinitionen.
...
| Feld | Wert |
|---|---|
| Script | [ArithmeticAbortSettingReview.sql](ArithmeticAbortSettingReview.sql) |
| Version | `1.0` |
| Typ | `didactic-lab` |
| Kapitel | `17_ANSI_NULL & Co` |
| Sicherheit | `read-only-tempdb` |
| Zweck | Reviewt typische ARITHABORT-Szenarien fuer Fehlersignale, Planvergleich und Feature-Guardrails. |
<!-- SQLDOC:SUMMARY_TABLE:END -->

## Einordnung

`ARITHABORT` wird in der Praxis oft erst dann bemerkt, wenn Fehlersignale, Performance-Vergleiche oder feature-nahe Deployments unerwartet unterschiedlich wirken. Das Skript strukturiert diese Beobachtungen als Review-Material und leitet daraus eine kompakte Entscheidungshilfe fuer Schulung, Troubleshooting und Deployment-Standards ab.

## Annahmen

- Es handelt sich um eine didaktische Erstversion mit modellierten Szenarien statt echter Session-Umschaltungen.
- Der Fokus liegt auf Review- und Diagnosefolgen von `ARITHABORT`, nicht auf einer vollstaendigen Produktsimulation aller Engine-Details.
- Feature-Beispiele werden bewusst auf Guardrails und Voraussetzungen reduziert, ohne produktive DDL-Aktionen auszufuehren.

## Anwendungsfall

Das Skript eignet sich fuer Unterricht, Review-Checklisten und Troubleshooting-Workshops. Es hilft dabei, `ARITHABORT` nicht isoliert zu betrachten, sondern als Teil einer reproduzierbaren Session-Baseline fuer Arithmetic-Fehler, Performance-Vergleiche und Feature-Voraussetzungen.
...
|---|---|---|---|
| `@OnlyRiskyScenarios` | `BIT` | Nein | Zeigt bei `1` nur Szenarien mit hohem Review-Risiko. |
| `@IncludeDecisionGuide` | `BIT` | Nein | Gibt bei `1` zusaetzlich einen Entscheidungs- und Guardrail-Guide aus. |
...
- `ScenarioInventory` sammelt typische ARITHABORT-bezogene Review-Situationen mit Risiko und Leitfrage.
- `BehaviorReview` stellt gegenueber, wie sich die Betrachtung bei `ARITHABORT ON` und `ARITHABORT OFF` verschiebt.
- Der `DecisionGuide` uebersetzt die Szenarien in Guardrails fuer Session-Baselines, Tests und Deployments.
...
|---|---|---|---|
| `1.0` | `2026-04-19` | `ER` | Erstversion des didaktischen ARITHABORT-Review-Skripts |
...
```mermaid
flowchart TD
    A[Parameter validieren] --> B[ScenarioInventory mit ARITHABORT-Szenarien fuellen]
    B --> C[BehaviorReview fuer ON und OFF aus ScenarioInventory ableiten]
    C --> D[DecisionGuide fuer Baseline, Tests und Deployments aufbauen]
    D --> E[ScenarioInventory nach Risiko filtern und ausgeben]
    E --> F[BehaviorReview ausgeben]
    F --> G{IncludeDecisionGuide = 1?}
    G -->|Ja| H[DecisionGuide ausgeben]
