# JoinSelectivityEstimator.sql

Dieses Skript schaetzt im Kapitel `03_JOIN`, wie selektiv typische Join-Pfade auf einer kleinen Demobasis wirken. Es kombiniert Distinct-Werte je Join-Key mit beobachteten Trefferzahlen, Coverage und maximaler Mehrfachheit pro linker Zeile.
...
| Feld | Wert |
|---|---|
| Script | [JoinSelectivityEstimator.sql](JoinSelectivityEstimator.sql) |
| Version | `1.0` |
| Typ | `didactic-lab` |
| Kapitel | `03_JOIN` |
| Sicherheit | `read-only-tempdb` |
| Zweck | Schaetzt die Selektivitaet typischer Join-Pfade ueber Rowcounts, Distinct-Werte und beobachtete Trefferraten. |
<!-- SQLDOC:SUMMARY_TABLE:END -->

## Einordnung

Im Join-Kapitel ist dieses Skript als Diagnose-Bruecke zwischen einfachem Join-Verstaendnis und spaeteren Fanout- oder Grain-Analysen gedacht. Es beantwortet nicht nur, ob ein Join funktioniert, sondern wie stark ein Pfad expandiert und wie gut er linke Zeilen abdeckt.

## Annahmen

- Die Demo nutzt nur tempdb-Objekte und kleine Trainingsdaten statt produktiver Tabellen.
- `OrdersToCustomers` modelliert absichtlich einen lueckenhaften Lookup ueber einen fehlenden `CustomerID`.
- `OrdersToOrderLines` und `CustomersToOrders` zeigen typische fachliche `1:n`-Pfade.
- `OrderLinesToEvents` dient als bewusst fanout-starker Prozesspfad mit mehreren Ereignissen pro Position.

## Anwendungsfall

Die erste Ausgabe listet Kennzahlen pro Join-Pfad, darunter Distinct-Keys, geschaetzte und beobachtete Treffer pro linker Zeile sowie Coverage und Maximal-Fanout. Die zweite Ausgabe uebersetzt diese Zahlen in didaktische Review-Signale. Optional folgt ein drittes Resultset mit konkreten Beispielzeilen.
...
|---|---|---|---|
| `@OnlyRiskyPaths` | `BIT` | Nein | Zeigt bei `1` nur Pfade mit Coverage- oder Fanout-Risiko. |
| `@MinActualMatchesPerLeftRow` | `DECIMAL(10,2)` | Nein | Untergrenze fuer beobachtete Treffer pro linker Zeile. |
| `@IncludeSampleRows` | `BIT` | Nein | Steuert ein zusaetzliches Resultset mit Beispieldaten pro Join-Pfad. |
...
- `EstimatedMatchesPerLeftRow` basiert auf der rechten Mehrfachheit pro Distinct-Key und ist bewusst eine grobe Heuristik.
- `ActualMatchesPerLeftRow` misst die tatsaechlich erzeugten Join-Zeilen pro linker Zeile.
- `MatchCoveragePct` macht sichtbar, wie viele linke Zeilen ueberhaupt einen Treffer finden.
- `RiskSignal` trennt Coverage-Luecken von Fanout-Hotspots und kombiniert beide Effekte, wenn sie gemeinsam auftreten.
...
|---|---|---|---|
| `1.0` | `2026-04-19` | `ER` | Erstversion fuer eine didaktische Schaetzung von Join-Selektivitaet |
...
```mermaid
flowchart TD
    A[Parameter validieren] --> B[Temp-Tabellen fuer Customers Orders OrderLines und Events aufbauen]
    B --> C[Demodaten mit Lookup 1:n und Prozesspfaden laden]
    C --> D[PathCatalog fuer vier Join-Pfade definieren]
    D --> E[Je Pfad Rowcounts Distinct-Keys Shared-Keys und Matches berechnen]
    E --> F[EstimatedMatchesPerLeftRow und ActualMatchesPerLeftRow ableiten]
    F --> G[Coverage MaxMatches SelectivityBand und RiskSignal bestimmen]
    G --> H[Pfad-Summary nach Parametern filtern und ausgeben]
    H --> I[Review-Signale und EstimationComment ableiten]
    I --> J{IncludeSampleRows = 1?}
    J -->|Ja| K[Beispielzeilen fuer Lookup-Luecken und Fanout-Pfade ausgeben]
