# BridgeTableCoverageAudit.sql

Dieses Skript prueft im Kapitel `03_JOIN` eine didaktische n:m-Zuordnung zwischen Studierenden und Modulen. Es kontrastiert eine explizit erwartete Paarmenge mit den realen Bridge-Eintraegen, damit fehlende oder doppelte Zuordnungen vor dem eigentlichen Join sichtbar werden.
...
| Feld | Wert |
|---|---|
| Script | [BridgeTableCoverageAudit.sql](BridgeTableCoverageAudit.sql) |
| Version | `1.0` |
| Typ | `didactic-lab` |
| Kapitel | `03_JOIN` |
| Sicherheit | `read-only-tempdb` |
| Zweck | Prueft eine didaktische n:m-Zwischentabelle auf fehlende, doppelte und optional unerwartete Zuordnungen. |
<!-- SQLDOC:SUMMARY_TABLE:END -->

## Einordnung

Bridge-Tabellen sind die typische technische Form fuer n:m-Beziehungen. Bevor ein `JOIN` auf dieser Struktur fachlich interpretiert wird, sollte klar sein, ob erwartete Paare fehlen oder ob Duplikate dieselbe Beziehung mehrfach abbilden. Genau diese Vorpruefung kapselt das Skript in einem kleinen tempdb-Labor.

## Annahmen

- Die Trainingsdaten modellieren Studierende, Module und eine erwartete Referenzmenge fuer verpflichtende Modulzuordnungen.
- `missing` bedeutet: ein erwartetes Paar existiert gar nicht in der Bridge-Tabelle.
- `duplicate` bedeutet: ein erwartetes Paar existiert mehrfach und kann spaeter zu Duplikat-Explosionen im Join fuehren.
- `unexpected` ist kein harter Fehler, sondern eine zusaetzliche Diagnose fuer reale Zuordnungen ausserhalb der Referenzmenge.

## Anwendungsfall

Die Detailausgabe zeigt fuer jedes erwartete Paar den Coverage-Status und die Zahl realer Bridge-Zeilen. Die Zusammenfassung verdichtet diese Signale pro Jahrgang. Optional erscheint ein drittes Resultset fuer tatsaechliche Zuordnungen, die nicht in der Erwartungsmenge hinterlegt sind.
...
|---|---|---|---|
| `@CohortFilter` | `NVARCHAR(20)` | Nein | Optionaler Filter auf einen Jahrgang oder Kursblock. |
| `@OnlyIssues` | `BIT` | Nein | Zeigt bei `1` nur auffaellige Zuordnungen, bei `0` auch gesunde Erwartungspaare. |
| `@IncludeUnexpectedAssignments` | `BIT` | Nein | Steuert ein zusaetzliches Resultset fuer reale, aber nicht erwartete Bridge-Zuordnungen. |
...
- Die Bridge-Pruefung arbeitet bewusst gegen eine explizite Erwartungsmenge, damit die Begriffe `missing` und `duplicate` fachlich klar bleiben.
- Doppelte Bridge-Zeilen koennen spaeter in normalen Joins zu ueberzaehlten Summen, mehrfachen Trefferzeilen oder scheinbar unerwarteter Kardinalitaet fuehren.
- Unerwartete Zuordnungen sind oft der erste Hinweis auf eine fehlerhafte Referenzliste, aber manchmal auch auf legitime Zusatzbelegungen ausserhalb des Standardpfads.
...
|---|---|---|---|
| `1.0` | `2026-04-17` | `ER` | Erstversion fuer die didaktische Bridge-Table-Coverage-Pruefung |
...
```mermaid
flowchart TD
    A[Parameter einlesen] --> B{OnlyIssues und IncludeUnexpectedAssignments gueltig?}
    B -->|Nein| C[THROW Fehler]
    B -->|Ja| D[Temp-Tabellen fuer Students Modules ExpectedBridge und StudentModuleBridge aufbauen]
    D --> E[Demodaten fuer erwartete und reale Zuordnungen laden]
    E --> F[ExpectedBridge optional nach Cohort filtern]
    F --> G[ActualBridge pro Student Modul aggregieren]
    G --> H[CoverageAudit mit missing duplicate covered ableiten]
    H --> I[BridgeCoverageAudit ausgeben]
    I --> J[BridgeCoverageSummary pro Cohort und Status aggregieren]
    J --> K{UnexpectedAssignments anzeigen?}
    K -->|Nein| L[Ende]
    K -->|Ja| M[Reale aber nicht erwartete Paare ermitteln]
    M --> L
```
<!-- SQLDOC:MERMAID:END -->
