...
Dieses Skript zeigt im Kapitel `03_JOIN`, wie verwaiste Kinddatensaetze auf einer kleinen tempdb-Demobasis gefunden werden. Die gleiche Orphan-Suche wird bewusst einmal mit `LEFT JOIN ... IS NULL` und einmal mit `NOT EXISTS` formuliert.
...
| Feld | Wert |
|---|---|
| Script | [FindOrphanRows.sql](FindOrphanRows.sql) |
| Version | `1.0` |
| Typ | `didactic-lab` |
| Kapitel | `03_JOIN` |
| Sicherheit | `read-only-tempdb` |
| Zweck | Findet verwaiste Kinddatensaetze und kontrastiert `LEFT JOIN ... IS NULL` mit `NOT EXISTS`. |
<!-- SQLDOC:SUMMARY_TABLE:END -->

## Einordnung

Verwaiste Zeilen entstehen typischerweise dann, wenn ein Fremdschluesselwert keinen passenden Stammdatensatz mehr hat oder nie hatte. In Trainings- und Diagnosekontexten ist genau diese Pruefung oft der erste Schritt, bevor weitere `JOIN`s oder Aggregationen auf den Daten ausgefuehrt werden.

## Annahmen

- Die Demobasis nutzt `#Customers` als Stammdaten und `#Orders` als Kindmenge.
- Als verwaist gilt hier jede Bestellung, deren `CustomerID` keinen Treffer in `#Customers` hat oder selbst `NULL` ist.
- Inaktive Kunden bleiben in diesem Skript bewusst gueltige Stammdatensaetze; der Orphan-Begriff bezieht sich nur auf fehlende Matches.
- Die optionale Vergleichsausgabe dient nur dazu, die Gleichwertigkeit der beiden Suchmuster auf derselben Datenmenge sichtbar zu machen.

## Anwendungsfall

Die ersten beiden Resultsets liefern dieselbe Orphan-Menge mit zwei unterschiedlichen SQL-Mustern. Das dritte Resultset verdichtet die Treffer pro Vertriebskanal. Optional zeigt ein viertes Resultset, ob beide Muster tatsaechlich dieselben `OrderID`s liefern.
...
|---|---|---|---|
| `@MinOrderDate` | `DATE` | Nein | Optionales Untergrenzen-Datum fuer die betrachteten Bestellungen. |
| `@MinimumOrderAmount` | `DECIMAL(10,2)` | Nein | Optionaler Mindestbetrag fuer die betrachteten Bestellungen. |
| `@IncludePatternComparison` | `BIT` | Nein | Steuert ein zusaetzliches Vergleichs-Resultset fuer `LEFT JOIN` und `NOT EXISTS`. |
...
- `LEFT JOIN ... IS NULL` ist oft sehr anschaulich, weil der fehlende Match direkt sichtbar wird.
- `NOT EXISTS` eignet sich besonders gut, wenn nur die Existenz oder Nicht-Existenz eines passenden Datensatzes geprueft werden soll.
- `NULL` in der Kindspalte ist hier absichtlich ebenfalls ein Orphan-Signal, weil kein gueltiger Stammdatensatz referenziert wird.
- Die Zusammenfassung zeigt, dass Orphan-Checks auch als vorgelagerte Datenqualitaetspruefung pro Segment nutzbar sind.
...
|---|---|---|---|
| `1.0` | `2026-04-17` | `ER` | Erstversion fuer die didaktische Orphan-Row-Suche mit `LEFT JOIN` und `NOT EXISTS` |
...
flowchart TD
    ...
    E --> F[ScopedOrders nach Datum und Mindestbetrag filtern]
    F --> G[OrphansLeftJoin per LEFT JOIN IS NULL ableiten]
    F --> H[OrphansNotExists per NOT EXISTS ableiten]
    F --> I[OrphanSummary je SalesChannel aggregieren]
    G --> J[Resultset OrphanRowsLeftJoin]
    H --> K[Resultset OrphanRowsNotExists]
    I --> L[Resultset OrphanSummary]
    L --> M{PatternComparison aktiv?}
    M -->|Ja| N[FULL OUTER JOIN auf beiden Orphan-Mengen]
    M -->|Nein| O[Ende]
    N --> O
```
<!-- SQLDOC:MERMAID:END -->
