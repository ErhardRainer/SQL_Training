# JoinSurvivorRowPreview.sql

Dieses Skript zeigt im Kapitel `03_JOIN`, welche Bestellzeilen nach mehreren fachlichen Join- und Filterstufen wirklich uebrig bleiben. Statt nur den finalen Join zu betrachten, macht es die schrittweise Reduktion von der Grundmenge bis zu den echten Survivor Rows sichtbar.
...
| Feld | Wert |
|---|---|
| Script | [JoinSurvivorRowPreview.sql](JoinSurvivorRowPreview.sql) |
| Version | `1.0` |
| Typ | `didactic-lab` |
| Kapitel | `03_JOIN` |
| Sicherheit | `read-only-tempdb` |
| Zweck | Zeigt, welche Bestellzeilen nach Status-, Rechnungs-, Versand- und Rueckgabepruefungen als Survivor Rows im komplexen Join verbleiben. |
<!-- SQLDOC:SUMMARY_TABLE:END -->

## Einordnung

Bei komplexeren Joins ist die wichtigste Frage oft nicht, wie viele Treffer technisch moeglich sind, sondern welche linke Zeilenmenge nach allen Zusatzbedingungen fachlich noch gueltig bleibt. Das Skript baut deshalb zuerst eine Basis aus Bestellzeilen auf und bewertet danach in separaten Stufen Order-Status, Rechnungsabdeckung, primaeren Versandtreffer und Rueckgabezustand.

## Annahmen

- Die Demo-Daten modellieren Bestellzeilen als linke Ausgangsmenge mit exakt einem erwarteten Rechnungsdatensatz pro Zeile.
- Der Versand-Join betrachtet nur Treffer mit `ShipmentRole = primary`; Backup- oder Audit-Zeilen zaehlen nicht als Survivor-Nachweis.
- Survivor Rows duerfen nur aus Orders mit Status `Released` oder `Shipped` stammen.
- Rueckgaben mit Status `pending` gelten als noch nicht freigegebene Zeilen und fallen deshalb aus der Survivor-Menge heraus.

## Anwendungsfall

Die erste Ausgabe listet nur die Zeilen, die alle Pruefstufen ueberstehen. Die zweite Ausgabe zeigt pro Schritt, wie stark die Menge schrumpft. Optional liefert ein drittes Resultset die ausgeschlossenen Zeilen samt dominanter Ausschlussursache, sodass die Wirkung einzelner Join-Bedingungen direkt sichtbar wird.
...
| `@MinimumInvoiceCoverage` | `DECIMAL(5,2)` | Nein | Legt die Mindestabdeckung fuer eine als ausreichend betrachtete Rechnungszuordnung fest. |
| `@IncludeExcludedRows` | `BIT` | Nein | Steuert ein zusaetzliches Resultset mit ausgeschlossenen Zeilen und Ausschlussgrund. |
...
- `JoinStepSummary` zeigt nicht nur die verbleibende Menge, sondern auch den Verlust pro Stufe.
- `PrimaryOutcome` markiert die erste Bedingung, an der eine Zeile aus dem Survivor-Pfad herausfaellt.
- `EvaluationPath` ist absichtlich lesbar gehalten, damit der komplette Entscheidungsweg ohne Debugging nachvollzogen werden kann.
...
|---|---|---|---|
| `1.0` | `2026-04-17` | `ER` | Erstversion fuer die Vorschau auf Survivor Rows nach einem komplexen Join |
...
```mermaid
flowchart TD
    A[Parameter einlesen] --> B{InvoiceCoverage und IncludeExcludedRows gueltig?}
    B -->|Nein| C[THROW Fehler]
    B -->|Ja| D[Temp-Tabellen fuer Customers Orders OrderLines InvoiceCoverage ShipmentMatches und ReturnFlags aufbauen]
    D --> E[Demodaten fuer linke Bestellzeilen und rechte Join-Quellen laden]
    E --> F[BaseOrderLines optional nach Region filtern]
    F --> G[PrimaryShipment pro OrderID und LineNo ueber ROW_NUMBER bestimmen]
    G --> H[ComplexJoinPreview mit Rechnungs Versand und Rueckgabeinformationen bilden]
    H --> I[SurvivorEvaluation mit Pass-Flags und PrimaryOutcome ableiten]
    I --> J[SurvivorRowPreview nur fuer IsSurvivor = 1 ausgeben]
    J --> K[JoinStepSummary fuer jede Reduktionsstufe aggregieren]
    K --> L{ExcludedRows anzeigen?}
    L -->|Nein| M[Ende]
    L -->|Ja| N[Ausgeschlossene Zeilen mit Ausschlussgrund und OutcomeRank ausgeben]
    N --> M
```
<!-- SQLDOC:MERMAID:END -->
