# JoinPredicatePlacementDemo.sql

Dieses Skript zeigt im Kapitel `03_JOIN`, wie stark die Filterplatzierung bei einem `LEFT JOIN` die sichtbare Ergebnismenge beeinflusst. Dieselbe Statusbedingung auf der rechten Lead-Tabelle liefert im `ON` NULL-erhaltene Kampagnenzeilen, waehrend sie im `WHERE` genau diese Zeilen wieder entfernt.
...
| Feld | Wert |
|---|---|
| Script | [JoinPredicatePlacementDemo.sql](JoinPredicatePlacementDemo.sql) |
| Version | `1.0` |
| Typ | `didactic-lab` |
| Kapitel | `03_JOIN` |
| Sicherheit | `read-only-tempdb` |
| Zweck | Vergleicht die Platzierung desselben Filters in `ON` und `WHERE` bei einem `LEFT JOIN`. |
<!-- SQLDOC:SUMMARY_TABLE:END -->

## Einordnung

Die Demo konzentriert sich auf die semantische Frage, wann linke Zeilen erhalten bleiben sollen. Ein Filter in `ON` reduziert nur die Treffer auf der rechten Seite; ein Filter in `WHERE` bewertet das bereits gejointe Resultset und kann dadurch die linke Zeile vollstaendig ausblenden.

## Annahmen

- Die Kampagnen- und Lead-Daten sind reine Trainingsdaten fuer JOIN-Semantik und keine produktive Marketinglogik.
- `@TargetLeadStatus` repraesentiert einen exemplarischen Filter auf der rechten Tabelle.
- Kampagnen ohne passenden Zielstatus-Lead sollen in der `ON`-Variante bewusst als NULL-erweiterte Zeilen sichtbar bleiben.
- Die `WHERE`-Variante demonstriert absichtlich den Effekt, dass ein `LEFT JOIN` dadurch fachlich in Richtung `INNER JOIN` kippen kann.

## Anwendungsfall

Das Skript eignet sich fuer Unterricht, Reviews und Query-Walkthroughs zu Outer Joins. Besonders nuetzlich ist es, wenn Lernende beurteilen sollen, ob sie eine Coverage-Frage beantworten wollen, bei der alle linken Stammdaten sichtbar bleiben muessen, oder ob sie wirklich nur gematchte Treffer benoetigen.
...
|---|---|---|---|
| `@TargetLeadStatus` | `NVARCHAR(20)` | Nein | Lead-Status auf der rechten Tabelle, der als Treffer gelten soll. |
| `@OnlyChangedCampaigns` | `BIT` | Nein | Zeigt bei `1` nur Kampagnen mit unterschiedlicher sichtbarer Zeilenmenge je Filterplatzierung. |
| `@IncludeSourceData` | `BIT` | Nein | Gibt bei `1` die Demo-Daten vor den JOIN-Auswertungen aus. |
...
- `#JoinFilterOn` behaelt jede Kampagne mindestens einmal im Resultset und markiert fehlende Zielstatus-Treffer ueber `NULL`.
- `#JoinFilterWhere` zeigt dieselbe Join-Basis, blendet aber Kampagnen ohne passenden Zielstatus-Lead vollstaendig aus.
- Die Summary verdichtet, fuer welche Kampagnen die Filterplatzierung die Diagnosefaehigkeit veraendert.
- Die optionale `INNER JOIN`-Referenz macht sichtbar, warum die `WHERE`-Variante bei rechten Filtern oft wie ein `INNER JOIN` wirkt.
...
|---|---|---|---|
| `1.0` | `2026-04-19` | `ER` | Erstversion fuer die Demo zur Filterplatzierung in ON und WHERE bei LEFT JOINs |
...
```mermaid
flowchart TD
    A[Parameter validieren] --> B[Temp-Tabellen fuer Campaigns und Leads anlegen]
    B --> C[Demo-Daten laden]
    C --> D{IncludeSourceData = 1?}
    D -->|Ja| E[Ausgangsdaten mit LEFT JOIN ausgeben]
    D -->|Nein| F[Direkt zu den Vergleichsvarianten]
    E --> F
    F --> G[JoinFilterOn mit Statusfilter im ON materialisieren]
    F --> H[JoinFilterWhere mit Statusfilter im WHERE materialisieren]
    G --> I[Rows und NULL-erweiterte Zeilen pro Kampagne aggregieren]
    H --> I
    I --> J[ON-Variante optional nur fuer geaenderte Kampagnen ausgeben]
    I --> K[WHERE-Variante optional nur fuer geaenderte Kampagnen ausgeben]
    I --> L[Summary mit Vergleich von Sichtbarkeit und NULL-Erhalt ausgeben]
    L --> M{IncludeInnerJoinReference = 1?}
    M -->|Ja| N[INNER-JOIN-Referenz mit identischem Statusfilter ausgeben]
    M -->|Nein| O[Ende]
    N --> O
```
<!-- SQLDOC:MERMAID:END -->
