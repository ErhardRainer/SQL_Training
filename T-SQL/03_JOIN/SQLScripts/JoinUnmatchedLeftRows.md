# JoinUnmatchedLeftRows.sql

Dieses Skript zeigt im Kapitel `03_JOIN`, wie ein `LEFT JOIN` gezielt fuer linksseitig nicht gematchte Zeilen genutzt wird. Die linke Seite besteht aus Kunden, die rechte Seite aus gefilterten Bestellungen; sichtbar bleiben genau die Kunden, fuer die auf der rechten Seite kein Treffer existiert.
...
| Feld | Wert |
|---|---|
| Script | [JoinUnmatchedLeftRows.sql](JoinUnmatchedLeftRows.sql) |
| Version | `1.0` |
| Typ | `didactic-lab` |
| Kapitel | `03_JOIN` |
| Sicherheit | `read-only-tempdb` |
| Zweck | Zeigt linksseitig nicht gematchte Zeilen ueber `LEFT JOIN` und `IS NULL` auf einer kleinen Demobasis. |
<!-- SQLDOC:SUMMARY_TABLE:END -->

## Einordnung

Im JOIN-Kapitel ist dieses Lab die direkte Sicht auf die Frage: Welche Zeilen der linken Tabelle verschwinden nicht, obwohl rechts kein Match existiert? Genau diese Situation ist fachlich wichtig, wenn fehlende Aktivitaet, fehlende Detaildaten oder ausbleibende Folgesaetze sichtbar gemacht werden sollen.

## Annahmen

- Die Demobasis nutzt `#Customers` als linke Seite und `#Orders` als rechte Seite.
- Als nicht gematcht gilt hier ein Kunde ohne Bestellung ab dem durch `@MinOrderDate` begrenzten Stichtag.
- Die erste Bestellzeile fuer `CustomerID = 3` liegt absichtlich vor dem Standard-Stichtag, damit ein zeitabhaengiger Nicht-Match sichtbar wird.
- Die Vergleichsausgabe fuer gematchte Kunden dient nur dazu, die Grenze zwischen Match und Nicht-Match auf derselben Datenbasis klar zu lesen.

## Anwendungsfall

Das Skript eignet sich fuer Unterricht, Reviews und Diagnoseabfragen, bei denen linksseitige Stammdatensaetze ohne passende Bewegungsdaten gefunden werden sollen. Typische fachliche Bilder sind Kunden ohne Bestellung, Produkte ohne Lagerbewegung oder Mitarbeitende ohne Zeiterfassung im betrachteten Zeitraum.
...
|---|---|---|---|
| `@RegionFilter` | `NVARCHAR(20)` | Nein | Optionaler Filter auf die Region der linken Kundenseite. |
| `@MinOrderDate` | `DATE` | Nein | Untergrenze fuer Bestellungen auf der rechten Seite. |
| `@ShowMatchedComparison` | `BIT` | Nein | Gibt bei `1` zusaetzlich die linksseitigen Zeilen mit Match aus. |
...
- `#ScopedOrders` reduziert die rechte Seite zuerst auf den betrachteten Zeitraum, damit das Nicht-Match fachlich klar definiert ist.
- `#UnmatchedLeftRows` ist das Kernmuster: `LEFT JOIN` behalten, dann `WHERE so.OrderID IS NULL`.
- Die Summary pro Region zeigt, dass linksseitige Nicht-Matches auch als Coverage-Kennzahl aggregiert werden koennen.
- Mit aktivierter Vergleichsausgabe laesst sich direkt sehen, dass dieselbe linke Tabelle sowohl gematchte als auch nicht gematchte Zeilen enthaelt.
...
|---|---|---|---|
| `1.0` | `2026-04-19` | `ER` | Erstversion fuer die didaktische Sicht auf linksseitig nicht gematchte Zeilen |
...
```mermaid
flowchart TD
    A[Parameter einlesen] --> B{ShowMatchedComparison gueltig?}
    B -->|Nein| C[THROW Fehler]
    B -->|Ja| D[Temp-Tabellen fuer Customers und Orders aufbauen]
    D --> E[Demo-Daten laden]
    E --> F[ScopedCustomers per Region filtern]
    E --> G[ScopedOrders per MinOrderDate filtern]
    F --> H[LEFT JOIN von ScopedCustomers auf ScopedOrders]
    G --> H
    H --> I[UnmatchedLeftRows ueber OrderID IS NULL ableiten]
    H --> J[CustomerMatchStatus je linker Zeile verdichten]
    J --> K[MatchCoverageSummary je Region aggregieren]
    F --> L[LeftSourcePreview ausgeben]
    I --> M[UnmatchedLeftRows ausgeben]
    K --> N[MatchCoverageSummary ausgeben]
    N --> O{ShowMatchedComparison = 1?}
    O -->|Ja| P[Gematchte Kunden mit INNER JOIN aggregiert ausgeben]
