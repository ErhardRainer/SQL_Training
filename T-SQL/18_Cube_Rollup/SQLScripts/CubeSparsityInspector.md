# CubeSparsityInspector.sql

Dieses Skript zeigt an einer kleinen Umsatzsicht, dass eine sparse Faktentabelle trotz weniger belegter Detailkombinationen ein deutlich groesseres `GROUP BY CUBE(...)`-Ergebnis erzeugen kann. Der Fokus liegt auf dem Kontrast zwischen belegten Detailzeilen, dichter Obergrenze und den zusaetzlichen Subtotal-Schichten.
...
| Feld | Wert |
|---|---|
| Script | [CubeSparsityInspector.sql](CubeSparsityInspector.sql) |
| Version | `1.0` |
| Typ | `didactic-lab` |
| Kapitel | `18_Cube_Rollup` |
| Sicherheit | `read-only-tempdb` |
| Zweck | Zeigt, wie sparse Kombinationen `CUBE`-Ergebnisse vergroessern koennen. |
<!-- SQLDOC:SUMMARY_TABLE:END -->

## Einordnung

Die Auswertung misst nicht die Laufzeit eines produktiven Workloads, sondern eine didaktische Wirkung: Eine sparse Faktentabelle besetzt nur einen kleinen Teil aller moeglichen Detailkombinationen, aber `CUBE` erzeugt trotzdem zusaetzliche Gruppierungszeilen fuer mehrere Aggregationstiefen.

## Annahmen

- Es handelt sich um eine didaktische Erstversion mit tempdb-basierten Demo-Daten.
- Die vier Dimensionen `RegionCode`, `ProductGroup`, `SalesChannel` und `FiscalQuarter` bilden absichtlich keine dichte Kreuzproduktmenge.
- Jede Faktzeile steht fuer eine bereits belegte Kombination; fehlende Kombinationen werden bewusst nicht kuenstlich aufgefuellt.
- Die Alert-Ausgabe bewertet Expansion nur relativ zur Anzahl der Faktzeilen, nicht als produktive Kapazitaetsaussage.

## Anwendungsfall

Das Skript eignet sich fuer Unterricht und Review, wenn erklaert werden soll, warum `CUBE` auch bei sparsamen Daten schnell mehr Ergebniszeilen als Ausgangszeilen erzeugt. Es passt gut vor Diskussionen ueber gezieltere `GROUPING SETS`, reduzierte Dimensionen oder eine bewusstere Report-Gestaltung.
...
|---|---|---|---|
| `@ShowCubeSample` | `BIT` | Nein | Gibt bei `1` eine kleine Stichprobe der erzeugten CUBE-Zeilen aus. |
| `@MinimumExpansionFactor` | `DECIMAL(9,2)` | Nein | Legt den Mindestfaktor fuer Alerts fest, bezogen auf kumulierte CUBE-Zeilen versus Faktzeilen. |
...
- rekursive CTE fuer alle 16 GroupingId-Ebenen
<!-- SQLDOC:DEPENDENCIES_LIST:END -->

## Hinweise

- `DenseDetailUpperBound` beschreibt die theoretische Vollbelegung aller Detailkombinationen.
- `OccupancyRatio` zeigt, wie klein der belegte Anteil der Detailfaktoren im Demo-Datensatz ist.
- `CumulativeExpansionVsFactRows` macht sichtbar, ab welcher Aggregationstiefe das gesamte CUBE-Ergebnis groesser als die Faktentabelle wird.
- Die Alert-Tabelle filtert genau diese Expansionsstufen ueber den einstellbaren Schwellwert.
...
|---|---|---|---|
| `1.0` | `2026-04-19` | `ER` | Erstversion des didaktischen Inspectors fuer CUBE-Sparsity und Expansion |
...
```mermaid
flowchart TD
    A[Parameter validieren] --> B[Sparse Demo-Fakttabelle mit vier Dimensionen aufbauen]
    B --> C[Dimensionsprofil und dichte Detailobergrenze berechnen]
    C --> D[Reales GROUP BY CUBE ueber Region, Produktgruppe, Kanal und Quartal ausfuehren]
    D --> E[Alle 16 GroupingId-Ebenen fuer die Theorie ableiten]
    E --> F[Reale CUBE-Zeilen und dichte Obergrenze je Aggregationstiefe verdichten]
    F --> G[Kumulative Expansion gegenueber FactRows berechnen]
    G --> H[Alert-Stufen ab MinimumExpansionFactor ausgeben]
    H --> I{ShowCubeSample = 1?}
    I -->|Ja| J[Stichprobe einzelner Detail-, Subtotal- und Grand-Total-Zeilen anzeigen]
    I -->|Nein| K[Run ohne Stichprobe beenden]
```
<!-- SQLDOC:MERMAID:END -->
