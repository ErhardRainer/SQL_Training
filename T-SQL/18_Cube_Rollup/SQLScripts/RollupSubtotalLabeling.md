# RollupSubtotalLabeling.sql

Dieses Skript zeigt, wie `ROLLUP`-Zeilen nach der Aggregation mit sprechenden Labels versehen werden koennen. Statt rohe `NULL`-Werte in Total- und Subtotal-Zeilen zu belassen, erzeugt die didaktische Erstversion lesbare Scope-Texte und feldweise Anzeige-Captions.
...
| Feld | Wert |
|---|---|
| Script | [RollupSubtotalLabeling.sql](RollupSubtotalLabeling.sql) |
| Version | `1.0` |
| Typ | `didactic-lab` |
| Kapitel | `18_Cube_Rollup` |
| Sicherheit | `read-only-tempdb` |
| Zweck | Beschriftet Detail-, Subtotal- und Grand-Total-Zeilen in `ROLLUP`-Ergebnissen lesbar. |
<!-- SQLDOC:SUMMARY_TABLE:END -->

## Einordnung

`ROLLUP` ist technisch sehr kompakt, aber die erzeugten Ergebniszeilen sind fuer Berichte oft noch zu roh. Dieses Beispiel trennt deshalb bewusst zwischen dem technischen Aggregationsergebnis und einer nachgelagerten Label-Schicht, die Totals, Subtotals und verbleibende Detailwerte klar benennt.

## Annahmen

- Die Umsetzung bleibt didaktisch und verwendet nur lokale Temp-Tabellen.
- Die Demo-Fakten modellieren Umsaetze ueber die Dimensionen `RegionCode`, `ProductGroup` und `SalesChannel`.
- Die Label-Texte sind generisch gehalten, damit sie spaeter leicht an Fachbegriffe oder UI-Konventionen angepasst werden koennen.
- `@UseCompactLabels` schaltet nur die Darstellung um; die zugrunde liegende `ROLLUP`-Logik bleibt unveraendert.

## Anwendungsfall

Das Muster eignet sich fuer Unterricht, Report-Prototypen und Review-Szenarien, in denen Total- und Subtotal-Zeilen nicht nur technisch korrekt, sondern auch fuer Menschen sofort verstaendlich sein sollen. Besonders nuetzlich ist es vor Matrix-Reports, Exporten oder Dashboard-Tabellen, in denen aggregierte Ebenen explizit beschriftet werden muessen.
...
| `@UseCompactLabels` | `BIT` | Nein | Verwendet bei `1` kompakte Scope-Labels statt ausfuehrlicher Beschriftungen. |
| `@GrandTotalLabel` | `VARCHAR(80)` | Nein | Legt die Beschriftung fuer die voll aggregierte Grand-Total-Zeile fest. |
<!-- SQLDOC:PARAMETERS_TABLE:END -->

## Abhaengigkeiten

<!-- SQLDOC:DEPENDENCIES_LIST:BEGIN -->
- `tempdb` fuer Demo-Fakten und Zwischenergebnisse
- `GROUP BY ROLLUP`
- `GROUPING`
- `GROUPING_ID`
- `CASE`
...
- `RawRollupPreview` zeigt zuerst die technische Aggregationssicht mit `GroupingId`, Aggregationsgrad und Level-Typ.
- `LabeledRollupResult` fuegt darauf sprechende Scope-Labels und feldweise Anzeige-Captions hinzu.
- `DisplayRegion`, `DisplayProductGroup` und `DisplaySalesChannel` sind praktisch fuer Exporte oder Frontends, die keine eigene Label-Logik mehr nachziehen sollen.
- `LabelingGuidance` fasst die uebertragbaren Regeln fuer produktionsnahe Reports knapp zusammen.
...
|---|---|---|---|
| `1.0` | `2026-04-19` | `ER` | Erstversion fuer sprechende Beschriftungen in `ROLLUP`-Ergebnissen |
...
flowchart TD
    ...
    C -->|Ja| D[SourceFactPreview ausgeben]
    C -->|Nein| E[Direkt zur ROLLUP-Aggregation wechseln]
    D --> E
    E --> F[RawRollupPreview per GROUP BY ROLLUP erzeugen]
    F --> G[GroupingId AggregatedDimensions und LevelKind ableiten]
    G --> H[LabeledRollupResult mit ScopeLabel und Display-Captions aufbauen]
    H --> I{UseCompactLabels = 1?}
    I -->|Ja| J[Kompakte Pipe-Labels verwenden]
    I -->|Nein| K[Ausfuehrliche Detail oder Subtotal-Beschriftung verwenden]
    J --> L[LabeledRollupResult ausgeben]
    K --> L
    L --> M[LabelingGuidance fuer Report-Uebernahme ausgeben]
```
<!-- SQLDOC:MERMAID:END -->
