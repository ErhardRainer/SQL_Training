# GroupingIdDecodeLab.sql

Dieses Labor macht `GROUPING_ID()` in einer `CUBE`-Auswertung schrittweise lesbar. Statt die Bitmaske nur als Zahl anzuzeigen, legt das Skript jede Dimension als eigenes Bit, als Gewicht und als nachpruefbare Summenformel offen.
...
| Feld | Wert |
|---|---|
| Script | [GroupingIdDecodeLab.sql](GroupingIdDecodeLab.sql) |
| Version | `1.0` |
| Typ | `didactic-lab` |
| Kapitel | `18_Cube_Rollup` |
| Sicherheit | `read-only-tempdb` |
| Zweck | Zerlegt `GROUPING_ID()` in Bits, Gewichte und lesbare Komponenten fuer `CUBE`-Subtotal-Ebenen. |
<!-- SQLDOC:SUMMARY_TABLE:END -->

## Einordnung

`GROUPING_ID()` ist praktisch, aber ohne Uebersetzung oft schwer greifbar. Das Skript zeigt deshalb zu jeder aggregierten Zeile nicht nur die fertige Maske, sondern auch die zugrunde liegenden `GROUPING()`-Bits, deren Bitgewichte und eine Kontrollspalte, mit der sich die berechnete Summe direkt gegen `GROUPING_ID()` pruefen laesst.

## Annahmen

- Das Skript ist eine didaktische Erstversion mit temporaeren Demo-Fakten statt produktiver Tabellen.
- Die Bitzuordnung folgt der T-SQL-ueblichen Lesart fuer `GROUPING_ID(RegionCode, ProductLine, CustomerTier, FiscalQuarter)` mit Gewichten `8`, `4`, `2` und `1`.
- Die Dekodierung konzentriert sich auf Verstaendlichkeit fuer Schulung und Review, nicht auf minimale Spaltenanzahl.
- `@FilterGroupingId` dient als Lernhilfe, um einzelne Masken gezielt zu untersuchen.

## Anwendungsfall

Das Labor ist hilfreich, wenn `CUBE`-Resultsets in Unterricht, Reviews oder Report-Templates auftauchen und klar sein muss, welche Dimensionen in einer bestimmten Zeile noch auf Detailniveau stehen und welche bereits zur ALL-Ebene aggregiert wurden. Es eignet sich auch als Vorstufe fuer spaetere Label- oder Sortierlogik.
...
|---|---|---|---|
| `@ShowSourcePreview` | `BIT` | Nein | Gibt bei `1` die Demo-Fakttabelle vor der CUBE-Auswertung aus. |
| `@FilterGroupingId` | `INT` | Nein | Zeigt bei `-1` alle Masken oder sonst nur den angegebenen `GROUPING_ID()`-Wert von `0` bis `15`. |
| `@ShowOnlyAggregatedRows` | `BIT` | Nein | Filtert bei `1` Detailzeilen aus und laesst nur Subtotals sowie Grand Total im Labor stehen. |
<!-- SQLDOC:PARAMETERS_TABLE:END -->

## Abhaengigkeiten

<!-- SQLDOC:DEPENDENCIES_LIST:BEGIN -->
- `tempdb` fuer Demo-Fakten und Labor-Zwischentabellen
- `GROUP BY CUBE`
- `GROUPING`
- `GROUPING_ID`
- `CASE`
...
- `GroupingIdBinary` zeigt die Bits in derselben Dimensionsreihenfolge wie der Aufruf von `GROUPING_ID()`.
- `CalculatedGroupingId` prueft explizit, ob die Summe der Bitgewichte zur gelieferten Maske passt.
- `DimensionScope` formuliert die aktuelle Zeile bewusst als lesbare Kombination aus Detail- und ALL-Ebenen.
- Die Legende am Ende hilft dabei, einzelne Bits bei spaeteren Report-Skripten schneller einzuordnen.
...
|---|---|---|---|
| `1.0` | `2026-04-19` | `ER` | Erstversion des Labors zur schrittweisen Dekodierung von `GROUPING_ID()`-Werten |
...
```mermaid
flowchart TD
    A[Parameter validieren] --> B[Demo-Fakttabelle fuer Region, Produktlinie, Segment und Quartal aufbauen]
    B --> C{ShowSourcePreview = 1?}
    C -->|Ja| D[Quellfakten ausgeben]
    C -->|Nein| E[Direkt zur CUBE-Auswertung]
    D --> E
    E --> F[GROUP BY CUBE ueber vier Dimensionen ausfuehren]
    F --> G[GROUPING_ID sowie GROUPING-Bits und Bitgewichte 8 4 2 1 ableiten]
    G --> H[CalculatedGroupingId, DecodeStatus, DimensionScope und AggregationLevelLabel fuellen]
    H --> I{FilterGroupingId oder nur aggregierte Zeilen aktiv?}
    I -->|Ja| J[Lab-Resultset entsprechend filtern]
    I -->|Nein| K[Vollstaendiges Lab-Resultset ausgeben]
    J --> L[Bit-Legende mit Positionen, Gewichten und Bedeutungen ausgeben]
    K --> L
```
<!-- SQLDOC:MERMAID:END -->
