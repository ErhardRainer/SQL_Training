# GroupingLevelLabelTemplate.sql

Dieses Skript liefert eine didaktische Vorlage dafuer, Aggregationszeilen aus `CUBE` oder `ROLLUP` mit sprechenden Labels zu versehen. Im Mittelpunkt steht nicht die reine Summenbildung, sondern die Uebersetzung von `GROUPING()`-Flags in lesbare Detail-, Subtotal- und Grand-Total-Bezeichnungen.
...
| Feld | Wert |
|---|---|
| Script | [GroupingLevelLabelTemplate.sql](GroupingLevelLabelTemplate.sql) |
| Version | `1.0` |
| Typ | `template` |
| Kapitel | `18_Cube_Rollup` |
| Sicherheit | `read-only-tempdb` |
| Zweck | Vorlage fuer sprechende Labels je Aggregationslevel in CUBE- oder ROLLUP-Ausgaben. |
<!-- SQLDOC:SUMMARY_TABLE:END -->

## Einordnung

Viele `CUBE`- oder `ROLLUP`-Resultsets sind technisch korrekt, aber fuer Berichte oder Schulungsausgaben zu roh. Dieses Template zeigt deshalb eine Zwischenstufe: Zuerst entsteht eine Aggregationsmenge mit `GROUPING_ID()`, danach werden aus den Aggregationsflags sprechende Texte und Sammelbezeichnungen pro Dimension abgeleitet.

## Annahmen

- Es handelt sich um eine didaktische Erstversion mit einer kleinen tempdb-basierten Faktentabelle.
- Das Beispiel nutzt drei Dimensionen: `RegionCode`, `ProductGroup` und `SalesChannel`.
- Die Label-Texte sind bewusst generisch gehalten und dienen als Vorlage fuer spaetere Fachbegriffe oder UI-Beschriftungen.
- `@UseCompactLabels` bildet einen einfachen Schalter zwischen kurzer und ausfuehrlicher Beschriftung, ohne die Aggregationslogik zu veraendern.

## Anwendungsfall

Das Skript eignet sich fuer Unterricht, Report-Prototypen und Reviews, wenn Totals und Subtotals zwar bereits vorliegen, aber noch nicht anwendergerecht benannt sind. Besonders nuetzlich ist es vor dem Bau von Matrix-Reports, Exportformaten oder Dashboard-Tabellen, in denen `NULL`-Spaltenwerte durch klare Sammelbezeichnungen ersetzt werden sollen.
...
|---|---|---|---|
| `@ShowSourcePreview` | `BIT` | Nein | Gibt bei `1` die Demo-Faktdaten vor der Aggregation aus. |
| `@UseCompactLabels` | `BIT` | Nein | Verwendet bei `1` kuerzere Ebenenlabels fuer kompakte Ausgaben. |
| `@GrandTotalLabel` | `VARCHAR(60)` | Nein | Legt die Beschriftung fuer die voll aggregierte Zeile fest. |
...
- `LevelKind` trennt Detailzeilen, Subtotals und Grand Totals fuer spaetere Report-Logik.
- `LevelLabel` zeigt die eigentliche Beschriftungsvorlage, waehrend `RegionLabel`, `ProductGroupLabel` und `SalesChannelLabel` feldweise Sammeltexte bereitstellen.
- `LabelTemplateHint` dient als didaktische Zusatzspalte und kann in produktiven Reports entfallen.
- Die gleiche Struktur laesst sich von `CUBE` leicht auf `ROLLUP` oder explizite `GROUPING SETS` uebertragen.
...
|---|---|---|---|
| `1.0` | `2026-04-19` | `ER` | Erstversion der Label-Vorlage fuer Aggregationslevel in CUBE-Ausgaben |
...
flowchart TD
    ...
    D --> E
    E --> F[GROUP BY CUBE ueber RegionCode, ProductGroup und SalesChannel ausfuehren]
    F --> G[GroupingId und AggregatedDimensions berechnen]
    G --> H[LevelKind, LevelLabel und feldweise Sammelbezeichnungen ableiten]
    H --> I[GroupingLevelLabelMatrix fuer Detail, Subtotal und Grand Total ausgeben]
    I --> J[LabelingGuidance mit Hinweisen fuer Report-Uebernahme ausgeben]
```
<!-- SQLDOC:MERMAID:END -->
