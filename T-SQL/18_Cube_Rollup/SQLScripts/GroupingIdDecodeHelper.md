# GroupingIdDecodeHelper.sql

Dieses Skript macht `GROUPING_ID()` fuer `ROLLUP`- und `CUBE`-Ausgaben leichter lesbar. Statt die Bitmaske nur als nackten Integer zu zeigen, dekodiert die Demo jede Dimension einzeln, stellt das Binarmuster dar und versieht Subtotal-Zeilen mit einer sprechenden Ebenenbeschreibung.
...
| Feld | Wert |
|---|---|
| Script | [GroupingIdDecodeHelper.sql](GroupingIdDecodeHelper.sql) |
| Version | `1.0` |
| Typ | `didactic-lab` |
| Kapitel | `18_Cube_Rollup` |
| Sicherheit | `read-only-tempdb` |
| Zweck | Dekodiert `GROUPING_ID()`-Werte pro Dimension in lesbare Subtotal-Hinweise. |
<!-- SQLDOC:SUMMARY_TABLE:END -->

## Einordnung

`GROUPING_ID()` ist praktisch, aber im Alltag oft schwer direkt zu lesen. Besonders bei mehreren Dimensionen ist ohne Hilfstabelle oder Kopfkino unklar, welche Bits fuer welche aggregierten Ebenen stehen. Der Helper uebersetzt deshalb die Maske in pro-Dimension-Flags, Labels und eine kurze Erlaeuterung der aktuellen Aggregationsebene.

## Annahmen

- Es handelt sich um eine didaktische Erstversion mit tempdb-basierten Demo-Fakten.
- Die Bitreihenfolge folgt der `GROUPING_ID(RegionCode, ProductLine, CustomerTier, FiscalQuarter)`-Definition im Skript.
- `@UseCube = 1` zeigt die vollstaendige Variantenvielfalt, waehrend `@UseCube = 0` eine linearere `ROLLUP`-Interpretation ermoeglicht.
- Die Beschriftungen wie `(alle Regionen)` oder `subtotal fuer ...` sind bewusst lehrorientiert und nicht als festes Reporting-Vokabular gedacht.

## Anwendungsfall

Das Skript eignet sich fuer Unterricht, Reviews und Debugging, wenn Teams `GROUPING_ID()` in Reports oder Diagnoseabfragen einsetzen, aber die Bedeutung einzelner Werte wie `3`, `7` oder `15` schnell nachschlagen muessen. Es hilft auch beim Bau eigener Label- oder Sortierlogiken, weil jede Bitposition separat sichtbar bleibt.
...
|---|---|---|---|
| `@UseCube` | `BIT` | Nein | Waehlt zwischen `GROUP BY CUBE` (`1`) und `GROUP BY ROLLUP` (`0`) fuer die Demo-Auswertung. |
| `@ShowOnlyAggregatedRows` | `BIT` | Nein | Filtert bei `1` Detailzeilen aus der dekodierten Ergebnisliste heraus. |
| `@ShowBinaryMask` | `BIT` | Nein | Blendet bei `1` die vierstellige Binardarstellung der `GROUPING_ID()`-Maske ein. |
<!-- SQLDOC:PARAMETERS_TABLE:END -->

## Abhaengigkeiten

<!-- SQLDOC:DEPENDENCIES_LIST:BEGIN -->
- `tempdb` fuer Demo-Fakten und Hilfstabellen
- `GROUP BY CUBE`
- `GROUP BY ROLLUP`
- `GROUPING`
- `GROUPING_ID`
...
- `GroupingIdBinary` zeigt die gleiche Logik wie `GroupingId`, aber bereits in der fuer Menschen schnelleren Bitdarstellung.
- Die vier Bitspalten `RegionBit`, `ProductLineBit`, `CustomerTierBit` und `FiscalQuarterBit` machen sofort sichtbar, welche Dimension auf ALL-Ebene aggregiert wurde.
- `AggregationLevelLabel` fasst die noch sichtbaren Gruppierungsdimensionen in einer lesbaren Subtotal-Beschreibung zusammen.
- Die Referenztabelle am Ende dient als kompaktes Nachschlagewerk fuer die Bitpositionen der Demo.
...
|---|---|---|---|
| `1.0` | `2026-04-19` | `ER` | Erstversion des Helpers zur lesbaren Dekodierung von `GROUPING_ID()`-Werten |
...
```mermaid
flowchart TD
    A[Parameter validieren] --> B[Demo-Fakttabelle fuer Region, Produktlinie, Kundensegment und Quartal aufbauen]
    B --> C[SourceFactPreview ausgeben]
    C --> D{UseCube = 1?}
    D -->|Ja| E[CUBE-Aggregation mit GROUPING_ID und Bitflags erzeugen]
    D -->|Nein| F[ROLLUP-Aggregation mit derselben Decode-Logik erzeugen]
    E --> G[Labels, Binarmaske und AggregationLevelLabel in #GroupingResult schreiben]
    F --> G
    G --> H[DecodedRows optional auf aggregierte Zeilen filtern]
    H --> I{ShowBinaryMask = 1?}
    I -->|Ja| J[GroupingIdBinary im Resultset einblenden]
    I -->|Nein| K[Binary-Spalte als NULL ausgeben]
    J --> L[BitReference fuer jede Dimension ausgeben]
    K --> L
```
<!-- SQLDOC:MERMAID:END -->
