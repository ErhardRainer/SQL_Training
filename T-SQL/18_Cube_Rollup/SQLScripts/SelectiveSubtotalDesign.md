# SelectiveSubtotalDesign.sql

Dieses Skript zeigt eine didaktische Alternative zur vollen `CUBE`-Expansion. Statt alle moeglichen Gruppierungsebenen auszugeben, wird eine kleine Menge fachlich begruendeter Subtotals ausgewaehlt und gegen den kompletten CUBE-Raum derselben vier Dimensionen gespiegelt.
...
| Feld | Wert |
|---|---|
| Script | [SelectiveSubtotalDesign.sql](SelectiveSubtotalDesign.sql) |
| Version | `1.0` |
| Typ | `didactic-lab` |
| Kapitel | `18_Cube_Rollup` |
| Sicherheit | `read-only-tempdb` |
| Zweck | Selektiert fachlich noetige Subtotal-Ebenen statt einer vollen CUBE-Expansion. |
<!-- SQLDOC:SUMMARY_TABLE:END -->

## Einordnung

Im Kapitel zu `CUBE` und `ROLLUP` ist oft nicht die technische Vollstaendigkeit entscheidend, sondern die Frage, welche Zwischensummen wirklich gebraucht werden. Das Skript baut deshalb zuerst eine Referenz auf alle `CUBE`-Ebenen und kontrastiert diese anschliessend mit einer kleineren, bewusst kuratierten Menge an Aggregationen.

## Annahmen

- Es handelt sich um eine didaktische Erstversion mit Demo-Daten in `tempdb`.
- Die vier Dimensionen `RegionCode`, `ProductGroup`, `SalesChannel` und `FiscalQuarter` repraesentieren typische Reporting-Schnitte, ohne produktive Tabellen vorauszusetzen.
- `Region + Produktgruppe`, `Region + Quartal` und `Kanal + Quartal` gelten hier als standardmaessig fachlich plausible Subtotals.
- `Produktgruppe + Quartal` und `Grand Total` bleiben bewusst optional, damit der Unterschied zwischen Pflicht- und Zusatzebenen sichtbar wird.

## Anwendungsfall

Das Skript eignet sich fuer Unterricht, Architektur-Reviews und Report-Design-Diskussionen, wenn Lernende oder Teams entscheiden sollen, ob `GROUP BY CUBE` zu viele ungenutzte Ebenen erzeugt. Hilfreich ist vor allem das Coverage-Resultset, weil es die Zahl der selektierten Ebenen und Ergebniszeilen direkt dem Voll-CUBE gegenueberstellt.
...
|---|---|---|---|
| `@ShowFullCubeReference` | `BIT` | Nein | Gibt bei `1` die Referenz aller CUBE-Ebenen mit realer Zeilenzahl aus. |
| `@IncludeProductQuarter` | `BIT` | Nein | Nimmt bei `1` zusaetzliche Subtotals je Produktgruppe und Quartal in das Design auf. |
| `@IncludeGrandTotal` | `BIT` | Nein | Fuegt bei `1` eine Grand-Total-Zeile als eigene Designentscheidung hinzu. |
...
- `GROUP BY CUBE`
- `GROUPING_ID`
- explizites `GROUPING SETS`-Designmuster ueber selektive Subtotal-Abfragen
- `UNION ALL`-artige Zusammenstellung gezielter Berichtsebenen
<!-- SQLDOC:DEPENDENCIES_LIST:END -->

## Hinweise

- Das Skript nutzt `GROUP BY CUBE` nur als Referenzrahmen, nicht als finale Berichtsausgabe.
- Die selektierten Ebenen sind als explizite Designentscheidungen modelliert und koennen damit separat begruendet oder abgeschaltet werden.
- `RowsAvoided` im Coverage-Resultset macht sichtbar, wie viele CUBE-Zeilen fuer das gezeigte Berichtsziel unnoetig waeren.
...
|---|---|---|---|
| `1.0` | `2026-04-19` | `ER` | Erstversion fuer die didaktische Auswahl gezielter Subtotal-Ebenen |
...
```mermaid
flowchart TD
    A[Parameter validieren] --> B[Demo-Fakttabelle aufbauen]
    B --> C[Volle CUBE-Referenz ueber vier Dimensionen berechnen]
    C --> D{ShowFullCubeReference = 1?}
    D -->|Ja| E[Alle erreichbaren CUBE-Ebenen ausgeben]
    D -->|Nein| F[Selektives Design mit Business-Fragen laden]
    E --> F
    F --> G[Pflicht-Subtotals fuer Region+Produkt, Region+Quartal und Kanal+Quartal erzeugen]
    G --> H{IncludeProductQuarter = 1?}
    H -->|Ja| I[Optionales Subtotal Produktgruppe+Quartal hinzufuegen]
    H -->|Nein| J[Optionales Produkt-Quartal ueberspringen]
    I --> K{IncludeGrandTotal = 1?}
    J --> K
    K -->|Ja| L[Grand Total hinzufuegen]
    K -->|Nein| M[Grand Total auslassen]
    L --> N[Coverage zwischen selektivem Design und Voll-CUBE berechnen]
    M --> N
    N --> O[Selektierte Subtotals ausgeben]
    O --> P[Design-Hinweise fuer GROUPING SETS ausgeben]
```
<!-- SQLDOC:MERMAID:END -->
...
dependencies:
  ...
  - "GROUP BY CUBE"
  - "GROUPING_ID"
  - "GROUPING SETS pattern via UNION ALL"
  - "STRING_AGG"
