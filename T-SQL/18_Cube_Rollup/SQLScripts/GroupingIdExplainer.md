# GroupingIdExplainer.sql

Dieses Skript erklaert `GROUPING()` und `GROUPING_ID()` an einem kleinen Report-Beispiel. Die Demo bleibt absichtlich kompakt, damit sich Aggregationszeilen, sichtbare `ALL`-Ebenen und die zugehoerige Bitmaske ohne langes Nachrechnen lesen lassen.
...
| Feld | Wert |
|---|---|
| Script | [GroupingIdExplainer.sql](GroupingIdExplainer.sql) |
| Version | `1.0` |
| Typ | `didactic-lab` |
| Kapitel | `18_Cube_Rollup` |
| Sicherheit | `read-only-tempdb` |
| Zweck | Erklaert `GROUPING()` und `GROUPING_ID()` in einem kompakten `CUBE`- oder `ROLLUP`-Beispiel. |
<!-- SQLDOC:SUMMARY_TABLE:END -->

## Einordnung

`GROUPING()` beantwortet pro Spalte die Frage, ob ein sichtbarer Wert noch Detailniveau hat oder bereits durch Aggregation entstanden ist. `GROUPING_ID()` fasst dieselbe Information als Bitmaske zusammen. Das Skript zeigt beides gleichzeitig und formuliert die Maske zusaetzlich in Klartext.

## Annahmen

- Das Skript ist eine didaktische Erstversion mit temporaeren Demo-Fakten statt produktiver Tabellen.
- Die Bitreihenfolge fuer `GROUPING_ID(RegionCode, ProductLine, FiscalQuarter)` nutzt die Gewichte `4`, `2` und `1`.
- `@AggregationMode` dient zum direkten Vergleich zwischen `CUBE` und `ROLLUP`, damit die Unterschiede bei moeglichen `GROUPING_ID()`-Werten sichtbar werden.

## Anwendungsfall

Das Skript eignet sich fuer Unterricht, Code-Reviews und Report-Designs, in denen Subtotal-Zeilen korrekt gelesen werden muessen. Besonders nuetzlich ist es als kurze Referenz, wenn `GROUPING()`-Flags und `GROUPING_ID()` im selben Resultset auftauchen.
...
|---|---|---|---|
| `@ShowSourcePreview` | `BIT` | Nein | Gibt bei `1` die Demo-Fakttabelle vor der Aggregation aus. |
| `@AggregationMode` | `VARCHAR(10)` | Nein | Waehlt `CUBE` oder `ROLLUP` als Aggregationsmodus fuer das Erklaerungsbeispiel. |
| `@ShowOnlyAggregatedRows` | `BIT` | Nein | Filtert bei `1` alle Detailzeilen aus und zeigt nur Zeilen mit aggregierten Dimensionen. |
| `@FilterGroupingId` | `INT` | Nein | Zeigt bei `-1` alle Zeilen oder sonst nur die angeforderte `GROUPING_ID()` von `0` bis `7`. |
<!-- SQLDOC:PARAMETERS_TABLE:END -->

## Abhaengigkeiten

<!-- SQLDOC:DEPENDENCIES_LIST:BEGIN -->
- `tempdb` fuer Demo-Fakten und Erklaerungs-Zwischentabellen
- `GROUP BY CUBE`
- `GROUP BY ROLLUP`
- `GROUPING`
- `GROUPING_ID`
...
- `GroupingBits` zeigt dieselben Einzelinformationen wie `GROUPING()`, nur kompakt als Dreiermaske.
- `GroupingMeaning` erklaert direkt pro Zeile, welche Dimensionen aggregiert wurden.
- `GroupingIdMeaning` schreibt die Gewichtsformel offen aus, damit die Zahl hinter `GROUPING_ID()` nachvollziehbar bleibt.
- Mit `ROLLUP` entstehen nur Prefix-Subtotals; `CUBE` erzeugt mehr Kombinationen und damit typischerweise mehr `GROUPING_ID()`-Werte.
...
|---|---|---|---|
| `1.0` | `2026-04-19` | `ER` | Erstversion der kompakten Erklaerung fuer `GROUPING()` und `GROUPING_ID()` |
...
```mermaid
flowchart TD
    A[Parameter validieren] --> B[Demo-Fakttabelle fuer Region, Produktlinie und Quartal aufbauen]
    B --> C{ShowSourcePreview = 1?}
    C -->|Ja| D[Quellfakten ausgeben]
    C -->|Nein| E[Direkt zur Aggregation]
    D --> E
    E --> F{AggregationMode}
    F -->|CUBE| G[GROUP BY CUBE ueber drei Dimensionen ausfuehren]
    F -->|ROLLUP| H[GROUP BY ROLLUP ueber drei Dimensionen ausfuehren]
    G --> I[GROUPING Flags, GroupingBits und GROUPING_ID Formel ableiten]
    H --> I
    I --> J[Labels und Klartext-Erklaerungen fuer Detail oder ALL-Ebenen fuellen]
    J --> K{Nur aggregierte Zeilen oder bestimmte GROUPING_ID filtern?}
    K -->|Ja| L[Ergebnis entsprechend filtern]
    K -->|Nein| M[Vollstaendiges Erklaerungs-Resultset ausgeben]
    L --> N[Bit-Referenz mit Gewichten 4 2 1 ausgeben]
    M --> N
```
<!-- SQLDOC:MERMAID:END -->
