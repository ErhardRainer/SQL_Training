# RollupRunningSubtotalDemo.sql

Dieses Skript zeigt, wie ein `ROLLUP`-Ergebnis nicht nur Totals liefert, sondern auch in eine lesbare Berichtssortierung ueberfuehrt und dort mit laufenden Zwischensummen angereichert werden kann. So wird sichtbar, an welcher Stelle einer Praesentation Produkt- oder Regionsbloecke abgeschlossen werden.
...
| Feld | Wert |
|---|---|
| Script | [RollupRunningSubtotalDemo.sql](RollupRunningSubtotalDemo.sql) |
| Version | `1.0` |
| Typ | `didactic-lab` |
| Kapitel | `18_Cube_Rollup` |
| Sicherheit | `read-only-tempdb` |
| Zweck | Demonstriert laufende Zwischensummen in einer praesentationsreifen `ROLLUP`-Ausgabe. |
<!-- SQLDOC:SUMMARY_TABLE:END -->

## Einordnung

Der didaktische Schwerpunkt liegt hier auf zwei Schritten: zuerst entsteht ein regulaeres `ROLLUP`-Ergebnis, danach wird dieses Ergebnis explizit sortiert und mit Window Functions kumuliert. Dadurch laesst sich erklaeren, warum laufende Zwischensummen nicht automatisch in `ROLLUP` enthalten sind, sondern aus dem geordneten Ausgabestrom abgeleitet werden.

## Annahmen

- Die Umsetzung bleibt rein didaktisch und arbeitet nur mit Temp-Tabellen.
- Die Demo-Buchungen verwenden wiederholte Leaf-Kombinationen, damit Monatswerte und spaetere Zwischensummen sichtbar anwachsen.
- Die Running-Subtotal-Felder folgen der definierten Berichtssortierung nach Region, Produktgruppe und Monat.
- `NULL` in `ROLLUP`-Zeilen wird als aggregierte Ebene interpretiert und nicht als fachlicher Quelldatenwert.

## Anwendungsfall

Das Muster eignet sich fuer Unterricht, Review und Reporting-Design, wenn Lernende verstehen sollen, wie sich kumulierte Zwischenstaende entlang einer `ROLLUP`-Hierarchie lesen lassen. Es ist besonders nuetzlich fuer Berichte, die nach jedem Produkt- oder Regionsblock einen sichtbaren Zwischenstand ausgeben sollen.
...
|---|---|---|---|
| `@ShowSourceData` | `BIT` | Nein | Gibt bei `1` die Demo-Buchungen vor der Aggregation aus. |
| `@IncludeLeafRows` | `BIT` | Nein | Behaelt bei `1` auch Leaf-Zeilen im Running-Subtotal-Resultset. |
| `@HighlightAmountFrom` | `DECIMAL(18,2)` | Nein | Schwellwert fuer auffaellige kumulierte Zwischenstaende. |
<!-- SQLDOC:PARAMETERS_TABLE:END -->

## Abhaengigkeiten

<!-- SQLDOC:DEPENDENCIES_LIST:BEGIN -->
- `tempdb` fuer lokale Demo- und Ergebnisobjekte
- `GROUP BY ROLLUP`
- `GROUPING_ID`
- Window Functions mit `SUM() OVER (...)`
- `CASE`
<!-- SQLDOC:DEPENDENCIES_LIST:END -->

## Hinweise

- `RollupBasePreview` zeigt das unmittelbare `ROLLUP`-Ergebnis, bevor eine Praesentationsreihenfolge definiert wird.
- `RunningSubtotalDemo` enthaelt drei kumulierte Sichten: innerhalb der Region, innerhalb der Produktgruppe einer Region und ueber den gesamten Bericht.
- `ResetScope` erklaert, an welcher Art von Zeile ein sichtbarer Blockabschluss erreicht wird.
- `RunningSubtotalHighlights` filtert groessere kumulierte Staende fuer Besprechungen oder Demo-Screens.
...
|---|---|---|---|
| `1.0` | `2026-04-19` | `ER` | Erstversion fuer laufende Zwischensummen in einem didaktischen `ROLLUP`-Bericht |
...
```mermaid
flowchart TD
    A[Parameter validieren] --> B[Demo-Buchungen in tempdb aufbauen]
    B --> C{ShowSourceData = 1?}
    C -->|Ja| D[Quelldaten ausgeben]
    C -->|Nein| E[Direkt zum ROLLUP wechseln]
    D --> E
    E --> F[RollupBase per GROUP BY ROLLUP ueber Region, Produktgruppe und Monat erzeugen]
    F --> G[LevelLabel, GroupingId und ScopeLabel fuer jede ROLLUP-Zeile ableiten]
    G --> H[Praesentationsreihenfolge mit Region-, Produkt- und Monatsordnung berechnen]
    H --> I[Window Functions fuer RunningAmountWithinRegion, RunningAmountWithinProductInRegion und RunningAmountAcrossReport anwenden]
    I --> J{IncludeLeafRows = 1?}
    J -->|Ja| K[Komplettes RunningSubtotalDemo ausgeben]
    J -->|Nein| L[Nur Subtotal- und Grand-Total-Zeilen ausgeben]
    K --> M[Highlights ab Schwellwert bestimmen]
    L --> M
    M --> N[ModelingHints fuer Sortierung, Reset-Logik und Reporting ausgeben]
```
<!-- SQLDOC:MERMAID:END -->
