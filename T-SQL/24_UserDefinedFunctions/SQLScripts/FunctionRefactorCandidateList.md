# FunctionRefactorCandidateList.sql

Diagnose-Skript fuer Kapitel `24_UserDefinedFunctions`, das moegliche Refactoring-Kandidaten unter Funktionen ueber einen transparenten Heuristik-Score priorisiert.
...
| Feld | Wert |
|---|---|
| Script | [FunctionRefactorCandidateList.sql](FunctionRefactorCandidateList.sql) |
| Version | `1.0` |
| Typ | `diagnostic-query` |
| Kapitel | `24_UserDefinedFunctions` |
| Sicherheit | `read-only` |
| Zweck | Listet User-Defined Functions auf, die anhand von Metadaten, Abhaengigkeiten und Definitionsmustern als moegliche Refactoring-Kandidaten priorisiert werden koennen. |
<!-- SQLDOC:SUMMARY_TABLE:END -->

## Einordnung

Das Skript ist als lesende Erstversion fuer Review, Backlog-Pflege und Unterricht zur Funktionsqualitaet gedacht. Es bildet keine automatische Umbauentscheidung ab, sondern eine nachvollziehbare Priorisierung fuer manuelle Nachpruefung.

## Annahmen

- Die Priorisierung basiert auf Metadaten, Katalog-Dependencies und einfachen Textmustern in `sys.sql_modules.definition`.
- Ein hoher Score signalisiert Review-Bedarf oder Refactoring-Potenzial, nicht automatisch einen fachlichen Fehler.
- Funktionen ohne referenzierte Objekte koennen trotzdem auffaellig werden, wenn Metadaten oder Definitionsmuster auf komplexe oder fragile Umsetzung hindeuten.

## Anwendungsfall

Nutze das Skript, wenn du schnell erkennen moechtest:

- welche Funktionen fuer Refactoring-Backlogs zuerst betrachtet werden sollten,
- wo fehlendes `SCHEMABINDING`, Datenzugriffe oder nicht deterministische Signale zusammenkommen,
- welche Funktionen viele Abhaengigkeiten oder Schleifen-/Cursor-Muster aufweisen.

## Hinweise und Grenzen

- Das Skript schreibt keine Daten und aendert keine Objekte.
- Die Textmuster erkennen nur offensichtliche Hinweise wie `CURSOR`, `WHILE`, `APPLY` oder ausgewaehlte nicht deterministische Built-ins.
- Die Ergebnisliste eignet sich fuer Priorisierung und Review, ersetzt aber keine fachliche oder Performance-Analyse im Detail.
...
| `@MinimumRefactorScore` | `INT` | `IN` | nein | Untergrenze fuer die Ausgabe im Kandidaten-Resultset |
| `@OnlyCandidates` | `BIT` | `IN` | nein | `1` = nur Funktionen mit Einstufung `review` oder `priority` ausgeben |
...
|---|---|---|---|
| `1.0` | `2026-04-22` | `ER` | Erstversion des Diagnose-Skripts fuer Refactoring-Kandidaten bei Funktionen |
...
```mermaid
flowchart TD
    A[Parameter vorbereiten] --> B{MinimumRefactorScore gueltig?}
    B -- Nein --> X[THROW]
    B -- Ja --> C{OnlyCandidates gueltig?}
    C -- Nein --> Y[THROW]
    C -- Ja --> D[FunctionCatalog aus Objekten, Schemas und SQL-Modulen lesen]
    D --> E[DependencySignals aus sys.sql_expression_dependencies verdichten]
    D --> F[DefinitionSignals aus Textmustern ableiten]
    E --> G[Audit mit Heuristik-Score berechnen]
    F --> G
    G --> H[RefactorBand und Empfehlungstexte ableiten]
    H --> I[Kandidatenliste nach Score und Filtern ausgeben]
    H --> J[Summary je RefactorBand aggregieren]
```
<!-- SQLDOC:MERMAID:END -->
