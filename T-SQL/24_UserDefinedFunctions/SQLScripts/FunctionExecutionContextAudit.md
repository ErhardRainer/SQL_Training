# FunctionExecutionContextAudit.sql

Diagnose-Skript fuer Kapitel `24_UserDefinedFunctions`, das Ausfuehrungskontext, Eigentuemerschaft und explizite Objektberechtigungen von Funktionen in einem kompakten Audit sichtbar macht.
...
| Feld | Wert |
|---|---|
| Script | [FunctionExecutionContextAudit.sql](FunctionExecutionContextAudit.sql) |
| Version | `1.0` |
| Typ | `diagnostic-query` |
| Kapitel | `24_UserDefinedFunctions` |
| Sicherheit | `read-only` |
| Zweck | Auditiert User-Defined Functions auf Ausfuehrungskontext, Eigentuemerschaft und explizite Objektberechtigungen, um Ownership-Chaining, EXECUTE AS und potenzielle Review-Faelle sichtbar zu machen. |
<!-- SQLDOC:SUMMARY_TABLE:END -->

## Einordnung

Das Skript ist als lesende Erstversion fuer Trainings- und Review-Zwecke gedacht. Es eignet sich, um Funktionen mit abweichendem Ausfuehrungskontext, expliziten Objektrechten oder ungewoehnlicher Owner-Konstellation schnell zu identifizieren.

## Annahmen

- Das Audit bewertet nur Katalogmetadaten innerhalb der aktuellen Datenbank.
- Fehlendes `EXECUTE AS` wird als Aufruf im Standardkontext des Callers interpretiert.
- Objektrechte werden ueber `sys.database_permissions` auf Klassenebene `OBJECT_OR_COLUMN` aggregiert; geerbte Rechte aus Rollen oder ueber Ownership-Chaining werden nicht vollstaendig simuliert.

## Anwendungsfall

Nutze das Skript, wenn du erkennen moechtest:

- welche Funktionen mit `EXECUTE AS OWNER` oder einem expliziten Principal laufen,
- wo explizite `GRANT`- oder `DENY`-Eintraege auf Funktionsobjekten hinterlegt sind,
- ob Objekt-Owner und Schema-Owner voneinander abweichen und deshalb ein Review sinnvoll ist.

## Hinweise und Grenzen

- Das Skript aendert keine Objekte und schreibt keine Daten.
- Nicht jede Abweichung ist automatisch problematisch; das Resultset liefert bewusst Review-Signale statt harter Verbote.
- Die effektive Laufzeitberechtigung kann durch Rollen, Zertifikate oder uebergeordnete Sicherheitskonzepte erweitert sein und wird hier nicht vollstaendig modelliert.
...
|---|---|---|---|---|
| `@SchemaFilter` | `SYSNAME` | `IN` | nein | Optionales Schema zur Eingrenzung der Analyse |
| `@OnlyFlagged` | `BIT` | `IN` | nein | `1` = nur Funktionen mit Warnhinweisen ausgeben |
...
|---|---|---|---|
| `1.0` | `2026-04-22` | `ER` | Erstversion des Diagnose-Skripts fuer Kontext-, Owner- und Berechtigungs-Audits bei Funktionen |
...
flowchart TD
    ...
    B -- Nein --> X[THROW]
    B -- Ja --> C[FunctionBase aus sys.objects sys.schemas und sys.sql_modules lesen]
    C --> D[PermissionRollup aus sys.database_permissions und sys.database_principals aggregieren]
    D --> E[Audit mit ExecutionContext Ownern PermissionSummary Risiko und ReviewNotes bilden]
    E --> F[Detailresultset FunctionExecutionContextAudit ausgeben]
    E --> G[Aggregierte AuditSummary ausgeben]
```
<!-- SQLDOC:MERMAID:END -->
