# FunctionDataAccessHeatmap.sql

Diagnose-Skript fuer Kapitel `24_UserDefinedFunctions`, das Datenzugriffe aus Funktionen als gewichtete Heatmap sichtbar macht. Die Auswertung verbindet aufgeloeste Katalog-Dependencies mit vorsichtigen Textsignalen aus der Funktionsdefinition.
...
| Feld | Wert |
|---|---|
| Script | [FunctionDataAccessHeatmap.sql](FunctionDataAccessHeatmap.sql) |
| Version | `1.0` |
| Typ | `diagnostic-query` |
| Kapitel | `24_UserDefinedFunctions` |
| Sicherheit | `read-only` |
| Zweck | Erstellt eine Heatmap fuer Datenzugriffe aus User-Defined Functions, indem lokale Dependencies, referenzierte Objektarten und heuristische Definition-Signale zu gewichteten Zugriffskategorien verdichtet werden. |
<!-- SQLDOC:SUMMARY_TABLE:END -->

## Einordnung

Das Skript ist als lesende Erstversion fuer Discovery-, Review- und Unterrichtszwecke gedacht. Es hilft dabei, schnell zu sehen, welche Funktionsbereiche relativ viele Datenzugriffssignale, externe Referenzen oder unaufgeloeste Abhaengigkeiten aufweisen.

## Annahmen

- Die Heatmap nutzt `sys.sql_expression_dependencies` als primaere Quelle fuer aufgeloeste Referenzen.
- Nicht aufgeloeste Namen und Textmuster in `sys.sql_modules.definition` werden nur als didaktische Heuristiken behandelt.
- Die Gewichtung dient der Priorisierung fuer Reviews und ist bewusst konservativ statt parsergenau.

## Anwendungsfall

Nutze das Skript, wenn du schnell erkennen moechtest:

- welche Funktionsschemas besonders viele direkte oder indirekte Datenzugriffe enthalten,
- wo externe oder mehrdeutige Referenzen eine manuelle Nachpruefung verdienen,
- welche Zugriffskategorien als Hotspots fuer Refactoring oder Dokumentation gelten koennen.

## Hinweise und Grenzen

- Das Skript veraendert keine Objekte und schreibt keine Daten.
- Dynamisches SQL oder rein textuelle Namensbildung kann nur eingeschraenkt ueber Heuristiken sichtbar werden.
- Eine hohe Heatmap-Bewertung ist ein Review-Signal, kein automatischer Fehlernachweis.
...
| `@OnlyHotspots` | `BIT` | `IN` | nein | `1` = nur Funktionen und Kategorien mit auffaelligen Zugriffssignalen ausgeben |
| `@MinimumHeatScore` | `INT` | `IN` | nein | Untergrenze fuer Heatmap-Zeilen im Summary-Resultset |
...
|---|---|---|---|
| `1.0` | `2026-04-22` | `ER` | Erstversion des Diagnose-Skripts fuer eine Heatmap von Datenzugriffen aus Funktionen |
...
```mermaid
flowchart TD
    A[Parameter vorbereiten] --> B{OnlyHotspots und MinimumHeatScore gueltig?}
    B -- Nein --> X[THROW]
    B -- Ja --> C[FunctionCatalog aus sys.objects sys.schemas und sys.sql_modules lesen]
    C --> D[DependencyBase und DependencyResolved mit sys.sql_expression_dependencies anreichern]
    D --> E[AccessSignals pro Funktion aggregieren]
    E --> F[DefinitionHeuristics aus FROM JOIN APPLY externen Mustern ableiten]
    F --> G[FunctionAudit mit HeatScore und HotspotLevel bilden]
    G --> H[Detailresultset je Funktion ausgeben]
    G --> I[HeatmapRows per Zugriffskategorie entfalten]
    I --> J[Heatmap je Funktionsschema und Zugriffskategorie aggregieren]
    G --> K[SchemaSummary mit Gesamt-HeatScore ausgeben]
```
<!-- SQLDOC:MERMAID:END -->
