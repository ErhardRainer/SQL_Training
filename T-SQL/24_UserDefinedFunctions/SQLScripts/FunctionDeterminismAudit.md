# FunctionDeterminismAudit.sql

Diagnose-Skript fuer Kapitel `24_UserDefinedFunctions`, das vorhandene Funktionen ueber Katalogmetadaten und heuristische Definition-Checks auf Determinismus-, Praezisions- und Seiteneffekt-Hinweise untersucht.
...
| Feld | Wert |
|---|---|
| Script | [FunctionDeterminismAudit.sql](FunctionDeterminismAudit.sql) |
| Version | `1.0` |
| Typ | `diagnostic-query` |
| Kapitel | `24_UserDefinedFunctions` |
| Sicherheit | `read-only` |
| Zweck | Prueft User-Defined Functions auf Metadaten- und Definitionshinweise zu Determinismus, Praezision, Schema Binding und potenziell seiteneffektbehafteten oder kontextabhaengigen Ausdruecken. |
<!-- SQLDOC:SUMMARY_TABLE:END -->

## Einordnung

Das Skript ist als lesende Erstversion fuer Trainings- und Review-Zwecke gedacht. Es eignet sich besonders, um Kandidaten fuer `SCHEMABINDING`, persistierte berechnete Spalten oder vertiefte Code-Reviews sichtbar zu machen.

## Annahmen

- Die Bewertung kombiniert `OBJECTPROPERTYEX` mit Textmustern in `sys.sql_modules.definition`.
- "Seiteneffekt" wird hier didaktisch als Warnsignal fuer nicht deterministische, kontextabhaengige oder manuell zu pruefende Konstrukte verwendet.
- Token wie `UPDATE`, `EXEC` oder `TRY` werden nur als Heuristik markiert und koennen in Kommentaren oder Bezeichnern false positives liefern.

## Anwendungsfall

Nutze das Skript, wenn du in einer Datenbank schnell erkennen moechtest:

- welche Funktionen laut Metadaten bereits deterministisch oder schema-gebunden wirken,
- wo eingebaute Zeit-, Zufalls- oder Session-Funktionen die Stabilitaet einschaenken,
- welche Definitionen eine manuelle Nachpruefung fuer Restriktionen oder Refactoring verdienen.

## Hinweise und Grenzen

- Das Skript veraendert keine Objekte und schreibt keine Daten.
- CLR-Funktionen ohne SQL-Definition koennen nur ueber Metadaten bewertet werden.
- Die Heuristiken ersetzen keine vollstaendige semantische Parser-Analyse.
...
|---|---|---|---|
| `1.0` | `2026-04-17` | `ER` | Erstversion des Diagnose-Skripts fuer Determinismus-Hinweise bei Funktionen |
...
flowchart TD
    ...
    C --> D[DefinitionSignals mit UPPER Definition ableiten]
    D --> E[FlagScan fuer Zeit Zufall Sequence Session Extern DML EXEC Error-Tokens]
    E --> F[Audit mit Risiko Status ReviewNotes und optionalem DefinitionSnippet bilden]
    F --> G[Detailresultset FunctionDeterminismAudit ausgeben]
    F --> H[Aggregierte FindingSummary ausgeben]
```
<!-- SQLDOC:MERMAID:END -->
