# FunctionSchemaBindingReadiness.sql

Diagnose-Skript fuer Kapitel `24_UserDefinedFunctions`, das Funktionen mit einem konservativen Readiness-Score fuer `SCHEMABINDING` versieht und die naechste sinnvolle Vorbereitungsmassnahme direkt mitliefert.
...
| Feld | Wert |
|---|---|
| Script | [FunctionSchemaBindingReadiness.sql](FunctionSchemaBindingReadiness.sql) |
| Version | `1.0` |
| Typ | `diagnostic-query` |
| Kapitel | `24_UserDefinedFunctions` |
| Sicherheit | `read-only` |
| Zweck | Bewertet User-Defined Functions mit einem nachvollziehbaren Readiness-Score fuer die Vorbereitung auf SCHEMABINDING und priorisiert konkrete Nacharbeiten fuer ein manuelles Review. |
<!-- SQLDOC:SUMMARY_TABLE:END -->

## Einordnung

Das Skript ist als didaktisch-konservative Vorpruefung gedacht. Es soll nicht automatisch `SCHEMABINDING` setzen, sondern auffaellige Muster frueh gruppieren und die manuelle Nacharbeit priorisieren.

## Annahmen

- Die Analyse bleibt rein lesend und arbeitet nur mit Katalogmetadaten sowie Definitionsmustern.
- Der Readiness-Score ist eine Heuristik zwischen `0` und `100`; er ersetzt keine fachliche Pruefung der konkreten Funktionslogik.
- Fehlende oder uneindeutige Abhaengigkeiten werden bewusst als Nacharbeitsbedarf modelliert, damit der Review konservativ bleibt.

## Anwendungsfall

Nutze das Skript, wenn du vor einer Ueberarbeitung von Funktionen schnell erkennen willst:

- welche Module bereits nah an einem `SCHEMABINDING`-Review sind,
- welche Funktionen zuerst Moduloptionen, Objektqualifizierung oder deterministische Ausdruecke bereinigen sollten,
- wo Cross-Database-, Cross-Server- oder dynamische Muster einen direkten Einsatz unplausibel machen.

## Hinweise und Grenzen

- Das Skript veraendert keine Funktionen und erzeugt keine DDL-Empfehlung mit Seiteneffekten.
- Die Textsignale fuer `SELECT *`, `EXEC`, `GETDATE()` oder `tempdb` sind bewusst einfach gehalten und koennen Kommentare oder Sonderfaelle nicht vollstaendig semantisch auswerten.
- `already_schema_bound` bedeutet nur, dass der Vorbereitungsstatus bereits erreicht ist; Consumer- und Deployment-Auswirkungen muessen weiterhin separat geprueft werden.
...
| `@MinimumReadinessScore` | `TINYINT` | `IN` | nein | Optionaler Mindestscore zwischen `0` und `100` fuer die Detailausgabe |
...
|---|---|---|---|
| `1.0` | `2026-04-22` | `ER` | Erstversion des Diagnose-Skripts fuer SCHEMABINDING-Readiness bei Funktionen |
...
```mermaid
flowchart TD
    A[Parameter vorbereiten] --> B{"Score und Bit-Parameter gueltig?"}
    B -- Nein --> X[THROW]
    ...
    D --> E[DefinitionSignals aus der Funktionsdefinition ableiten]
    E --> F[Assessment mit Score Status und naechster Aktion berechnen]
    F --> G[Score auf 0 bis 100 normalisieren]
    G --> H[Detailresultset ab Mindestscore ausgeben]
    G --> I[Summary je Readiness-Status aggregieren]
```
<!-- SQLDOC:MERMAID:END -->
