# ExceptIntersectAudit.sql

Dieses Skript erstellt eine kompakte Audit-Sicht auf zwei Ergebnismengen und zeigt dabei, welche Zeilen nur links, nur rechts oder auf beiden Seiten vorkommen. Die Demobasis ist absichtlich klein, enthaelt aber Dubletten und `NULL`-Werte, damit die DISTINCT-Semantik von `EXCEPT` und `INTERSECT` sichtbar wird.
...
| Feld | Wert |
|---|---|
| Script | [ExceptIntersectAudit.sql](ExceptIntersectAudit.sql) |
| Version | `1.0` |
| Typ | `didactic-lab` |
| Kapitel | `09_Set_Operations` |
| Sicherheit | `read-only-tempdb` |
| Zweck | Erstellt eine Audit-Sicht auf gemeinsame und unterschiedliche Zeilen zweier Demo-Mengen mit EXCEPT und INTERSECT. |
<!-- SQLDOC:SUMMARY_TABLE:END -->

## Einordnung

Das Muster eignet sich fuer Reviews von Staging-Ergebnissen, Snapshot-Vergleichen oder Abnahmelisten, wenn zwei Resultsets zunaechst zeilenweise gegeneinander gehalten werden sollen. Statt einen Join mit vielen Bedingungszweigen zu bauen, nutzt das Skript bewusst `EXCEPT` und `INTERSECT`, um Gleichheiten und Unterschiede aus derselben, klar ausgerichteten Spaltenliste abzuleiten.

## Annahmen

- Beide Mengen repraesentieren Monats-Snapshots derselben fachlichen Struktur.
- Die Vergleichsspalten sind auf beiden Seiten bereits typ- und positionskompatibel ausgerichtet.
- Dubletten werden fuer `EXCEPT` und `INTERSECT` absichtlich nicht mehrfach gezaehlt, weil diese Operatoren DISTINCT-basiert arbeiten.
- Die optionale Duplikatdiagnostik dient nur dazu, den Unterschied zwischen Rohdaten und Set-Semantik sichtbar zu machen.

## Anwendungsfall

Die erste Ausgabe liefert ein Detailbild mit `left_only`, `right_only` und optional `in_both`. Die zweite Ausgabe verdichtet diese Sicht zu einer kleinen Audit-Zusammenfassung. Ein drittes optionales Resultset zeigt, wo die Rohmengen vor dem Set-Vergleich Dubletten enthalten und damit moeglicherweise mehr Zeilen im Quellsystem existieren als im DISTINCT-basierten Set-Ergebnis sichtbar werden.
...
|---|---|---|---|
| `@OnlyShowDifferences` | `BIT` | Nein | Zeigt bei `1` im Detail nur `left_only` und `right_only`. |
| `@IncludeSharedRows` | `BIT` | Nein | Gibt bei `1` zusaetzlich die gemeinsamen Zeilen aus `INTERSECT` aus. |
| `@IncludeDuplicateDiagnostics` | `BIT` | Nein | Aktiviert bei `1` ein zusaetzliches Resultset fuer Dubletten pro Seite. |
...
- `NULL`-Werte koennen in Set-Operatoren als gleich behandelt werden, was im Demo-Datensatz ueber die Support-Zeile sichtbar wird.
- `CH-ZH / Platform / 2026-01-01` unterscheidet sich nur im `TicketCount` und erscheint deshalb einmal links und einmal rechts als Differenz.
- Die Dubletten in beiden Rohmengen verschwinden im Set-Vergleich, bleiben aber im optionalen Diagnose-Resultset nachvollziehbar.
...
|---|---|---|---|
| `1.0` | `2026-04-18` | `ER` | Erstversion fuer eine Audit-Sicht mit EXCEPT und INTERSECT auf zwei Demo-Mengen |
...
```mermaid
flowchart TD
    A[Bit-Parameter validieren] --> B[Temp-Tabellen fuer linke und rechte Menge neu aufbauen]
    B --> C[Demo-Zeilen mit Dubletten und NULL-Werten laden]
    C --> D[Distinct-Sichten fuer beide Mengen ableiten]
    D --> E[LeftOnly ueber EXCEPT berechnen]
    D --> F[RightOnly ueber EXCEPT berechnen]
    D --> G[SharedRows ueber INTERSECT berechnen]
    E --> H[Detailsicht mit left_only, right_only und optional in_both zusammensetzen]
    F --> H
    G --> H
    H --> I[AuditSummary pro Kategorie aggregieren]
    I --> J{IncludeDuplicateDiagnostics = 1?}
    J -->|Ja| K[Dubletten pro Seite aus den Rohmengen ausgeben]
    J -->|Nein| L[Ohne Dubletten-Resultset enden]
    K --> L
```
<!-- SQLDOC:MERMAID:END -->
