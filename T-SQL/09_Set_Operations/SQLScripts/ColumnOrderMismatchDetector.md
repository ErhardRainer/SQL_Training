# ColumnOrderMismatchDetector.sql

Dieses Diagnoseskript zeigt vor einer Set-Operation, ob zwei Projektionen dieselbe Spaltenlogik in derselben Reihenfolge liefern. Die Demo nutzt lokale Temp-Tabellen in `tempdb`, damit Vertauschungen und implizite Typkonflikte sichtbar werden, bevor `UNION`, `EXCEPT` oder `INTERSECT` fachlich falsche Ergebnisse erzeugen.
...
| Feld | Wert |
|---|---|
| Script | [ColumnOrderMismatchDetector.sql](ColumnOrderMismatchDetector.sql) |
| Version | `1.0` |
| Typ | `diagnostic-query` |
| Kapitel | `09_Set_Operations` |
| Sicherheit | `read-only-tempdb` |
| Zweck | Prueft Projektionen auf vertauschte Spaltenreihenfolge und implizite Typkonflikte vor Set-Operationen. |
<!-- SQLDOC:SUMMARY_TABLE:END -->

## Einordnung

Das Artefakt passt zu den Kompatibilitaetsregeln von Set-Operationen. Im Fokus steht nicht die Set-Operation selbst, sondern die vorgelagerte Frage, ob beide Teilabfragen positionsgleich dieselben Fachattribute liefern und ob SQL Server dabei problematische implizite Konvertierungen vornehmen wuerde.

## Annahmen

- Die Erstversion arbeitet bewusst mit Demo-Projektionen in lokalen Temp-Tabellen statt mit produktiven Quelltabellen.
- Die rechte Projektion ist absichtlich fachlich fehlgeordnet: `RegionCode` steht an Position 1, obwohl links dort `CustomerCode` liegt.
- `OrderAmount` ist rechts absichtlich als `NVARCHAR(30)` modelliert, um eine typische implizite Konvertierungsfalle vorzufuehren.
- Der Parameter `@SetOperator` beschriftet nur die Auswertung; das Skript fuehrt keine echte `UNION`-, `EXCEPT`- oder `INTERSECT`-Abfrage aus.

## Anwendungsfall

Das Skript eignet sich fuer Reviews von Staging-Abfragen, ETL-Zwischenschritten oder Schulungsbeispielen, in denen zwei Resultsets vor einer Set-Operation zuerst normiert werden muessen. Es laesst sich leicht auf reale Projektionen erweitern, indem die Demo-Temp-Tabellen durch zwei konkrete Teilabfragen ersetzt werden.
...
|---|---|---|---|
| `@SetOperator` | `NVARCHAR(20)` | Nein | Beschriftet die geplante Set-Operation im Bericht, zum Beispiel `UNION` oder `EXCEPT`. |
| `@IncludeSampleData` | `BIT` | Nein | Gibt bei `1` die Beispielzeilen beider Projektionen zusaetzlich aus. |
...
- `ColumnAlignmentReport` bewertet jede Spaltenposition getrennt nach Namensausrichtung und Typkompatibilitaet.
- `CompatibilitySummary` fasst zusammen, ob die geplante Set-Operation sofort ausfuehrbar ist oder vorab explizite `CAST`- bzw. Projektionsanpassungen braucht.
- Die Beispielresultsets zeigen sichtbar, warum eine reine Positionslogik bei Set-Operationen gefaehrlich sein kann.
...
|---|---|---|---|
| `1.0` | `2026-04-18` | `ER` | Erstversion eines Diagnose-Skripts fuer Spaltenreihenfolge und Typkonflikte vor Set-Operationen |
...
```mermaid
flowchart TD
    A[Parameter validieren] --> B[Demo-Temp-Tabellen neu aufbauen]
    B --> C[Linke und rechte Projektion mit Beispielwerten befuellen]
    C --> D[Metadaten aus tempdb.sys.columns und sys.types lesen]
    D --> E[Spalten paarweise nach column_id gegenueberstellen]
    E --> F[Namens- und Typabgleich je Position bewerten]
    F --> G[ColumnAlignmentReport mit Empfehlung ausgeben]
    G --> H[Gesamtrisiken zu CompatibilitySummary verdichten]
    H --> I{IncludeSampleData = 1?}
    I -->|Ja| J[Beispielzeilen der linken Projektion ausgeben]
    J --> K[Beispielzeilen der rechten Projektion ausgeben]
    I -->|Nein| L[Lauf endet nach der Zusammenfassung]
    K --> L
```
<!-- SQLDOC:MERMAID:END -->
