# SetUnionOrderReview.sql

Dieses Diagnose-Skript zeigt an einer kleinen `UNION`-Szenerie, dass die sichtbare Reihenfolge erst durch das finale `ORDER BY` des kombinierten Resultsets entsteht. Die Demo trennt deshalb bewusst lokale Branch-Reihenfolgen von einer fachlichen Gesamtsortierung und stellt diese beide nebeneinander.
...
| Feld | Wert |
|---|---|
| Script | [SetUnionOrderReview.sql](SetUnionOrderReview.sql) |
| Version | `1.0` |
| Typ | `diagnostic-query` |
| Kapitel | `09_Set_Operations` |
| Sicherheit | `read-only-tempdb` |
| Zweck | Reviewt lokale Branch-Sortierung, finales ORDER BY und distincte UNION-Semantik in einem Demo-Szenario. |
<!-- SQLDOC:SUMMARY_TABLE:END -->

## Einordnung

Das Artefakt eignet sich fuer Reviews von Query-Umschreibungen, Reporting-Statements und ETL-Abfragen, bei denen mehrere Teilmengen per `UNION` oder `UNION ALL` zusammengefuehrt werden. Im Fokus steht nicht die reine Ergebnismenge, sondern die Frage, welche Reihenfolge Lernende oder Reviewende nach der Kombination tatsaechlich sehen.

## Annahmen

- Die Erstversion arbeitet mit einer lokalen Temp-Tabelle und kleinen Demo-Zeilen statt mit produktiven Quellabfragen.
- Die linke und rechte Branch enthalten absichtlich einen gemeinsamen Business-Fall fuer `TicketID = 102`, damit der Unterschied zwischen `UNION ALL` und distincter `UNION` sichtbar wird.
- `BUSINESS` sortiert final nach `BusinessPriority` und `TicketID`, waehrend `SOURCE_THEN_LOCAL` die lokale Reihenfolge je Branch explizit mitnimmt.
- Das Skript beschreibt den Effekt finaler Sortierung; es bildet keine absichtlich ungueltigen Unterabfragen mit branch-lokalem `ORDER BY` nach.

## Anwendungsfall

Die Auswertung passt fuer Unterricht zu Set-Operationen, fuer Code Reviews bei Reporting-Queries und fuer Regressionstests, wenn ein neues `ORDER BY` eingefuehrt wird. In realen Szenarien kann die Demo-Struktur leicht durch zwei konkrete Teilabfragen mit eigener Sortierlogik ersetzt werden.
...
|---|---|---|---|
| `@PreferredOutputOrder` | `NVARCHAR(30)` | Nein | `BUSINESS` sortiert nach Fachschluessel, `SOURCE_THEN_LOCAL` behaelt Branch und lokale Reihenfolge sichtbar. |
| `@IncludeDistinctProjection` | `BIT` | Nein | Gibt bei `1` die distincte `UNION`-Projektion ohne Branch-Metadaten zusaetzlich aus. |
| `@ShowRawBranches` | `BIT` | Nein | Gibt bei `1` die vorbereiteten Branch-Zeilen mit lokaler Sortierung zusaetzlich aus. |
...
- `OrderReviewSummary` zeigt auf einen Blick, ob die finale Sortierung die sichtbare Reihenfolge gegenueber der Branch-Vorbereitung veraendert.
- `OrderSensitivityReview` stellt lokale Branch-Position und finale Anzeige-Position pro Zeile direkt gegenueber.
- `DistinctUnionProjection` blendet Branch-Metadaten bewusst aus und zeigt damit, wie eine `UNION`-Sicht dieselbe Business-Zeile nur einmal ausgeben kann.
...
|---|---|---|---|
| `1.0` | `2026-04-18` | `ER` | Erstversion fuer Sortierungs- und Reihenfolgereviews bei UNION-Szenarien |
...
```mermaid
flowchart TD
    A[Parameter validieren] --> B[Demo-Tabelle #OrderReviewInput in tempdb aufbauen]
    B --> C[BranchRows mit lokaler Reihenfolge per ROW_NUMBER vorbereiten]
    C --> D[UnionAllRows aus beiden Branches zusammenfuehren]
    D --> E[FinalOrderedRows mit gewaehlter finaler ORDER-BY-Strategie berechnen]
    C --> F[DistinctUnionRows als UNION-Projektion ohne Branch-Metadaten bilden]
    E --> G[OrderReviewSummary ausgeben]
    G --> H[OrderSensitivityReview mit lokaler und finaler Position ausgeben]
    H --> I{IncludeDistinctProjection = 1?}
    I -->|Ja| J[DistinctUnionProjection ausgeben]
    I -->|Nein| K[Distinct-Projektion ueberspringen]
    J --> L{ShowRawBranches = 1?}
    K --> L
    L -->|Ja| M[RawBranchRows je Branch ausgeben]
    L -->|Nein| N[Lauf endet]
    M --> N
```
<!-- SQLDOC:MERMAID:END -->
