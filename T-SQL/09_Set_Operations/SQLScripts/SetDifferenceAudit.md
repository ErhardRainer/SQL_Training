# SetDifferenceAudit.sql

Dieses Diagnose-Skript vergleicht zwei Snapshot-Mengen als Sets und zeigt mit `EXCEPT` sowie `INTERSECT`, welche Zeilen neu, fehlend oder in beiden Staenden vorhanden sind. Die Erstversion bleibt bewusst in `tempdb`, damit das Muster fuer Delta-Audits und Freigabepruefungen ohne produktive Abhaengigkeiten nachvollziehbar bleibt.
...
| Feld | Wert |
|---|---|
| Script | [SetDifferenceAudit.sql](SetDifferenceAudit.sql) |
| Version | `1.0` |
| Typ | `diagnostic-query` |
| Kapitel | `09_Set_Operations` |
| Sicherheit | `read-only-tempdb` |
| Zweck | Vergleicht zwei Snapshot-Mengen mit EXCEPT und INTERSECT fuer Delta-Analysen. |
<!-- SQLDOC:SUMMARY_TABLE:END -->

## Einordnung

Das Artefakt deckt einen typischen Audit-Fall fuer Set-Operationen ab: Eine fachlich gleich aufgebaute Baseline-Menge wird mit einem spaeteren Snapshot verglichen. Statt Summen oder Statuszahlen zu aggregieren, bewertet das Skript die eigentliche Ergebnismenge und macht so Additionen, Wegfaelle und stabile Schnittmengen sichtbar.

## Annahmen

- Die Erstversion verwendet eine lokale Temp-Tabelle mit Demo-Snapshots statt produktiver Staging- oder Reporting-Tabellen.
- Verglichen wird eine distincte Projektion aus `OrderID`, `CustomerCode`, `FulfillmentStatus` und `WarehouseCode`.
- Die Beispielwerte zeigen absichtlich alle drei Faelle: neue Zeilen, fehlende Zeilen und stabile Schnittmengen.
- Eine Statusaenderung derselben `OrderID` wird als Mengendifferenz behandelt, weil die Fachprojektion den Status explizit enthaelt.

## Anwendungsfall

Das Skript eignet sich fuer Snapshot-Abgleiche in ETL-Strecken, Release-Checks nach Datenmigrationen oder Schulungen zu `EXCEPT` und `INTERSECT`. In realen Szenarien kann die Demo-Tabelle durch zwei fachliche Teilabfragen oder durch zwei Snapshot-Staende derselben Quelle ersetzt werden.
...
|---|---|---|---|
| `@BaselineSnapshotLabel` | `NVARCHAR(30)` | Nein | Bezeichner der Baseline-Menge. |
| `@CandidateSnapshotLabel` | `NVARCHAR(30)` | Nein | Bezeichner der Vergleichsmenge. |
| `@IncludeIntersectionDetails` | `BIT` | Nein | Gibt bei `1` die Schnittmenge beider Snapshots als zusaetzliches Resultset aus. |
...
- `AuditSummary` verdichtet die Mengenanzahl beider Snapshots und klassifiziert das Delta als `snapshots-match`, `candidate-additions-only`, `candidate-missing-rows` oder `bidirectional-drift`.
- `OnlyInCandidate` zeigt Zeilen, die nur im juengeren Snapshot vorhanden sind.
- `OnlyInBaseline` zeigt Zeilen, die im Vergleichssnapshot fehlen.
- `InBothSnapshots` bleibt optional, damit das Hauptmuster auch bei grossen Mengen schlank eingesetzt werden kann.
...
|---|---|---|---|
| `1.0` | `2026-04-18` | `ER` | Erstversion fuer Delta-Audits mit EXCEPT und INTERSECT |
...
```mermaid
flowchart TD
    A[Parameter und Snapshot-Labels validieren] --> B[Demo-Tabelle #OrderSnapshots in tempdb neu aufbauen]
    B --> C[Baseline- und Candidate-Snapshot mit Beispielzeilen befuellen]
    C --> D[Distincte BaselineSet-Projektion bilden]
    D --> E[Distincte CandidateSet-Projektion bilden]
    E --> F[Mit EXCEPT nur im Candidate vorhandene Zeilen ermitteln]
    F --> G[Mit EXCEPT nur in der Baseline vorhandene Zeilen ermitteln]
    G --> H[Mit INTERSECT stabile Schnittmenge beider Snapshots bestimmen]
    H --> I[AuditSummary mit Outcome und Empfehlung ausgeben]
    I --> J[OnlyInCandidate ausgeben]
    J --> K[OnlyInBaseline ausgeben]
    K --> L{IncludeIntersectionDetails = 1?}
    L -->|Ja| M[InBothSnapshots ausgeben]
    L -->|Nein| N[Lauf endet ohne Schnittmengenliste]
    M --> N
```
<!-- SQLDOC:MERMAID:END -->
