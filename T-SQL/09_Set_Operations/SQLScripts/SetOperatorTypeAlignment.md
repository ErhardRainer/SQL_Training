# SetOperatorTypeAlignment.sql

Dieses Diagnose-Skript prueft, ob zwei fachlich gleich gemeinte Eingangsprojektionen vor `UNION`, `EXCEPT` oder `INTERSECT` auf gemeinsame Zieltypen ausgerichtet werden koennen. Die Erstversion arbeitet mit zwei lokalen Temp-Quellen und zeigt bewusst sowohl saubere Konvertierungen als auch problematische Importwerte.
...
| Feld | Wert |
|---|---|
| Script | [SetOperatorTypeAlignment.sql](SetOperatorTypeAlignment.sql) |
| Version | `1.0` |
| Typ | `diagnostic-query` |
| Kapitel | `09_Set_Operations` |
| Sicherheit | `read-only-tempdb` |
| Zweck | Prueft die Typ-Ausrichtung beider Quellprojektionen vor Set-Operationen. |
<!-- SQLDOC:SUMMARY_TABLE:END -->

## Einordnung

Set-Operatoren verlangen nicht nur dieselbe Spaltenanzahl und Reihenfolge, sondern auch zueinander passende Datentypen. Das Skript zeigt deshalb eine kleine Vorstufe vor der eigentlichen Mengenoperation: Zuerst werden Quelltypen, Zieltypen und Konvertierbarkeit geprueft, danach kann optional eine explizit ausgerichtete `UNION ALL`-Projektion betrachtet werden.

## Annahmen

- Die Erstversion vergleicht eine aktuelle Fachquelle mit einer importierten Textquelle in lokalen Temp-Tabellen.
- Der kanonische Zieltyp wird pro logischer Spalte didaktisch vorgegeben, damit die Ausrichtungsentscheidung im Resultset nachvollziehbar bleibt.
- `100X`, `1499,95` und `Y` sind absichtlich problematische Importwerte, um Konvertierungsrisiken sichtbar zu machen.
- Die optionale Projektionsvorschau dient nur als Vorstufe fuer spaetere Set-Operationen und fuehrt selbst noch keine `UNION`, `EXCEPT` oder `INTERSECT`-Bewertung fachlicher Unterschiede aus.

## Anwendungsfall

Das Muster eignet sich fuer Staging-Checks, Importpruefungen oder Unterrichtseinheiten zu Mengenoperatoren. Vor allem bei CSV-, Excel- oder Fremdsystem-Importen hilft eine explizite Typ-Ausrichtung, bevor zwei Resultsets wirklich zu einer Menge kombiniert werden.
...
|---|---|---|---|
| `@OnlyShowMismatches` | `BIT` | Nein | Zeigt bei `1` nur Spalten mit Ausrichtungsbedarf oder Konvertierungsrisiko. |
| `@IncludeAlignedProjection` | `BIT` | Nein | Gibt bei `1` eine bereits typausgerichtete `UNION ALL`-Vorschau aus. |
...
- `AlignmentSummary` klassifiziert den Gesamtzustand als `ready`, `ready-after-explicit-alignment` oder `not-ready`.
- `ColumnAlignmentDetails` zeigt pro logischer Spalte die Ausgangstypen, den Zieltyp und empfohlene Ausdruecke fuer die Alignmentschicht.
- `conversion-risk` markiert Werte, die selbst mit `TRY_CONVERT` nicht sauber in den Zieltyp ueberfuehrt werden.
- Die optionale Projektionsvorschau zeigt, welche Importzeilen vor einer echten Set-Operation noch bereinigt werden muessen.
...
|---|---|---|---|
| `1.0` | `2026-04-18` | `ER` | Erstversion fuer Typ-Ausrichtungschecks vor Set-Operationen |
...
```mermaid
flowchart TD
    A[Parameter validieren] --> B[Temp-Quellen CurrentOrders und ImportedOrders neu aufbauen]
    B --> C[Didaktische Beispielzeilen fuer aktuelle Quelle und Importquelle laden]
    C --> D[Spaltenweise Alignment-Matrix per VALUES definieren]
    D --> E[TRY_CONVERT-Risiken und Zieltypen pro logischer Spalte bewerten]
    E --> F[AlignmentSummary mit Gesamtstatus berechnen]
    F --> G[ColumnAlignmentDetails optional auf Mismatches filtern]
    G --> H{IncludeAlignedProjection = 1?}
    H -->|Ja| I[Aktuelle Quelle und Importquelle explizit auf Zieltypen projizieren]
    I --> J[Ausgerichtete UNION ALL-Vorschau mit RowAlignmentStatus ausgeben]
    H -->|Nein| K[Lauf endet nach den Diagnose-Resultsets]
    J --> K
```
<!-- SQLDOC:MERMAID:END -->
