# SetChangeWindowCompare.sql

Dieses didaktische Skript vergleicht zwei Zeitfenster als Mengen aktiver Customer-Product-Kombinationen. Statt Einzelereignisse direkt gegeneinanderzustellen, werden zuerst distinct Mengen pro Fenster gebildet und anschliessend mit `EXCEPT` und `INTERSECT` auf Zugaenge, Abgaenge und stabile Schnittmengen geprueft.
...
| Feld | Wert |
|---|---|
| Script | [SetChangeWindowCompare.sql](SetChangeWindowCompare.sql) |
| Version | `1.0` |
| Typ | `didactic-lab` |
| Kapitel | `09_Set_Operations` |
| Sicherheit | `read-only-tempdb` |
| Zweck | Vergleicht aktive Customer-Product-Kombinationen zwischen zwei Zeitfenstern mit `EXCEPT` und `INTERSECT`. |
<!-- SQLDOC:SUMMARY_TABLE:END -->

## Einordnung

Der Schwerpunkt liegt auf Set-Operationen ueber zeitlich gefilterte Bestandsmengen. Das Muster eignet sich fuer Drift-Analysen, Delta-Pruefungen oder fachliche Snapshot-Vergleiche, wenn nicht jede Einzeltransaktion, sondern die im jeweiligen Zeitraum gueltige Menge relevant ist.

## Annahmen

- Die Erstversion nutzt eine kleine Demo-Historie in lokalen Temp-Tabellen statt produktiver Bewegungsdaten.
- Verglichen werden distinct Customer-Product-Kombinationen, damit Mehrfachquellen oder wiederholte Bewegungen die Set-Ergebnisse nicht kuenstlich aufblasen.
- Eine offene Gueltigkeit wird ueber `ValidTo IS NULL` modelliert und fuer den Vergleich als lang laufend behandelt.
- Das optionale vierte Resultset wird nur bei `@IncludeStableOverlap = 1` ausgegeben.

## Anwendungsfall

Das Skript passt zu Szenarien wie Sortimentswechseln, Freischaltungsfenstern oder Berechtigungsvergleichen zwischen zwei Stichtags- oder Monatsfenstern. Fuer reale Nutzung kann die Demo-Tabelle durch eine echte Historien- oder Snapshot-Quelle ersetzt werden, solange pro Fenster dieselbe Projektion geliefert wird.
...
|---|---|---|---|
| `@WindowAStart` | `DATETIME2(0)` | Nein | Start des ersten Vergleichsfensters. |
| `@WindowAEnd` | `DATETIME2(0)` | Nein | Ende des ersten Vergleichsfensters. |
| `@WindowBStart` | `DATETIME2(0)` | Nein | Start des zweiten Vergleichsfensters. |
| `@WindowBEnd` | `DATETIME2(0)` | Nein | Ende des zweiten Vergleichsfensters. |
| `@IncludeStableOverlap` | `BIT` | Nein | Gibt bei `1` die in beiden Fenstern stabil vorhandenen Kombinationen zusaetzlich aus. |
...
- Die Gueltigkeitslogik behandelt jedes Zeitfenster als Intervallueberlappung: ein Datensatz zaehlt, wenn er im Fenster mindestens teilweise aktiv war.
- `OnlyInWindowA` zeigt Kombinationen, die vor dem zweiten Fenster weggefallen sind.
- `OnlyInWindowB` zeigt Kombinationen, die spaeter hinzugekommen sind.
- Die Demo enthaelt absichtlich einen doppelten Quellhinweis fuer `C-106` und `P-GAMMA`, damit sichtbar bleibt, warum `DISTINCT` vor den Set-Operationen sinnvoll ist.
...
|---|---|---|---|
| `1.0` | `2026-04-18` | `ER` | Erstversion eines didaktischen Zeitfenstervergleichs fuer Set-Operationen |
...
```mermaid
flowchart TD
    A[Zeitfenster und BIT-Parameter validieren] --> B[Demo-Historie in tempdb neu aufbauen]
    B --> C[Distinct Menge fuer WindowA ueber Gueltigkeitsintervall bilden]
    C --> D[Distinct Menge fuer WindowB ueber Gueltigkeitsintervall bilden]
    D --> E[Mit EXCEPT Kombinationen nur aus WindowA ableiten]
    E --> F[Mit EXCEPT Kombinationen nur aus WindowB ableiten]
    F --> G[Mit INTERSECT stabile Schnittmenge bilden]
    G --> H[Summary mit Mengenanzahlen ausgeben]
    H --> I[RemovedBeforeWindowB ausgeben]
    I --> J[AddedByWindowB ausgeben]
    J --> K{IncludeStableOverlap = 1?}
    K -->|Ja| L[StableAcrossWindows ausgeben]
    K -->|Nein| M[Lauf endet nach den Delta-Resultsets]
    L --> M
```
<!-- SQLDOC:MERMAID:END -->
