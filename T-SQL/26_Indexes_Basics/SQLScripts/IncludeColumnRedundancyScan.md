# IncludeColumnRedundancyScan.sql

Dieses Skript prueft Nichtclustered-Indizes auf Include-Spalten, die auf Tabellen mit Clusterschluessel bereits implizit in den Blattseiten mitgefuehrt werden. Dadurch eignet sich die Auswertung gut fuer Reviews zu schlanken INCLUDE-Listen, Indexbreite und Metadatenanalyse.
...
| Feld | Wert |
|---|---|
| Script | [IncludeColumnRedundancyScan.sql](IncludeColumnRedundancyScan.sql) |
| Version | `1.0` |
| Typ | `diagnostic-query` |
| Kapitel | `26_Indexes_Basics` |
| Sicherheit | `read-only` |
| Zweck | Scannt Nichtclustered-Indizes auf explizite Include-Spalten, die bereits implizit ueber den Clusterschluessel vorhanden sind. |
<!-- SQLDOC:SUMMARY_TABLE:END -->

## Einordnung

Im Kapitel `26_Indexes_Basics` ergaenzt das Skript die vorhandenen Auswertungen zu Definition, Nutzung und Fragmentierung um eine Review-Perspektive auf Indexdesign. Es beantwortet nicht, ob ein Index fachlich benoetigt wird, sondern zeigt gezielt Stellen, an denen die INCLUDE-Liste unnoetig breit sein koennte.

## Annahmen

- Das Skript arbeitet rein lesend auf Systemkatalogen der aktuellen Datenbank.
- Redundanz wird bewusst konservativ nur gegen Spalten des Clusterschluessels derselben Tabelle bewertet.
- Vor einer echten Indexaenderung sollte die Workload mit Ausfuehrungsplaenen und Nutzungsmustern validiert werden.
- Tabellen ohne Clusterschluessel liefern in diesem Skript keine direkte Redundanz-Markierung fuer ihre Include-Spalten.

## Anwendungsfall

Die Detailausgabe eignet sich fuer Code Reviews, Tuning-Sessions und Unterrichtsgespraeche zu Blattbreite, Covering-Indizes und implizit mitgefuehrten Schluesselspalten. Die zweite Ausgabe verdichtet die Findings je Index und hilft dabei, Kandidaten fuer eine tiefergehende Designpruefung zu priorisieren.
...
| `@OnlyRedundant` | `BIT` | Nein | Zeigt bei `1` nur redundante Include-Spalten, bei `0` alle Include-Spalten mit Klassifikation. |
...
- `IncludedColumnReview` listet je Include-Spalte die erkannte Redundanz, die Clusterschluesselspalte und eine kurze Handlungsempfehlung.
- `IndexRedundancySummary` aggregiert dieselben Findings je Index und berechnet den Anteil redundanter Include-Spalten.
- Gefilterte Indizes werden nicht ausgeschlossen; ihre Filterdefinition wird in der Detailansicht mit ausgegeben, damit Reviews den Kontext nicht verlieren.
...
|---|---|---|---|
| `1.0` | `2026-04-17` | `ER` | Erstversion fuer den Scan redundanter Include-Spalten |
...
```mermaid
flowchart TD
    A[Parameter einlesen] --> B{OnlyRedundant gueltig?}
    B -->|Nein| C[THROW Fehler]
    B -->|Ja| D[TargetIndexes aus sys.tables und sys.indexes filtern]
    D --> E[Include-Spalten pro Nichtclustered-Index ermitteln]
    E --> F[Clustered-Key-Spalten derselben Tabellen sammeln]
    F --> G[Include-Spalten gegen Clustered Key klassifizieren]
    G --> H{OnlyRedundant = 1?}
    H -->|Ja| I[Nur redundante Detailzeilen ausgeben]
    H -->|Nein| J[Alle Include-Spalten mit Klassifikation ausgeben]
    I --> K[Summary je Index aggregieren]
    J --> K
```
<!-- SQLDOC:MERMAID:END -->
