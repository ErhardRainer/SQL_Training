# HeapTablesInventory.sql

Dieses Skript inventarisiert Heap-Tabellen der aktuellen Datenbank und verdichtet ihre Metadaten zu einer vorsichtigen Review-Sicht. Der Fokus liegt auf Unterricht, Diagnose und Priorisierung, nicht auf einer automatischen Forderung nach einem Clustered Index.
...
| Feld | Wert |
|---|---|
| Script | [HeapTablesInventory.sql](HeapTablesInventory.sql) |
| Version | `1.0` |
| Typ | `diagnostic-query` |
| Kapitel | `26_Indexes_Basics` |
| Sicherheit | `read-only` |
| Zweck | Inventarisiert Heap-Tabellen und markiert konservative Review-Kandidaten fuer Clustering-, Key- und Weiterleitungs-Analysen. |
<!-- SQLDOC:SUMMARY_TABLE:END -->

## Einordnung

Im Kapitel `26_Indexes_Basics` ergaenzt das Skript die bisherigen Auswertungen zu doppelten, gefilterten und zu breiten Indizes um die Frage, welche Tabellen ueberhaupt als Heap betrieben werden. Sichtbar werden Groesse, physische Auffaelligkeiten und einige typische Review-Signale wie viele `Forwarded Records`, scan-lastige Nutzung oder fehlende Key-Constraints.

## Annahmen

- Das Skript arbeitet rein lesend auf Systemkatalogen, `sys.dm_db_index_usage_stats` und `sys.dm_db_index_physical_stats`.
- Ein Heap ist nicht automatisch ein Designfehler; die Ausgabe priorisiert nur konservative Review-Anlaesse.
- `ForwardedRecordCount` ist als starkes Warnsignal modelliert, weil Weiterleitungen insbesondere bei Updates und Scans teurer werden koennen.
- Eine fehlende `Primary Key`- oder `Unique Constraint`-Markierung bedeutet nicht zwangslaufig fehlende fachliche Schluessel, sondern nur, dass sie in den gelesenen Indexmetadaten nicht sichtbar ist.

## Anwendungsfall

Die Detailausgabe zeigt je Heap die geschaetzte Groesse, physische Kennzahlen, Nutzungszaehler und eine Review-Klasse. Die zweite Ausgabe aggregiert diese Signale zu einer kurzen Priorisierung, damit groessere oder technisch auffaellige Heaps zuerst untersucht werden koennen.
...
| `@OnlyCandidates` | `BIT` | Nein | Zeigt bei `1` nur Heaps mit erkennbarem Review-Signal, bei `0` alle Heaps. |
| `@MinRowCount` | `BIGINT` | Nein | Mindestanzahl geschaetzter Zeilen fuer groessere Heap-Kandidaten. |
...
- `forwarded-record-hotspot` priorisiert Heaps mit vielen Weiterleitungen, weil diese besonders haeufig in Tuning- oder Pflegegespraechen auffallen.
- `large-scan-oriented-heap` ist ein konservatives Signal fuer groessere Heaps, die eher gescannt als gesucht werden und keinen `Primary Key` tragen.
- `large-heap-without-nci` markiert groessere Heaps ohne aktive Nonclustered-Indizes; das kann bewusst minimalistisch sein, sollte aber nachvollziehbar sein.
- `heap-without-key-constraint` hebt kleinere oder gemischte Faelle hervor, bei denen die eindeutige Zeilenidentifikation aus Metadaten nicht klar wird.
...
|---|---|---|---|
| `1.0` | `2026-04-17` | `ER` | Erstversion fuer die Inventur und Review-Priorisierung von Heap-Tabellen |
...
```mermaid
flowchart TD
    A[Parameter einlesen] --> B{OnlyCandidates und MinRowCount gueltig?}
    B -->|Nein| C[THROW Fehler]
    B -->|Ja| D[Heap-Basis aus sys.tables sys.indexes und Partition Stats laden]
    D --> E[Physische Heap-Werte aus sys.dm_db_index_physical_stats holen]
    E --> F[Nutzungszaehler sowie Nonclustered- und Constraint-Summaries anreichern]
    F --> G[ReviewClass aus Forwarded Records Groesse Nutzung und Key-Status ableiten]
    G --> H{OnlyCandidates = 1?}
    H -->|Ja| I[Nur Heaps mit Review-Signal ausgeben]
    H -->|Nein| J[Alle Heaps inventarisieren]
    I --> K[Review-Klassen zu HeapCandidateSummary aggregieren]
    J --> K
```
<!-- SQLDOC:MERMAID:END -->
