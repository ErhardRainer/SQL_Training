# IncludedColumnBloatReview.sql

Dieses Skript bewertet Nichtclustered-Indizes anhand ihrer Include-Listen und markiert Kandidaten, deren Blattzeilen durch viele, breite oder MAX- beziehungsweise LOB-aehnliche Include-Spalten unnoetig aufgeblasen wirken koennten. Die Auswertung ist bewusst als Review-Hilfe fuer Unterricht, Architekturgespraeche und erste Tuning-Sichtungen ausgelegt.
...
| Feld | Wert |
|---|---|
| Script | [IncludedColumnBloatReview.sql](IncludedColumnBloatReview.sql) |
| Version | `1.0` |
| Typ | `diagnostic-query` |
| Kapitel | `26_Indexes_Basics` |
| Sicherheit | `read-only` |
| Zweck | Markiert Nichtclustered-Indizes mit potenziell zu breiten Include-Listen anhand konservativer Breitenheuristiken. |
<!-- SQLDOC:SUMMARY_TABLE:END -->

## Einordnung

Im Kapitel `26_Indexes_Basics` ergaenzt das Skript die vorhandenen Metadatenabfragen um eine Diagnose auf Leaf-Level-Breite. Es schaut nicht auf Fragmentierung oder echte Laufzeitplaene, sondern auf die Struktur der Include-Liste selbst und liefert damit eine fruehe Review-Perspektive fuer Covering-Indizes.

## Annahmen

- Das Skript arbeitet ausschliesslich lesend auf den Systemkatalogen der aktuellen Datenbank.
- Eine Markierung bedeutet nur Review-Bedarf und nicht automatisch, dass ein Index fachlich falsch ist.
- Die Breitenheuristik summiert nur bekannte feste oder deklarierte Maximalbreiten und ersetzt keine physische Seitenauswertung.
- MAX- und LOB-aehnliche Typen werden separat markiert, weil ihre reale Speicherwirkung vom konkreten Zugriffsmuster und vom Indexdesign abhaengt.

## Anwendungsfall

Die erste Ausgabe priorisiert Indexe, deren INCLUDE-Listen gegen konfigurierbare Schwellwerte laufen. Die zweite Ausgabe zeigt je Include-Spalte, welche Typ- und Breitenannahme in die Bewertung eingeflossen ist. Damit lassen sich breite Covering-Indizes fuer Reviews, Refactorings oder Lernbeispiele gezielt herausfiltern.
...
| `@MinIncludedColumnCount` | `INT` | Nein | Untergrenze fuer die Anzahl der Include-Spalten, ab der ein Index markiert wird. |
| `@MinEstimatedIncludeBytes` | `INT` | Nein | Untergrenze fuer die geschaetzte feste Include-Byte-Breite, ab der ein Index markiert wird. |
| `@ShowOnlyFlagged` | `BIT` | Nein | Zeigt bei `1` nur markierte Indizes, bei `0` alle geprueften Indizes. |
...
- Die Summary-Ausgabe kombiniert drei Signale: Anzahl der Include-Spalten, geschaetzte Byte-Breite und Vorhandensein von MAX- oder LOB-aehnlichen Includes.
- Die Detailausgabe hilft beim Nachvollziehen, welche Spalten den Review-Hinweis ausloesen oder verstaerken.
- Gefilterte Indizes bleiben im Scope und tragen ihre Filterdefinition in die Zusammenfassung, damit der Review-Kontext sichtbar bleibt.
...
|---|---|---|---|
| `1.0` | `2026-04-17` | `ER` | Erstversion fuer die Review potenziell zu breiter Include-Listen |
...
flowchart TD
    ...
    B -->|Ja| D[Zielindizes aus sys.tables und sys.indexes filtern]
    D --> E[Include-Spalten mit Typ- und Breitenmetadaten sammeln]
    E --> F[Indexweite Kennzahlen aggregieren]
    F --> G[Schwellwerte fuer Include-Anzahl, Byte-Breite und MAX LOB pruefen]
    G --> H[Summary je Index ausgeben]
    G --> I[Detailzeilen je Include-Spalte mit Beitragsbewertung ausgeben]
```
<!-- SQLDOC:MERMAID:END -->
