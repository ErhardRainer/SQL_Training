/* Index fragmentation report + recommendation (Refresh/Rebuild) FOR ONE TABLE */

DECLARE @SchemaName SYSNAME = N'sap';
DECLARE @TableName  SYSNAME = N'JournalEntry_CO_CBC';

DECLARE @MinPageCount INT   = 1000;  -- ignore tiny indexes
DECLARE @RefreshFrag  FLOAT = 5.0;   -- >= 5%  => Refresh (REORGANIZE)
DECLARE @RebuildFrag  FLOAT = 30.0;  -- >= 30% => Rebuild (REBUILD)

DECLARE @ObjectId INT = OBJECT_ID(QUOTENAME(@SchemaName) + N'.' + QUOTENAME(@TableName));

IF @ObjectId IS NULL
BEGIN
    THROW 50000, N'Table not found in current database.', 1;
END;
...
(
    SELECT
        ...
        page_count
    FROM sys.dm_db_index_physical_stats(DB_ID(), @ObjectId, NULL, NULL, 'SAMPLED')
    WHERE index_id > 0
),
agg AS
(
