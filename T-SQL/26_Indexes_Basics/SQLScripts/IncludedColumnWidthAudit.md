# IncludedColumnWidthAudit.sql

Dieses Skript bewertet Nichtclustered-Indizes anhand der geschaetzten Breite ihrer Include-Spalten und ordnet die Ergebnisse in Bezug auf Leaf-Zeilenbreite und Seitendichte ein. Die Auswertung ist als konservative Metadatenanalyse fuer Unterricht, Reviews und erste Tuning-Sichtungen gedacht.
...
| Feld | Wert |
|---|---|
| Script | [IncludedColumnWidthAudit.sql](IncludedColumnWidthAudit.sql) |
| Version | `1.0` |
| Typ | `diagnostic-query` |
| Kapitel | `26_Indexes_Basics` |
| Sicherheit | `read-only` |
| Zweck | Bewertet breite Include-Spalten und deren Einfluss auf die geschaetzte Blattzeilenbreite von Nichtclustered-Indizes. |
<!-- SQLDOC:SUMMARY_TABLE:END -->

## Einordnung

Im Kapitel `26_Indexes_Basics` ergaenzt das Skript die vorhandenen Reviews zu redundanten und breiten INCLUDE-Listen. Es zaehlt nicht nur die Include-Spalten, sondern addiert auch die Breite des Nonclustered-Schluessels und des impliziten Row-Locators, damit die Leaf-Kosten eines Covering-Indexes didaktisch greifbarer werden.

## Annahmen

- Das Skript arbeitet rein lesend auf den Systemkatalogen der aktuellen Datenbank.
- Die Breitenwerte sind deklarierte Metadatenheuristiken und keine physische Seitenmessung.
- Auf Tabellen mit Clusterschluessel wird dessen Schluesselbreite als impliziter Locator beruecksichtigt.
- Auf Heaps wird der Locator didaktisch mit acht Bytes als RID angenaehert.
- MAX- und LOB-aehnliche Include-Spalten werden separat markiert, weil ihre Wirkung nicht sauber ueber `max_length` modelliert werden kann.

## Anwendungsfall

Die erste Ausgabe priorisiert Nichtclustered-Indizes nach geschaetzter Leaf-Breite, Include-Breite und Seitendichte. Die zweite Ausgabe zeigt je Include-Spalte ihren geschaetzten Anteil an der gesamten Include-Breite. Dadurch lassen sich breite Covering-Indizes fuer Refactoring, Review und Unterricht gezielt herausfiltern.
...
| `@WarnIncludedBytes` | `INT` | Nein | Schwellwert fuer die geschaetzte Include-Gesamtbreite. |
| `@WarnLeafBytes` | `INT` | Nein | Schwellwert fuer die geschaetzte Leaf-Zeilenbreite. |
| `@ShowOnlyFlagged` | `BIT` | Nein | Zeigt bei `1` nur markierte Faelle, bei `0` alle geprueften Faelle. |
...
- `IndexWidthSummary` kombiniert Schluesselbreite, Include-Breite und Locator-Breite zu einer geschaetzten Leaf-Zeilenbreite.
- `EstimatedRowsPerPage` ist nur eine Rechenhilfe auf Basis von `8096 / EstimatedLeafRowBytes`.
- `WidthClass` unterscheidet kompakte, moderate und review-pflichtige Faelle.
- Gefilterte Indizes bleiben im Scope; ihre Filterdefinition wird mit ausgegeben.
...
|---|---|---|---|
| `1.0` | `2026-04-17` | `ER` | Erstversion fuer das Audit breiter Include-Spalten |
...
flowchart TD
    ...
    D --> E[Key- und Include-Spalten mit Typ- und Breitenmetadaten sammeln]
    E --> F[Clustered-Key oder Heap-RID als Locator ableiten]
    F --> G[Key Include und Locator je Index aggregieren]
    G --> H[Leaf-Breite und EstimatedRowsPerPage berechnen]
    H --> I[WidthClass und Review-Signal festlegen]
    I --> J[Summary je Index ausgeben]
    I --> K[Detailzeilen je Include-Spalte ausgeben]
```
<!-- SQLDOC:MERMAID:END -->
