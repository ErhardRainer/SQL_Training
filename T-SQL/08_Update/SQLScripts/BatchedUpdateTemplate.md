# BatchedUpdateTemplate.sql

Dieses Skript zeigt ein kontrolliertes Batch-Update-Muster fuer groessere `UPDATE`-Strecken. Die Demo arbeitet ausschliesslich in `tempdb`, trennt jede Portion ueber eine eigene Transaktion ab und dokumentiert pro Batch, ob committed oder nur als Dry-Run simuliert wurde.
...
| Feld | Wert |
|---|---|
| Script | [BatchedUpdateTemplate.sql](BatchedUpdateTemplate.sql) |
| Version | `1.0` |
| Typ | `template` |
| Kapitel | `08_Update` |
| Sicherheit | `demo-write-tempdb` |
| Zweck | Vorlage fuer grosse Updates in kontrollierten Batches mit Transaktionsgrenzen. |
<!-- SQLDOC:SUMMARY_TABLE:END -->

## Einordnung

Das Artefakt ist bewusst als didaktisch ausfuehrbares Template gebaut. Es zeigt kein produktives Massenupdate gegen Fachtabellen, sondern ein uebertragbares Muster fuer stabile Batch-Selektion, saubere Commit-Grenzen und einen vorab pruefbaren Dry-Run.

## Annahmen

- Die Erstversion arbeitet ausschliesslich mit Demo-Tabellen in `tempdb`.
- Das Muster priorisiert Nachvollziehbarkeit vor maximaler Performance und nutzt daher eine einfache `TOP (@BatchSize)`-Selektion mit stabiler Sortierung nach `WorkItemID`.
- `@DryRun = 1` rollt den ersten Batch bewusst wieder zurueck und beendet den Lauf danach, damit der Effekt klar sichtbar bleibt.
- Die Demo verwendet Kreditlimit-Anpassungen als neutrale Update-Story, ohne produktive Fachregeln zu behaupten.

## Anwendungsfall

Das Skript passt zu Kapitelabschnitten ueber Batching, Wiederanlauf und kontrollierte Transaktionsgroessen. Es kann als Ausgangspunkt fuer produktionsnahe Varianten mit echten Zieltabellen, zusaetzlichen Filterkriterien, WAITFOR-Throttling oder erweitertem Monitoring dienen.
...
|---|---|---|---|
| `@BatchSize` | `INT` | Nein | Maximale Anzahl Zeilen, die pro Batch aktualisiert werden. |
| `@MaxBatches` | `INT` | Nein | Obergrenze fuer die Anzahl auszufuehrender Batches. |
| `@DryRun` | `BIT` | Nein | Simuliert bei `1` die Batch-Updates und rollt jede Batch-Transaktion wieder zurueck. |
...
- `demo.BatchUpdateQueue` haelt die noch offenen und bereits aktualisierten Demo-Zeilen.
- `demo.BatchUpdateAudit` sammelt nur bei committeten Laeufen die Detailzeilen je Batch; die eigentliche Batch-Uebersicht wird immer ueber `#BatchLog` ausgegeben.
- Die Zusatzzeile mit `DecisionLabel = stopped` markiert bewusst den Fall, dass `@MaxBatches` erreicht wurde und eine Restmenge fuer spaetere Laeufe bestehen bleibt.
...
|---|---|---|---|
| `1.0` | `2026-04-17` | `ER` | Erstversion eines didaktischen Batch-Update-Templates mit Transaktionsgrenzen |
...
flowchart TD
    ...
    B --> C[Demo-Tabellen fuer Queue und Audit anlegen]
    C --> D{ResetDemoData = 1?}
    D -->|Ja| E[Demo-Daten neu befuellen]
    D -->|Nein| F[Vorhandene Demo-Daten weiterverwenden]
    E --> G[BatchLog-Temp-Tabelle vorbereiten]
    F --> G
    G --> H[Restmenge offener Updates zaehlen]
    H --> I{Restmenge > 0 und Batch-Limit nicht erreicht?}
    I -->|Nein| N[BatchLog ausgeben]
    I -->|Ja| J[TOP BatchSize per WorkItemID in CurrentBatch laden]
    J --> K[Transaktion starten und Batch-Update ausfuehren]
    K --> L{DryRun = 1?}
    L -->|Ja| M[ROLLBACK und Batch als rolled_back protokollieren]
    L -->|Nein| P[COMMIT und Batch als committed protokollieren]
    M --> Q[Loop beenden]
    P --> R[Restmenge neu zaehlen]
    R --> I
    Q --> N
    N --> O[Finale Queue ausgeben]
    O --> S[TemplateChecklist ausgeben]
    S --> T{DropDemoObjects = 1?}
    T -->|Ja| U[Demo-Tabellen entfernen]
    T -->|Nein| V[Demo-Objekte fuer weitere Experimente belassen]
```
<!-- SQLDOC:MERMAID:END -->
