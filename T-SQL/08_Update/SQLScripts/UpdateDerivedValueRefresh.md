# UpdateDerivedValueRefresh.sql

Dieses Skript zeigt in `tempdb`, wie abgeleitete Kopfwerte nach Aenderungen an Detailzeilen kontrolliert neu berechnet und nur bei echtem Delta per `UPDATE` in die Kopf-Tabelle zurueckgeschrieben werden. Die Erstversion bleibt bewusst im didaktischen Demo-Rahmen und vermeidet produktive Abhaengigkeiten.
...
| Feld | Wert |
|---|---|
| Script | [UpdateDerivedValueRefresh.sql](UpdateDerivedValueRefresh.sql) |
| Version | `1.0` |
| Typ | `didactic-lab` |
| Kapitel | `08_Update` |
| Sicherheit | `demo-write-tempdb` |
| Zweck | Aktualisiert nach Detailaenderungen veraltete abgeleitete Kopfwerte einer Demo-Zieltabelle. |
<!-- SQLDOC:SUMMARY_TABLE:END -->

## Einordnung

Das Muster passt zu Header- oder Summary-Tabellen, in denen offene Positionen, Rueckstaende, Gesamtbetraege oder Gesamtstatus aus Detailzeilen abgeleitet werden. Statt alle Kopfzeilen blind neu zu schreiben, vergleicht das Skript gespeicherte Werte mit frisch aggregierten Ableitungen und erlaubt ueber `@OnlyChangedRows`, nur echte Delta-Zeilen zu aktualisieren.

## Annahmen

- Die Erstversion verwendet ausschliesslich Demo-Objekte in `tempdb`.
- `demo.UpdateDerivedValueHeader` repraesentiert gespeicherte Kopfwerte.
- `demo.UpdateDerivedValueLine` repraesentiert die Detaildaten, aus denen Summen und Status abgeleitet werden.
- Das Beispiel fokussiert auf die Refresh-Logik selbst; eine separate Change-Queue wird hier didaktisch durch den Vorher-/Nachher-Vergleich ersetzt.
- `OrderID = 404` dient als bereits erfuellte Kopfzeile ohne offene oder Rueckstandspositionen.

## Anwendungsfall

Das Artefakt eignet sich fuer periodische Refresh-Jobs, Nachzuege nach Detailupdates oder Validierungslaeufe bei materialisierten Kopfwerten. In produktionsnahen Varianten kann das Muster mit einer Delta-Queue, CDC oder Triggern kombiniert werden.
...
|---|---|---|---|
| `@ApplyUpdate` | `BIT` | Nein | Fuehrt bei `1` das Refresh-Update aus, sonst nur Vorschau. |
| `@OnlyChangedRows` | `BIT` | Nein | Schreibt bei `1` nur Kopfzeilen mit echtem Delta, sonst alle Kandidaten. |
...
- `LineAggregate` berechnet offene Positionen, Rueckstaende, Gesamtbetrag und Fulfillment-Status pro `OrderID` neu.
- `#RefreshPreview` stellt gespeicherte Kopfwerte den neu abgeleiteten Werten gegenueber und markiert `NeedsRefresh`.
- `@OnlyChangedRows = 1` ist die sichere Standardvariante fuer echte Delta-Refreshes.
- `#AppliedRefresh` dokumentiert per `OUTPUT`, welche Kopfwerte im Lauf wirklich geschrieben wurden.
...
|---|---|---|---|
| `1.0` | `2026-04-17` | `ER` | Erstversion eines didaktischen Refresh-Skripts fuer abgeleitete Kopfwerte nach Detailaenderungen |
...
flowchart TD
    ...
    B --> C[Header- und Line-Demo-Tabellen bei Bedarf anlegen]
    C --> D{ResetDemoData = 1?}
    D -->|Ja| E[Demo-Kopf- und Detaildaten neu befuellen]
    D -->|Nein| F[Vorhandene Demo-Daten beibehalten]
    E --> G[RefreshPreview und AppliedRefresh vorbereiten]
    F --> G
    G --> H[LineAggregate aus Detailzeilen je Order berechnen]
    H --> I[Gespeicherte und neu abgeleitete Kopfwerte vergleichen]
    I --> J{ApplyUpdate = 1?}
    J -->|Ja| K{OnlyChangedRows = 1?}
    K -->|Ja| L[Nur Preview-Zeilen mit NeedsRefresh = 1 aktualisieren]
    K -->|Nein| M[Alle Refresh-Kandidaten zur Demonstration aktualisieren]
    L --> N[Aenderungen per OUTPUT in AppliedRefresh protokollieren]
    M --> N
    J -->|Nein| O[Nur Vorschau ohne Schreibvorgang]
    N --> P[RefreshPreview, AppliedRefresh und FinalHeaderState ausgeben]
    O --> P
    P --> Q[RefreshSummary ausgeben]
