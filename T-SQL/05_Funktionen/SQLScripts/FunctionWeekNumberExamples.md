# FunctionWeekNumberExamples.sql

Dieses Skript vergleicht mehrere Wochenlogiken direkt nebeneinander. Der Schwerpunkt liegt auf ISO-Wochen, `DATEFIRST`-abhaengigen Kalenderwochen und typischen Jahreswechsel-Effekten, die in Reports oder Berechnungen schnell zu Missverstaendnissen fuehren koennen.
...
| Feld | Wert |
|---|---|
| Script | [FunctionWeekNumberExamples.sql](FunctionWeekNumberExamples.sql) |
| Version | `1.0` |
| Typ | `didactic-lab` |
| Kapitel | `05_Funktionen` |
| Sicherheit | `read-only-tempdb` |
| Zweck | Vergleicht ISO-, Sonntag- und Montag-Definitionen fuer Kalenderwochen an reproduzierbaren Demo-Daten. |
<!-- SQLDOC:SUMMARY_TABLE:END -->

## Einordnung

Wochenlogik wirkt in T-SQL oft eindeutig, ist es aber nicht. `DATEPART(ISO_WEEK)` folgt ISO-8601, waehrend `DATEPART(WEEK)` von `DATEFIRST` abhaengt. Dadurch koennen dieselben Datumswerte je nach Kalenderdefinition unterschiedliche Wochenzahlen oder sogar unterschiedliche Wochenjahre erhalten.

## Annahmen

- Das Skript ist eine didaktische Erstversion ohne produktive Kalender- oder Faktentabellen.
- Die Vergleichswerte fuer Sonntag- und Montag-Kalenderwochen werden bewusst ueber `SET DATEFIRST` erzeugt.
- `@AnchorDate` ist ein neutraler Referenzpunkt fuer eine kleine Boundary-Vorschau und keine fachliche Steuerlogik.
- ISO-Wochenjahr und Kalenderjahr werden absichtlich getrennt gezeigt, damit Jahreswechsel-Effekte sichtbar werden.

## Anwendungsfall

Das Lab eignet sich fuer Unterricht, Review-Sessions und Debugging von Datumsberechnungen. Es ist besonders hilfreich, wenn ein Team dieselbe Kennzahl einmal mit ISO-Woche und einmal mit einer lokalen Wochenlogik auswertet und Unterschiede nachvollziehen muss.
...
|---|---|---|---|
| `@CalendarMode` | `VARCHAR(20)` | Nein | Filtert `all`, `iso`, `us-sunday` oder `us-monday` fuer die Lehrperspektive. |
| `@AnchorDate` | `DATE` | Nein | Legt den Referenztag fuer die Boundary-Vorschau fest. |
| `@PreviewDays` | `TINYINT` | Nein | Steuert die Anzahl der Tage vor und nach dem Referenztag. |
...
- `WeekNumberExamples` zeigt alle drei Perspektiven nebeneinander, auch wenn mit `@CalendarMode` nur eine Sicht hervorgehoben wird.
- `WeekDefinitionSummary` fasst pro Perspektive zusammen, wie gross der beobachtete Wertebereich und wie haeufig Grenzfaelle sind.
- `BoundaryPreview` baut rund um `@AnchorDate` eine kleine Tagesachse auf, damit Wochenstarts sichtbar werden.
- Besonders interessant sind die Daten um den Jahreswechsel, weil ISO-Wochenjahr und Kalenderjahr dort auseinanderlaufen koennen.
...
|---|---|---|---|
| `1.0` | `2026-04-17` | `ER` | Erstversion fuer didaktische Beispiele zu Wochenlogik und Kalenderdefinitionen |
...
```mermaid
flowchart TD
    A[Parameter und Guardrails pruefen] --> B[#WeekSampleDates mit Jahreswechsel- und Anchor-Beispielen fuellen]
    B --> C[#WeekComparisons mit ISO-Werten und Wochenstarts vorbereiten]
    C --> D[DATEFIRST auf Sunday setzen und SundayWeekNumber berechnen]
    D --> E[DATEFIRST auf Monday setzen und MondayWeekNumber berechnen]
    E --> F[DATEFIRST auf Ursprungswert zuruecksetzen]
    F --> G[WeekNumberExamples mit TeachingFlag und FocusedWeekLabel ausgeben]
    G --> H[WeekDefinitionSummary je Kalenderperspektive aggregieren]
    H --> I[BoundaryPreview rund um AnchorDate ausgeben]
```
<!-- SQLDOC:MERMAID:END -->
