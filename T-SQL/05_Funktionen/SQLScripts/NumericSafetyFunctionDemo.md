# NumericSafetyFunctionDemo.sql

Dieses Skript vergleicht rohe numerische Risiken mit defensiven Rechenmustern auf einer kleinen Demo-Datenbasis. Im Fokus stehen Quotienten mit `NULLIF` gegen Division durch Null sowie breit gerechnete Zwischenwerte, die vor einer engen `INT`-Konvertierung mit `TRY_CONVERT` oder `CASE` abgesichert werden.
...
| Feld | Wert |
|---|---|
| Script | [NumericSafetyFunctionDemo.sql](NumericSafetyFunctionDemo.sql) |
| Version | `1.0` |
| Typ | `didactic-lab` |
| Kapitel | `05_Funktionen` |
| Sicherheit | `read-only-tempdb` |
| Zweck | Demonstriert defensive Muster gegen Division durch Null und Overflow in numerischen Ausdruecken. |
<!-- SQLDOC:SUMMARY_TABLE:END -->

## Einordnung

Defensive Numerik ist in T-SQL oft weniger eine Frage einzelner Funktionen als der Reihenfolge von Typwahl, Guardrails und Konvertierung. Das Lab trennt deshalb den Quotienten selbst, einen skalierten Folgewert und die Konvertierung in einen engeren Zieltyp sichtbar voneinander.

## Annahmen

- Das Skript nutzt ausschliesslich eine temporaere Demo-Tabelle und keine produktiven Fachtabellen.
- Riskante rohe Ausdruecke werden nicht direkt ausgefuehrt, sondern als Risiko markiert und durch sichere Varianten ersetzt.
- `@FallbackRatio` steht fuer einen bewusst expliziten Ersatzwert; in produktiven Ablaufen kann stattdessen auch `NULL` oder ein Fehlerpfad fachlich richtiger sein.
- Overflow wird hier an der Grenze des SQL-Server-Typs `INT` demonstriert, weil der Effekt fuer Schulungszwecke schnell lesbar ist.

## Anwendungsfall

Das Skript eignet sich fuer Unterricht, Reviews und Diagnosesitzungen, wenn Kennzahlen, Prozentwerte oder skalierte Zaehler erst berechnet und anschliessend in einen engeren Zieltyp ueberfuehrt werden sollen. Die Muster lassen sich spaeter auf echte KPI-, Pricing- oder Telemetrieabfragen uebertragen.
...
|---|---|---|---|
| `@FallbackRatio` | `DECIMAL(12, 4)` | Nein | Ersatzwert fuer Quotienten, wenn der Nenner `0` ist. |
| `@TargetScale` | `TINYINT` | Nein | Anzahl der Nachkommastellen fuer die Quotientenanzeige. |
| `@ShowOnlyRiskCases` | `BIT` | Nein | `1` filtert das Detail-Resultset auf Divide-by-zero- oder Overflow-Risiken. |
...
- `CASE`
- `TRY_CONVERT`
- breite `DECIMAL`-Zwischenwerte
- CTEs fuer Risikoprofile und didaktische Hinweise
<!-- SQLDOC:DEPENDENCIES_LIST:END -->

## Hinweise

- `SafeRatioRounded` zeigt den Quotienten nur dann direkt, wenn der Nenner unkritisch ist.
- `RatioWithFallbackRounded` macht sichtbar, welcher fachliche Ersatzwert in Zero-Division-Faellen weitergereicht wuerde.
- `SafeScaledRatioRounded` rechnet zunaechst breit, damit die eigentliche Skalierung vom spaeteren Zieltyp entkoppelt bleibt.
- `TryConvertScaledInt` und `GuardedScaledInt` zeigen zwei defensive Varianten fuer enge Zieltypen: stilles `NULL` per `TRY_CONVERT` oder eine explizite Grenzwertpruefung.
...
|---|---|---|---|
| `1.0` | `2026-04-17` | `ER` | Erstversion fuer defensive Numerikmuster gegen Divide-by-zero und Overflow |
...
```mermaid
flowchart TD
    A[Parameter pruefen] --> B[#NumericCases anlegen]
    B --> C[Demo-Faelle fuer Ratio Pricing Telemetry und Overflow laden]
    C --> D[PreparedCases markiert Divide-by-zero und INT-Overflow-Risiken]
    D --> E[SafetyMatrix berechnet NULLIF-Fallback breite DECIMAL-Werte TRY_CONVERT und GuardedInt]
    E --> F[NumericSafetyMatrix als Detailsicht ausgeben]
    F --> G[RiskSummary nach ScenarioGroup aggregieren]
    G --> H[GuidanceBase ableiten]
    H --> I[GuidanceNotes mit defensiven Empfehlungen ausgeben]
```
<!-- SQLDOC:MERMAID:END -->
