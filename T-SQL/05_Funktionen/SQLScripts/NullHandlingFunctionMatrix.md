# NullHandlingFunctionMatrix.sql

Dieses Skript vergleicht NULL-Behandlungsfunktionen nicht isoliert, sondern als zusammenhaengende Matrix. Die Demo-Faelle zeigen, wann `ISNULL` fuer einen direkten Ersatz reicht, wann `COALESCE` mit mehreren Rueckfallstufen staerker ist und wie `NULLIF` zunaechst unerwuenschte Gleichstaende oder Sentinel-Werte auf `NULL` zuruecksetzt.
...
| Feld | Wert |
|---|---|
| Script | [NullHandlingFunctionMatrix.sql](NullHandlingFunctionMatrix.sql) |
| Version | `1.0` |
| Typ | `didactic-lab` |
| Kapitel | `05_Funktionen` |
| Sicherheit | `read-only-tempdb` |
| Zweck | Vergleicht `ISNULL`, `COALESCE`, `NULLIF` und angrenzende NULL-Muster auf derselben Demo-Datenbasis. |
<!-- SQLDOC:SUMMARY_TABLE:END -->

## Einordnung

Im Alltag wirken NULL-Funktionen oft austauschbar, obwohl sie unterschiedliche Fragen beantworten. `ISNULL` ersetzt direkt einen fehlenden Wert, `COALESCE` prueft eine ganze Kette moeglicher Quellen, `NULLIF` markiert Gleichstaende oder Sentinel-Werte erst einmal als fehlend, und `CONCAT` oder Folgeausdruecke zeigen anschliessend, wie daraus dennoch lesbare Ergebnisfelder entstehen.

## Annahmen

- Das Skript verwendet nur eine temporaere Demo-Tabelle und keine produktiven Datenquellen.
- `@StringFallback` ist bewusst laenger als `ShortCode`, damit der Unterschied zwischen direktem Ersatz und mehrstufigem Fallback sichtbar wird.
- `0.00` wird in zwei Szenarien als didaktischer Suppressionswert behandelt, damit `NULLIF` vor einem anschliessenden `COALESCE` demonstriert werden kann.
- Die String-Kette fuer `COALESCE` castet `ShortCode` auf `VARCHAR(20)`, damit die Rueckgabelaenge fuer den Vergleich klar erkennbar bleibt.

## Anwendungsfall

Das Lab eignet sich fuer Schulungen zu Ausdruckslogik, fuer Reviews von Reporting-Abfragen und fuer Diagnose-Skripte, in denen fehlende Werte sauber aufbereitet werden muessen. Besonders nuetzlich ist es, wenn sowohl Text- als auch Zahlenspalten unterschiedliche Rueckfallregeln haben und die gewaehlte Funktion spaeter erklaert werden soll.
...
|---|---|---|---|
| `@StringFallback` | `VARCHAR(20)` | Nein | Textlicher Rueckgabewert fuer fehlende Codes. |
| `@NumericFallback` | `DECIMAL(10,2)` | Nein | Numerischer Rueckgabewert fuer fehlende Werte. |
| `@ShowOnlyDifferences` | `BIT` | Nein | `1` zeigt nur Szenarien mit unterschiedlichem Funktionsverhalten. |
...
- `NULLIF`
- `CONCAT`
- CTEs fuer Matrixaufbereitung und Filterung
<!-- SQLDOC:DEPENDENCIES_LIST:END -->

## Hinweise

- `IsNullCodeLength` und `CoalesceCodeLength` machen sichtbar, dass String-Fallbacks je nach Ausdruckskette unterschiedlich ausfallen koennen.
- `DifferenceSummary` gruppiert die Demo-Faelle nach beobachtetem Unterschiedsprofil, damit nicht nur einzelne Zeilen, sondern Muster sichtbar werden.
- `GuidanceMatrix` uebersetzt die Resultate in knappe Einsatzempfehlungen fuer `ISNULL`, `COALESCE`, `NULLIF` und zusammengesetzte Anzeigeausdruecke.
- Mit `@ShowOnlyDifferences = 1` laesst sich das Skript auf jene Szenarien zuspitzen, in denen Funktionswahl wirklich einen beobachtbaren Effekt hat.
...
|---|---|---|---|
| `1.0` | `2026-04-17` | `ER` | Erstversion fuer die didaktische Matrix zu Null-Behandlungsfunktionen |
...
```mermaid
flowchart TD
    A[Parameter pruefen] --> B[#NullSamples anlegen]
    B --> C[Demo-Szenarien mit Text Zahl und Suppressionswerten laden]
    C --> D[PreparedMatrix berechnet ISNULL COALESCE NULLIF und Folgeausdruecke]
    D --> E[FilteredMatrix filtert optional auf echte Unterschiede]
    E --> F[FunctionMatrixRows mit Code- und Zahlenvergleich ausgeben]
    F --> G[DifferenceSummary nach Unterschiedsprofil aggregieren]
    G --> H[GuidanceMatrix mit Einsatzempfehlungen je Funktion ausgeben]
```
<!-- SQLDOC:MERMAID:END -->
