# FunctionWhitespaceCleanupDemo.sql

Dieses Skript zeigt anhand weniger Demo-Werte, wie Leerzeichen, Tabs, Zeilenumbrueche und unsichtbare Unicode-Zeichen in T-SQL sichtbar gemacht und bereinigt werden koennen. Der Schwerpunkt liegt nicht auf einer einzigen "richtigen" Funktion, sondern auf der Kombination mehrerer Schritte fuer robuste Stringvergleiche.
...
| Feld | Wert |
|---|---|
| Script | [FunctionWhitespaceCleanupDemo.sql](FunctionWhitespaceCleanupDemo.sql) |
| Version | `1.0` |
| Typ | `didactic-lab` |
| Kapitel | `05_Funktionen` |
| Sicherheit | `read-only-tempdb` |
| Zweck | Demonstriert Bereinigung von Leerzeichen, Tabs, Zeilenumbruechen und unsichtbaren Zeichen. |
<!-- SQLDOC:SUMMARY_TABLE:END -->

## Einordnung

Stringwerte wirken oft sauber, obwohl sie fuer Vergleiche, Joins oder Importpruefungen problematische Zeichen enthalten. Dieses Lab zerlegt den Bereinigungspfad daher in sichtbare Zwischenschritte: erst sichtbar machen, dann trimmen, danach spezielle Leerzeichen und Zeilenumbrueche angleichen und zum Schluss mehrfache Leerzeichen kollabieren.

## Annahmen

- Das Skript arbeitet mit einem didaktischen Demo-Katalog in einer temporaeren Tabelle statt mit produktiven Importdaten.
- `NCHAR(160)` und `NCHAR(8203)` repraesentieren typische Copy-and-Paste-Probleme aus Browsern, Office oder externen Systemen.
- Die mehrfachen `REPLACE`-Schritte fuer doppelte Leerzeichen sind bewusst einfach gehalten, damit der Effekt ohne Hilfsfunktion sichtbar bleibt.
- `@CleanupMode = trim-only` zeigt absichtlich, dass `TRIM` allein Tabs, Zeilenumbrueche oder Zero-Width-Spaces nicht vollstaendig beseitigt.

## Anwendungsfall

Das Skript eignet sich fuer Unterricht, ETL-Reviews und erste Diagnose bei auffaelligen Textvergleichen. Es ist besonders hilfreich, wenn scheinbar identische Codes, Namen oder Labels wegen unsichtbarer Zeichen nicht matchen oder wenn Imports vor der eigentlichen Validierung vereinheitlicht werden sollen.
...
|---|---|---|---|
| `@CleanupMode` | `VARCHAR(20)` | Nein | Steuert `all`, `trim-only` oder `normalize`. |
| `@ReplaceTabsWithSpace` | `BIT` | Nein | `1` ersetzt Tabs und Zeilenumbrueche durch Leerzeichen. |
| `@ShowOnlyChanged` | `BIT` | Nein | `1` zeigt nur Zeilen mit echter Bereinigung. |
...
- `VisibleRawValue` ersetzt problematische Zeichen durch Marker wie `<TAB>`, `<NBSP>` oder `<ZWSP>`, damit unsichtbare Inhalte im Resultset lesbar werden.
- `TrimValue` zeigt den Unterschied zwischen klassischem Trim und einer vollstaendigen Normalisierung.
- `WhitespaceIssueSummary` gruppiert die Demo-Zeilen nach Problemtyp und eignet sich als kleine Diagnoseuebersicht fuer Importstrecken.
- `FunctionEffectMatrix` macht die einzelnen Bereinigungsschritte pro Zeile vergleichbar, statt nur das Endergebnis zu zeigen.
...
|---|---|---|---|
| `1.0` | `2026-04-17` | `ER` | Erstversion fuer eine didaktische Demo zur Bereinigung von Leerzeichen und unsichtbaren Zeichen |
...
```mermaid
flowchart TD
    A[Parameter pruefen] --> B[#WhitespaceSamples mit Problemfaellen fuellen]
    B --> C[PreparedSamples macht Tabs CR LF NBSP und ZWSP sichtbar und markiert Issue-Flags]
    C --> D[NormalizedSteps berechnet LTRIM/RTRIM TRIM und erste Ersetzungen fuer NBSP ZWSP Tabs]
    D --> E[CollapsedWhitespace vereinheitlicht Zeilenumbrueche]
    E --> F[FinalCleanup kollabiert mehrfache Leerzeichen und trimmt das Endergebnis]
    F --> G[PreviewRows waehlt SelectedCleanupValue und baut IssueLabels]
    G --> H[WhitespaceCleanupPreview ausgeben]
    H --> I[WhitespaceIssueSummary nach Problemtyp aggregieren]
    I --> J[FunctionEffectMatrix mit sichtbaren Zwischenschritten ausgeben]
```
<!-- SQLDOC:MERMAID:END -->
