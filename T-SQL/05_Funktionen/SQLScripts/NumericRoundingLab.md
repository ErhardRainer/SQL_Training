# NumericRoundingLab.sql

Dieses Skript stellt typische Rundungssituationen auf einer kleinen Demo-Basis gegenueber. Es zeigt, wann `ROUND` kaufmaennisch rundet, wann die Truncate-Variante nur Stellen abschneidet, wie `CEILING` und `FLOOR` bei negativen Zahlen reagieren und was beim Wechsel nach `DECIMAL` oder `INT` sichtbar bleibt.
...
| Feld | Wert |
|---|---|
| Script | [NumericRoundingLab.sql](NumericRoundingLab.sql) |
| Version | `1.0` |
| Typ | `didactic-lab` |
| Kapitel | `05_Funktionen` |
| Sicherheit | `read-only-tempdb` |
| Zweck | Vergleicht `ROUND`, `CEILING`, `FLOOR` und Datentypwechsel auf derselben Demo-Datenbasis. |
<!-- SQLDOC:SUMMARY_TABLE:END -->

## Einordnung

Rundungsfragen tauchen in SQL nicht nur bei Preisen auf, sondern auch bei Kennzahlen, Mengen und Schwellenwerten. Das Lab setzt deshalb bewusst positive und negative Werte nebeneinander, damit die Unterschiede zwischen kaufmaennischem Runden, Abschneiden von Stellen und ganzzahligen Grenzfunktionen direkt lesbar werden.

## Annahmen

- Das Skript nutzt ausschliesslich eine temporaere Demo-Tabelle und keine produktiven Datenquellen.
- `@DisplayScale` ist auf kleine didaktische Werte begrenzt, damit Unterschiede in der Ausgabe kompakt bleiben.
- `@LeftRoundDigits` demonstriert Rundung links vom Dezimalpunkt nur in einem sicheren Bereich zwischen `-3` und `0`.
- Der Vergleich zwischen `CAST` und `CONVERT` dient hier der Lesbarkeit; beide Varianten werden auf denselben Demo-Wert angewendet.

## Anwendungsfall

Das Lab eignet sich fuer Unterricht, Code-Reviews und kleine Diagnoseaufgaben, wenn Betragswerte oder Kennzahlen vor einer Ausgabe, einer Klassifikation oder einer Uebergabe an weitere SQL-Ausdruecke normiert werden muessen. Die Demo-Szenarien koennen spaeter leicht gegen echte Preis-, Ratio- oder Mengenfelder ausgetauscht werden.
...
|---|---|---|---|
| `@DisplayScale` | `TINYINT` | Nein | Anzahl der Nachkommastellen fuer `ROUND` und `DECIMAL`-Konvertierungen. |
| `@LeftRoundDigits` | `SMALLINT` | Nein | Negative Rundungsstelle fuer Vergleiche links vom Dezimalpunkt. |
| `@ShowNegativeScaleComparison` | `BIT` | Nein | `1` blendet den Vergleich fuer negative `ROUND`-Stellen ein. |
...
- `CAST`
- `CONVERT`
- CTEs fuer Aufbereitung und didaktische Hinweise
<!-- SQLDOC:DEPENDENCIES_LIST:END -->

## Hinweise

- `RoundAtScale` und `TruncateAtScale` zeigen denselben Eingabewert mit und ohne kaufmaennische Rundung.
- `CeilingValue` und `FloorValue` machen bei negativen Zahlen sichtbar, dass die Richtung nicht mit "auf" und "ab" im alltaeglichen Sprachgebrauch verwechselt werden sollte.
- `CastInt` und `ConvertInt` illustrieren die Wirkung eines Typsprungs auf Ganzzahlwerte getrennt von den eigentlichen Rundungsfunktionen.
- `RoundAtLeftDigits` blendet einen zweiten Blick auf Hunderter- oder Zehnerstellen ein, wenn `@ShowNegativeScaleComparison = 1` gesetzt ist.
...
|---|---|---|---|
| `1.0` | `2026-04-17` | `ER` | Erstversion fuer ein didaktisches Lab zu Rundungsfunktionen und Typkonvertierungen |
...
```mermaid
flowchart TD
    A[Parameter pruefen] --> B[#NumericSamples anlegen]
    B --> C[Demo-Werte fuer Preise Ratios und Mengen laden]
    C --> D[PreparedSamples berechnet CAST und CONVERT Varianten]
    D --> E[RoundingMatrix vergleicht ROUND Truncate CEILING FLOOR und linke Rundungsstelle]
    E --> F[RoundingMatrix als Detailergebnis ausgeben]
    F --> G[FunctionSummary nach Kategorien aggregieren]
    G --> H[GuidanceBase ableiten]
    H --> I[GuidanceNotes mit Einsatzhinweisen ausgeben]
```
<!-- SQLDOC:MERMAID:END -->
