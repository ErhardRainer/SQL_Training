# ConditionalLogicCasePatterns.sql

Dieses Skript zeigt, wie `CASE` in T-SQL mehrere Aufgaben gleichzeitig uebernehmen kann: Klassifikation, Bucketing, Statuskennzeichen und umschaltbare Sortierschluessel. Die Demo-Daten bleiben bewusst klein und transparent, damit jede abgeleitete Spalte direkt auf den zugrunde liegenden Regeln nachvollziehbar ist.
...
| Feld | Wert |
|---|---|
| Script | [ConditionalLogicCasePatterns.sql](ConditionalLogicCasePatterns.sql) |
| Version | `1.0` |
| Typ | `didactic-lab` |
| Kapitel | `05_Funktionen` |
| Sicherheit | `read-only-tempdb` |
| Zweck | Buendelt CASE-Muster fuer Klassifikation, Umsatz-Bucketing und Sortierschluessel. |
<!-- SQLDOC:SUMMARY_TABLE:END -->

## Einordnung

`CASE` ist in T-SQL weit mehr als ein Ersatz fuer `if/else` in einzelnen Ausdruecken. In Reports, Diagnoseabfragen und Lernskripten dient es oft dazu, rohe Messwerte in fachlich lesbare Kategorien zu uebersetzen. Das Lab kombiniert deshalb mehrere typische Einsatzfelder in einem zusammenhaengenden Beispiel.

## Annahmen

- Das Skript nutzt einen didaktischen Demo-Bestand in `tempdb`-nahen Objekten statt produktiver CRM- oder ERP-Tabellen.
- Die Segmentlabels `core`, `growth` und `watch` sind Lehrmuster und keine verbindliche Vertriebslogik.
- `@SortMode` schaltet bewusst nur zwischen drei klar abgegrenzten Priorisierungen um, damit der Einfluss von `CASE` im Sortierschluessel sichtbar bleibt.
- Inaktive Kunden bleiben optional sichtbar, um zu zeigen, dass `CASE` und Filterlogik parallel eingesetzt werden koennen.

## Anwendungsfall

Das Skript eignet sich fuer Schulungen zu `CASE`, fuer SQL-Reviews mit Business-Labels und fuer erste Reporting-Prototypen. Besonders nuetzlich ist es, wenn rohe Zahlen zwar vorhanden sind, aber in der Ausgabe noch in lesbare Kategorien und Prioritaeten uebersetzt werden muessen.
...
|---|---|---|---|
| `@SegmentFocus` | `VARCHAR(20)` | Nein | Filtert `all`, `core`, `growth` oder `watch`. |
| `@SortMode` | `VARCHAR(20)` | Nein | Steuert den Sortierschluessel ueber `priority`, `revenue` oder `overdue`. |
| `@IncludeInactive` | `BIT` | Nein | `1` zeigt auch inaktive Kunden, `0` blendet sie aus. |
...
- `CASE`
- CTEs fuer vorbereitete und gefilterte Sicht
- `ROW_NUMBER` fuer eine stabile Anzeige-Reihenfolge
- `STRING_AGG` fuer kompakte Beispielkunden je Bucket
<!-- SQLDOC:DEPENDENCIES_LIST:END -->

## Hinweise

- `PreparedSignals` zeigt die typischen `CASE`-Muster fuer Label, Bucket, Status und Sortierlogik in einer zentralen Ableitung.
- `FilteredSignals` trennt die didaktische Klassifikation von der spaeteren Parameterfilterung.
- `RankedSignals` macht sichtbar, dass `CASE` auch in `ORDER BY` und nicht nur in Anzeige-Spalten relevant ist.
- Die drei Resultsets decken Detailansicht, Aggregation und Meta-Erklaerung des aktiven Sortiermodus ab.
...
|---|---|---|---|
| `1.0` | `2026-04-17` | `ER` | Erstversion fuer didaktische CASE-Muster in Funktionen |
...
```mermaid
flowchart TD
    A[Parameter pruefen] --> B[#CustomerSignals anlegen]
    B --> C[Demo-Kunden mit Umsatz, Rabatt, Tickets und Verzug laden]
    C --> D[PreparedSignals berechnet SegmentLabel RevenueBucket ReceivableState RelationshipFlag und Sortierlogik mit CASE]
    D --> E[FilteredSignals wendet SegmentFocus und IncludeInactive an]
    E --> F[RankedSignals erzeugt DisplayOrder ueber CASE-gesteuerten Sortierschluessel]
    F --> G[CasePatternRows als Detailansicht ausgeben]
    G --> H[BucketSummary je Segment und RevenueBucket aggregieren]
    H --> I[SortModeSummary mit TeachingNote zum aktiven Sortiermodus ausgeben]
```
<!-- SQLDOC:MERMAID:END -->
