# FunctionConditionalLabeling.sql

Dieses Skript zeigt, wie `CASE` gemeinsam mit Funktionen wie `UPPER`, `COALESCE`, `CONCAT` und `DATEDIFF` fachlich lesbare Labels erzeugt. Die Demo-Daten bleiben bewusst kompakt, damit die abgeleiteten Umsatz-, Service- und Risikoklassen direkt aus dem SQL nachvollziehbar sind.
...
| Feld | Wert |
|---|---|
| Script | [FunctionConditionalLabeling.sql](FunctionConditionalLabeling.sql) |
| Version | `1.0` |
| Typ | `didactic-lab` |
| Kapitel | `05_Funktionen` |
| Sicherheit | `read-only-tempdb` |
| Zweck | Kombiniert CASE mit Funktionen fuer fachlich lesbare Umsatz-, Service- und Risikolabels. |
<!-- SQLDOC:SUMMARY_TABLE:END -->

## Einordnung

Viele SQL-Abfragen liefern Rohwerte wie Umsatz, Verzugstage oder offene Tickets, aber noch keine sofort lesbaren Fachlabels. Dieses Lab zeigt deshalb, wie Ausdrucksfunktionen und `CASE` zusammenwirken koennen: Namen normalisieren, fehlende Werte absichern, Alterswerte berechnen und daraus anschliessend Fachklassen bilden.

## Annahmen

- Das Skript arbeitet mit einer didaktischen Demo-Tabelle in `tempdb`-nahen Objekten statt mit produktiven CRM- oder Support-Daten.
- Die Labels `strategic`, `growth`, `core`, `develop`, `service-hot` und `risk-critical` sind Lehrmuster und keine verbindliche Fachlogik.
- Fehlende Kontakt- oder Owner-Informationen werden bewusst mit `COALESCE` aufgefuellt, damit Nullbehandlung im Ergebnis sichtbar bleibt.
- Die Eskalationsqueue ist ein anschauliches Textmuster fuer kombinierte Labels und kein reales Routing-System.

## Anwendungsfall

Das Skript eignet sich fuer Unterricht, Reviews und Reporting-Prototypen, wenn Fachlabels direkt in T-SQL abgeleitet werden sollen. Besonders nuetzlich ist es, wenn mehrere Signale zusammen bewertet und zugleich in lesbare Queue- oder Kontaktkennzeichen uebersetzt werden muessen.
...
|---|---|---|---|
| `@LabelMode` | `VARCHAR(20)` | Nein | Filtert `all`, `revenue`, `service` oder `risk`. |
| `@ReferenceDate` | `DATE` | Nein | Referenzdatum fuer Kontaktalter und Faelligkeitslabels. |
| `@ShowOnlyEscalated` | `BIT` | Nein | `1` zeigt nur eskalationsrelevante Zeilen, `0` zeigt alle Demo-Faelle. |
...
- `PreparedSignals` verbindet Normalisierung, Nullbehandlung und erste Label-Ableitungen in einer einzigen Vorstufe.
- `FilteredSignals` zeigt, wie der aktive Label-Fokus per Parameter umgeschaltet werden kann, ohne die Basisregeln mehrfach zu definieren.
- `ContactLabel` und `EscalationQueue` machen sichtbar, dass `CASE` nicht nur Buckets, sondern auch zusammengesetzte Textlabels erzeugen kann.
- Das dritte Resultset konzentriert sich nur auf eskalierte Konten und eignet sich deshalb als Vorlage fuer Queue- oder Arbeitslisten.
...
|---|---|---|---|
| `1.0` | `2026-04-17` | `ER` | Erstversion fuer didaktische Labels aus CASE und Funktionen |
...
```mermaid
flowchart TD
    A[Parameter pruefen] --> B[#AccountSignals mit Demo-Konten fuellen]
    B --> C[PreparedSignals normalisiert Namen und leitet RevenueLabel ServiceLabel RiskLabel DaysSinceContact und EscalationQueue ab]
    C --> D[FilteredSignals baut ActiveLabel ContactLabel und IsEscalated nach Parametern]
    D --> E[RankedSignals sortiert eskalierte Konten nach Verzug Tickets Umsatz und Name]
    E --> F[ConditionalLabelRows als Detailansicht ausgeben]
    F --> G[LabelSummary nach ActiveLabel aggregieren]
    G --> H[EscalationPreview nur fuer eskalierte Konten ausgeben]
```
<!-- SQLDOC:MERMAID:END -->
