# FunctionDateWindowBuilder.sql

Dieses Skript baut aus einer Anchor-Date mehrere typische Reporting-Zeitfenster auf und macht deren Grenzen explizit sichtbar. Dadurch laesst sich gut nachvollziehen, wie `DATEADD`, `DATEFROMPARTS`, `EOMONTH` und `DATEPART` gemeinsam fuer rollierende und kalenderbasierte Reports eingesetzt werden.
...
| Feld | Wert |
|---|---|
| Script | [FunctionDateWindowBuilder.sql](FunctionDateWindowBuilder.sql) |
| Version | `1.0` |
| Typ | `didactic-lab` |
| Kapitel | `05_Funktionen` |
| Sicherheit | `read-only-tempdb` |
| Zweck | Baut Reporting-Zeitfenster relativ zu einer Anchor-Date auf und ordnet Demo-Ereignisse diesen Fenstern zu. |
<!-- SQLDOC:SUMMARY_TABLE:END -->

## Einordnung

Reporting-Logik benoetigt oft nicht nur einzelne Datumsfunktionen, sondern ein stabiles Muster zur Fensterbildung. Das Lab zeigt deshalb gleichzeitig rollierende Fenster wie die letzten sieben oder dreissig Tage sowie kalenderbasierte Fenster fuer Monat und Quartal.

## Annahmen

- Das Skript ist eine didaktische Erstversion mit Demo-Ereignissen in einer temporaeren Tabelle.
- Vergleichsfenster liegen direkt vor dem jeweils aktiven Hauptfenster und haben dieselbe Dauer.
- `@AnchorDate` repraesentiert den Auswertungsstichtag eines Reports und nicht zwingend das aktuelle Tagesdatum.
- Quartalsgrenzen werden aus `DATEPART(QUARTER, @AnchorDate)` und `DATEFROMPARTS` abgeleitet, damit die Logik transparent bleibt.

## Anwendungsfall

Das Skript eignet sich fuer Unterricht, Report-Reviews und ETL-Vorbereitung, wenn Datumsfenster nicht implizit im Kopf bleiben sollen. Besonders nuetzlich ist es, wenn Teams erklaeren muessen, warum rollierende Zeitfenster anders reagieren als abgeschlossene Kalenderperioden.
...
|---|---|---|---|
| `@AnchorDate` | `DATE` | Nein | Stichtag, relativ zu dem die Reporting-Fenster aufgebaut werden. |
| `@WindowPreset` | `VARCHAR(20)` | Nein | Waehlt `last7d`, `rolling30d`, `current_month`, `prior_month`, `current_quarter` oder `all`. |
| `@IncludeComparisonWindow` | `BIT` | Nein | `1` fuegt fuer das aktive Hauptfenster ein direkt davor liegendes Vergleichsfenster hinzu. |
...
- `DATEPART`
- `DATENAME`
- CTEs fuer Fensterkatalog und Kennzahlen
- `CASE`
<!-- SQLDOC:DEPENDENCIES_LIST:END -->

## Hinweise

- `BaseWindows` definiert die Hauptfenster fuer rollierende und kalenderbasierte Auswertungen.
- `FilteredWindows` reduziert den Katalog bei Bedarf auf ein einzelnes Preset, ohne die Grundlogik zu duplizieren.
- `WindowCatalog` ergaenzt optional direkt vorgelagerte Vergleichsfenster mit derselben Dauer.
- `WindowMetrics` verbindet die Fenster mit Demo-Ereignissen und markiert leere Fenster explizit.
...
|---|---|---|---|
| `1.0` | `2026-04-19` | `ER` | Erstversion fuer didaktische Reporting-Zeitfenster mit Datumsfunktionen |
...
```mermaid
flowchart TD
    A[Parameter und Guardrails pruefen] --> B[#ReportSignals mit Demo-Ereignissen fuellen]
    B --> C[BaseWindows leitet Hauptfenster aus Anchor-Date ab]
    C --> D[FilteredWindows begrenzt optional auf ein Preset]
    D --> E{ComparisonWindow aktiv?}
    E -->|Ja| F[WindowCatalog fuegt vorgelagerte Vergleichsfenster hinzu]
    E -->|Nein| G[WindowCatalog behaelt nur Hauptfenster]
    F --> H[WindowMetrics verknuepft Fenster mit Demo-Ereignissen]
    G --> H
    H --> I[WindowCatalog mit Grenzen und Dauer ausgeben]
    I --> J[WindowMetrics mit Kennzahlen je Fenster ausgeben]
    J --> K[WindowGuidance mit didaktischer Einordnung ausgeben]
```
<!-- SQLDOC:MERMAID:END -->
