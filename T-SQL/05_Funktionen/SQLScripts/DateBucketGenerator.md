# DateBucketGenerator.sql

Dieses Skript erzeugt aus einer kleinen Demo-Zeitreihe stabile Buckets fuer Tage, Wochen und Monate. Der Fokus liegt darauf, die Grenzen der Intervalle explizit sichtbar zu machen, damit spaetere Verdichtungen in Reports oder ETL-Stufen nachvollziehbar vorbereitet werden koennen.
...
| Feld | Wert |
|---|---|
| Script | [DateBucketGenerator.sql](DateBucketGenerator.sql) |
| Version | `1.0` |
| Typ | `didactic-lab` |
| Kapitel | `05_Funktionen` |
| Sicherheit | `read-only-tempdb` |
| Zweck | Erzeugt nachvollziehbare Tages-, Wochen- und Monats-Buckets fuer Verdichtungen in T-SQL. |
<!-- SQLDOC:SUMMARY_TABLE:END -->

## Einordnung

Viele Reports benoetigen keine Einzelzeilen pro Datum, sondern saubere Perioden wie Tag, Woche oder Monat. Das Skript zeigt deshalb nicht nur eine Aggregation, sondern den vorgelagerten Schritt: Wie werden Bucket-Grenzen reproduzierbar gebildet, und wie lassen sich dabei auch leere Perioden sichtbar machen?

## Annahmen

- Das Skript verwendet eine didaktische Demo-Zeitreihe statt produktiver Bestell- oder Faktentabellen.
- Wochen-Buckets werden bewusst ueber einen expliziten Parameter gesteuert, damit der Wochenstart transparent bleibt.
- Leere Buckets dienen hier als Lehrmittel fuer vollstaendige Zeitachsen in Dashboards oder ETL-Vorverdichtungen.
- Die Demo-Daten enthalten nur Tageswerte; Wochen- und Monatswerte werden ausschliesslich daraus abgeleitet.

## Anwendungsfall

Das Lab eignet sich fuer Schulungen zu `DATEADD`, `DATEDIFF`, Monatsgrenzen und reporttauglichen Zeitachsen. Es kann auch als Startpunkt dienen, wenn spaeter eine echte Faktentabelle nach Tages-, Wochen- oder Monatsintervallen aggregiert werden soll.
...
|---|---|---|---|
| `@BucketLevel` | `VARCHAR(10)` | Nein | Waehlt `day`, `week`, `month` oder `all` fuer die sichtbaren Buckets. |
| `@WeekStart` | `VARCHAR(10)` | Nein | Legt `monday` oder `sunday` als Wochenstart fuer das Wochen-Bucketing fest. |
| `@IncludeEmptyBuckets` | `BIT` | Nein | `1` zeigt auch Buckets ohne Messwert, `0` blendet leere Buckets aus. |
...
- CTEs fuer Datumsspanne, Kalenderbasis und aggregierte Buckets
- `DATEADD`
- `DATEDIFF`
- `DATEPART`
- `ROW_NUMBER` fuer stabile Bucket-Sequenzen
<!-- SQLDOC:DEPENDENCIES_LIST:END -->

## Hinweise

- `DateSpan` bildet zuerst eine lueckenlose Tagesachse zwischen erstem und letztem Demo-Datum.
- `BaseCalendar` leitet daraus Monats- und Wochenstarts ab, ohne auf Session-Einstellungen wie `DATEFIRST` angewiesen zu sein.
- `ExpandedBuckets` erzeugt daraus parallel drei Granularitaeten, damit `all` direkt vergleichbar bleibt.
- `VisibleBuckets` zeigt optional auch leere Buckets und macht dadurch den Unterschied zwischen vorhandenen Messwerten und vollstaendiger Zeitachse sichtbar.
...
|---|---|---|---|
| `1.0` | `2026-04-17` | `ER` | Erstversion fuer didaktische Tages-, Wochen- und Monats-Buckets |
...
```mermaid
flowchart TD
    A[Parameter pruefen] --> B[#DailySignals mit Demo-Tageswerten anlegen]
    B --> C[Min und Max Datum aus der Quelle bestimmen]
    C --> D[DateSpan erzeugt lueckenlose Tagesachse]
    D --> E[BaseCalendar leitet WeekStartDate und MonthStartDate ab]
    E --> F[ExpandedBuckets erzeugt day week und month Buckets]
    F --> G[AggregatedBuckets verknuepft Buckets mit Demo-Messwerten]
    G --> H[VisibleBuckets filtert BucketLevel und leere Buckets]
    H --> I[BucketRows mit Grenzen und Summen ausgeben]
    I --> J[BucketSummary je Granularitaet berechnen]
    J --> K[BucketGuide mit aktueller Parameterauswahl ausgeben]
```
<!-- SQLDOC:MERMAID:END -->
