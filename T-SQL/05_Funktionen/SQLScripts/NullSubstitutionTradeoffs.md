# NullSubstitutionTradeoffs.sql

Dieses Skript vergleicht `ISNULL`, `COALESCE` und `NULLIF` nicht nur funktional, sondern entlang ihrer typischen Tradeoffs. Die Demo-Faelle zeigen, wann direkte Ersetzung ausreicht, wann eine Fallback-Kette noetig ist und wann ein Wert zunaechst per `NULLIF` neutralisiert werden sollte.
...
| Feld | Wert |
|---|---|
| Script | [NullSubstitutionTradeoffs.sql](NullSubstitutionTradeoffs.sql) |
| Version | `1.0` |
| Typ | `didactic-lab` |
| Kapitel | `05_Funktionen` |
| Sicherheit | `read-only-tempdb` |
| Zweck | Vergleicht `ISNULL`, `COALESCE` und `NULLIF` entlang von Rueckfallketten, Typbreite und Sentinel-Unterdrueckung. |
<!-- SQLDOC:SUMMARY_TABLE:END -->

## Einordnung

Die drei Funktionen beantworten unterschiedliche Fragen: `ISNULL` setzt sofort einen Ersatzwert, `COALESCE` prueft mehrere Quellen in Reihenfolge und `NULLIF` markiert bestimmte Werte erst einmal als fehlend. Gerade in Report- und Bereinigungsausdruecken entstehen daraus unterschiedliche fachliche und technische Konsequenzen.

## Annahmen

- Das Skript arbeitet mit einer kleinen Demo-Tabelle in `tempdb`-nahen Objekten statt mit produktiven Tabellen.
- `ShortCode` ist bewusst nur `VARCHAR(6)`, damit der Unterschied zwischen schmalem Erstargument und breiterer Fallback-Kette sichtbar wird.
- `0.00` dient in einzelnen Szenarien als didaktischer Sentinel-Wert, der per `NULLIF` vor einem spaeteren Fallback entfernt wird.
- Leere Kommentare werden erst per `NULLIF` und anschliessend per `COALESCE` auf einen lesbaren Platzhalter gebracht.

## Anwendungsfall

Das Lab eignet sich fuer Schulungen zu Ausdruckslogik, fuer SQL-Reviews bei NULL-Aufbereitung und als Muster, wenn Teams die Wahl zwischen kompaktem Ersatz und expliziter Fallback-Kette diskutieren. Besonders hilfreich ist es, wenn Rueckgabetyp, Stringlaenge oder Sentinel-Werte fachlich relevant werden.
...
|---|---|---|---|
| `@StringFallback` | `VARCHAR(12)` | Nein | Textlicher Rueckfallwert fuer fehlende oder bereinigte Codes. |
| `@NumericFallback` | `DECIMAL(10,2)` | Nein | Numerischer Rueckfallwert fuer NULL- oder Sentinel-Faelle. |
| `@ShowTypeDetails` | `BIT` | Nein | `1` zeigt zusaetzlich Typ-, Precision- und Laengenbeobachtungen. |
...
- `TradeoffRows` zeigt pro Szenario, wie direkte Ersetzung, Kettenlogik und vorgelagerte Bereinigung unterschiedliche Resultate erzeugen.
- `IsNullCodeLength` und `CoalesceCodeLength` machen sichtbar, dass Rueckfallwerte nicht nur inhaltlich, sondern auch in ihrer resultierenden Breite variieren koennen.
- `TypeLengthObservations` ist optional schaltbar, damit das Skript sowohl als schneller Vergleich als auch als tieferes Typ-Lab genutzt werden kann.
- `RecommendationMatrix` uebersetzt die beobachteten Muster in kompakte Einsatzhinweise fuer echte SQL-Ausdruecke.
...
|---|---|---|---|
| `1.0` | `2026-04-17` | `ER` | Erstversion fuer ein didaktisches Lab zu Null-Substitution und Tradeoffs |
...
```mermaid
flowchart TD
    A[Parameter und Guardrails pruefen] --> B[#NullTradeoffSamples anlegen]
    B --> C[Demo-Szenarien mit ShortCode BackupCode und SentinelMetric laden]
    C --> D[PreparedTradeoffs berechnet ISNULL COALESCE NULLIF und bereinigte Folgeausdruecke]
    D --> E[TradeoffRows leitet Beobachtungen zu Stringbreite Fallback und Sentinel-Unterdrueckung ab]
    E --> F[TradeoffRows als Hauptvergleich ausgeben]
    F --> G{ShowTypeDetails = 1?}
    G -->|Ja| H[TypeLengthObservations mit SQL_VARIANT_PROPERTY ausgeben]
    G -->|Nein| I[Direkt zur Empfehlungsmatrix wechseln]
    H --> I[RecommendationMatrix fuer ISNULL COALESCE NULLIF und Kombinationen ausgeben]
```
<!-- SQLDOC:MERMAID:END -->
