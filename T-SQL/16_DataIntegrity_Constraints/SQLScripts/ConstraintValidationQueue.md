# ConstraintValidationQueue.sql

Dieses Skript modelliert in `tempdb` eine kleine Queue fuer nachgelagerte Constraint-Validierungen. Aus Constraint-Metadaten werden priorisierte Queue-Eintraege erzeugt, anschliessend wird der naechste Arbeitsbatch fuer einen fiktiven Worker reserviert.
...
| Feld | Wert |
|---|---|
| Script | [ConstraintValidationQueue.sql](ConstraintValidationQueue.sql) |
| Version | `1.0` |
| Typ | `didactic-lab` |
| Kapitel | `16_DataIntegrity_Constraints` |
| Sicherheit | `demo-write-tempdb` |
| Zweck | Modelliert eine Queue fuer nachgelagerte Constraint-Validierungen und claimt den naechsten Batch. |
<!-- SQLDOC:SUMMARY_TABLE:END -->

## Einordnung

Das Artefakt passt zu Szenarien nach Bulk Loads, Staging-Merges oder Wartungsfenstern, in denen Constraints zeitweise deaktiviert oder ohne Vollpruefung wieder aktiviert wurden. Statt sofort alle Revalidierungen auszufuehren, demonstriert das Skript ein Queue-Muster: problematische Constraints werden erst inventarisiert, priorisiert und dann in kontrollierten Batches zur Nachbearbeitung reserviert.

## Annahmen

- Die Erstversion arbeitet ausschliesslich mit Demo-Objekten im Schema `demo` innerhalb von `tempdb`.
- `FK_ConstraintValidationChild_Parent` wird absichtlich deaktiviert gelassen, damit ein hoch priorisierter Queue-Eintrag entsteht.
- `CK_ConstraintValidationChild_RetryCount` wird absichtlich erst deaktiviert und danach ohne `WITH CHECK` wieder aktiviert, damit ein enabled-not-trusted-Zustand sichtbar wird.
- Revalidierungsbefehle werden nur vorgeschlagen und nicht automatisch ausgefuehrt.

## Anwendungsfall

Ein typischer Einsatz ist die Planung von Nacharbeiten nach Imports oder Migrationsschritten. Das Skript zeigt, wie ein Team auffaellige Constraints in eine Warteschlange ueberfuehren, den naechsten Batch claimen und die eigentlichen `ALTER TABLE ... WITH CHECK CHECK CONSTRAINT`-Befehle erst im passenden Betriebsfenster ausfuehren kann.
...
|---|---|---|---|
| `@WorkBatchSize` | `INT` | Nein | Anzahl der Queue-Eintraege, die in diesem Lauf auf `claimed` gesetzt werden. |
| `@IncludeTrusted` | `BIT` | Nein | Fuegt bei `1` auch bereits trusted Constraints mit niedriger Prioritaet in die Queue ein. |
| `@ResetDemoObjects` | `BIT` | Nein | Baut bei `1` das Demo-Modell in `tempdb` vor der Analyse neu auf. |
...
- `ConstraintQueueInventory` zeigt die gesamte erzeugte Warteschlange samt Prioritaet, Grund und empfohlenem Befehl.
- `ClaimedValidationBatch` isoliert die in diesem Lauf reservierten Eintraege und macht Claim-Zeit sowie Worker sichtbar.
- `ValidationRunbook` verdichtet den geclaimten Batch zu einer direkt lesbaren Handlungsanweisung fuer das naechste Wartungsfenster.
- Mit `@IncludeTrusted = 1` laesst sich dieselbe Queue-Idee auch fuer turnusmaessige Reviews statt nur fuer Problemfaelle demonstrieren.
...
|---|---|---|---|
| `1.0` | `2026-04-18` | `ER` | Erstversion einer didaktischen Queue fuer nachgelagerte Constraint-Validierungen. |
...
flowchart TD
    ...
    C -->|Ja| D[Demo-Tabellen und Constraints anlegen]
    D --> E[Beispieldaten laden]
    E --> F[Foreign Key deaktivieren]
    F --> G[RetryCount-Check deaktivieren und ohne WITH CHECK wieder aktivieren]
    ...
    H --> I
    I --> J[Problematische Constraints in Queue-Eintraege ueberfuehren]
    J --> K[Nach Prioritaet sortieren und naechsten Batch claimen]
    K --> L[Queue-Inventar ausgeben]
    L --> M[ClaimedValidationBatch und ValidationRunbook erzeugen]
    M --> N[QueueStatusSummary aggregieren]
    N --> O{DropDemoObjects = 1?}
    O -->|Ja| P[Demo-Objekte entfernen]
    O -->|Nein| Q[Demo-Objekte fuer weitere Durchlaeufe behalten]
```
<!-- SQLDOC:MERMAID:END -->
