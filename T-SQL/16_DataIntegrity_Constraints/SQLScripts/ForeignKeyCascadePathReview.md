# ForeignKeyCascadePathReview.sql

Dieses Skript liest die Foreign-Key-Metadaten der aktuellen Datenbank und verfolgt daraus ableitbare Kaskadenpfade fuer `DELETE`- und `UPDATE`-Regeln. Der Schwerpunkt liegt auf Zielen, die vom selben Root aus ueber mehrere unterschiedliche Pfade erreicht werden koennen.
...
| Feld | Wert |
|---|---|
| Script | [ForeignKeyCascadePathReview.sql](ForeignKeyCascadePathReview.sql) |
| Version | `1.0` |
| Typ | `diagnostic-query` |
| Kapitel | `16_DataIntegrity_Constraints` |
| Sicherheit | `read-only` |
| Zweck | Analysiert FK-Kaskadenpfade und markiert potenzielle Mehrfachpfade vom selben Root zur selben Zieltabelle. |
<!-- SQLDOC:SUMMARY_TABLE:END -->

## Einordnung

Das Artefakt unterstuetzt Schema-Reviews, Migrationsvorbereitung und Risikoanalysen fuer referenzielle Integritaet. Statt DML auszufuehren, wertet das Skript ausschliesslich SQL-Server-Katalogsichten aus und rekonstruiert daraus moegliche Pfade, ueber die Folgeaktionen an nachgelagerte Tabellen weitergereicht werden koennen.

## Annahmen

- Die Analyse beschraenkt sich auf die aktuelle Datenbank und betrachtet keine datenbankuebergreifenden Abhaengigkeiten.
- Mehrfachpfade sind Diagnose- und Review-Signale. Ob daraus tatsaechlich ein Problem entsteht, muss im Kontext des konkreten Datenmodells beurteilt werden.
- Mit `@IncludeSetActions = 1` werden `SET_NULL` und `SET_DEFAULT` wie `CASCADE` als relevante Folgepfade behandelt, weil auch diese Regeln nachgelagerte Tabellen veraendern.
- Zyklen werden in der rekursiven Pfadlogik ueber besuchte Objekt-IDs begrenzt, damit selbstreferenzielle oder ringfoermige Beziehungen die Auswertung nicht endlos fortsetzen.

## Anwendungsfall

Vor dem Einfuehren neuer Foreign Keys oder vor groesseren Loesch- und Update-Prozessen laesst sich pruefen, ob einzelne Zieltabellen vom selben Ursprung aus mehrfach erreicht werden. Solche Muster sind hilfreich fuer Design-Diskussionen, weil sie tiefe Kaskadenketten und moegliche Mehrfachpfade frueh sichtbar machen.
...
| `@IncludeSetActions` | `BIT` | Nein | `1` bewertet `SET_NULL` und `SET_DEFAULT` als Folgepfade; `0` fokussiert auf `CASCADE`. |
| `@MaxDepth` | `INT` | Nein | Begrenzt die rekursive Pfadexpansion fuer die Diagnose. |
...
- `ForeignKeyEdgeInventory` zeigt das Ausgangsinventar aller Foreign Keys mit den Delete-/Update-Regeln und der Markierung, ob daraus ein verfolgbarer Pfad entsteht.
- `CascadePathReview` listet jeden rekursiv gefundenen Pfad inklusive Root, Ziel, Modus, Tabellenkette und FK-Kette.
- `HasMultiplePathsToTarget` markiert in der Pfadliste, ob dasselbe Ziel vom selben Root und Modus aus mehrfach erreicht wird.
- `MultiplePathRiskSummary` verdichtet nur die auffaelligen Ziele und gibt Tabellen- sowie FK-Pfade als Review-Grundlage aus.
...
|---|---|---|---|
| `1.0` | `2026-04-19` | `ER` | Erstversion einer Pfadpruefung fuer potenzielle Mehrfachpfade bei FK-Kaskaden. |
...
```mermaid
flowchart TD
    A[Parameter validieren] --> B[Foreign Keys und Spaltenabbildung aus Katalogsichten lesen]
    B --> C[Delete und Update Regeln je Kante als ForeignKeyEdgeInventory bereitstellen]
    C --> D[Tracked Kanten fuer DELETE und UPDATE als CascadeSeedEdges ableiten]
    D --> E[Rekursive CascadePaths bis MaxDepth ueber ReferencedObjectID erweitern]
    E --> F[CascadePathReview mit Root, Ziel, Pfadtext und PathSignature ausgeben]
    F --> G{Mehr als ein Pfad je Root, Ziel und Modus?}
    G -->|Ja| H[MultiplePathRiskSummary mit PathCount und Beispielpfaden erzeugen]
    G -->|Nein| I[Keine Mehrfachpfade im Summary-Resultset]
```
<!-- SQLDOC:MERMAID:END -->
