# UniqueConstraintOverlapCheck.sql

Dieses Skript analysiert `UNIQUE`-Constraints und optional eigenstaendige eindeutige Indizes auf derselben Tabelle. Es zeigt zuerst das Inventar der gefundenen Definitionen, danach identische oder links-praefixartige Ueberlappungen und schliesst mit einem konservativen Review-Backlog fuer moegliche Redundanzen.
...
| Feld | Wert |
|---|---|
| Script | [UniqueConstraintOverlapCheck.sql](UniqueConstraintOverlapCheck.sql) |
| Version | `1.0` |
| Typ | `diagnostic-query` |
| Kapitel | `16_DataIntegrity_Constraints` |
| Sicherheit | `read-only` |
| Zweck | Findet ueberlappende `UNIQUE`-Definitionen mit moeglichem Redundanzpotenzial. |
<!-- SQLDOC:SUMMARY_TABLE:END -->

## Einordnung

Das Artefakt ist fuer Architektur-Reviews, Datenbankbereinigungen und Governance-Checks gedacht. Es arbeitet ausschliesslich auf SQL-Server-Katalogsichten und markiert Stellen, an denen mehrere eindeutige Definitionen dieselbe Tabelle in sehr aehnlicher Weise absichern. Dadurch wird sichtbar, wo eine Doppeldefinition, eine praefixartige Ueberschneidung oder zumindest ein Dokumentationsbedarf bestehen koennte.

## Annahmen

- Die Erstversion liest nur Metadaten der aktuellen SQL-Server-Datenbank und erzeugt keine DDL.
- Eine gemeldete Ueberlappung ist bewusst nur ein Review-Signal. Ein zusaetzlicher eindeutiger Index kann fuer Query-Planauswahl, Namenskonventionen oder Governance trotzdem gewollt sein.
- Praefixlogik wird ueber die Reihenfolge der Schluesselspalten bewertet. `UNIQUE (A)` gegen `UNIQUE (A, B)` wird daher als moegliche Redundanz markiert.
- Gefilterte eindeutige Indizes werden nicht automatisch als redundant gewertet, sondern mit niedrigerem Review-Fokus angezeigt, weil der Filter fachlich relevant sein kann.

## Anwendungsfall

Das Skript ist nuetzlich, wenn gewachsene Datenbanken auf doppelte Integritaetsdefinitionen untersucht werden sollen oder wenn vor Refactorings sichtbar werden muss, welche `UNIQUE`-Definitionen dieselbe Fachlogik mehrfach abbilden. Mit den Parametern laesst sich der Fokus auf einzelne Schemata oder Tabellen einschraenken und bei Bedarf um eigenstaendige eindeutige Indizes erweitern.
...
| `@IncludeUniqueIndexes` | `BIT` | Nein | `1` bezieht eigenstaendige eindeutige Indizes ohne `UNIQUE`-Constraint ein. |
| `@IncludeExactMatches` | `BIT` | Nein | `1` zeigt auch exakt gleiche Schluesselsignaturen in den Paarergebnissen. |
| `@MinSharedLeadingColumns` | `TINYINT` | Nein | Minimale Zahl gemeinsamer fuehrender Schluesselspalten fuer eine gemeldete Ueberlappung. |
...
- `UniqueDefinitionInventory` listet jede betrachtete eindeutige Definition mit Typ, Schluesselspalten, geschaetzter Breite und Filtereigenschaften.
- `UniqueOverlapPairs` bildet Paare pro Tabelle und markiert `exact_match`, `left_is_prefix_of_right` oder `right_is_prefix_of_left`.
- `UniqueRedundancyBacklog` leitet daraus einen konservativen naechsten Review-Schritt ab, ohne automatisch eine Definition zum Entfernen vorzuschlagen.
- Die Analyse konzentriert sich auf Schluesselspalten in ihrer Reihenfolge. Included Columns sind absichtlich nicht Teil der Redundanzbewertung.
...
|---|---|---|---|
| `1.0` | `2026-04-19` | `ER` | Erstversion eines read-only Checks fuer ueberlappende `UNIQUE`-Definitionen. |
...
flowchart TD
    ...
    B --> C[UNIQUE-Constraints und optional eindeutige Indizes aus sys.indexes lesen]
    C --> D[Schluesselspalten ueber sys.index_columns und sys.columns aufloesen]
    D --> E[Schluesselsignaturen und Breiten je Definition aggregieren]
    E --> F[DefinitionInventory in Temp-Tabelle schreiben]
    F --> G[Paare je Tabelle bilden]
    G --> H[Exact Match und Praefix-Ueberlappung klassifizieren]
    H --> I[OverlapPairs mit Prioritaet und SuggestedNextStep erzeugen]
    I --> J[Inventar ausgeben]
    J --> K[Paarsicht ausgeben]
    K --> L[Review-Backlog ausgeben]
```
<!-- SQLDOC:MERMAID:END -->
