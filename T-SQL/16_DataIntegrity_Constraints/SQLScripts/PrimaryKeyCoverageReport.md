# PrimaryKeyCoverageReport.sql

Dieses Skript erstellt einen read-only Report ueber Tabellen ohne `PRIMARY KEY` sowie ueber Tabellen mit auffaelligen Schluesselmustern in der aktuellen Datenbank. Der Fokus liegt auf schnell sichtbaren Review-Signalen rund um Schluesselbreite, Komplexitaet und fuehrende Datentypen.
...
| Feld | Wert |
|---|---|
| Script | [PrimaryKeyCoverageReport.sql](PrimaryKeyCoverageReport.sql) |
| Version | `1.0` |
| Typ | `diagnostic-query` |
| Kapitel | `16_DataIntegrity_Constraints` |
| Sicherheit | `read-only` |
| Zweck | Listet Tabellen ohne Primary Key und markiert auffaellige PK-Muster fuer Reviews. |
<!-- SQLDOC:SUMMARY_TABLE:END -->

## Einordnung

Das Artefakt passt zu Datenbank-Assessments, Refactoring-Vorbereitung und Architektur-Reviews. Es veraendert keine DDL, sondern verdichtet SQL-Server-Katalogsichten so, dass fehlende Primary Keys und potenziell unguenstige Key-Muster schnell priorisiert werden koennen.

## Annahmen

- Die Erstversion arbeitet ausschliesslich gegen die aktuelle Datenbank und verwendet keine externen Governance-Tabellen.
- Ein markiertes Muster ist ein Review-Hinweis und kein automatischer Modellierungsfehler. Zusammengesetzte oder GUID-basierte Schluessel koennen fachlich sinnvoll sein.
- Die geschaetzte Key-Breite basiert auf deklarierter Spaltenlaenge aus den Katalogsichten und dient der Einordnung, nicht als exakter Laufzeitbeweis.
- Ein nicht geclusterter Primary Key ist bewusst nur als Design-Signal markiert, weil Cluster-Entscheidungen stark vom Zugriffsmuster abhaengen.

## Anwendungsfall

Vor Migrationen, vor Performance-Analysen oder beim Aufraeumen gewachsener Datenmodelle laesst sich mit dem Report schnell erkennen, welche Tabellen gar keinen Primary Key besitzen und bei welchen Tabellen sich eine manuelle Schluesselpruefung lohnt. Die Backlog-Sicht ordnet die auffaelligen Tabellen direkt einer naechsten Review-Aktion zu.
...
| `@IncludeMsShipped` | `BIT` | Nein | `1` bezieht systemnahe Tabellen ein; `0` fokussiert auf benutzerdefinierte Tabellen. |
| `@IncludeHealthyTables` | `BIT` | Nein | `1` zeigt alle Tabellen; `0` fokussiert auf fehlende oder auffaellige Primary-Key-Muster. |
| `@CompositeKeyThreshold` | `TINYINT` | Nein | Ab wie vielen Schluesselspalten ein zusammengesetzter Primary Key als Review-Signal markiert wird. |
| `@WideKeyBytesThreshold` | `SMALLINT` | Nein | Ab welcher geschaetzten Byte-Breite ein Primary Key als breit markiert wird. |
...
- `PrimaryKeyCoverageReport` ist die Hauptsicht mit PK-Metadaten, Pattern-Flags und einer einfachen `ReviewCategory`.
- `PrimaryKeyPatternSummary` verdichtet, wie viele Tabellen ohne Primary Key oder mit breiten, zusammengesetzten oder auffaelligen Leading Keys vorkommen.
- `PrimaryKeyReviewBacklog` leitet fuer jede auffaellige Tabelle eine konservative naechste Aktion ab, zum Beispiel Schluesseldesign pruefen oder einen fehlenden PK ergaenzen.
- Die Flags koennen ueber `@CompositeKeyThreshold` und `@WideKeyBytesThreshold` an lokale Standards angepasst werden.
...
|---|---|---|---|
| `1.0` | `2026-04-19` | `ER` | Erstversion eines read-only Reports fuer fehlende und auffaellige Primary-Key-Muster. |
...
flowchart TD
    ...
    B --> C[Primary-Key-Basis aus sys.key_constraints und sys.indexes lesen]
    C --> D[Key-Spalten ueber sys.index_columns sys.columns und sys.types aufloesen]
    D --> E[Schluesselstatistik mit Spaltenanzahl Breite Signatur und Leading Type aggregieren]
    E --> F[Alle Tabellen mit PK-Statistik in PrimaryKeyCoverage zusammenfuehren]
    F --> G{IncludeHealthyTables = 1?}
    G -->|Ja| H[Alle Tabellen mit ReviewCategory ausgeben]
    G -->|Nein| I[Nur fehlende oder auffaellige PK-Muster ausgeben]
    H --> J[PatternSummary ueber alle betrachteten Tabellen berechnen]
    I --> J
    J --> K[ReviewBacklog mit SuggestedNextStep ausgeben]
```
<!-- SQLDOC:MERMAID:END -->
