# ForeignKeyCascadeMap.sql

Dieses Skript liest die Foreign-Key-Metadaten der aktuellen Datenbank und zeichnet daraus eine kompakte Map der Delete- und Update-Regeln. Sichtbar werden Richtung, Spaltenabbildung, Aktivitaetsstatus und eine Mermaid-taugliche Kantenliste fuer Dokumentation oder Architektur-Reviews.
...
| Feld | Wert |
|---|---|
| Script | [ForeignKeyCascadeMap.sql](ForeignKeyCascadeMap.sql) |
| Version | `1.0` |
| Typ | `diagnostic-query` |
| Kapitel | `16_DataIntegrity_Constraints` |
| Sicherheit | `read-only` |
| Zweck | Zeichnet eine Map der Foreign-Key-Kaskadenregeln ueber referenzierte und referenzierende Tabellen. |
<!-- SQLDOC:SUMMARY_TABLE:END -->

## Einordnung

Das Artefakt eignet sich fuer Schema-Reviews, Change-Impact-Analysen und Dokumentationsaufgaben rund um referenzielle Integritaet. Statt Transaktionen auszufuehren, liest das Skript nur Katalogmetadaten und zeigt damit, welche Folgeaktionen ein `DELETE` oder `UPDATE` auf Elternseiten nach sich ziehen kann.

## Annahmen

- Die Erstversion bewertet ausschliesslich die aktuelle Datenbank und beruecksichtigt keine datenbankuebergreifenden Abhaengigkeiten.
- Als kaskadenrelevant gelten `CASCADE`, `SET_NULL` und `SET_DEFAULT`, weil diese Aktionen den Zustand der Kindtabelle aktiv veraendern.
- Deaktivierte oder nicht vertrauenswuerdige Foreign Keys werden nicht versteckt, sondern im Statusfeld kenntlich gemacht, damit Governance-Luecken sichtbar bleiben.
- Die Mermaid-Kantenliste dient als Textreprasentation der Metadatenkarte und nicht als Nachweis tatsaechlich ausgefuehrter DML-Pfade.

## Anwendungsfall

Vor Loeschkonzepten, Stammdatenbereinigungen oder Migrationsschritten laesst sich schnell ablesen, welche Kindtabellen potenziell mitbetroffen sind und ob Delete- und Update-Regeln gleich oder unterschiedlich modelliert wurden. Die Summary hilft ausserdem, besonders stark verknuepfte Elternobjekte zu priorisieren.
...
| `@IncludeNoAction` | `BIT` | Nein | `1` zeigt auch Foreign Keys ohne Folgeaktion; `0` fokussiert auf Cascade- oder Set-Regeln. |
| `@IncludeDisabled` | `BIT` | Nein | `1` laesst deaktivierte Foreign Keys in der Karte stehen; `0` blendet sie aus. |
...
- `ForeignKeyCascadeMap` liefert die Detailkarte je Foreign Key inklusive Spaltenzuordnung und Integritaetsstatus.
- `CascadeActionSummary` verdichtet je Elternseite und Modus (`DELETE` oder `UPDATE`), wie viele Kaskaden- oder Set-Regeln auf nachgelagerte Tabellen zeigen.
- `CascadeMermaidEdges` gibt direkt verwendbare Kantenstrings fuer Mermaid- oder Graph-Dokumentation aus.
- Mit `@IncludeNoAction = 1` wird aus der Kaskadenkarte ein vollstaendiger FK-Regelplan, der auch blockierende `NO_ACTION`-Beziehungen zeigt.
...
|---|---|---|---|
| `1.0` | `2026-04-19` | `ER` | Erstversion einer Kaskadenkarte fuer Foreign-Key-Regeln. |
...
```mermaid
flowchart TD
    A[Parameter validieren] --> B[Foreign Keys und Spaltenabbildung aus sys.foreign_keys plus sys.foreign_key_columns lesen]
    B --> C{IncludeDisabled = 1?}
    C -->|Ja| D[Alle gefilterten FK-Regeln in die Karte aufnehmen]
    C -->|Nein| E[Deaktivierte FK-Regeln ausblenden]
    D --> F{IncludeNoAction = 1?}
    E --> F
    F -->|Ja| G[Auch NO_ACTION-Regeln in Detail- und Mermaid-Sicht belassen]
    F -->|Nein| H[Nur Cascade und Set-Regeln ausgeben]
    G --> I[ForeignKeyCascadeMap als Detailinventar ausgeben]
    H --> I
    I --> J[CascadeActionSummary je Elternseite und Aktionsart aggregieren]
    J --> K[CascadeMermaidEdges fuer Dokumentationsgraphen ableiten]
```
<!-- SQLDOC:MERMAID:END -->
