# CheckConstraintViolationProbe.sql

Dieses Skript bewertet einen didaktischen Staging-Datensatz gegen geplante CHECK-Constraint-Kandidaten. Die Ausgabe zeigt sowohl einzelne Verletzungen als auch eine verdichtete Readiness-Sicht, bevor ein Constraint in einer echten Tabelle aktiviert wird.
...
| Feld | Wert |
|---|---|
| Script | [CheckConstraintViolationProbe.sql](CheckConstraintViolationProbe.sql) |
| Version | `1.0` |
| Typ | `didactic-lab` |
| Kapitel | `16_DataIntegrity_Constraints` |
| Sicherheit | `read-only-tempdb` |
| Zweck | Bewertet Datenbereiche gegen geplante CHECK-Regeln und zeigt, welche Zeilen vor dem Aktivieren bereinigt werden sollten. |
<!-- SQLDOC:SUMMARY_TABLE:END -->

## Einordnung

Das Artefakt eignet sich fuer Workshops zu Datenqualitaet, Staging-Pruefungen und Constraint-Einfuehrungen. Statt sofort `ALTER TABLE ... WITH CHECK` auf produktive Daten auszufuehren, laesst sich mit derselben Denklogik zuerst sichtbar machen, welche Regelverletzungen und Ausreisser noch im Bestand liegen.

## Annahmen

- Die Erstversion verwendet ausschliesslich einen lokalen Demo-Datensatz in Temp-Tabellen und keine produktiven Tabellen.
- Geplante CHECK-Regeln werden explizit im Skript modelliert, damit der Ablauf ohne vorhandene Metadaten nachvollziehbar bleibt.
- `SeverityRank` dient nur der didaktischen Priorisierung zwischen Regelverletzungen und ersetzt kein echtes Governance-Modell.
- `ColumnHotspots` arbeitet mit textuell dargestellten Beobachtungswerten, um sehr unterschiedliche Zielspalten in einer gemeinsamen Sicht zu zeigen.

## Anwendungsfall

Typisch ist der Einsatz vor einer Constraint-Nachruestung auf gewachsenen Tabellen. Das Skript laesst sich leicht auf reale Staging-Daten uebertragen, indem die Temp-Tabelle und die geplanten Regeln durch echte Quellobjekte und projektspezifische Praedikate ersetzt werden.
...
|---|---|---|---|
| `@OnlyViolations` | `BIT` | Nein | `1` zeigt nur verletzende Regelzeilen; `0` zeigt alle geprueften Regelbewertungen. |
| `@SeverityFloor` | `TINYINT` | Nein | Mindestschwere `1` bis `3` fuer die Ausgabe geplanter Regeln und Verletzungen. |
| `@ConstraintNamePattern` | `NVARCHAR(128)` | Nein | Optionales `LIKE`-Muster zur Auswahl bestimmter geplanter CHECK-Constraint-Namen. |
...
- `ViolationDetails` ist die operative Sicht fuer Datenbereinigung und zeigt Sollbereich, Ist-Wert und Diagnose pro betroffener Regel.
- `ConstraintReadiness` verdichtet pro Regel, ob ein kleiner Nachbereinigungsbatch reicht oder ob vor dem Constraint-Aktivieren noch groessere Korrekturen noetig sind.
- `ColumnHotspots` ist absichtlich generisch gehalten, damit bei einer spaeteren Uebernahme in echte Tabellen dieselbe Sicht fuer unterschiedliche Spaltentypen weiterverwendet werden kann.
- Mit `@ConstraintNamePattern` lassen sich Teilmengen wie `CK_OrderLine_ShipDate%` isoliert betrachten.
...
|---|---|---|---|
| `1.0` | `2026-04-18` | `ER` | Erstversion eines didaktischen Probeskripts fuer geplante CHECK-Constraints. |
...
```mermaid
flowchart TD
    A[Parameter validieren] --> B[Demo-Stagingdaten in #StagingOrderLine anlegen]
    B --> C[Geplante CHECK-Regeln in #PlannedChecks erfassen]
    C --> D[Je Regel eine explizite Bewertung ueber UNION ALL erzeugen]
    D --> E[Nach Severity und ConstraintNamePattern filtern]
    E --> F[ViolationDetails mit Sollbereich und Diagnose ausgeben]
    F --> G[ConstraintReadiness pro Regel verdichten]
    G --> H[ColumnHotspots pro Zielspalte ableiten]
    H --> I[Temp-Tabellen entfernen]
```
<!-- SQLDOC:MERMAID:END -->
