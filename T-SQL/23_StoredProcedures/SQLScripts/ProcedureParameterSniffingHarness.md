# ProcedureParameterSniffingHarness.sql

Dieses Skript baut in `tempdb` eine didaktische Harness fuer moegliche Parameter-Sniffing-Effekte auf. Es vergleicht einen seltenen und einen haeufigen Parameterwert in einer Demo-Procedure und stellt dazu DMV-Deltas fuer Plan-Reuse versus Recompile gegenueber.
...
| Feld | Wert |
|---|---|
| Script | [ProcedureParameterSniffingHarness.sql](ProcedureParameterSniffingHarness.sql) |
| Version | `1.0` |
| Typ | `didactic-lab` |
| Kapitel | `23_StoredProcedures` |
| Sicherheit | `demo-write-tempdb` |
| Zweck | Vergleicht selektive und breite Parameterwerte in einer Demo-Procedure, um moegliche Sniffing-Effekte sichtbar zu machen. |
<!-- SQLDOC:SUMMARY_TABLE:END -->

## Einordnung

Parameter Sniffing entsteht, wenn eine Procedure fuer einen konkreten Parameterwert kompiliert wird und derselbe Plan spaeter fuer deutlich andere Kardinalitaeten wiederverwendet wird. Dieses Lab erzeugt bewusst eine schiefe Datenverteilung und stellt Reuse-Szenarien einem frischen Recompile gegenueber.

## Annahmen

- Es handelt sich um eine didaktische Erstversion mit Demo-Objekten im Schema `demo` in `tempdb`.
- Die Auswertung nutzt `sys.dm_exec_procedure_stats` als Diagnosehilfe; reale Unterschiede haengen von SQL-Server-Version, Cache-Zustand und Umgebung ab.
- Das Skript zeigt moegliche Sniffing-Indizien, ersetzt aber keine tiefe Plananalyse mit Execution Plans oder Query Store.
- `RAREX` steht fuer einen seltenen, `DB100` fuer einen haeufigen Parameterwert in der Demo-Datenbasis.

## Anwendungsfall

Das Skript eignet sich fuer Kapitelabschnitte, in denen erklaert werden soll, warum dieselbe Procedure mit unterschiedlichen Parametern stark abweichende Laufzeit- oder Read-Kennzahlen zeigen kann. Die vier Szenarien helfen dabei, Compile-Reihenfolge, Cache-Reuse und Recompile als getrennte Stellhebel zu besprechen.
...
|---|---|---|---|
| `@SelectiveRegionCode` | `NVARCHAR(20)` | Nein | Selten vorkommender Parameterwert fuer die selektive Compile-Phase. |
| `@BroadRegionCode` | `NVARCHAR(20)` | Nein | Haeufig vorkommender Parameterwert fuer die breite Reuse-Phase. |
| `@MinimumSeats` | `INT` | Nein | Untergrenze fuer die beruecksichtigten Demo-Zeilen. |
| `@DropDemoObjects` | `BIT` | Nein | Entfernt Demo-Tabelle und Demo-Procedure am Ende wieder aus `tempdb`, wenn `1`. |
...
- `SelectiveCompile` recompiliert vor dem seltenen Parameterwert und liefert damit die Ausgangslage fuer einen selektiv kompilierten Plan.
- `BroadReuse` nutzt denselben Procedure-Plan ohne Recompile fuer den haeufigen Parameterwert.
- `BroadRecompile` zeigt denselben haeufigen Parameterwert nach explizitem `sp_recompile`.
- `SelectiveReuseAfterBroad` prueft, ob der seltene Parameterwert nach einer breiten Compile-Phase auffaellige DMV-Deltas zeigt.
...
|---|---|---|---|
| `1.0` | `2026-04-19` | `ER` | Erstversion des didaktischen Harness fuer moegliche Parameter-Sniffing-Effekte |
...
flowchart TD
    ...
    B --> C[Demo-Tabelle mit schiefer Region-Verteilung erzeugen]
    C --> D[Demo-Procedure fuer RegionCode und MinimumSeats anlegen]
    D --> E[Szenarioliste fuer selective und broad Werte aufbauen]
    E --> F{Szenario verlangt Recompile?}
    F -->|Ja| G[sp_recompile auf Demo-Procedure ausfuehren]
    F -->|Nein| H[Vorhandenen Cache-Plan weiterverwenden]
    G --> I[DMV-Snapshot vor Ausfuehrung lesen]
    H --> I
    I --> J[Procedure mit aktuellem Szenario ausfuehren]
    J --> K[DMV-Snapshot nach Ausfuehrung lesen]
    K --> L[Delta-Metriken in ScenarioResult schreiben]
    L --> M{Weitere Szenarien offen?}
    M -->|Ja| F
    M -->|Nein| N[ScenarioMetrics ausgeben]
    N --> O[ProcedureStatsSnapshot aus DMV ausgeben]
    O --> P[Didaktische SniffingAssessment-Vergleiche bilden]
    P --> Q{DropDemoObjects = 1?}
    Q -->|Ja| R[Demo-Procedure und Demo-Tabelle entfernen]
    Q -->|Nein| S[Demo-Objekte fuer weitere Experimente belassen]
```
<!-- SQLDOC:MERMAID:END -->
