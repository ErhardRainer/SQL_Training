# ProcedureRecompileDecisionAid.sql

Dieses Skript erstellt fuer die aktuelle Datenbank eine didaktische Entscheidungshilfe rund um `WITH RECOMPILE` bei Stored Procedures. Es kombiniert DMV-Metriken mit strukturellen Signalen aus dem Modultext und ordnet Procedures als eher `helpful`, `balanced` oder `risky` ein.
...
| Feld | Wert |
|---|---|
| Script | [ProcedureRecompileDecisionAid.sql](ProcedureRecompileDecisionAid.sql) |
| Version | `1.0` |
| Typ | `diagnostic-query` |
| Kapitel | `23_StoredProcedures` |
| Sicherheit | `read-only` |
| Zweck | Zeigt, wann `WITH RECOMPILE` fuer eine Procedure eher hilfreich, eher riskant oder zunaechst nur weiter zu messen ist. |
<!-- SQLDOC:SUMMARY_TABLE:END -->

## Einordnung

`WITH RECOMPILE` kann bei parameterabhaengigen Planunterschieden hilfreich sein, fuehrt aber auch zu wiederholten Compile-Kosten. Das Skript trifft deshalb keine starre Ja/Nein-Entscheidung, sondern stellt Nutzen- und Risikosignale nebeneinander.

## Annahmen

- Die Analyse laeuft rein lesend gegen die aktuelle Datenbank und betrachtet vorhandene Stored Procedures.
- Hinweise auf optionale Parameter, Control Flow oder Dynamic SQL werden textuell aus `sys.sql_modules` abgeleitet und bleiben daher heuristisch.
- DMV-Werte aus `sys.dm_exec_procedure_stats` sind nur verfuegbar, wenn zu einer Procedure bereits Cache- oder Laufzeitdaten vorliegen.
- Die Empfehlung ersetzt keine Plananalyse mit Query Store, Execution Plans oder reproduzierbaren Lasttests.

## Anwendungsfall

Das Skript eignet sich fuer Reviews, Schulungen und erste Performance-Sichtungen, wenn entschieden werden soll, ob `WITH RECOMPILE` ueberhaupt in Betracht gezogen werden sollte. Besonders nuetzlich ist es vor tieferer Analyse einzelner Procedures mit vielen Parametern, optionalen Filtern oder starker Verzweigungslogik.
...
| `@MinExecutionCount` | `BIGINT` | Nein | Mindestanzahl an Ausfuehrungen fuer eine belastbarere Empfehlung. |
| `@HighAvgWorkerMs` | `DECIMAL(18,3)` | Nein | Schwellwert fuer durchschnittlich hohe Worker-Zeit je Ausfuehrung. |
| `@TopN` | `INT` | Nein | Maximalzahl der priorisierten Procedures in der Hauptausgabe. |
...
- Die erste Ausgabe priorisiert Procedures samt Empfehlung, Score und den wichtigsten Begruendungen.
- Die zweite Ausgabe legt alle verwendeten Signale offen, damit die Bewertung nachvollziehbar bleibt.
- `helpful` bedeutet nicht automatisch, dass `WITH RECOMPILE` gesetzt werden sollte; es markiert nur einen sinnvollen Pruefpunkt.
- `risky` bedeutet nicht, dass Recompile unmoeglich ist, sondern dass Compile-Overhead, vorhandene Hinweise oder alternative Muster zunaechst staerker wiegen.
...
|---|---|---|---|
| `1.0` | `2026-04-22` | `ER` | Erstversion der didaktischen Entscheidungshilfe fuer `WITH RECOMPILE` |
...
flowchart TD
    ...
    C --> D[Textuelle Signale fuer Dynamic SQL Control Flow Temp-Objekte optionale Parameter und vorhandene Recompile-Hinweise ableiten]
    D --> E[DMV-Metriken aus sys.dm_exec_procedure_stats fuer die aktuelle Datenbank lesen]
    E --> F[HelpfulScore und RiskScore je Procedure berechnen]
    F --> G[Recommendation helpful balanced oder risky bestimmen]
    G --> H[Priorisierte RecompileDecisionAid ausgeben]
    H --> I[DecisionSignalBreakdown mit allen Einzelsignalen ausgeben]
    I --> J[Didaktische DecisionAidNotes ausgeben]
```
<!-- SQLDOC:MERMAID:END -->
