# ProcedureOutputContractReview.sql

Dieses Skript baut in `tempdb` drei Demo-Prozeduren mit unterschiedlichen Rueckgabe- und Output-Mustern auf und bewertet deren Vertrag aus Sicht eines Reviews. Die Auswertung kombiniert Katalogmetadaten, Output-Parameter-Inventar und das Profil des ersten Resultsets.
...
| Feld | Wert |
|---|---|
| Script | [ProcedureOutputContractReview.sql](ProcedureOutputContractReview.sql) |
| Version | `1.0` |
| Typ | `didactic-lab` |
| Kapitel | `23_StoredProcedures` |
| Sicherheit | `demo-write-tempdb` |
| Zweck | Reviewt Rueckgabe- und Output-Kontrakte von Demo-Prozeduren ueber Katalogsicht und Resultset-Metadaten. |
<!-- SQLDOC:SUMMARY_TABLE:END -->

## Einordnung

Stored Procedures haben oft mehr als einen Rueckgabekanal. Neben dem eigentlichen Resultset spielen Output-Parameter und Returncodes eine Rolle, besonders fuer Batch-Aufrufer, ETL-Strecken oder API-nahe Wrapper. Das Skript zeigt, wie sich diese Kontrakte systematisch reviewen lassen, ohne jede Procedure manuell auszufuehren.

## Annahmen

- Es handelt sich um eine didaktische Erstversion ohne produktive Prozeduren oder produktive Review-Regeln.
- Alle Demo-Objekte werden ausschliesslich in `tempdb` angelegt.
- Das Review konzentriert sich bewusst auf Output-Parameter, die Nutzung des `RETURN`-Statements und das erste Resultset.
- Die drei Demo-Prozeduren repraesentieren unterschiedliche Vertragsmuster: Output plus Returncode, reiner Output-Fokus und reines Resultset.

## Anwendungsfall

Das Skript eignet sich fuer Kapitelabschnitte, in denen Stored Procedures als vertragliche Schnittstellen betrachtet werden. Es ist besonders nuetzlich, wenn Lernende oder Reviewer schnell pruefen wollen, ob eine Procedure nur Daten liefert oder zusaetzlich klare Statuskanaele fuer Automatisierung und Fehlerbehandlung anbietet.
...
|---|---|---|---|
| `@ProcedureNamePattern` | `NVARCHAR(128)` | Nein | LIKE-Filter fuer die zu reviewenden Demo-Prozeduren. |
| `@IncludeSystemShipped` | `BIT` | Nein | Nimmt bei `1` auch `system_shipped`-Prozeduren in die Review-Menge auf. |
...
- Das erste Resultset fasst pro Procedure zusammen, wie viele Output-Parameter sichtbar sind und ob der Modultext einen expliziten `RETURN`-Pfad enthaelt.
- Das Output-Parameter-Inventar hilft bei Reviews, wenn Status- und Kennzahlparameter schnell identifiziert werden sollen.
- Die dritte Ausgabe zeigt ueber `sys.dm_exec_describe_first_result_set_for_object`, ob sich der fachliche Resultset-Vertrag ohne Client-Code beschreiben laesst.
...
|---|---|---|---|
| `1.0` | `2026-04-17` | `ER` | Erstversion des didaktischen Reviews fuer Procedure-Output- und Rueckgabevertraege |
...
flowchart TD
    ...
    B --> C[Demo-Tabelle mit Vertragsbeispielen aufbauen]
    C --> D[Drei Demo-Prozeduren mit verschiedenen Vertragsmustern erzeugen]
    D --> E[Zielmenge der reviewenden Procedures nach Name und Scope sammeln]
    E --> F[Output-Parameter und erstes Resultset je Procedure aggregieren]
    F --> G[ProcedureContractReview mit ReviewStatus und Note ausgeben]
    G --> H[OutputParameterInventory aus sys.parameters ausgeben]
    H --> I[FirstResultSetMetadata ueber dm_exec_describe_first_result_set_for_object ausgeben]
    I --> J[Didaktische ReviewChecklist ausgeben]
    J --> K{DropDemoObjects = 1?}
    K -->|Ja| L[Demo-Prozeduren und Demo-Tabelle entfernen]
    K -->|Nein| M[Demo-Objekte fuer weitere Experimente belassen]
```
<!-- SQLDOC:MERMAID:END -->
