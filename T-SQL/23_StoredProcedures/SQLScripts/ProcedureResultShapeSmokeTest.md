# ProcedureResultShapeSmokeTest.sql

Dieses Skript baut in `tempdb` drei Demo-Prozeduren mit stabil gedachten Rueckgabeformen auf und prueft dann ueber Systemmetadaten, ob deren erstes Resultset weiterhin den dokumentierten Vertrag einhaelt. Der Fokus liegt auf einem kompakten Smoke Test fuer Spaltennamen, Reihenfolge, Datentypen und Nullable-Verhalten.
...
| Feld | Wert |
|---|---|
| Script | [ProcedureResultShapeSmokeTest.sql](ProcedureResultShapeSmokeTest.sql) |
| Version | `1.0` |
| Typ | `didactic-lab` |
| Kapitel | `23_StoredProcedures` |
| Sicherheit | `demo-write-tempdb` |
| Zweck | Prueft ueber Metadaten, ob Demo-Prozeduren ihre erwarteten Resultset-Spalten unveraendert liefern. |
<!-- SQLDOC:SUMMARY_TABLE:END -->

## Einordnung

Stored Procedures wirken aus Sicht von Reports, ETL-Strecken oder Anwendungen oft wie kleine APIs. Genau deshalb reicht ein optischer Blick auf SQL-Code haeufig nicht aus: Schon eine verschobene Spaltenreihenfolge oder ein geaenderter Datentyp kann bestehende Konsumenten brechen. Dieses Lab zeigt ein didaktisches Muster, wie sich solche Rueckgabeformen ueber Metadaten als schneller Smoke Test absichern lassen.

## Annahmen

- Die Umsetzung ist bewusst ein didaktisches `tempdb`-Lab und erzeugt keine produktiven Objekte.
- Der Smoke Test betrachtet nur das erste Resultset je Procedure, weil dies in vielen Integrationsszenarien der vertraglich relevante Standardfall ist.
- Das LIKE-Muster `usp_ResultShapeSmoke%` grenzt die Demo-Prozeduren bewusst auf eine kleine, kontrollierte Testmenge ein.
- Die erwarteten Resultset-Spalten werden im Skript selbst als kompakter Vertrag gepflegt, damit Abweichungen sofort positionsbasiert sichtbar werden.

## Anwendungsfall

Das Skript eignet sich fuer Schulungen zu Procedure-Vertraegen, fuer Team-Reviews vor Deployments und fuer alle Situationen, in denen ein schneller Regressionstest auf Resultset-Formen hilfreicher ist als eine manuelle Sichtpruefung. Besonders nuetzlich ist es fuer Teams, die Stored Procedures als stabile Schnittstellen fuer Reporting oder Batch-Verarbeitung behandeln.
...
|---|---|---|---|
| `@ProcedureNameLike` | `SYSNAME` | Nein | Optionales LIKE-Muster fuer die zu pruefenden Demo-Prozeduren. |
| `@IncludeDefinitionPreview` | `BIT` | Nein | Zeigt bei `1` zusaetzlich die registrierten Demo-Prozeduren mit Kurzbeschreibung. |
| `@DropDemoObjects` | `BIT` | Nein | Entfernt Demo-Tabellen und Demo-Prozeduren am Ende wieder aus `tempdb`. |
...
- `#ExpectedColumns` definiert den Resultset-Vertrag je Demo-Procedure bewusst explizit im Skript.
- `sys.dm_exec_describe_first_result_set_for_object` liest die tatsaechliche erste Rueckgabeform aus, ohne dass dafuer externer Testcode geschrieben werden muss.
- `SmokeSummary` verdichtet die Detailpruefung pro Procedure auf `pass`, `needs_review` oder `metadata_error`.
- `SmokeFindings` zeigt positionsbasiert, ob Spalten fehlen, unerwartet auftauchen oder in Name, Typ oder Nullable-Eigenschaft vom Vertrag abweichen.
...
|---|---|---|---|
| `1.0` | `2026-04-22` | `ER` | Erstversion des tempdb-Labs fuer Metadaten-basierte Resultset-Smoke-Tests |
...
flowchart TD
    ...
    B --> C[Demo-Module und Demo-Learner anlegen und fuellen]
    C --> D[Drei Demo-Prozeduren fuer Summary, Certification und AtRisk erzeugen]
    D --> E[Procedure-Katalog ueber sys.procedures und Pattern aufbauen]
    E --> F[Erwartete Resultset-Spalten in #ExpectedColumns hinterlegen]
    F --> G[Tatsaechliche Metadaten je Procedure ueber sys.dm_exec_describe_first_result_set_for_object lesen]
    G --> H[Erwartete und tatsaechliche Spalten positionsbasiert zusammenfuehren]
    H --> I[FindingStatus fuer Match, Missing, Unexpected oder Typabweichung ableiten]
    I --> J{IncludeDefinitionPreview = 1?}
    J -->|Ja| K[Registrierte Demo-Prozeduren zuerst ausgeben]
    J -->|Nein| L[Direkt mit Vertragsdaten weitermachen]
    K --> M[ExpectedColumnCatalog und ActualColumnMetadata ausgeben]
    L --> M
    M --> N[SmokeSummary und SmokeFindings ausgeben]
    N --> O{DropDemoObjects = 1?}
    O -->|Ja| P[Demo-Prozeduren und Demo-Tabellen wieder entfernen]
    O -->|Nein| Q[Demo-Objekte fuer weitere Experimente belassen]
```
<!-- SQLDOC:MERMAID:END -->
