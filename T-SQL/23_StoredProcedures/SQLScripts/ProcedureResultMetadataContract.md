# ProcedureResultMetadataContract.sql

Dieses Skript baut in `tempdb` eine kleine Demo-Procedure mit bewusst stabilem Rueckgabe-Vertrag auf. Anschliessend liest es die Parameter-Metadaten aus `sys.parameters`, beschreibt das erste Resultset mit `sys.dm_exec_describe_first_result_set` und zeigt eine Beispielausfuehrung mit Output-Parameter und Returncode.
...
| Feld | Wert |
|---|---|
| Script | [ProcedureResultMetadataContract.sql](ProcedureResultMetadataContract.sql) |
| Version | `1.0` |
| Typ | `didactic-lab` |
| Kapitel | `23_StoredProcedures` |
| Sicherheit | `demo-write-tempdb` |
| Zweck | Zeigt einen stabil dokumentierten Stored-Procedure-Vertrag fuer Resultset-Metadaten, Output-Parameter und Returncodes. |
<!-- SQLDOC:SUMMARY_TABLE:END -->

## Einordnung

Der Fokus liegt auf dem API-Charakter von Stored Procedures. Das Skript zeigt nicht nur eine fachliche Abfrage, sondern vor allem, welche Teile des Vertrags fuer Konsumenten stabil gehalten werden sollten: Resultset-Form, Parameterliste, Output-Status und Returncode.

## Annahmen

- Es handelt sich um eine didaktische Erstversion ohne produktive Tabellen oder Prozeduren.
- Die Demo-Prozedur wird ausschliesslich in `tempdb` angelegt und kann am Ende wieder entfernt werden.
- Der Vertrag konzentriert sich auf genau ein erstes Resultset plus einen Output-Parameter und einen numerischen Returncode.
- `ReturnCode = 0` steht fuer Erfolg und `ReturnCode = 10` fuer einen gueltigen No-Data-Fall.

## Anwendungsfall

Das Skript eignet sich fuer Kapitelabschnitte, in denen Stored Procedures als stabile Schnittstellen behandelt werden. Besonders nuetzlich ist es, wenn Lernende zwischen fachlichem Resultset, technischem Status ueber Output-Parameter und kompaktem Rueckgabecode unterscheiden sollen.
...
|---|---|---|---|
| `@CourseCode` | `NVARCHAR(20)` | Nein | Filtert die Demo-Prozedur auf einen Kurscode. |
| `@IncludeArchived` | `BIT` | Nein | Schaltet archivierte Demo-Zeilen bei `1` frei. |
...
- Die erste Ausgabe beschreibt die didaktischen Vertragsregeln explizit, bevor Systemmetadaten abgefragt werden.
- `sys.dm_exec_describe_first_result_set` hilft, das erste Resultset zu pruefen, ohne einen Client schreiben zu muessen.
- Die Beispielausfuehrung zeigt den Unterschied zwischen den Zeilen im Resultset und den zusaetzlichen Statuskanaelen `@ExecutionState` und `ReturnCode`.
...
|---|---|---|---|
| `1.0` | `2026-04-17` | `ER` | Erstversion des didaktischen Procedure-Contract-Labs fuer Resultset-Metadaten |
...
flowchart TD
    ...
    B --> C[Vertrags- und Protokolltabellen in tempdb aufbauen]
    C --> D[Demo-Prozedur mit Output-Parameter und Returncode erzeugen]
    D --> E[Didaktischen Vertragskatalog ausgeben]
    E --> F[Parameter-Metadaten aus sys.parameters lesen]
    F --> G[Erstes Resultset mit sys.dm_exec_describe_first_result_set beschreiben]
    G --> H[Demo-Prozedur ausfuehren und Resultset puffern]
    H --> I[Resultset plus OutputParameterState und ReturnCode zeigen]
    I --> J{DropDemoObjects = 1?}
    J -->|Ja| K[Demo-Prozedur und Demo-Tabellen entfernen]
    J -->|Nein| L[Demo-Objekte fuer weitere Experimente belassen]
```
<!-- SQLDOC:MERMAID:END -->
