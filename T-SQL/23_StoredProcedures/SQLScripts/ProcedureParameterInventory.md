# ProcedureParameterInventory.sql

Dieses Skript baut in `tempdb` mehrere Demo-Prozeduren mit unterschiedlichen Parameterlisten auf und erstellt daraus ein Inventar ueber Datentypen, Richtungen und Default-Hinweise. Der Fokus liegt darauf, Parameter-Metadaten fuer Reviews oder Schulungssituationen sichtbar und vergleichbar zu machen.
...
| Feld | Wert |
|---|---|
| Script | [ProcedureParameterInventory.sql](ProcedureParameterInventory.sql) |
| Version | `1.0` |
| Typ | `didactic-lab` |
| Kapitel | `23_StoredProcedures` |
| Sicherheit | `demo-write-tempdb` |
| Zweck | Inventarisiert Stored-Procedure-Parameter inklusive Datentypen, Richtungen und Default-Hinweisen. |
<!-- SQLDOC:SUMMARY_TABLE:END -->

## Einordnung

Parameter sind ein zentraler Teil des Vertrags einer Stored Procedure. Das Skript zeigt, wie sich dieser Vertrag ueber Katalogsichten und den Procedure-Text systematisch inventarisieren laesst, ohne produktive Objekte vorauszusetzen.

## Annahmen

- Es handelt sich um eine didaktische Erstversion mit Demo-Prozeduren im Schema `demo` in `tempdb`.
- Default-Ausdruecke werden zusaetzlich aus `sys.sql_modules` gelesen, damit T-SQL-Defaults im Inventar sichtbar werden.
- Die Inventarisierung konzentriert sich auf Parameterstruktur, nicht auf Laufzeit- oder Performance-Merkmale der Prozeduren.
- `INOUT` markiert Output-Parameter, die zugleich Eingabewerte akzeptieren oder Rueckmeldungen liefern.

## Anwendungsfall

Das Skript eignet sich fuer Kapitelabschnitte, in denen Stored Procedures als stabile Schnittstellen verstanden werden sollen. Es ist besonders nuetzlich, wenn Lernende oder Reviewer schnell sehen wollen, welche Parameter eine Procedure besitzt, welche Defaults hinterlegt sind und welche Parameter als Output-Kanal dienen.
...
|---|---|---|---|
| `@ProcedureNamePattern` | `NVARCHAR(128)` | Nein | LIKE-Filter fuer die zu inventarisierenden Demo-Prozeduren. |
| `@IncludeDemoOnly` | `BIT` | Nein | Beschraenkt die Inventarisierung bei `1` auf das Schema `demo`. |
...
- Die erste Ausgabe verdichtet je Procedure, wie viele Parameter insgesamt, mit Defaults und als Output vorhanden sind.
- Die Detailausgabe zeigt neben dem technischen Typ auch die gelesene Parameterdeklaration aus dem Modultext.
- Die didaktischen Hinweise am Ende helfen, das Inventar fuer Reviews oder refaktorierende Arbeiten einzuordnen.
...
|---|---|---|---|
| `1.0` | `2026-04-19` | `ER` | Erstversion des didaktischen Parameter-Inventars fuer Stored Procedures |
...
flowchart TD
    ...
    B --> C[Demo-Tabelle mit Procedure-Beispieldaten aufbauen]
    C --> D[Drei Demo-Prozeduren mit verschiedenen Parameterprofilen erzeugen]
    D --> E[Zielmenge der Procedures nach Name und Scope sammeln]
    E --> F[sys.parameters und sys.types fuer technische Parameterdaten lesen]
    F --> G[Parameterdeklarationen aus sys.sql_modules ausschneiden]
    G --> H[Inventartabelle mit Richtung Typ und Default-Hinweisen fuellen]
    H --> I[Summary je Procedure ausgeben]
    I --> J[Detailliertes Parameterinventar ausgeben]
    J --> K[Didaktische Hinweise ausgeben]
