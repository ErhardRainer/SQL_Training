# ProcedureDynamicSqlFinder.sql

Dieses Skript baut in `tempdb` mehrere Demo-Prozeduren mit statischem und dynamischem SQL auf und analysiert anschliessend ueber `sys.sql_modules`, welche typischen Muster fuer dynamisches SQL in Stored Procedures vorkommen. Der Schwerpunkt liegt auf einer leicht nachvollziehbaren Erstversion fuer Unterricht, Reviews und spaetere Governance-Regeln.
...
| Feld | Wert |
|---|---|
| Script | [ProcedureDynamicSqlFinder.sql](ProcedureDynamicSqlFinder.sql) |
| Version | `1.0` |
| Typ | `didactic-lab` |
| Kapitel | `23_StoredProcedures` |
| Sicherheit | `demo-write-tempdb` |
| Zweck | Findet Prozeduren, die dynamisches SQL verwenden. |
<!-- SQLDOC:SUMMARY_TABLE:END -->

## Einordnung

Das Skript zeigt, wie sich dynamisches SQL in Stored Procedures pragmatisch und ohne vollstaendigen Parser sichten laesst. Dabei werden sowohl sichere als auch bewusst riskantere Demo-Muster aufgebaut, damit Unterschiede zwischen `sp_executesql`, `EXEC(@sql)` und `QUOTENAME` greifbar bleiben.
...
- Die Erkennung basiert auf heuristischen Textmustern in `sys.sql_modules` und nicht auf einer vollstaendigen T-SQL-Parserlogik.
- `QUOTENAME` wird als nuetzliches Schutzsignal fuer Objektbezeichner gewertet, aber nicht als allgemeiner Ersatz fuer Parameterisierung.

## Anwendungsfall

Das Skript eignet sich fuer Reviews von Procedure-Portfolios, Trainings zu dynamischem SQL und erste Governance-Checks. Lernende koennen nachvollziehen, warum frei zusammengesetzte `EXEC(@sql)`-Aufrufe hoehere Review-Prioritaet erhalten als parameterisierte `sp_executesql`-Varianten.
...
|---|---|---|---|
| `@ProcedureNamePattern` | `NVARCHAR(128)` | Nein | LIKE-Filter fuer die zu analysierenden Stored Procedures. |
| `@IncludeDemoSetup` | `BIT` | Nein | Baut Demo-Tabelle und Demo-Prozeduren in `tempdb` auf, wenn `1`. |
| `@DropDemoObjects` | `BIT` | Nein | Entfernt die Demo-Objekte nach der Analyse wieder, wenn `1`. |
...
- `ProcedureDynamicSqlSignals` zeigt pro Procedure mehrere Heuristik-Flags sowie eine kompakte Kategorien- und Prioritaetsbewertung.
- `ProcedureDynamicSqlSummary` verdichtet das Portfolio in statische, parameterisierte und review-beduerftige Muster.
- Der Demo-Fall `usp_DynamicSqlUnsafeSearch` illustriert bewusst eine Literal-Ersetzung mit `EXEC(@sql)`, damit ein kontrastierender High-Risk-Fall sichtbar bleibt.
...
|---|---|---|---|
| `1.0` | `2026-04-17` | `ER` | Erstversion des didaktischen Labs zur Erkennung von dynamischem SQL in Stored Procedures |
...
flowchart TD
    ...
    C -->|Ja| D[Demo-Tabelle und vier Demo-Prozeduren anlegen]
    C -->|Nein| E[Bestehende Procedures im aktuellen Filter verwenden]
    D --> F[Procedure-Definitionen aus sys.sql_modules lesen]
    E --> F
    F --> G[Heuristische Signale fuer sp_executesql, EXEC string, QUOTENAME und SQL-Aufbau ableiten]
    G --> H[ProcedureDynamicSqlSignals je Procedure ausgeben]
    H --> I[ProcedureDynamicSqlSummary nach Kategorien aggregieren]
    I --> J[Checklist fuer Review und Governance ausgeben]
    J --> K{IncludeDemoSetup und DropDemoObjects = 1?}
    K -->|Ja| L[Demo-Prozeduren und Demo-Tabelle entfernen]
    K -->|Nein| M[Demo-Objekte fuer weitere Analysen belassen]
```
<!-- SQLDOC:MERMAID:END -->
