# InListParameterizationDemo.sql

Dieses Skript zeigt zwei robuste Muster fuer `WHERE ... IN (...)` im Trainingskontext: eine kleine stabile Liste per `VALUES`-Konstruktor und eine groessere, wechselnde Liste per CSV-Aufbereitung mit `STRING_SPLIT`.
...
| Feld | Wert |
|---|---|
| Script | [InListParameterizationDemo.sql](InListParameterizationDemo.sql) |
| Version | `1.0` |
| Typ | `didactic-lab` |
| Kapitel | `04_Where` |
| Sicherheit | `read-only-tempdb` |
| Zweck | Demonstriert Muster fuer kleine und groessere `IN`-Listen. |
<!-- SQLDOC:SUMMARY_TABLE:END -->

## Einordnung

Im `WHERE`-Kapitel ist `IN` zwar syntaktisch einfach, praktisch aber oft von der Eingabeform abhaengig. Eine kurze feste Liste kann direkt im Statement lesbar bleiben. Bei groesseren oder variierenden Listen ist eine vorbereitete Join-Menge meist wartbarer und besser validierbar als eine sehr lange Literal-Liste.

## Annahmen

- Das Skript ist eine didaktische Erstversion und arbeitet ausschliesslich mit tempdb-nahen Demo-Daten.
- Die groessere Liste wird als CSV uebergeben, weil T-SQL in dieser Trainingsumgebung keine echte Table-Valued-Parameter-Schnittstelle voraussetzt.
- Ungueltige Tokens wie `abc` werden bewusst sichtbar aussortiert, damit die Listenbereinigung als eigener Lernschritt erkennbar bleibt.
- Die Strategiegrenzen sind didaktische Heuristiken und keine harte Optimizer-Regel.

## Anwendungsfall

Das Lab eignet sich fuer Unterricht, Code Reviews und Refactorings rund um Filterparameter. Es beantwortet die Frage, wann eine kleine `IN`-Liste direkt im Query bleiben darf und wann ein vorgelagerter Aufbereitungsschritt fuer eine groessere Menge sinnvoller ist.
...
|---|---|---|---|
| `@IncludeLargeList` | `BIT` | Nein | Fuehrt bei `1` zusaetzlich das Muster fuer die groessere Liste aus. |
| `@LargeCustomerCsv` | `NVARCHAR(MAX)` | Nein | CSV-Liste fuer das groessere Listenmuster mit CustomerIDs. |
| `@MinOrderAmount` | `DECIMAL(10,2)` | Nein | Untergrenze fuer die Demo-Bestellsumme im Ergebnis. |
...
- `STRING_SPLIT` fuer die groessere CSV-Liste
- `TRY_CONVERT` fuer die Validierung numerischer Tokens
- `ROW_NUMBER` fuer eine stabile Token-Reihenfolge
<!-- SQLDOC:DEPENDENCIES_LIST:END -->

## Hinweise

- `SmallListMatches` zeigt das kompakte Muster fuer wenige feste Werte mit sehr wenig Vorbereitungslogik.
- `LargeListMatches` baut zuerst eine bereinigte und deduplizierte Listenmenge auf und filtert dann per Join.
- `StrategyComparison` fasst beide Muster mit Tokenqualitaet und didaktischer Empfehlung zusammen.
- Das Skript demonstriert Parameterisierung und Listenaufbereitung, nicht physische Performance-Messung.
...
|---|---|---|---|
| `1.0` | `2026-04-17` | `ER` | Erstversion fuer ein didaktisches Lab zu kleinen und groesseren IN-Listen |
...
```mermaid
flowchart TD
    A[Parameter pruefen] --> B[Demo-Bestellungen in #Orders laden]
    B --> C[CSV-Tokens mit STRING_SPLIT in #LargeListTokens aufbereiten]
    C --> D[Kleine Liste per VALUES-Konstruktor definieren]
    D --> E[SmallListMatches ueber Join auf #Orders ausgeben]
    E --> F[Gueltige CustomerIDs aus CSV deduplizieren]
    F --> G{IncludeLargeList = 1?}
    G -->|Ja| H[LargeListMatches ueber Join auf bereinigte Liste ausgeben]
    G -->|Nein| I[Leeres grosses Listenmuster zulassen]
    H --> J[Tokenqualitaet und Strategievergleich verdichten]
    I --> J
    J --> K[StrategyComparison mit Empfehlungen ausgeben]
```
<!-- SQLDOC:MERMAID:END -->
