# WhereSelectiveFilterProfiler.sql

Dieses Skript profiliert mehrere typische `WHERE`-Bedingungen auf derselben Demo-Auftragsmenge. Statt nur einzelne Trefferzahlen zu zeigen, stellt es Zeilenanteil, Kundenbreite und Umsatzabdeckung nebeneinander, damit Selektivitaet fuer Reviews und Unterricht schneller vergleichbar wird.
...
| Feld | Wert |
|---|---|
| Script | [WhereSelectiveFilterProfiler.sql](WhereSelectiveFilterProfiler.sql) |
| Version | `1.0` |
| Typ | `didactic-lab` |
| Kapitel | `04_Where` |
| Sicherheit | `read-only-tempdb` |
| Zweck | Profiliert typische `WHERE`-Filter nach Trefferquote, Abdeckung und Selektivitaetsband. |
<!-- SQLDOC:SUMMARY_TABLE:END -->

## Einordnung

Das Lab knuepft an klassische Filterdiskussionen an, geht aber einen Schritt weiter als ein einfacher Trefferzaehler. Jeder Filter wird nicht nur nach Zeilenanzahl, sondern auch nach Kundenstreuung und Umsatzanteil bewertet. Dadurch wird sichtbar, dass ein Filter zwar wenige Zeilen treffen kann, aber trotzdem noch einen grossen Teil des Volumens umfasst.

## Annahmen

- Das Skript ist eine didaktische Erstversion und arbeitet ausschliesslich mit tempdb-nahen Demo-Daten.
- Der Namensfilter wird bewusst als Prefix-Suche mit `LIKE 'X%'` modelliert, weil dies in Suchmasken haeufiger ist als freie Wildcard-Suchen.
- Der Monatsfilter wird als Datumsbereich statt ueber `MONTH(OrderDate)` umgesetzt, damit die Profilierung keine Funktionslogik direkt auf der Spalte vormacht.
- Die Selektivitaetsbaender sind didaktische Heuristiken und keine unmittelbare Aussage ueber echte Optimizer-Kosten.

## Anwendungsfall

Das Skript eignet sich fuer Schulungen, Query-Reviews und erste Indexgespraeche im Kapitel `WHERE`. Es hilft insbesondere dann, wenn Teams mehrere moegliche Filterkriterien gegeneinander abwaegen und schnell erkennen wollen, welche Bedingungen eher eng, mittel oder breit auf die Ausgangsmenge wirken.
...
|---|---|---|---|
| `@TargetRegionCode` | `CHAR(2)` | Nein | Optionaler Regionenwert fuer `DE`, `AT` oder `CH`. |
| `@MinNetAmount` | `DECIMAL(10,2)` | Nein | Optionaler Schwellwert fuer einen Bereichsfilter auf `NetAmount`. |
| `@CustomerNamePrefix` | `NVARCHAR(20)` | Nein | Optionales Namenspraefix fuer eine `LIKE`-Prefix-Suche. |
| `@TargetMonth` | `TINYINT` | Nein | Optionaler Monatswert von `1` bis `12` fuer einen Datumsbereich. |
| `@PriorityOnly` | `BIT` | Nein | Aktiviert bei `1` einen Filter auf priorisierte Auftraege. |
<!-- SQLDOC:PARAMETERS_TABLE:END -->

## Abhaengigkeiten

<!-- SQLDOC:DEPENDENCIES_LIST:BEGIN -->
- `tempdb` fuer die temporaeren Demo-Tabellen und Profil-Container
- `CASE` fuer Selektivitaetsbaender und Empfehlungen
- `DATEFROMPARTS` und `EOMONTH` fuer den Monatsbereich
- `LIKE` fuer das Prefix-Filtermuster
- `UNION ALL` fuer das einheitliche Profiling mehrerer Filterformen
<!-- SQLDOC:DEPENDENCIES_LIST:END -->

## Hinweise

- `FilterCatalog` beschreibt die profilierten Filtermuster und ihren didaktischen Zweck.
- `FilterProfile` zeigt pro Filter sowohl Zeilenanteil als auch Umsatzabdeckung, damit Selektivitaet differenzierter gelesen werden kann.
- `CompositeFilterPreview` macht sichtbar, wie sich mehrere Einzelpraedikate gemeinsam weiter verengen.
- Das Skript ist bewusst klein und deterministisch gehalten, damit die Profilierung ohne produktiven Kontext lesbar bleibt.
...
|---|---|---|---|
| `1.0` | `2026-04-17` | `ER` | Erstversion fuer ein didaktisches Profiling typischer WHERE-Filter |
...
```mermaid
flowchart TD
    A[Parameter validieren] --> B[#SalesOrders, #FilterCatalog und #FilterProfile anlegen]
    B --> C[Demo-Auftraege in #SalesOrders laden]
    C --> D[FilterCatalog fuer Equality, Range, Prefix LIKE, Date Range und BIT-Filter fuellen]
    D --> E[Pro Filter MatchingRows, MatchingCustomers und MatchingAmount berechnen]
    E --> F[RowSelectivityPct, AmountCoveragePct und SelectivityBand in #FilterProfile ableiten]
    F --> G[FilterCatalog ausgeben]
    G --> H[FilterProfile nach engsten Filtern sortiert ausgeben]
    H --> I[Kombinierte Filter-Szenarien ueber CompositeFilterPreview bewerten]
```
<!-- SQLDOC:MERMAID:END -->
