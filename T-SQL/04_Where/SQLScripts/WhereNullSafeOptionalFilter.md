# WhereNullSafeOptionalFilter.sql

Dieses Skript zeigt ein robustes Muster fuer optionale Filter auf nullable Spalten. Im Zentrum steht der Unterschied zwischen einem bequemen, aber fehleranfaelligen Shortcut und einem expliziten Filtermodus, der `NULL` sachlich und vorhersehbar behandelt.
...
| Feld | Wert |
|---|---|
| Script | [WhereNullSafeOptionalFilter.sql](WhereNullSafeOptionalFilter.sql) |
| Version | `1.0` |
| Typ | `didactic-lab` |
| Kapitel | `04_Where` |
| Sicherheit | `read-only-tempdb` |
| Zweck | Zeigt null-sichere optionale Filter auf nullable Spalten und kontrastiert sie mit einem unsicheren Shortcut. |
<!-- SQLDOC:SUMMARY_TABLE:END -->

## Einordnung

Optional bedeutet fachlich oft "Filter ist aus". Bei nullable Spalten ist das aber nicht gleichbedeutend mit einem Vergleichsausdruck, der `NULL` an dieselbe Stelle setzt wie einen echten Suchwert. Genau deshalb ist `Spalte = COALESCE(@Parameter, Spalte)` fuer Reviews gefaehrlich: Sobald `Spalte` selbst `NULL` enthalten kann, verschwinden Zeilen im scheinbar filterlosen Modus.

## Annahmen

- Das Skript arbeitet ausschliesslich mit einer kleinen tempdb-nahen Demo-Menge und nullable `AccountManager`-Werten.
- `@ManagerFilterMode` trennt bewusst drei fachliche Zustaende: kein Managerfilter, exakter Managerfilter und nur unzugewiesene Portfolios.
- Der unsichere Vergleich wird nur didaktisch gezeigt; fuer neue Abfragen ist das null-sichere Muster der Zielzustand.
- Das Lab fokussiert auf semantische Korrektheit und Lesbarkeit, nicht auf Optimizer-Metriken oder Indexdesign.

## Anwendungsfall

Das Skript eignet sich fuer Suchmasken, Reporting-Abfragen und Query-Reviews, bei denen nullable Fachattribute optional gefiltert werden. Es ist besonders nuetzlich, wenn ein Parameter gleichzeitig "nicht gesetzt" und "suche NULL-Werte" repraesentieren koennte.
...
|---|---|---|---|
| `@RegionCode` | `CHAR(2)` | Nein | Optionaler Regionsfilter; `NULL` deaktiviert die Regionseinschraenkung. |
| `@ManagerFilterMode` | `VARCHAR(20)` | Nein | Waehlt `all`, `exact` oder `unassigned` fuer den Managerfilter. |
| `@TargetManager` | `NVARCHAR(80)` | Nein | Managername fuer `exact`; ausserhalb davon muss der Parameter `NULL` bleiben. |
| `@IncludeUnsafeDemo` | `BIT` | Nein | Zeigt bei `1` zusaetzlich die fehleranfaellige Vergleichsstrategie. |
...
- `CASE`
- `COALESCE`
- `ROW_NUMBER` fuer die stabile Ausgabe-ID
<!-- SQLDOC:DEPENDENCIES_LIST:END -->

## Hinweise

- `FilterParameterSnapshot` macht zuerst sichtbar, ob wirklich kein Managerfilter aktiv ist oder ob gezielt `exact` beziehungsweise `unassigned` gemeint ist.
- `unsafe-shortcut` demonstriert das Review-Antimuster `AccountManager = COALESCE(@TargetManager, AccountManager)`.
- `null-safe` benutzt stattdessen einen expliziten Modus und behandelt den `IS NULL`-Fall getrennt.
- Im Modus `all` wird sichtbar, dass nur die sichere Variante auch Portfolios ohne Manager korrekt im Resultset behaelt.
...
|---|---|---|---|
| `1.0` | `2026-04-17` | `ER` | Erstversion fuer null-sichere optionale Filter auf nullable Spalten |
...
```mermaid
flowchart TD
    A[Parameter pruefen] --> B[#CustomerPortfolio und #StrategyResults anlegen]
    B --> C[Demo-Portfolios mit zugewiesenem und NULL-AccountManager laden]
    C --> D[FilterParameterSnapshot fuer Modus und Parameter ausgeben]
    D --> E{IncludeUnsafeDemo = 1?}
    E -->|Ja| F[unsafe-shortcut mit COALESCE Shortcut auswerten]
    E -->|Nein| G[unsafe-shortcut ueberspringen]
    F --> H[null-safe Strategie mit all / exact / unassigned auswerten]
    G --> H
    H --> I[StrategyRows fuer beide Strategien ausgeben]
    I --> J[StrategyAssessment mit NULL-Anteil und ReviewConclusion aggregieren]
```
<!-- SQLDOC:MERMAID:END -->
