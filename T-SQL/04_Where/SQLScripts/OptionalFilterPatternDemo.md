# OptionalFilterPatternDemo.sql

Dieses Skript zeigt drei gaengige Muster fuer optionale Parameterfilter in `WHERE`-Klauseln. Auf derselben Demo-Datenbasis werden statisches `OR`, verzweigtes Branching und `OPTION (RECOMPILE)` gegenuebergestellt.
...
| Feld | Wert |
|---|---|
| Script | [OptionalFilterPatternDemo.sql](OptionalFilterPatternDemo.sql) |
| Version | `1.0` |
| Typ | `didactic-lab` |
| Kapitel | `04_Where` |
| Sicherheit | `read-only-tempdb` |
| Zweck | Vergleicht optionale Filtermuster und ihre didaktischen Planwirkungen. |
<!-- SQLDOC:SUMMARY_TABLE:END -->

## Einordnung

Optionale Filter wirken in UIs und Reports bequem, fuehren in SQL aber schnell zu Zielkonflikten. Ein einziges Statement mit vielen `OR`-Bedingungen ist kompakt, kann aber unguenstige Query-Shapes produzieren. Branching verbessert oft die Praedikate, erhoeht jedoch den Codeumfang. `OPTION (RECOMPILE)` liegt dazwischen: wenig Sondercode, aber zusaetzliche Kompilierung pro Lauf.

## Annahmen

- Das Skript arbeitet ausschliesslich mit einer kleinen tempdb-nahen Demo-Tabelle und setzt keine produktiven Tabellen voraus.
- Planwirkungen werden didaktisch beschrieben und nicht ueber echte Ausfuehrungsplaene oder IO-Metriken gemessen.
- Die Eingaben `DE`, `AT` und `CH` genuegen als kompakter DACH-Rahmen fuer optionale Selektivitaet.
- `@PriorityOnly` nutzt bewusst ein einfaches Bool-Muster, damit der Unterschied zwischen deaktiviertem und aktivem Filter klar sichtbar bleibt.

## Anwendungsfall

Das Lab eignet sich fuer Query-Reviews, Schulungen und Refactorings von Suchmasken. Besonders hilfreich ist es, wenn Reports mit vielen optionalen Parametern entweder zu generisch formuliert wurden oder zu stark in Spezialpfade zerfallen.
...
|---|---|---|---|
| `@RegionCode` | `CHAR(2)` | Nein | Optionaler Regionsfilter; `NULL` deaktiviert den Regionsteil. |
| `@MinNetAmount` | `DECIMAL(10,2)` | Nein | Optionale Untergrenze fuer den Nettobetrag; `NULL` deaktiviert den Range-Filter. |
| `@PriorityOnly` | `BIT` | Nein | `1` beschraenkt die Ergebnisse auf High-Priority-Auftraege. |
| `@Mode` | `VARCHAR(20)` | Nein | Filtert `all`, `static-or`, `branching` oder `recompile`. |
...
- `CASE`
- `OPTION (RECOMPILE)`
- `ROW_NUMBER` fuer eine stabile Ausgabe-ID
<!-- SQLDOC:DEPENDENCIES_LIST:END -->

## Hinweise

- `FilterParameterSnapshot` macht zuerst sichtbar, welche optionalen Filter im aktuellen Lauf aktiv sind.
- `StrategyRows` zeigt dann dieselbe Treffermenge je Strategie, damit der Unterschied im Query-Muster und nicht in den Daten liegt.
- `static-or` steht fuer den kompakten Standardansatz mit deaktivierbaren Praedikaten.
- `branching` zeigt, wie eigene Codepfade deaktivierte Filter ganz aus der Bedingung entfernen koennen.
- `recompile` benutzt dieselbe Filterlogik wie `static-or`, markiert aber den Fall, in dem eine frische Kompilierung pro Lauf sinnvoll sein kann.
...
|---|---|---|---|
| `1.0` | `2026-04-17` | `ER` | Erstversion fuer didaktische Muster zu optionalen Parameterfiltern |
...
```mermaid
flowchart TD
    A[Parameter pruefen] --> B[#SalesOrders und #StrategyResults anlegen]
    B --> C[Demo-Auftragsdaten fuer DE, AT und CH laden]
    C --> D[FilterParameterSnapshot fuer aktive und inaktive Filter ausgeben]
    D --> E{Mode enthaelt static-or?}
    E -->|Ja| F[static-or Ergebnis ueber deaktivierbare OR-Praedikate einfuegen]
    E -->|Nein| G[static-or ueberspringen]
    F --> H{Mode enthaelt branching?}
    G --> H
    H -->|Ja| I[passenden Branching-Pfad waehlen und Ergebnis einfuegen]
    H -->|Nein| J[branching ueberspringen]
    I --> K{Mode enthaelt recompile?}
    J --> K
    K -->|Ja| L[recompile Ergebnis mit OPTION RECOMPILE einfuegen]
    K -->|Nein| M[recompile ueberspringen]
    L --> N[StrategyRows mit allen gesammelten Treffern ausgeben]
    M --> N
    N --> O[StrategyAssessment je Strategie aggregieren und didaktisch bewerten]
```
<!-- SQLDOC:MERMAID:END -->
