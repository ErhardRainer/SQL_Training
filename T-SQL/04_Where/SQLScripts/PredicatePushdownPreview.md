# PredicatePushdownPreview.sql

Dieses Skript zeigt an einer kleinen Bestellmenge, welche Filter direkt auf Basiszeilen wirken und welche Bedingungen erst nach einer Aggregation sinnvoll bewertet werden koennen. Der Fokus liegt nicht auf echten Ausfuehrungsplaenen, sondern auf einem gut sichtbaren Datenfluss durch fruehe und spaete Filterstufen.
...
| Feld | Wert |
|---|---|
| Script | [PredicatePushdownPreview.sql](PredicatePushdownPreview.sql) |
| Version | `1.0` |
| Typ | `didactic-lab` |
| Kapitel | `04_Where` |
| Sicherheit | `read-only-tempdb` |
| Zweck | Macht fruehe Basiszeilen-Filter und spaete Aggregat-Filter im Datenfluss sichtbar. |
<!-- SQLDOC:SUMMARY_TABLE:END -->

## Einordnung

Im SQL-Alltag wird oft gesagt, ein Filter sei "frueh" oder "spaet", ohne dass klar wird, woran das fachlich haengt. Das Lab trennt deshalb bewusst zwei Ebenen: Filter auf Basisattributen wie Region, Datum oder Einzelbetrag koennen sofort auf einzelne Zeilen wirken, waehrend ein Kriterium wie `SUM(OrderAmount)` erst nach `GROUP BY` ueberhaupt bekannt ist.

## Annahmen

- Das Skript ist eine didaktische Erstversion und arbeitet ausschliesslich mit tempdb-nahen Demo-Daten.
- Predicate Pushdown wird hier ueber nachvollziehbare Stufen im Query-Aufbau illustriert und nicht ueber einen konkreten SQL-Server-Ausfuehrungsplan bewiesen.
- `@MinCustomerRevenue` ist absichtlich als spaeter Aggregatfilter modelliert, damit der Unterschied zu den fruehen WHERE-geeigneten Bedingungen klar bleibt.
- Das Lab zeigt einen Datenfluss fuer Reviews und Schulungen, nicht ein allgemeingueltiges Performance-Benchmark.

## Anwendungsfall

Das Skript eignet sich fuer Schulungen und Query-Reviews, wenn Teams erklaeren muessen, warum manche Suchkriterien bereits auf Zeilenebene wirken und andere erst nach einer Verdichtung. Besonders hilfreich ist es bei Diskussionen ueber `WHERE` versus `HAVING` oder ueber die Lesbarkeit mehrstufiger Filterlogik in Reports.
...
|---|---|---|---|
| `@RegionCode` | `CHAR(2)` | Nein | Optionaler frueher Regionsfilter fuer `DE`, `AT` oder `CH`. |
| `@MinOrderDate` | `DATE` | Nein | Optionaler frueher Filter auf das Bestelldatum. |
| `@MinOrderAmount` | `DECIMAL(10,2)` | Nein | Optionaler frueher Filter auf den einzelnen Auftragswert. |
| `@MinCustomerRevenue` | `DECIMAL(10,2)` | Nein | Optionaler spaeter Filter auf aggregierten Umsatz je Kunde. |
...
- `CASE` fuer die sichtbare Einordnung der Filterergebnisse
- `GROUP BY` fuer den Kundenumsatz nach fruehen Filtern
- `HAVING` fuer die spaete Filterung auf Aggregatwerten
<!-- SQLDOC:DEPENDENCIES_LIST:END -->

## Hinweise

- `FilterPreview` ordnet jeden aktiven Filter als `early` oder `late` ein und erklaert kurz den Grund.
- `CandidateFlow` zeigt pro Auftrag, ob er schon an einer Basisbedingung ausscheidet oder erst am spaeten Umsatzfilter scheitert.
- `StageSummary` verdichtet, wie viele Zeilen und Kunden nach jeder Filterstufe noch uebrig bleiben.
- Das Skript bleibt absichtlich klein, damit die Wirkung der vier Filter ohne Zusatzkomplexitaet nachvollziehbar bleibt.
...
|---|---|---|---|
| `1.0` | `2026-04-17` | `ER` | Erstversion fuer ein didaktisches Lab zu fruehen und spaeten Filterstufen |
...
```mermaid
flowchart TD
    A[Parameter pruefen] --> B[#SalesOrders und #FilterPreview anlegen]
    B --> C[Demo-Bestellungen laden und Filterkatalog fuellen]
    C --> D[BaseOrders als Eingangsstufe lesen]
    D --> E[#EarlyEvaluation berechnet Region, Datum und Einzelbetrag als fruehe Filter]
    E --> F[#EarlyFiltered laesst nur Basiszeilen mit allen fruehen Treffern weiter]
    F --> G[#CustomerRevenue aggregiert Umsatz und Anzahl je Kunde]
    G --> H[#LateQualifiedCustomers prueft den spaeten Umsatzfilter mit HAVING]
    H --> I[#CandidateFlow markiert je Auftrag den Ausstieg oder das Durchkommen]
    I --> J[FilterPreview ausgeben]
    J --> K[CandidateFlow ausgeben]
    K --> L[StageSummary fuer Basis, nach fruehen Filtern und nach spaetem Filter ausgeben]
```
<!-- SQLDOC:MERMAID:END -->
