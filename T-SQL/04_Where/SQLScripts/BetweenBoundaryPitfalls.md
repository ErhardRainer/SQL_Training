# BetweenBoundaryPitfalls.sql

Dieses Skript zeigt typische Stolperstellen rund um `BETWEEN`: inklusive Grenzen bei Zahlen, Off-by-one-Effekte an der Obergrenze und das verbreitete Missverstaendnis, dass ein fachliches Enddatum bei `DATETIME` automatisch den ganzen letzten Tag abdeckt.
...
| Feld | Wert |
|---|---|
| Script | [BetweenBoundaryPitfalls.sql](BetweenBoundaryPitfalls.sql) |
| Version | `1.0` |
| Typ | `didactic-lab` |
| Kapitel | `04_Where` |
| Sicherheit | `read-only-tempdb` |
| Zweck | Macht inklusive Grenzen, Datumsenden und Off-by-one-Faelle bei `BETWEEN` sichtbar. |
<!-- SQLDOC:SUMMARY_TABLE:END -->

## Einordnung

`BETWEEN` ist kompakt, aber nicht neutral: Die Schreibweise ist immer beidseitig inklusiv. Genau daraus entstehen in Reviews zwei typische Fehlerbilder. Erstens wird `< Obergrenze` versehentlich als gleichwertig zu `BETWEEN` gelesen. Zweitens wird bei `DATETIME`-Spalten ein fachliches Enddatum haeufig als kompletter letzter Kalendertag verstanden, obwohl `BETWEEN Start AND Enddatum` technisch nur bis Mitternacht des Endtags reicht.

## Annahmen

- Das Skript ist eine didaktische Erstversion und arbeitet nur mit tempdb-basierten Demo-Daten.
- Die numerische Demo verwendet kleine Ganzzahlen, damit Grenzwerte und Off-by-one-Faelle direkt lesbar bleiben.
- Die Datumsdemo behandelt `@WindowEndDate` fachlich als ganzen Kalendertag und kontrastiert deshalb ein naives `BETWEEN`-Muster mit einem halb-offenen Fenster.
- Der Fokus liegt auf der Bedeutungsseite der Praedikate, nicht auf Ausfuehrungsplaenen oder Performance-Messungen.

## Anwendungsfall

Das Lab eignet sich fuer Schulung, Code-Review und Refactoring von Bereichsfiltern. Es ist besonders nuetzlich, wenn numerische Grenzlogik oder Tagesfenster in Reports, Suchmasken oder Audit-Abfragen erklaert und gegen Randfehler abgesichert werden sollen.
...
|---|---|---|---|
| `@ScoreLowerBound` | `INT` | Ja | Untere inklusive Grenze fuer die numerische Bereichsdemo. |
| `@ScoreUpperBound` | `INT` | Ja | Obere inklusive Grenze fuer die numerische Bereichsdemo. |
| `@WindowStartDate` | `DATE` | Ja | Fachlicher Starttag fuer die Datumsdemo. |
| `@WindowEndDate` | `DATE` | Ja | Fachlicher Endtag fuer die Datumsdemo. |
...
- `BETWEEN` fuer die inklusive Bereichsschreibweise
- `DATEADD` fuer das exklusive Ende des halb-offenen Tagesfensters
- `CAST` fuer die saubere Ableitung von `DATETIME2`-Grenzen aus `DATE`
- `CASE` fuer Grenzrollen und didaktische Diagnosen
- `DATETIME2` fuer sichtbare Uhrzeitanteile am Enddatum
<!-- SQLDOC:DEPENDENCIES_LIST:END -->

## Hinweise

- `NumericBoundaryPitfalls` zeigt direkt, dass `BETWEEN` und `>=` plus `<=` logisch gleichwertig sind, waehrend `< Obergrenze` den oberen Grenzwert verliert.
- `DatetimeBoundaryPitfalls` macht sichtbar, dass `BETWEEN` auf `DATE`-Grenzen nur Mitternacht des Endtags einschliesst, nicht aber spaetere Uhrzeiten desselben Tages.
- `BoundaryTeachingNotes` verdichtet die Kernbotschaften fuer Unterricht, Reviews oder kurze Team-Notizen.
- Das ausgegebene Parameterfenster zeigt bewusst sowohl das naive Ende um Mitternacht als auch das robuste exklusive Ende am Folgetag.
...
|---|---|---|---|
| `1.0` | `2026-04-22` | `ER` | Erstversion fuer BETWEEN-Grenzfaelle und Off-by-one-Pitfalls im WHERE-Kapitel |
...
```mermaid
flowchart TD
    A[Parameter und Datumsgrenzen vorbereiten] --> B[Guardrails fuer Grenzen und ResultMode pruefen]
    B --> C[#NumericSamples und #DatetimeSamples anlegen]
    C --> D[Numerische Demo-Werte knapp unter auf und ueber den Grenzen laden]
    D --> E[Datetime-Demo fuer Start Enddatum und Endtags-Uhrzeiten laden]
    E --> F[Parameterfenster mit Mitternachtsende und exklusivem Folgetag ausgeben]
    F --> G[NumericEvaluation fuer BETWEEN inclusive und < Obergrenze berechnen]
    G --> H[NumericBoundaryPitfalls mit Off-by-one-Diagnose ausgeben]
    H --> I[DatetimeEvaluation fuer BETWEEN und halb-offenes Fenster berechnen]
    I --> J[DatetimeBoundaryPitfalls mit Endtags-Pitfalls ausgeben]
    J --> K[BoundaryTeachingNotes als kompakte Lernzusammenfassung ausgeben]
```
<!-- SQLDOC:MERMAID:END -->
