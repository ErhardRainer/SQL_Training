# WhereBooleanPrecedenceLab.sql

Dieses Skript zeigt an einer kleinen Demo-Menge, wie SQL Server gemischte `AND`/`OR`-Filter ohne Klammern interpretiert. Der Fokus liegt darauf, die Standardpraezedenz sichtbar zu machen und die Treffermenge einer enger geklammerten Variante direkt daneben zu vergleichen.
...
| Feld | Wert |
|---|---|
| Script | [WhereBooleanPrecedenceLab.sql](WhereBooleanPrecedenceLab.sql) |
| Version | `1.0` |
| Typ | `didactic-lab` |
| Kapitel | `04_Where` |
| Sicherheit | `read-only-tempdb` |
| Zweck | Demonstriert die Wirkung von `AND`/`OR`-Praezedenz und den Unterschied zwischen Standardbindung und enger Klammerung. |
<!-- SQLDOC:SUMMARY_TABLE:END -->

## Einordnung

`AND` bindet in SQL staerker als `OR`. Viele Filterketten sehen beim schnellen Lesen so aus, als ob ein linker Pflichtfilter fuer die ganze Bedingung gelten wuerde, obwohl ungeklammerte `OR`-Teile in Wahrheit wieder aus diesem Scope ausbrechen koennen.

## Annahmen

- Das Skript ist eine didaktische Erstversion und nutzt ausschliesslich eine Temp-Tabelle mit Demo-Leads.
- `Unparenthesized` und `GroupedDefault` sind absichtlich logisch identisch, damit die echte SQL-Standardbindung sichtbar wird.
- `RegionScoped` zeigt die haeufig gemeinte Geschaeftslesart, bei der der Regionsfilter fuer beide rechten Alternativen gelten soll.
- `@VipOverride = 1` aktiviert bewusst einen OR-Zweig, der VIP-Leads ausserhalb der Region in die Treffermenge ziehen kann.

## Anwendungsfall

Das Lab eignet sich fuer den Einstieg in boolesche Logik in `WHERE`-Klauseln, fuer Code-Reviews mit gemischten Filtern und fuer Schulungsfolien, in denen eine kleine Aenderung an den Klammern sofort als andere Treffermenge sichtbar werden soll.
...
|---|---|---|---|
| `@RegionCode` | `CHAR(2)` | Nein | Region, die als linker Pflichtfilter in den Vergleichspraedikaten dient. |
| `@PriorityStage` | `VARCHAR(20)` | Nein | Stage-Wert fuer den linken `AND`-Zweig. |
| `@VipOverride` | `BIT` | Nein | `1` aktiviert den VIP-Override im rechten `OR`-Zweig. |
...
- `CASE` fuer die explizite Abbildung der Wahrheitswerte
- `ROW_NUMBER` fuer eine stabile Reihenfolge innerhalb jeder Filterform
- `STRING_AGG` fuer die kompakte Lead-Signatur im Vergleich der Treffermengen
<!-- SQLDOC:DEPENDENCIES_LIST:END -->

## Hinweise

- `PredicateCatalog` zeigt die ungeklammerte Form, die explizite Standardbindung und die enger geklammerte Region-Variante nebeneinander.
- `LeadEvaluation` macht pro Zeile sichtbar, welche atomaren Wahrheitswerte (`RegionMatches`, `StageMatches`, `VipMatches`) vorliegen.
- `PredicateSummary` verdichtet die resultierenden Treffermengen und zeigt, ob eine Variante dieselbe Signatur wie die Standardbindung liefert.
- Der didaktische Kern liegt nicht auf Performance, sondern auf Lesart und logischer Auswirkung kleiner Klammerunterschiede.
...
|---|---|---|---|
| `1.0` | `2026-04-19` | `ER` | Erstversion fuer ein didaktisches Lab zur AND/OR-Praezedenz in `WHERE`-Ketten |
...
```mermaid
flowchart TD
    A[Parameter validieren] --> B[#LeadPipeline, #PredicateCatalog und #LeadEvaluation anlegen]
    B --> C[Demo-Leads mit Region, Stage und VIP-Flag laden]
    C --> D[PredicateCatalog mit drei Filterformen fuellen]
    D --> E[Atomare Wahrheitswerte fuer Region, Stage und VIP je Lead berechnen]
    E --> F[Unparenthesized und GroupedDefault als Standardbindung auswerten]
    F --> G[RegionScoped mit enger Klammerung auswerten]
    G --> H[Alle Bewertungen in #LeadEvaluation mit MatchRank schreiben]
    H --> I[PredicateCatalog ausgeben]
    I --> J[LeadEvaluation mit Wahrheitswerten und Trefferstatus ausgeben]
    J --> K[PredicateSummary mit MatchCount, VIP-Overrides und Lead-Signatur bilden]
```
<!-- SQLDOC:MERMAID:END -->
