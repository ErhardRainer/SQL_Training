# WherePredicateShortList.sql

Dieses Skript sammelt mehrere kurze und gut lesbare `WHERE`-Praedikatmuster auf derselben kleinen Demo-Menge. So lassen sich kompakte Equality-, Range-, `LIKE`-, `IN`-, `IS NULL`- und Datumsfilter direkt nebeneinander lesen und vergleichen.
...
| Feld | Wert |
|---|---|
| Script | [WherePredicateShortList.sql](WherePredicateShortList.sql) |
| Version | `1.0` |
| Typ | `didactic-lab` |
| Kapitel | `04_Where` |
| Sicherheit | `read-only-tempdb` |
| Zweck | Liefert eine kompakte Sammlung kurzer und gut lesbarer WHERE-Praedikatmuster. |
<!-- SQLDOC:SUMMARY_TABLE:END -->

## Einordnung

Das Skript ist kein Benchmark, sondern eine Leseliste mit lauffaehigen Mini-Beispielen. Der Fokus liegt auf einfachen Praedikaten, die in Schulungen und Reviews schnell wiedererkennbar sind und ohne zusaetzliche Branches oder komplexe Dynamik auskommen.

## Annahmen

- Das Skript ist eine didaktische Erstversion und nutzt ausschliesslich Demo-Daten in einer Temp-Tabelle.
- `@StatusCsv` wird ueber `STRING_SPLIT` in eine kleine Statusliste fuer das `IN`-Muster ueberfuehrt.
- Der Monatsfilter wird absichtlich als halb-offene Datumsrange formuliert statt ueber eine Funktion direkt auf der Spalte.
- Wenn `@RequireUnassigned = 0` gesetzt ist, zeigt das NULL-Muster bewusst alle Zeilen, damit der Schaltereffekt sichtbar bleibt.

## Anwendungsfall

Das Lab eignet sich fuer den Einstieg in `WHERE`-Klauseln, fuer Review-Checklisten und fuer Schulungsfolien mit kleinen, gut lesbaren Mustern. Es zeigt, wie typische Filterformen knapp formuliert werden koennen, ohne sofort in optionale Branching- oder Performance-Sonderfaelle abzudriften.
...
|---|---|---|---|
| `@RegionCode` | `CHAR(2)` | Nein | Optionaler Equality-Filter fuer `DE`, `AT` oder `CH`. |
| `@MinNetAmount` | `DECIMAL(10,2)` | Nein | Optionaler Range-Filter fuer einen Mindestumsatz. |
| `@NameStartsWith` | `NVARCHAR(20)` | Nein | Optionales Praefix fuer ein `LIKE 'wert%'`-Muster. |
| `@StatusCsv` | `VARCHAR(100)` | Nein | Kommagetrennte Statusliste fuer das `IN`-Muster. |
| `@RequireUnassigned` | `BIT` | Nein | `1` zeigt nur Zeilen ohne `AccountManager`. |
| `@OrderMonth` | `TINYINT` | Nein | Optionaler Monatsfilter fuer eine halb-offene Datumsrange. |
...
- `STRING_SPLIT` fuer die Aufbereitung der Statusliste
- `DATEFROMPARTS` und `EOMONTH` fuer den halb-offenen Monatsfilter
- `ROW_NUMBER` fuer eine stabile Trefferreihenfolge je Praedikat
- `STRING_AGG` fuer die kompakte Signatur im Summary-Resultset
<!-- SQLDOC:DEPENDENCIES_LIST:END -->

## Hinweise

- `PredicateShortList` zeigt jedes Muster zusammen mit einer kurzen Beispielschreibweise und dem aktiven Vergleichswert.
- `PredicateMatches` listet die Treffer pro Muster separat, damit der Zusammenhang zwischen Praedikat und Resultset direkt sichtbar bleibt.
- `PredicateSummary` verdichtet Trefferzahl, Umsatzsumme und Order-Signatur pro Praedikat.
- Das Skript fuehrt keine persistenten Aenderungen aus und bleibt auf lesbare Mini-Muster beschraenkt.
...
|---|---|---|---|
| `1.0` | `2026-04-17` | `ER` | Erstversion fuer eine kompakte Sammlung lesbarer WHERE-Praedikatmuster |
...
```mermaid
flowchart TD
    A[Parameter validieren] --> B[#Orders, #StatusFilter, #PredicateCatalog und #PredicateMatches anlegen]
    B --> C[Demo-Auftraege in #Orders laden]
    C --> D{Enthaelt @StatusCsv Werte?}
    D -->|ja| E[Statuswerte ueber STRING_SPLIT nach #StatusFilter laden]
    D -->|nein| F[Leere Statusliste beibehalten]
    E --> G[PredicateCatalog mit Equality, Range, LIKE, IN, NULL und Date-Range fuellen]
    F --> G
    G --> H[Sechs kurze Praedikatabfragen per UNION ALL zu PredicateMatches zusammenfuehren]
    H --> I[PredicateShortList ausgeben]
    I --> J[PredicateMatches je Praedikat sortiert ausgeben]
    J --> K[PredicateSummary mit MatchCount, Umsatz und Order-Signatur bilden]
```
<!-- SQLDOC:MERMAID:END -->
