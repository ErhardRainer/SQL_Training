# TransactionRollbackAudit.sql

Dieses Skript zeigt an kleinen Fehlerszenarien, wie sich ein `COMMIT` oder `ROLLBACK` auf Nutzdaten und Audit-Spuren auswirkt. Der Fokus liegt auf der praktischen Frage, welche Informationen innerhalb der Transaktion verloren gehen und welche Details erst nach dem Rollback belastbar protokolliert werden koennen.
...
| Feld | Wert |
|---|---|
| Script | [TransactionRollbackAudit.sql](TransactionRollbackAudit.sql) |
| Version | `1.0` |
| Typ | `didactic-lab` |
| Kapitel | `25_ErrorHandling_TryCatch` |
| Sicherheit | `read-only-tempdb` |
| Zweck | Zeigt Rollback-Pfade und moegliche Audit-Infos nach Fehlern in Transaktionen. |
<!-- SQLDOC:SUMMARY_TABLE:END -->

## Einordnung

Das Lab trennt bewusst zwischen transaktionsinternem Audit und einem Post-Mortem-Audit nach dem `ROLLBACK`. Dadurch wird sichtbar, dass eine Logzeile im selben Transaktionskontext fuer Fehleranalysen oft nicht ausreicht, weil sie zusammen mit den Nutzdaten zurueckgerollt werden kann.

## Annahmen

- Das Skript ist eine didaktische Erstversion und arbeitet ausschliesslich mit `tempdb`-Objekten.
- Audit-Eintraege in `#TxnAuditScratch` werden absichtlich innerhalb derselben Transaktion geschrieben wie die Demo-Daten.
- Das belastbare Audit in `#AuditTrail` entsteht erst nach `COMMIT` oder nach dem `ROLLBACK` im `CATCH`.
- `duplicate-key` simuliert einen technischen Fehler ueber einen vorhandenen Primaerschluessel; `validation-stop` simuliert einen fachlichen Abbruch ueber `THROW`.

## Anwendungsfall

Das Skript eignet sich fuer Schulungen, Reviews und Design-Gespraeche zu Fehlerbehandlung in Stored Procedures, ETL-Ablaufen oder Service-Operationen. Besonders nuetzlich ist es, wenn Teams entscheiden muessen, welche Audit-Informationen innerhalb der Transaktion bleiben duerfen und welche Daten im Fehlerfall zwingend ausserhalb der Transaktion gesichert werden sollten.
...
|---|---|---|---|
| `@ScenarioFilter` | `VARCHAR(30)` | Nein | Waehlt `success`, `duplicate-key`, `validation-stop` oder `all`. |
...
- `THROW`
- `XACT_STATE()`
- Primaerschluessel-Constraint fuer den simulierten Duplicate-Key-Fehler
<!-- SQLDOC:DEPENDENCIES_LIST:END -->

## Hinweise

- `TransactionAuditLog` zeigt pro Szenario zuerst die Baseline und danach entweder den Commit- oder Rollback-Pfad.
- `TxnAuditRowsVisible` macht sichtbar, ob transaktionsinterne Audit-Spuren nach dem Lauf noch vorhanden sind.
- `ScenarioSummary` verdichtet, ob die Kandidatenzeile nach dem Lauf sichtbar blieb und ob ein Rollback stattfand.
- `SET XACT_ABORT ON` haelt das Lab streng und sorgt fuer klar erkennbare Rollback-Pfade.
...
|---|---|---|---|
| `1.0` | `2026-04-17` | `ER` | Erstversion fuer ein Lab zu Rollback-Pfaden und Post-Mortem-Audit |
...
```mermaid
flowchart TD
    A[Parameter pruefen und Temp-Tabellen anlegen] --> B[Szenariokatalog und Seed-Daten laden]
    B --> C[Je Szenario Baseline-Audit ausserhalb der Transaktion schreiben]
    C --> D[Transaktion starten]
    D --> E[TxnAuditScratch vor der Aenderung fuellen]
    E --> F[WorkQueue-Zeile einfuegen]
    F --> G[TxnAuditScratch nach der Aenderung fuellen]
    G --> H{Fehlerart?}
    H -->|none| I[COMMIT]
    H -->|duplicate-key| J[CATCH mit Constraint-Fehler]
    H -->|validation-stop| J
    I --> K[AuditTrail nach COMMIT schreiben]
    J --> L[XACT_STATE und Error-Daten sichern]
    L --> M[ROLLBACK ausfuehren]
    M --> N[AuditTrail nach ROLLBACK ausserhalb der Transaktion schreiben]
    K --> O[TransactionAuditLog ausgeben]
    N --> O
    O --> P[ScenarioSummary mit Sichtbarkeit von Daten und Txn-Audit ableiten]
```
<!-- SQLDOC:MERMAID:END -->
