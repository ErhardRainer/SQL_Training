# ValidationErrorThrowTemplate.sql

Dieses Skript zeigt ein kleines, didaktisches Muster fuer fachliche Validierungen mit `THROW`. Ein vorbereitetes Demo-Payload wird gegen einfache Fachregeln geprueft; bei Fehlern liefert der `CATCH`-Block Diagnoseinformationen und kann den Fehler optional unveraendert erneut werfen.
...
| Feld | Wert |
|---|---|
| Script | [ValidationErrorThrowTemplate.sql](ValidationErrorThrowTemplate.sql) |
| Version | `1.0` |
| Typ | `didactic-lab` |
| Kapitel | `25_ErrorHandling_TryCatch` |
| Sicherheit | `read-only-tempdb` |
| Zweck | Demonstriert fachliche Guard Clauses mit eigenen Fehlernummern und sauberem `THROW`. |
<!-- SQLDOC:SUMMARY_TABLE:END -->

## Einordnung

Im Mittelpunkt stehen kurze Guard Clauses mit stabilen Fehlernummern ab `51010`. Dadurch bleibt die fachliche Regel direkt am Pruefpunkt lesbar, waehrend der `CATCH`-Block nur Diagnose und optionales Rethrow uebernimmt. Das passt gut zu Service-, API- oder Stored-Procedure-Templates, bei denen Business-Validierung klar von technischen Fehlern getrennt bleiben soll.

## Annahmen

- Das Skript ist eine didaktische Erstversion mit Demo-Daten in `tempdb`.
- Das Kreditlimit pro Kunde ist bewusst klein und statisch, damit der Validierungsfall nachvollziehbar bleibt.
- Fehlernummern zwischen `51010` und `51016` repraesentieren hier ausschliesslich fachliche Validierungsregeln.
- Ein leeres `RequestedBy` wird als fachlicher Fehler behandelt, nicht als technischer Laufzeitfehler.

## Anwendungsfall

Das Template eignet sich als Ausgangspunkt fuer Prozeduren oder Freigabeflows, in denen Pflichtfelder, Statuspruefungen und einfache Grenzwerte frueh und explizit validiert werden sollen. Fuer Demo-Zwecke kann ueber `@Scenario` gezielt zwischen gueltigen und ungueltigen Payloads gewechselt werden.
...
|---|---|---|---|
| `@Scenario` | `VARCHAR(30)` | Nein | Bereitet eines von mehreren Demo-Szenarien wie `valid` oder `unsupported-currency` vor. |
| `@RequestedAmount` | `DECIMAL(12,2)` | Nein | Ueberschreibt optional den vorbereiteten Beispielbetrag. |
| `@ReThrowInCatch` | `BIT` | Nein | Wirft bei `1` den gefangenen Fehler nach der Diagnose erneut. |
...
- `ScenarioInput` wird immer zuerst ausgegeben, damit der vorbereitete Testfall transparent bleibt.
- `ApprovedPayload` erscheint nur, wenn alle Guard Clauses erfolgreich durchlaufen werden.
- Im Fehlerfall liefert `CatchDiagnostics` eine kompakte Trennung zwischen `business-validation` und `unexpected`.
- Mit `@ReThrowInCatch = 1` eignet sich das Skript auch als Minimalbeispiel fuer reines Logging plus anschliessendes Rethrow.
...
|---|---|---|---|
| `1.0` | `2026-04-17` | `ER` | Erstversion fuer fachliche Validierungen mit sauberem `THROW` |
...
```mermaid
flowchart TD
    A[Parameter pruefen] --> B[Demo-Kataloge und Auftrags-Payload in tempdb aufbauen]
    B --> C[ScenarioInput ausgeben]
    C --> D[Payload normalisieren und Guard Clauses starten]
    D --> E{Pflichtfelder, Kunde, Waehrung, Limit gueltig?}
    E -->|Ja| F[ApprovedPayload mit Freigabehinweis ausgeben]
    E -->|Nein| G[Business-THROW mit Fehlernummer 51010 bis 51016]
    G --> H[CATCH liest ERROR_* Metadaten]
    H --> I{ReThrowInCatch = 1?}
    I -->|Nein| J[CatchDiagnostics ausgeben]
    I -->|Ja| K[THROW ohne Parameter fuer sauberes Rethrow]
```
<!-- SQLDOC:MERMAID:END -->
