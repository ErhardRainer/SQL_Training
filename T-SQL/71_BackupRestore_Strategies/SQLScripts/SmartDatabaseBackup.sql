...
-- Ab hier: alle Ausgaenge muessen bei @WhatIf = 0 ueber die CATCH-Bloecke oder das Laufende
-- der Batch-Ausfuehrung den Lock wieder freigeben - siehe Schritt 10.
...
BEGIN TRY
    ...
    -- Datenbankgroesse in MB (Summe aller Datendateien, ohne Log) fuer die Mindestgroessen-Regel.
    SELECT @DatabaseSizeMB = SUM(CAST(mf.size AS BIGINT) * 8.0 / 1024)
    FROM sys.master_files AS mf
    WHERE mf.database_id = DB_ID(@TargetDatabaseName) AND mf.type = 0; -- 0 = ROWS (Datendateien)

    -- Anteil geaenderter Extents seit dem letzten Full-Backup (Naeherung ueber die
    -- Differential-Changed-Map) fuer die Differential->Full-Eskalationsentscheidung.
    -- dm_db_file_space_usage liefert die Werte je Datenbankdatei; hier ueber alle
    -- Datendateien der Zieldatenbank aggregiert per dynamischem SQL (DB-Kontext-Wechsel
    -- ist fuer diese DMF nicht per Cross-Database-Query moeglich).
    ...
    END TRY
    BEGIN CATCH
        -- Aeltere SQL-Server-Versionen ohne modified_extent_page_count oder Berechtigungsprobleme:
        -- Eskalationslogik faellt dann auf "nicht ermittelbar" zurueck und wird uebersprungen.
    ...
    -- Log-Aufkommen seit dem letzten Log-Backup in MB (nur relevant bei FULL/BULK_LOGGED).
    ...
    BEGIN
        -- Im SIMPLE Model ist nur Full oder Differential moeglich (kein Log-Backup).
    ...
    ELSE
    BEGIN
        -- FULL oder BULK_LOGGED: Differential vor Log pruefen.
    ...
    -- 7a. Wurde grundsaetzlich Differential ausgewaehlt: Mindestgroessen-Regel und
    --     Aenderungsanteil-Eskalation pruefen, bevor der Typ endgueltig festgelegt wird.
