# BackupChainIntegrityCheck.sql

Dieses Skript prüft für eine oder alle Datenbanken, ob die **Log-Backup-Kette (LSN-Kette)** seit dem letzten Full-/Differential-Backup lückenlos ist — also ob jedes Log-Backup nahtlos an die `last_lsn` des Vorgängers anschließt. Eine Lücke bedeutet, dass ein **Point-in-Time-Restore über diese Stelle hinweg nicht möglich ist**, selbst wenn alle Backup-Dateien physisch vorhanden sind.
...
| Feld | Wert |
|---|---|
| Script | [BackupChainIntegrityCheck.sql](BackupChainIntegrityCheck.sql) |
| Version | `1.0` |
| Typ | `diagnostic` |
| Kapitel | `71_BackupRestore_Strategies` |
| Sicherheit | `read-only` |
| Zweck | Prüft die LSN-Kette der Log-Backups auf Lücken und optional die physische Existenz der Backup-Dateien. |
<!-- SQLDOC:SUMMARY_TABLE:END -->

## Einordnung

Dieses Skript ist die notwendige **Vorprüfung** zu [RestoreDatabaseFromLatestBackup.sql](RestoreDatabaseFromLatestBackup.md): Bevor eine automatisch ermittelte Restore-Kette tatsächlich verwendet wird, sollte geprüft werden, ob sie überhaupt lückenlos ist. Die fachliche Grundlage (was eine LSN-Kette ist und warum Lücken PITR verhindern) ist in [71_BackupRestore_Strategies.md](../71_BackupRestore_Strategies.md) Abschnitt 4.2 beschrieben; die Backup-Historie-Abfrage als Ausgangsbasis findet sich zusätzlich in [SuspectOrRecoveryPendingDatabase_RepairOptions.md](../SuspectOrRecoveryPendingDatabase_RepairOptions.md) Abschnitt 5.

## Annahmen

- Eine LSN-Lücke bedeutet **nicht zwingend Datenverlust** im Sinne einer beschädigten Datenbank — sie bedeutet, dass ein Point-in-Time-Restore über die Lücke hinweg nicht möglich ist. Typische Ursachen: ein manuell gelöschtes/übersehenes Log-Backup, ein Recovery-Model-Wechsel zu `SIMPLE` und zurück, oder ein fehlgeschlagener Log-Backup-Job.
- Ohne `@TargetDatabaseName` werden nur Datenbanken im Recovery Model `FULL`/`BULK_LOGGED` mit mindestens einem Full-Backup geprüft — im `SIMPLE`-Modell gibt es keine Log-Kette.
- Die physische Dateiexistenzprüfung (`sys.dm_os_file_exists`) funktioniert nur für Pfade, die vom SQL-Server-Dienstkonto erreichbar sind — bei fehlenden Berechtigungen oder unerreichbaren Netzlaufwerken wird korrekt "Nicht prüfbar" statt fälschlich "fehlt" ausgewiesen.

## Anwendungsfall

Regelmäßige Kontrolle (z. B. wöchentlich), ob die Backup-Ketten aller produktiven Datenbanken tatsächlich für einen Point-in-Time-Restore nutzbar sind — Lücken bleiben sonst oft unbemerkt, bis sie im Ernstfall den Restore verhindern.
...
|---|---|---|---|
| `@TargetDatabaseName` | `SYSNAME` | Nein | Name einer einzelnen zu prüfenden Datenbank; `NULL` (Default) prüft alle Datenbanken mit Recovery Model `FULL`/`BULK_LOGGED`, die mindestens ein Full-Backup besitzen. |
| `@CheckPhysicalFileExistence` | `BIT` | Nein | `1` (Default) = prüft zusätzlich, ob die referenzierten Backup-Dateien physisch existieren; `0` = nur LSN-Kettenprüfung. |
...
- `BackupChainIntegrityCheck` zeigt je Log-Backup in der Kette die erwartete und tatsächliche `first_lsn`, den Kettenstatus (`OK`/`LUECKE`) und optional den Dateistatus.
- `BackupChainSummary` fasst je Datenbank zusammen: Anzahl der Lücken, Kettenbeginn/-ende, und eine Klartext-Einordnung (`KEINE KETTE` / `LUECKENHAFT` / `INTAKT`), sortiert nach Kritikalität.
...
|---|---|---|---|
| `1.0` | `2026-08-14` | `ER` | Erstversion: LSN-Ketten-Prüfung für Log-Backups seit dem letzten Full/Differential je Datenbank, mit optionaler physischer Dateiexistenzprüfung |
...
```mermaid
flowchart TD
    A[Parameter validieren] --> B[Zu pruefende Datenbanken ermitteln]
    B --> C[Letztes Full/Diff-Backup je DB als Kettenbasis]
    C --> D[Alle Log-Backups seit der Basis ermitteln]
    D --> E[Je Log-Backup: first_lsn mit last_lsn\ndes Vorgaengers vergleichen]
    E --> F{Physische Pruefung aktiviert?}
    F -->|Ja| G[sys.dm_os_file_exists je Backup-Datei]
    F -->|Nein| H[Detailergebnis ausgeben]
    G --> H
    H --> I[Zusammenfassung je Datenbank ausgeben]
```
<!-- SQLDOC:MERMAID:END -->
