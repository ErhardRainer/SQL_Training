# MergeMatchPriorityDemo.sql

Dieses Skript demonstriert ein didaktisches `MERGE`, bei dem die eigentliche Match-Reihenfolge vorab ueber Kandidatenprofile aufgeloest wird. Exakte `CustomerCode`-Treffer erhalten Vorrang vor Legacy-Fallbacks, und zusaetzliche `WHEN MATCHED`-Bedingungen steuern, welche Fallback-Treffer wirklich aktualisiert werden duerfen.
...
| Feld | Wert |
|---|---|
| Script | [MergeMatchPriorityDemo.sql](MergeMatchPriorityDemo.sql) |
| Version | `1.0` |
| Typ | `didactic-lab` |
| Kapitel | `13_Merge` |
| Sicherheit | `demo-write-tempdb` |
| Zweck | Zeigt, wie Match-Prioritaet und Zusatzbedingungen Exact-, Fallback- und Insert-Pfade in einem MERGE steuern. |
<!-- SQLDOC:SUMMARY_TABLE:END -->

## Annahmen

- Die Erstversion arbeitet ausschliesslich mit temporaeren Demo-Tabellen statt mit produktiven Ziel- oder Staging-Tabellen.
- Exakte `CustomerCode`-Treffer werden vor Legacy-Fallbacks priorisiert, damit konkurrierende Kandidaten nicht direkt im `MERGE` aufgeloest werden muessen.
- Fallback-Updates bleiben ohne Aktion, wenn `AllowFallbackUpdate = 0` oder der Zielsatz bereits auf `manual-review` steht.

## Anwendungsfall

Das Skript eignet sich fuer folgende Leitfragen:

- Wie laesst sich eine Match-Reihenfolge fuer `MERGE` vorbereiten, wenn mehrere Identifier unterschiedlich vertrauenswuerdig sind?
- Welche Zusatzbedingungen sollten Fallback-Updates trotz vorhandenem Match bewusst blockieren?
- Wie lassen sich Exact-Matches, Fallback-Matches, Insert-Kandidaten und reine Review-Faelle in getrennten Resultsets nachvollziehen?
...
- `MergeMatchCandidates` zeigt alle moeglichen Match-Kandidaten mit Prioritaet und erkannter Mehrdeutigkeit.
- `MergeMatchResolution` verdichtet pro Source-Zeile, ob der spaetere Pfad `ExactMatch`, `FallbackMatch`, `InsertCandidate` oder `Review` ist.
- `MergeMatchActionSummary` stellt die tatsaechlich ausgefuehrten `MERGE`-Aktionen den Match-Prioritaeten gegenueber.
- `MergeMatchAudit` erklaert pro Source-Zeile, ob ein Exact-Update, ein erlaubter Fallback, ein Insert oder bewusst keine Aktion erfolgt ist.
...
|---|---|---|---|
| `1.0` | `2026-04-18` | `ER` | Erstversion eines didaktischen MERGE-Labors fuer Match-Priorisierung und Zusatzbedingungen |
...
```mermaid
flowchart TD
    A[Temp Tabellen fuer Target Source Kandidaten Audit anlegen] --> B[Demo-Ziel- und Quellbestand befuellen]
    B --> C[MatchCandidates fuer ExactCustomerCode und LegacyCodeSameChannel ableiten]
    C --> D[RankedCandidates per MatchPriority und ROW_NUMBER ordnen]
    D --> E[ResolvedSource als ExactMatch FallbackMatch InsertCandidate oder Review aufbauen]
    E --> F[MatchCandidates Resultset ausgeben]
    E --> G[MergeResolution Resultset ausgeben]
    G --> H[MERGE auf TargetAccountId ausfuehren]
    H --> I[WHEN MATCHED ExactMatch aktualisiert direkt]
    H --> J[WHEN MATCHED FallbackMatch aktualisiert nur bei AllowFallbackUpdate und ohne manual-review]
    H --> K[WHEN NOT MATCHED BY TARGET fuegt InsertCandidate als neue Zielzeile ein]
    I --> L[OUTPUT schreibt Aktion und Vorher-Nachher-Werte in #MergeAudit]
    J --> L
    K --> L
    L --> M[ActionSummary nach Aktion und MatchPriority ausgeben]
    L --> N[Audit mit DecisionExplanation ausgeben]
    L --> O[Finalen Zielbestand nach dem MERGE ausgeben]
```
<!-- SQLDOC:MERMAID:END -->
