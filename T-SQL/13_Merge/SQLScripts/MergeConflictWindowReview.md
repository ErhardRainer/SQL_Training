# MergeConflictWindowReview.sql

Dieses Skript simuliert parallele `MERGE`-Laeufe als Session-Fenster und macht sichtbar, wo sich auf demselben Business Key konkurrierende Match-, Write- oder Delete/Insert-Phasen zeitlich ueberschneiden. Die Umsetzung bleibt auf temporaere Demo-Daten beschraenkt und dient damit als Review-Werkzeug fuer Konfliktrisiken, bevor echte Parallelisierungsmuster in produktionsnahe Prozesse uebernommen werden.
...
| Feld | Wert |
|---|---|
| Script | [MergeConflictWindowReview.sql](MergeConflictWindowReview.sql) |
| Version | `1.0` |
| Typ | `didactic-lab` |
| Kapitel | `13_Merge` |
| Sicherheit | `read-only-tempdb` |
| Zweck | Bewertet zeitlich ueberlappende MERGE-Fenster pro Business Key und leitet Review-Hinweise ab. |
<!-- SQLDOC:SUMMARY_TABLE:END -->

## Annahmen

- Die Erstversion arbeitet mit simulierten Session-Fenstern statt mit echten Sperr- oder Trace-Datenbanken.
- Konfliktfenster werden pro `CustomerCode` und Zeitueberlappung heuristisch als `High`, `Medium` oder `Low` bewertet.
- Das Skript fuehrt kein echtes `MERGE` aus, sondern bereitet Review-Entscheidungen fuer parallele `MERGE`-Designs vor.

## Anwendungsfall

Das Skript eignet sich fuer folgende Leitfragen:

- Welche Session-Paare greifen im selben Zeitfenster auf denselben Business Key zu?
- Wo konkurrieren `UPDATE`, `DELETE` oder spaete `INSERT`-Aktionen miteinander?
- Welche didaktischen Gegenmassnahmen sind fuer `HOLDLOCK`, Batch-Segmentierung oder materialisierte Quellmengen naheliegend?
...
- `MergeConflictWindowSummary` verdichtet die Zahl und Dauer kritischer Ueberlappungen pro `CustomerCode`.
- `MergeConflictWindowPairs` zeigt die beteiligten Sessions, Phasen, Aktionen und eine direkte Review-Interpretation.
- `MergeConflictWindowMitigations` leitet aus Konfliktmuster und Risiko passende Gegenmassnahmen fuer Review und Schulung ab.
...
|---|---|---|---|
| `1.0` | `2026-04-18` | `ER` | Erstversion eines didaktischen Konfliktfenster-Reviews fuer parallele MERGE-Laeufe |
...
```mermaid
flowchart TD
    A[Temp Tabellen fuer Sessions und Fenster anlegen] --> B[Demo-Sessions mit Isolation und HOLDLOCK-Merkmalen befuellen]
    B --> C[Konfliktfenster je Session Business Key und Phase laden]
    C --> D{Doppelte oder ungueltige Fenster?}
    D -->|Ja| E[THROW und Review abbrechen]
    D -->|Nein| F[WindowPairs per Self Join fuer ueberlappende Zeitfenster bilden]
    F --> G[ConflictPairs mit Muster Risiko und Ueberlappungsdauer bewerten]
    G --> H[Summary pro CustomerCode ausgeben]
    G --> I[Detail-Paare mit Review-Interpretation ausgeben]
