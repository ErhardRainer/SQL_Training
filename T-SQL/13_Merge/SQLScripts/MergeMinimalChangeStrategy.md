# MergeMinimalChangeStrategy.sql

Dieses Skript zeigt ein didaktisches `MERGE`-Muster, das breite Update-Wellen vermeidet. Vor dem eigentlichen `MERGE` werden Quell- und Zielbestand verglichen, unveraenderte Business Keys bewusst uebersprungen und nur echte Delta-Zeilen in ein Stage fuer `UPDATE` oder `INSERT` uebernommen.
...
| Feld | Wert |
|---|---|
| Script | [MergeMinimalChangeStrategy.sql](MergeMinimalChangeStrategy.sql) |
| Version | `1.0` |
| Typ | `didactic-lab` |
| Kapitel | `13_Merge` |
| Sicherheit | `demo-write-tempdb` |
| Zweck | Filtert No-Change-Zeilen vor einem MERGE aus und uebernimmt nur echte Deltas in das Update- oder Insert-Stage. |
<!-- SQLDOC:SUMMARY_TABLE:END -->

## Annahmen

- Die Erstversion arbeitet ausschliesslich mit temporaeren Demo-Tabellen statt mit produktiven Fachtabellen.
- `TargetOnly`-Faelle werden sichtbar gemacht, aber bewusst nicht geloescht, damit der Fokus auf minimalen `UPDATE`- und `INSERT`-Aenderungen bleibt.
- Die Delta-Ermittlung betrachtet fuenf exemplarische Fachspalten und dokumentiert geaenderte Spalten komprimiert ueber `STRING_AGG`.

## Anwendungsfall

Das Skript eignet sich fuer folgende Leitfragen:

- Wie lassen sich unveraenderte Business Keys bereits vor dem `MERGE` aussortieren?
- Welche Zeilen sollten wirklich aktualisiert werden, und welche koennen unveraendert bestehen bleiben?
- Wie kann ein schlankes Delta-Stage aufgebaut werden, damit ein `MERGE` nicht unnoetig breite Updates ausloest?
...
- `MergeMinimalChangeSummary` zeigt, wie viele Business Keys vor dem `MERGE` als `NoChange`, `UpdateNeeded`, `InsertNeeded` oder `TargetOnly` klassifiziert wurden.
- `MergeMinimalChangeCandidates` macht sichtbar, welche Spalten pro Business Key tatsaechlich abweichen.
- `MergeMinimalChangeAudit` protokolliert nur die Aktionen, die aus dem Delta-Stage wirklich im `MERGE` angekommen sind.
...
|---|---|---|---|
| `1.0` | `2026-04-18` | `ER` | Erstversion eines didaktischen Minimal-Change-MERGE-Musters |
...
```mermaid
flowchart TD
    A[Temp Tabellen fuer Target Source DeltaStage und Audit anlegen] --> B[Demo-Zielbestand und Quellbestand befuellen]
    B --> C{Doppelte CustomerCode Werte in Quelle oder Ziel?}
    C -->|Ja| D[THROW und Verarbeitung abbrechen]
    C -->|Nein| E[Baseline per FULL OUTER JOIN ueber CustomerCode bilden]
    E --> F[Abweichende Spalten ueber CROSS APPLY und Vergleich der Fachspalten ableiten]
    F --> G[CandidateStatus mit NoChange UpdateNeeded InsertNeeded und TargetOnly verdichten]
    G --> H[Nur UpdateNeeded und InsertNeeded in #MergeDeltaStage uebernehmen]
    H --> I[Summary und Kandidatenliste fuer das Delta ausgeben]
    I --> J[MERGE mit HOLDLOCK nur gegen das Delta-Stage ausfuehren]
    J --> K[OUTPUT schreibt echte INSERT- und UPDATE-Aktionen in #MergeAudit]
    K --> L[Auditdetails ausgeben]
    K --> M[Finalen Zielbestand nach dem Minimal-Change-MERGE ausgeben]
```
<!-- SQLDOC:MERMAID:END -->
