# MergeDuplicateSourceCheck.sql

Dieses Skript prueft eine didaktische `MERGE`-Quellmenge vor dem eigentlichen `MERGE` auf doppelte Business Keys in der Quelle sowie auf mehrdeutige Zieltreffer. Die Demo bleibt auf temporaere Tabellen beschraenkt und eignet sich damit fuer Preflight-Reviews, bevor eine Quellmenge in ein produktionsnahes `MERGE` uebernommen wird.
...
| Feld | Wert |
|---|---|
| Script | [MergeDuplicateSourceCheck.sql](MergeDuplicateSourceCheck.sql) |
| Version | `1.0` |
| Typ | `didactic-lab` |
| Kapitel | `13_Merge` |
| Sicherheit | `read-only-tempdb` |
| Zweck | Prueft Quellmengen vor MERGE auf Dubletten, mehrdeutige Zieltreffer und Preflight-Entscheidungen. |
<!-- SQLDOC:SUMMARY_TABLE:END -->

## Annahmen

- Die Erstversion arbeitet mit temporaeren Demo-Tabellen statt mit echten Staging- oder Zieltabellen.
- Mehrfachtreffer werden sowohl fuer doppelte `CustomerCode`-Werte in der Quelle als auch fuer nicht eindeutige Zielzeilen betrachtet.
- Das Skript fuehrt kein `MERGE` aus, sondern bewertet nur, ob ein Business Key vor einem `MERGE` blockiert, gewaehlt oder direkt weiterverarbeitet werden sollte.

## Anwendungsfall

Das Skript eignet sich fuer folgende Leitfragen:

- Welche Business Keys erscheinen mehrfach in der Quellmenge und muessen vor dem `MERGE` bereinigt werden?
- Wo bietet der Zielbestand mehr als eine Zeile fuer denselben Join-Key und wuerde dadurch ein mehrdeutiger Treffer entstehen?
- Fuer welche Keys ist nur ein `INSERT` zu erwarten und fuer welche Keys ist die Match-Beziehung bereits eindeutig genug fuer den naechsten Schritt?
...
- `MergePreflightSummary` verdichtet, wie viele Business Keys im aktuellen Demo-Batch als `Block`, `Warn`, `Proceed` oder `Info` eingestuft werden.
- `MergeSourceDuplicates` zeigt den Zeilenkontext fuer doppelte Source-Keys direkt an.
- `MergeMatchMultiplicity` macht sichtbar, wie viele Quell- und Zielzeilen pro Business Key vorliegen.
- `MergePreflightAssessment` leitet pro Business Key eine direkte Handlungsempfehlung vor dem eigentlichen `MERGE` ab.
...
|---|---|---|---|
| `1.0` | `2026-04-18` | `ER` | Erstversion eines didaktischen Preflight-Checks fuer MERGE-Quellmengen |
...
```mermaid
flowchart TD
    A[Temp Tabellen fuer Target und Source anlegen] --> B[Demo-Ziel- und Quellbestand befuellen]
    B --> C[SourceKeyProfile und TargetKeyProfile je CustomerCode aggregieren]
    C --> D[MatchProfile ueber Full Outer Join der Key-Profile bilden]
    D --> E[PreflightAssessment mit Block Warn Proceed und Info ableiten]
    E --> F[Summary der Entscheidungen ausgeben]
    E --> G[Doppelte Source-Zeilen per DuplicateSourceRows mit ROW_NUMBER anzeigen]
    E --> H[MatchMultiplicity pro Business Key mit Source- und Target-Treffern ausgeben]
    E --> I[Empfehlung pro Business Key fuer Bereinigung INSERT-Pfad oder MERGE-Freigabe ausgeben]
```
<!-- SQLDOC:MERMAID:END -->
