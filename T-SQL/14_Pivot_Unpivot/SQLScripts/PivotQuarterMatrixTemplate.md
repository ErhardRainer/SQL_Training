# PivotQuarterMatrixTemplate.sql

Dieses Skript liefert eine didaktische Vorlage fuer Quartalsmatrizen mit `PIVOT`. Die Demo kombiniert eine feste Quartals-Whitelist mit einer verdichteten Demo-Quelle, damit eine wiederverwendbare Matrix pro Portfolio und Kennzahlengruppe entsteht.
...
| Feld | Wert |
|---|---|
| Script | [PivotQuarterMatrixTemplate.sql](PivotQuarterMatrixTemplate.sql) |
| Version | `1.0` |
| Typ | `template` |
| Kapitel | `14_Pivot_Unpivot` |
| Sicherheit | `read-only-tempdb` |
| Zweck | Vorlage fuer Quartalsmatrizen mit stabiler Q1-bis-Q4-Achse und optionaler Gesamtsumme. |
<!-- SQLDOC:SUMMARY_TABLE:END -->

## Annahmen

- Die Erstversion verwendet ausschliesslich temporaere Demo-Tabellen und keine produktiven Portfolio- oder Reporting-Objekte.
- Die Quartalsachse bleibt ueber eine Whitelist bewusst stabil bei `Q1` bis `Q4`, auch wenn einzelne Kombinationen in einem Quartal keine Daten besitzen.
- Fehlende Quartalswerte werden fuer die Matrix absichtlich als `0.00` sichtbar gemacht, damit das Template auch fuer sparse Ausgangsdaten nachvollziehbar bleibt.

## Anwendungsfall

Die Vorlage eignet sich fuer folgende Leitfragen:

- Wie wird eine feste Quartalsachse aufgebaut, bevor das eigentliche `PIVOT` ausgefuehrt wird?
- Wo sollte eine optionale Gesamtsumme ueber alle Quartalsspalten im Ablauf eines Pivot-Templates sitzen?
- Wie laesst sich eine Quartalsmatrix so aufbauen, dass auch unbelegte Quartale in der Ausgabe sichtbar bleiben?
...
|---|---|---|---|
| `@ReportYear` | `int` | nein | Filtert die Demo-Quelle auf ein Berichtsjahr fuer die Quartalsmatrix. |
| `@IncludeQuarterTotal` | `bit` | nein | Erweitert die Matrix um eine berechnete `TotalQuarterAmount`-Spalte, wenn der Wert `1` ist. |
...
- `QuarterBucketPreview` zeigt die fuer das Template freigegebenen Quartalsspalten inklusive sicherem Alias.
- `QuarterAggregationPreview` verdichtet die Demo-Quelle bereits vor dem dynamischen Pivot auf Portfolio-, Kennzahlengruppen- und Quartalsebene.
- Das dynamische SQL verdichtet zuerst die Zeilenachse gegen alle Quartale und erzeugt dadurch auch fuer nicht belegte Quartale stabile `0.00`-Spalten.
...
|---|---|---|---|
| `1.0` | `2026-04-18` | `ER` | Erstversion einer didaktischen Quartalsmatrix-Vorlage mit stabilem Quarter-Axis-Template. |
...
```mermaid
flowchart TD
    A[Parameter und Temp Tabellen vorbereiten] --> B[Demo Quelle und Quartals Whitelist befuellen]
    B --> C{ReportYear sowie Bit Parameter gueltig und Quelldaten vorhanden?}
    C -->|Nein| D[THROW und Skript abbrechen]
    C -->|Ja| E[Freigegebene Quartals Buckets Q1 bis Q4 mit QUOTENAME vorbereiten]
    E --> F{Mindestens ein Quarter Bucket vorhanden?}
    F -->|Nein| G[THROW wegen leerer Quartals Whitelist]
    F -->|Ja| H[Quartalsaggregation pro Portfolio und MeasureGroup berechnen]
    H --> I[STRING_AGG baut Pivot IN Liste Projektion und optionalen Total Ausdruck]
    I --> J[Dynamic SQL bildet Row Axis plus Cross Join auf alle Quartale]
    J --> K[Quellvorschau Bucket Liste Aggregation und SQL Preview ausgeben]
    K --> L{ExecutePivot = 1?}
    L -->|Ja| M[Quartalsmatrix per sp_executesql ausfuehren]
