# PivotCalendarMatrix.sql

Dieses Skript baut eine didaktische Kalender-Matrix mit dynamischem `PIVOT`. Aus einem kleinen Demo-Worklog werden wahlweise Monats- oder ISO-Wochen-Buckets abgeleitet und anschliessend als Matrix pro Team und Aktivitaet ausgegeben.
...
| Feld | Wert |
|---|---|
| Script | [PivotCalendarMatrix.sql](PivotCalendarMatrix.sql) |
| Version | `1.0` |
| Typ | `didactic-lab` |
| Kapitel | `14_Pivot_Unpivot` |
| Sicherheit | `read-only-tempdb` |
| Zweck | Monats- oder ISO-Wochen-Matrix aus Demo-Arbeitsdaten per Pivot aufbauen. |
<!-- SQLDOC:SUMMARY_TABLE:END -->

## Annahmen

- Die Erstversion verwendet ausschliesslich temporaere Demo-Tabellen und keine produktiven Kalender- oder Reporting-Objekte.
- Die Zeilenachse besteht aus `TeamName` und `ActivityName`, waehrend die Kalenderspalten dynamisch aus dem gefilterten Datumsbereich entstehen.
- Die Wochenansicht nutzt `DATEPART(ISO_WEEK, ...)` und beschriftet die Buckets als `YYYY-Www`.

## Anwendungsfall

Die Demo eignet sich fuer folgende Leitfragen:

- Wie werden Kalenderspalten erst aus Datumswerten abgeleitet und danach dynamisch gepivotet?
- Wo sitzt die Entscheidung zwischen Monats- und Wochenansicht in einem wiederverwendbaren Muster?
- Wie laesst sich dieselbe Quellmenge fuer Vorschau, Bucket-Liste und Matrix wiederverwenden?
...
|---|---|---|---|
| `@CalendarMode` | `varchar(10)` | nein | Steuert die Kalendersicht mit den Werten `Month` oder `Week`. |
| `@StartDate` | `date` | nein | Untergrenze fuer die Demo-Daten, die in die Kalender-Matrix eingehen. |
...
- `CalendarSourcePreview` zeigt den gefilterten Demo-Worklog vor der Kalenderverschneidung.
- `CalendarBucketPreview` dokumentiert die wirklich verwendeten Monats- oder Wochen-Buckets samt Sortierschluessel.
- `CalendarMatrix` liefert die verdichtete Matrix mit Stunden je Team, Aktivitaet und Kalender-Bucket.
...
|---|---|---|---|
| `1.0` | `2026-04-18` | `ER` | Erstversion einer Kalender-Matrix fuer Monats- und ISO-Wochen-Pivots. |
...
```mermaid
flowchart TD
    A[Parameter und Temp Tabellen vorbereiten] --> B[Demo Worklog befuellen]
    B --> C{CalendarMode gueltig und Daten ab StartDate vorhanden?}
    C -->|Nein| D[THROW und Skript abbrechen]
    C -->|Ja| E[Monats oder ISO-Wochen Buckets aus TaskDate ableiten]
    E --> F[Bucket Labels mit QUOTENAME fuer Pivot freigeben]
    F --> G{Mindestens ein Kalender Bucket vorhanden?}
    G -->|Nein| H[THROW wegen leerer Bucket Liste]
    G -->|Ja| I[STRING_AGG baut dynamische IN Liste und Projektion]
    I --> J[Quellvorschau Bucket Liste und SQL Preview ausgeben]
    J --> K{ExecutePivot = 1?}
    K -->|Ja| L[Kalender Matrix per sp_executesql ausfuehren]
