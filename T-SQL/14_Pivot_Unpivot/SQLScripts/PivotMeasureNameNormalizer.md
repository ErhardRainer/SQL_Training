# PivotMeasureNameNormalizer.sql

Dieses Skript normalisiert uneinheitliche Kennzahlennamen fuer spaetere Pivot-Layouts. Die Demo zeigt, wie mehrere Rohschreibweisen auf stabile kanonische Kennzahlen und freigegebene Pivot-Spalten abgebildet werden, ohne problematische oder ungepruefte Rohwerte stillschweigend in die Matrix zu uebernehmen.
...
| Feld | Wert |
|---|---|
| Script | [PivotMeasureNameNormalizer.sql](PivotMeasureNameNormalizer.sql) |
| Version | `1.0` |
| Typ | `didactic-lab` |
| Kapitel | `14_Pivot_Unpivot` |
| Sicherheit | `read-only-tempdb` |
| Zweck | Normalisiert Kennzahlennamen und erzeugt eine pivot-taugliche Quellmenge mit stabilen Spaltennamen. |
<!-- SQLDOC:SUMMARY_TABLE:END -->

## Annahmen

- Die Erstversion verwendet ausschliesslich temporaere Demo-Tabellen und keine produktiven Reporting-Feeds.
- Mehrere Rohschreibweisen wie `Gross Sales`, `gross_sales` und `Gross-Sales` duerfen bewusst auf dieselbe Pivot-Spalte zusammenlaufen.
- Nicht freigegebene oder ungemappte Kennzahlen bleiben im Review sichtbar und werden standardmaessig nicht in die pivot-taugliche Quellmenge uebernommen.

## Anwendungsfall

Das Muster eignet sich fuer Leitfragen wie:

- Wie werden uneinheitliche Feed-Namen vor einem `PIVOT` in stabile Kennzahlenschluessel ueberfuehrt?
- Welche Rohwerte koennen direkt freigegeben werden und welche muessen erst fachlich geklaert werden?
- Wann ist ein technischer Fallback-Alias hilfreich und wann sollte ein Wert lieber als `unmapped` sichtbar bleiben?
...
|---|---|---|---|
| `@ReportMonth` | `char(7)` | nein | Filtert die Demo-Quelle auf einen Berichtsmonat im Format `YYYY-MM`. |
| `@AllowFallbackAlias` | `bit` | nein | Erlaubt fuer nicht gemappte Rohwerte einen technischen Fallback-Alias, wenn der Wert `1` ist. |
...
- `NormalizationRules` trennt fachlich freigegebene Regeln von bewusst blockierten Kennzahlen wie `Promo Credit`.
- `NormalizedMeasureReview` zeigt fuer jeden Rohwert, ob ein freigegebener Match, ein blockierter Match oder ein ungemappter Fall vorliegt.
- `PivotReadySource` enthaelt nur die Zeilen, die mit stabilem Kennzahlnamen und belastbarer Pivot-Spalte weiterverarbeitet werden duerfen.
...
|---|---|---|---|
| `1.0` | `2026-04-18` | `ER` | Erstversion eines didaktischen Normalisierers fuer Pivot-Kennzahlennamen. |
...
```mermaid
flowchart TD
    A[Parameter und Temp Tabellen vorbereiten] --> B[Demo-Feed und Normalisierungsregeln befuellen]
    B --> C{ReportMonth gueltig und Fallback-Flag 0 oder 1?}
    C -->|Nein| D[THROW und Skript abbrechen]
    C -->|Ja| E{Teilen genehmigte Regeln eine Pivot-Spalte fuer verschiedene Canonicals?}
    E -->|Ja| F[THROW wegen inkonsistenter Regelbasis]
    E -->|Nein| G[SourcePrepared erzeugt bereinigte RawMeasureKey Werte]
    G --> H[RulePrepared und MatchedMeasures verbinden Rohwerte mit Regeln]
    H --> I[RawMeasurePreview und NormalizationRules ausgeben]
    I --> J[NormalizedMeasureReview mit approved not-approved fallback oder unmapped Status ausgeben]
    J --> K[ApprovedColumns und PivotReadySource fuer freigegebene Pivot-Spalten ableiten]
    K --> L[PivotReadyColumnList und PivotReadySource fuer spaetere Pivot-Layouts ausgeben]
```
<!-- SQLDOC:MERMAID:END -->
