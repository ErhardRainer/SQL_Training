# PivotFallbackCasePattern.sql

Dieses Skript zeigt ein CASE-basiertes Fallback-Muster fuer Crosstab-Ausgaben im Kapitel `14_Pivot_Unpivot`. Die Demo-Matrix mischt bewusst mehrere Filter- und Aggregationsregeln, damit sichtbar wird, wann conditional aggregation lesbarer und stabiler ist als ein reines `PIVOT`.
...
| Feld | Wert |
|---|---|
| Script | [PivotFallbackCasePattern.sql](PivotFallbackCasePattern.sql) |
| Version | `1.0` |
| Typ | `didactic-lab` |
| Kapitel | `14_Pivot_Unpivot` |
| Sicherheit | `read-only-tempdb` |
| Zweck | CASE-basiertes Fallback fuer Crosstab-Ausgaben mit gemischten Aggregationsregeln. |
<!-- SQLDOC:SUMMARY_TABLE:END -->

## Annahmen

- Die Erstversion verwendet ausschliesslich temporaere Demo-Tabellen und keine produktiven Reporting-Objekte.
- Die Zielmatrix kombiniert absichtlich `SUM`, `COUNT` und `MAX`, damit der Mehrwert eines CASE-Fallbacks gegenueber einem einfachen `PIVOT` klar wird.
- Forecast-Zeilen koennen ueber einen Parameter ausgeblendet werden, ohne die Spaltenstruktur der Ergebnismatrix zu aendern.

## Anwendungsfall

Das Muster eignet sich fuer Leitfragen wie:

- Wann ist `PIVOT` unhandlich, weil unterschiedliche Zielspalten unterschiedliche Regeln brauchen?
- Wie lassen sich Kennzahlen und Kontrollspalten in einer einzigen, stabilen Matrix je Region kombinieren?
- Wo hilft conditional aggregation dabei, die Logik explizit und wartbar zu halten?
...
|---|---|---|---|
| `@ReportYear` | `int` | nein | Filtert die Demo-Quelle auf ein Berichtsjahr fuer die Ausgabematrix. |
| `@IncludeForecastRows` | `bit` | nein | Beruecksichtigt Forecast-Zeilen in Vorschau und Matrix, wenn der Wert `1` ist. |
...
- `CaseSourcePreview` zeigt die gefilterte Demo-Quelle in derselben Logik, die spaeter fuer die Matrix verwendet wird.
- `CasePatternMapping` macht die Zielspalten und ihre jeweiligen Fachregeln transparent.
- `PivotLimitationReview` verdichtet pro Region, warum ein einheitliches `PIVOT`-Aggregat hier unpassend waere.
...
|---|---|---|---|
| `1.0` | `2026-04-18` | `ER` | Erstversion eines CASE-basierten Fallback-Musters fuer gemischte Crosstab-Regeln. |
...
```mermaid
flowchart TD
    A[Parameter und Temp Tabellen vorbereiten] --> B[Demo-Quelle und CASE-Mapping befuellen]
    B --> C{ReportYear gesetzt und IncludeForecastRows gueltig?}
    C -->|Nein| D[THROW und Skript abbrechen]
    C -->|Ja| E{Doppelte OutputColumnName im Mapping?}
    E -->|Ja| F[THROW wegen inkonsistentem CASE-Mapping]
    E -->|Nein| G[FilteredSource per Jahr und Forecast-Flag bilden]
    G --> H[Quellvorschau ausgeben]
    H --> I[CASE-Mapping und Pivot-Limitation-Review ausgeben]
    I --> J[CASE-basierte Matrix mit SUM COUNT und MAX je Region berechnen]
    J --> K[CaseFallbackMatrix geordnet ausgeben]
```
<!-- SQLDOC:MERMAID:END -->
