# PivotSparseCategoryDemo.sql

Dieses Skript zeigt eine didaktische Sparse-Pivot-Demo fuer duenn besetzte Kategorien. Die Demo verdichtet pro Team und Queue gegen eine feste Kategorienachse, damit fehlende Pivot-Werte nicht verschwinden, sondern bewusst als `0.00` sichtbar bleiben.
...
| Feld | Wert |
|---|---|
| Script | [PivotSparseCategoryDemo.sql](PivotSparseCategoryDemo.sql) |
| Version | `1.0` |
| Typ | `didactic-lab` |
| Kapitel | `14_Pivot_Unpivot` |
| Sicherheit | `read-only-tempdb` |
| Zweck | Sparse Pivot mit fester Kategorienachse, Gap-Review und explizit als `0.00` sichtbaren Luecken. |
<!-- SQLDOC:SUMMARY_TABLE:END -->

## Annahmen

- Die Erstversion verwendet ausschliesslich temporaere Demo-Tabellen und keine produktiven Ticket- oder Service-Queues.
- Die Pivot-Achse bleibt ueber die freigegebenen Kategorien `Critical`, `High`, `Medium`, `Low` und `Deferred` bewusst stabil.
- Fehlende Kategorien werden pro Team und Queue absichtlich mit `0.00` verdichtet, damit sparse Quellmengen im Pivot nicht zu unsichtbaren Luecken werden.

## Anwendungsfall

Das Skript eignet sich fuer folgende Leitfragen:

- Wie wird eine feste Kategorienachse aufgebaut, bevor `PIVOT` auf sparse Quelldaten angewendet wird?
- Wie lassen sich fehlende Kategorien pro Zeilenachse sichtbar machen, statt sie stillschweigend aus der Matrix zu verlieren?
- Wann ist ein vorgeschalteter Coverage-Check sinnvoll, um nur Zeilen mit echten Kategorienluecken hervorzuheben?
...
|---|---|---|---|
| `@SnapshotMonth` | `tinyint` | nein | Filtert die Demo-Quelle auf einen Berichtsmonat fuer die Sparse-Pivot-Demo. |
| `@OnlyRowsWithGaps` | `bit` | nein | Beschraenkt die finale Matrix auf Team-Queue-Kombinationen mit mindestens einer unbelegten Kategorie. |
...
- `SparseCoverageReview` zeigt pro Team und Queue, wie viele freigegebene Kategorien in der Quellmenge fehlen oder bereits belegt sind.
- Die finale Matrix basiert nicht direkt auf der Rohquelle, sondern auf einer densifizierten Kombination aus Zeilenachse und Kategorienachse.
- Mit `@OnlyRowsWithGaps = 1` bleibt der Fokus auf den Kombinationen, bei denen ein klassisches Pivot sonst leicht irrefuehrende Leerstellen verdecken wuerde.
...
|---|---|---|---|
| `1.0` | `2026-04-18` | `ER` | Erstversion einer didaktischen Sparse-Pivot-Demo mit Kategorienachse und Gap-Review. |
...
```mermaid
flowchart TD
    A[Parameter und Temp Tabellen vorbereiten] --> B[Demo Quelle und Kategorienkatalog befuellen]
    B --> C{SnapshotMonth und Gap Schalter gueltig?}
    C -->|Nein| D[THROW und Skript abbrechen]
    C -->|Ja| E{Quelldaten fuer den Monat und freigegebene Kategorien vorhanden?}
    E -->|Nein| F[THROW wegen leerer Quelle oder Kategorienachse]
    E -->|Ja| G[Gefilterte Quelle und freigegebene Kategorien als Vorschau ausgeben]
    G --> H[Row Axis mit Team und Queue sowie Kategorienachse kreuzen]
    H --> I[Dense Coverage pro Kategorie mit COALESCE auf 0.00 verdichten]
    I --> J[SparseCoverageReview mit MissingCategoryCount berechnen]
    J --> K[EligibleRows optional auf Zeilen mit echten Gaps filtern]
    K --> L[DenseSource per PIVOT in feste Kategorienmatrix drehen]
    L --> M[Finale SparsePivotMatrix mit MissingCategoryCount ausgeben]
```
<!-- SQLDOC:MERMAID:END -->
