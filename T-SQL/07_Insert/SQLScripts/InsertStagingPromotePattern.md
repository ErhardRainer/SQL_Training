# InsertStagingPromotePattern.sql

Dieses Skript zeigt ein bewusst kleines Stage-to-target-Muster fuer `INSERT`: Erst werden Stage-Zeilen einer Charge auf fachliche und technische Promotability geprueft, danach werden nur die freigegebenen Datensaetze in die Zielstruktur uebernommen.
...
| Feld | Wert |
|---|---|
| Script | [InsertStagingPromotePattern.sql](InsertStagingPromotePattern.sql) |
| Version | `1.0` |
| Typ | `didactic-lab` |
| Kapitel | `07_Insert` |
| Sicherheit | `demo-write-tempdb` |
| Zweck | Promotet gepruefte Stage-Zeilen kontrolliert per `INSERT` in eine Zielstruktur und dokumentiert Ablehnungsgruende. |
<!-- SQLDOC:SUMMARY_TABLE:END -->

## Einordnung

Das Muster trennt bewusst zwischen Stage, Review und Ziel. Dadurch bleibt sichtbar, dass eine Stage-Tabelle nicht automatisch insert-reif ist: Vor dem eigentlichen Promote-Lauf werden Freigabestatus, Pflichtfelder, Duplikate gegen das Ziel und Duplikate innerhalb der Batch geprueft.

## Annahmen

- Die gesamte Demo arbeitet ausschliesslich in `tempdb`.
- `ValidationStatus = 'validated'` und `QualityGatePassed = 1` gelten hier als minimale Freigabe fuer eine Promotion.
- Die natuerliche Zielkombination besteht aus `LearnerCode`, `CourseCode` und `DeliveryMonth`.
- Bereits vorhandene Zielkombinationen und unvollstaendige Stage-Zeilen werden nicht nachgebessert, sondern nur als Ablehnungsfall markiert.

## Anwendungsfall

Das Skript passt zu Unterrichtssituationen rund um Staging, ETL-Vorbereitung und sichere `INSERT`-Prozesse. Lernende sehen, wie eine Charge zuerst in einer pruefbaren Zwischenschicht bewertet und erst danach selektiv in das Ziel uebernommen wird.
...
|---|---|---|---|
| `@BatchLabel` | `VARCHAR(20)` | Nein | Begrenzt die Promotion auf eine bestimmte Demo-Charge. |
| `@PromoteOnlyValidated` | `BIT` | Nein | Fuehrt bei `1` den eigentlichen Promote-Insert fuer freigegebene Zeilen aus. |
...
- Das erste Resultset `StageValidationReview` ist der Kern der Demo: Dort steht pro Stage-Zeile, ob sie `ready` oder `rejected` ist und warum.
- Die Promotion schreibt nur `ready`-Zeilen in das Ziel und legt parallel ein kleines Audit mit Stage- und Zielschluesseln an.
- Eine bereits vorhandene Zielzeile und mehrere absichtlich fehlerhafte Stage-Zeilen demonstrieren die typischen Blocker fuer ein kontrolliertes Promote-Muster.
...
|---|---|---|---|
| `1.0` | `2026-04-17` | `ER` | Erstversion des didaktischen Stage-to-target-Promote-Musters |
...
flowchart TD
    ...
    B --> C[Ziel Stage und Audit-Tabelle neu aufbauen]
    C --> D[Bestehende Zielzeile und Demo-Stage-Chargen laden]
    D --> E[Batch per CTE filtern und Review mit Ablehnungsgruenden erzeugen]
    E --> F[StageValidationReview ausgeben]
    F --> G{PromoteOnlyValidated = 1?}
    G -->|Nein| H[Promotion ueberspringen]
    G -->|Ja| I[Nur ready-Zeilen per INSERT in das Ziel promoten]
    I --> J[Promotion ueber OUTPUT in Audit protokollieren]
    H --> K[PromotionAudit und CurrentTargetRows ausgeben]
    J --> K[PromotionAudit und CurrentTargetRows ausgeben]
    K --> L{DropDemoObjects = 1?}
    L -->|Ja| M[Demo-Objekte in tempdb entfernen]
    L -->|Nein| N[Demo-Objekte fuer weitere Tests stehen lassen]
```
<!-- SQLDOC:MERMAID:END -->
