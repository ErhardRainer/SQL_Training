# UpsertInsertNotExists.sql

Dieses Skript zeigt ein bewusst konservatives Upsert-Muster fuer reine Insert-Strecken. Statt `MERGE` arbeitet die Demo mit einer vorgelagerten Kandidatenbewertung und einem `INSERT ... WHERE NOT EXISTS`, damit der Ablauf fuer Unterricht und Review klar nachvollziehbar bleibt.
...
| Feld | Wert |
|---|---|
| Script | [UpsertInsertNotExists.sql](UpsertInsertNotExists.sql) |
| Version | `1.0` |
| Typ | `didactic-lab` |
| Kapitel | `07_Insert` |
| Sicherheit | `demo-write-tempdb` |
| Zweck | Demonstriert ein robustes Insert-Muster mit `NOT EXISTS` als Alternative zu `MERGE`. |
<!-- SQLDOC:SUMMARY_TABLE:END -->

## Einordnung

Die Demo fokussiert auf den Fall, dass bestehende Zielzeilen unveraendert bleiben sollen und nur neue Schluessel eingefuegt werden. Dadurch entsteht ein klarer Lernpfad: zuerst Batch-Dubletten bereinigen, danach vorhandene Zielschluessel erkennen und erst dann die verbleibenden Kandidaten mit `NOT EXISTS` schreiben.

## Annahmen

- Alle Demo-Objekte liegen ausschliesslich in `tempdb`.
- `LearnerCode + CourseCode + TermCode` bilden den didaktischen Zielschluessel der Einschreibung.
- Bei Batch-Dubletten gewinnt die Zeile mit der kleineren `SourcePriority`; weitere Varianten werden bewusst verworfen statt gemischt.
- Die Variante mit `UPDLOCK` und `HOLDLOCK` dient der Einordnung eines robusteren Musters gegen Race Conditions, ohne echte Parallelitaet zu simulieren.

## Anwendungsfall

Das Skript eignet sich fuer Reviews von Lade- oder Integrationsstrecken, bei denen nur neue Zeilen eingefuegt werden sollen. Es zeigt, wie sich ein einfaches Upsert-Ziel auch ohne `MERGE` lesbar strukturieren laesst, wenn die Update-Komponente gar nicht benoetigt wird.
...
|---|---|---|---|
| `@ApplyInsert` | `BIT` | Nein | Fuehrt bei `1` den `NOT EXISTS`-Insert aus; bei `0` bleibt es bei der Kandidatenbewertung. |
| `@UseSerializableKeyProtection` | `BIT` | Nein | Verwendet bei `1` `UPDLOCK` und `HOLDLOCK` in der Zielpruefung. |
...
- `CandidateAssessment` zeigt, welche Batch-Zeilen schon im Ziel existieren, welche nur Batch-Dubletten sind und welche als echte Insert-Kandidaten uebrig bleiben.
- `InsertAudit` bleibt leer, wenn `@ApplyInsert = 0`, und dokumentiert sonst die tatsaechlich geschriebenen Zeilen.
- Das Pattern fuehrt absichtlich keine Updates aus; vorhandene Zielzeilen werden nur erkannt und ausgeschlossen.
...
|---|---|---|---|
| `1.0` | `2026-04-17` | `ER` | Erstversion des didaktischen `NOT EXISTS`-Upsert-Musters fuer Insert-Strecken |
...
flowchart TD
    ...
    B --> C[Demo-Zieltabelle Eingangsbatch und Audit-Tabellen anlegen]
    C --> D[Vorhandene Zielzeilen und Batch-Zeilen laden]
    D --> E[RankedBatch mit ROW_NUMBER fuer Batch-Dubletten berechnen]
    E --> F[CandidateAssessment mit Zieltreffern und Batch-Rang fuellen]
    F --> G{ApplyInsert = 1?}
    G -->|Nein| H[Nur Bewertungs-Resultsets ausgeben]
    G -->|Ja| I[Transaktion starten]
    I --> J{UseSerializableKeyProtection = 1?}
    J -->|Ja| K[Insert-Kandidaten mit NOT EXISTS plus UPDLOCK HOLDLOCK einfuegen]
    J -->|Nein| L[Insert-Kandidaten mit einfachem NOT EXISTS einfuegen]
    K --> M[InsertAudit per OUTPUT fuellen und WasInserted markieren]
    L --> M
    M --> N[Transaktion committen]
    N --> O[CandidateAssessment InsertAudit CurrentTargetRows und Summary ausgeben]
    H --> O
