# LagLeadChangeDetector.sql

Dieses Skript markiert Aenderungen in einer geordneten Snapshot-Folge je Auftrag. `LAG()` liefert die vorherigen Status-, Betrags- und Terminauspraegungen, waehrend `LEAD()` den Ausblick auf den naechsten Snapshot und dessen Abstand liefert.
...
| Feld | Wert |
|---|---|
| Script | [LagLeadChangeDetector.sql](LagLeadChangeDetector.sql) |
| Version | `1.0` |
| Typ | `didactic-lab` |
| Kapitel | `11_WindowFunctions` |
| Sicherheit | `read-only-tempdb` |
| Zweck | Markiert Status- und Wertewechsel je Auftrag ueber `LAG()` und `LEAD()`. |
<!-- SQLDOC:SUMMARY_TABLE:END -->

## Einordnung

Die Demo verwendet zeitlich geordnete Auftrags-Snapshots. Dadurch wird sichtbar, dass sich nicht nur der Prozessstatus aendern kann, sondern auch begleitende Fachwerte wie `NetAmount` oder `PlannedShipDate`.

Folgende Annahmen werden dabei bewusst festgehalten:

- Das Skript arbeitet ausschliesslich mit Demo-Daten in Temp-Tabellen.
- Kleine Betragsabweichungen bis einschliesslich `@AmountTolerance` gelten als nicht relevant.
- Ein Terminwechsel wird zusammen mit Wertewechseln unter derselben diagnostischen Kategorie betrachtet.
- Der letzte Snapshot je Auftrag wird als aktueller Endzustand ohne spaeteren Nachfolger ausgewiesen.
...
|---|---|---|---|
| `@OnlyShowDetectedChanges` | `BIT` | Ja | `1` zeigt nur Initial-, Status- und relevante Wertewechsel, `0` zeigt die komplette Snapshot-Folge. |
| `@AmountTolerance` | `DECIMAL(10,2)` | Ja | Betragsdifferenzen bis einschliesslich dieses Werts gelten als unveraendert. |
| `@ShowSourceData` | `BIT` | Nein | Gibt bei `1` die sortierten Demo-Snapshots vor der Analyse aus. |
...
- `SnapshotWithNeighbors` zeigt den direkten Vor- und Nachbar-Snapshot pro Auftrag und ist die analytische Grundlage fuer alle weiteren Resultsets.
- `ChangeCategory` unterscheidet zwischen Initialzustand, reinem Statuswechsel, reinem Wertewechsel, kombinierten Aenderungen und stabilen Snapshots.
- Die Position `ORD-200` mit `450.00 -> 454.00` demonstriert bewusst einen Betrag innerhalb der Toleranz und bleibt daher ohne relevanten Wertewechsel.
- `STRING_AGG` verdichtet die erkannte Historie pro Auftrag zu einer lesbaren Change-Timeline.
...
|---|---|---|---|
| `1.0` | `2026-04-18` | `ER` | Erstversion des didaktischen Labs zur Erkennung von Status- und Wertewechseln mit LAG und LEAD |
...
```mermaid
flowchart TD
    A[Parameter validieren] --> B[Demo-Snapshots in Temp-Tabelle laden]
    B --> C{ShowSourceData = 1?}
    C -->|Ja| D[Quell-Snapshots sortiert ausgeben]
    C -->|Nein| E[Weiter ohne Vorschau]
    D --> E
    E --> F[Vorherige und naechste Status- und Wertauspraegungen mit LAG und LEAD berechnen]
    F --> G[Status-, Betrag- und Terminwechsel je Snapshot klassifizieren]
    G --> H[Relevante Changes inklusive Delta und Abstand zum Folgesnapshot ausgeben]
    H --> I[Change-Timeline je Auftrag mit STRING_AGG verdichten]
    I --> J[Letzten bekannten Snapshot ohne Nachfolger markieren]
```
<!-- SQLDOC:MERMAID:END -->
