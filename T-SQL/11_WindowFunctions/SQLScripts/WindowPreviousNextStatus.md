# WindowPreviousNextStatus.sql

Dieses Skript zeigt eine kompakte Window-Functions-Sicht auf Statusfolgen. Pro Ereignis werden vorheriger, aktueller und naechster Status nebeneinander gestellt, damit Startpunkte, stabile Fenster und echte Uebergaenge klar erkennbar werden.
...
| Feld | Wert |
|---|---|
| Script | [WindowPreviousNextStatus.sql](WindowPreviousNextStatus.sql) |
| Version | `1.0` |
| Typ | `didactic-lab` |
| Kapitel | `11_WindowFunctions` |
| Sicherheit | `read-only-tempdb` |
| Zweck | Vergleicht mit `LAG()` und `LEAD()` den vorherigen, aktuellen und naechsten Status je Ereignis. |
<!-- SQLDOC:SUMMARY_TABLE:END -->

## Einordnung

Die Demo arbeitet mit Bestellstatus in Temp-Tabellen statt mit produktiven Workflow-Objekten. Dadurch laesst sich der didaktische Fokus auf die Window-Logik legen, ohne Fachtabellen vorauszusetzen.

Folgende Annahmen werden bewusst festgehalten:

- Die Reihenfolge innerhalb einer Order ergibt sich deterministisch aus `StatusTime`.
- Mehrfach auftretende gleiche Statuswerte bilden absichtlich stabile Fenster.
- Ein Fokus-Ereignis liegt vor, wenn sich der aktuelle Status vom vorherigen oder naechsten Status unterscheidet.
- `NULL` bei `PreviousStatusCode` oder `NextStatusCode` markiert den Rand einer Sequenz.
...
|---|---|---|---|
| `@ShowSourceData` | `BIT` | Nein | Gibt bei `1` die Demo-Ereignisse vor der Window-Auswertung aus. |
| `@OnlyChangedRows` | `BIT` | Ja | `1` zeigt nur Start-, End- und Wechselzeilen, `0` zeigt die komplette Sequenz. |
...
- `SequenceRole` beschreibt, ob eine Zeile am Anfang, Ende oder mitten in einem stabilen Statusfenster liegt.
- `TransitionLabel` verdichtet den Kontext in eine lesbare Uebergangsbezeichnung wie `Approved -> OnHold -> Approved`.
- Die Minutenabstaende zu Vorher und Nachher helfen, Statusdauern und Umschaltpunkte zu lesen.
- Einzelne Sequenzen mit nur einer Zeile werden explizit als `single_row_sequence` behandelt.
...
|---|---|---|---|
| `1.0` | `2026-04-18` | `ER` | Erstversion des Window-Functions-Labs zum Vergleich von vorherigem, aktuellem und naechstem Status |
...
```mermaid
flowchart TD
    A[BIT-Parameter validieren] --> B[Demo-Statushistorie in Temp-Tabelle laden]
    B --> C{ShowSourceData = 1?}
    C -->|Ja| D[Quellhistorie sortiert ausgeben]
    C -->|Nein| E[Direkt zur Window-Sequenz]
    D --> E
    E --> F[Vorherigen und naechsten Status mit LAG und LEAD berechnen]
    F --> G[SequenceRole fuer Start, Ende und stabile Fenster ableiten]
    G --> H[TransitionFocus mit Vergleich zu Vorher und Nachher aufbauen]
    H --> I[Wechselzeilen oder komplette Sequenz ausgeben]
    I --> J[BoundarySummary je Order aggregieren]
```
<!-- SQLDOC:MERMAID:END -->
