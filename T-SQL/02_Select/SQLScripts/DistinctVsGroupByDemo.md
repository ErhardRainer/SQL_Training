# DistinctVsGroupByDemo.sql

Dieses Skript stellt `DISTINCT` und `GROUP BY` auf demselben Demo-Datensatz direkt gegenueber. Der Schwerpunkt liegt darauf, dass beide Varianten zwar oft dieselbe Menge eindeutiger Kombinationen liefern, `GROUP BY` aber zusaetzlich Aggregationen und Kennzahlen in derselben Abfrageform aufnehmen kann.
...
| Feld | Wert |
|---|---|
| Script | [DistinctVsGroupByDemo.sql](DistinctVsGroupByDemo.sql) |
| Version | `1.0` |
| Typ | `didactic-lab` |
| Kapitel | `02_Select` |
| Sicherheit | `read-only` |
| Zweck | Vergleicht `DISTINCT` und `GROUP BY` hinsichtlich Ergebnisform, Duplikatentfernung und typischer Plancharakteristik. |
<!-- SQLDOC:SUMMARY_TABLE:END -->

## Einordnung

Im Kapitel `02_Select` ist dieses Lab als Bruecke zwischen einfacher Projektion und fruehen Aggregationen gedacht. Es zeigt, dass `DISTINCT` vor allem auf eindeutige Ergebniszeilen zielt, waehrend `GROUP BY` dieselben Gruppenschluessel als Grundlage fuer weitere Kennzahlen verwendet.

## Annahmen

- Das Skript arbeitet ausschliesslich mit eingebetteten Demo-Daten und greift auf keine produktiven Tabellen zu.
- Die Planhinweise beschreiben typische Operatorfamilien wie Stream Aggregate oder Hash Match Aggregate, ohne einen konkreten grafischen Ausfuehrungsplan zu exportieren.
- `@OnlyDuplicateGroups` fokussiert didaktisch auf Gruppen mit Mehrfachvorkommen, damit der Nutzen von Duplikatentfernung und Aggregation leichter lesbar wird.
- Die Kategorieauswertung ist absichtlich klein gehalten und dient als Anschlussbeispiel fuer spaetere `GROUP BY`-Vertiefungen.

## Anwendungsfall

Das Skript eignet sich fuer Unterrichtssequenzen, in denen Lernende entscheiden sollen, ob eine Anforderung nur eindeutige Werte benoetigt oder bereits aggregierte Kennzahlen pro Gruppe. Besonders hilfreich ist die Gegenueberstellung, wenn anschliessend ueber `COUNT(*)`, `SUM(...)` und die Erweiterbarkeit von Gruppierungsabfragen gesprochen wird.
...
|---|---|---|---|
| `@IncludeDetailedRows` | `BIT` | Nein | Gibt bei `1` Rohdaten sowie DISTINCT- und GROUP-BY-Resultset zusaetzlich aus. |
| `@IncludeCategoryBreakdown` | `BIT` | Nein | Gibt bei `1` eine erweiterte GROUP-BY-Auswertung je Region und Kategorie aus. |
| `@OnlyDuplicateGroups` | `BIT` | Nein | Beschraenkt bei `1` die Detailausgaben auf Kombinationen mit mehrfach vorkommenden Zeilen. |
...
- `DistinctProjection` und `GroupedProjection` verwenden denselben Gruppenschluessel, damit die Ergebniszeilen direkt vergleichbar bleiben.
- `ComparisonSummary` macht sichtbar, dass beide Muster oft aehnliche physische Operatoren nutzen koennen, die Ergebnisform aber unterschiedlich reichhaltig ist.
- `CategoryBreakdown` zeigt den typischen naechsten Schritt: Sobald Kennzahlen pro Gruppe benoetigt werden, ist `GROUP BY` fachlich meist der natuerlichere Einstieg.
...
|---|---|---|---|
| `1.0` | `2026-04-19` | `ER` | Erstversion des Labs zum Vergleich von DISTINCT und GROUP BY |
...
```mermaid
flowchart TD
    A[Parameter validieren] --> B[Demo-Datensatz im CTE SalesSample aufbauen]
    B --> C[BaseRows mit RevenueBand ableiten]
    C --> D[DistinctProjection fuer eindeutige Region-Kategorie-Kombinationen erzeugen]
    C --> E[GroupedProjection mit COUNT und SUM erzeugen]
    D --> F[ComparisonSummary aus beiden Varianten verdichten]
    E --> F
    F --> G{IncludeDetailedRows = 1?}
    G -->|Ja| H[Rohdaten, DISTINCT-Resultset und GROUP-BY-Resultset ausgeben]
    G -->|Nein| I[Direkt zur Zusammenfassung]
    H --> J{OnlyDuplicateGroups = 1?}
    J -->|Ja| K[Nur Gruppen mit Mehrfachvorkommen zeigen]
    J -->|Nein| L[Alle Gruppen zeigen]
    K --> I
    L --> I
    I --> M[ComparisonSummary ausgeben]
    M --> N{IncludeCategoryBreakdown = 1?}
    N -->|Ja| O[Erweiterte GROUP-BY-Auswertung je Region und Kategorie zeigen]
