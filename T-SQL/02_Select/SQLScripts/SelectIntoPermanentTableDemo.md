# SelectIntoPermanentTableDemo.sql

Dieses Lab demonstriert `SELECT INTO` als schnelle Tabellenkopie in eine permanente Demo-Tabelle unter `tempdb.dbo`. Der Fokus liegt auf dem typischen Staging-Muster: Quelldaten vorbereiten, Zieltabelle per Projektion materialisieren, kurz pruefen und optional wieder aufraeumen.
...
| Feld | Wert |
|---|---|
| Script | [SelectIntoPermanentTableDemo.sql](SelectIntoPermanentTableDemo.sql) |
| Version | `1.0` |
| Typ | `didactic-lab` |
| Kapitel | `02_Select` |
| Sicherheit | `demo-write-tempdb` |
| Zweck | Demonstriert `SELECT INTO` fuer eine schnelle Tabellenkopie in `tempdb` als Start fuer einfache Staging-Strukturen. |
<!-- SQLDOC:SUMMARY_TABLE:END -->

## Einordnung

Im Kapitel `02_Select` schliesst das Skript die Luecke zwischen reinen Projektionen und spaeteren Themen wie Staging, ETL-Vorbereitung oder temporaeren Arbeitskopien. Es zeigt bewusst kein produktives Ladeverfahren, sondern ein nachvollziehbares Grundmuster fuer eine schnelle Tabellenanlage aus einer bestehenden Abfrage.

## Annahmen

- Das Skript nutzt ausschliesslich eingebettete Demo-Daten und schreibt nur nach `tempdb.dbo`.
- Die Zieltabelle gilt als permanente Demo-Tabelle innerhalb von `tempdb`, also nicht nur als Session-Temp-Objekt.
- `@DropExisting = 1` ist die sichere Voreinstellung, damit wiederholte Demos reproduzierbar bleiben.
- `@KeepStageTable = 0` entfernt die erzeugte Tabelle standardmaessig wieder, damit keine Altobjekte im Demo-System liegen bleiben.
- Fuer variable Zieltabellennamen wird `sys.sp_executesql` zusammen mit einer whitelist-aehnlichen Namenspruefung verwendet.

## Anwendungsfall

Das Lab eignet sich fuer Unterricht und Code-Review, wenn besprochen werden soll, wie `SELECT INTO` in einer Staging- oder Analysephase eingesetzt wird. Sichtbar werden dabei drei Kernideen: die Projektion darf zugleich Daten formen, `SELECT INTO` erzeugt die Zielstruktur implizit und ein nachgelagerter Kontrollschritt sollte die Materialisierung pruefen, bevor mit der Tabelle weitergearbeitet wird.
...
|---|---|---|---|
| `@AsOfDate` | `DATE` | Nein | Stichtag fuer reproduzierbare Status- und Altersableitungen. |
| `@StageTableName` | `SYSNAME` | Nein | Name der in `tempdb.dbo` zu erzeugenden permanenten Demo-Staging-Tabelle. |
| `@DropExisting` | `BIT` | Nein | Loescht bei `1` eine vorhandene Demo-Staging-Tabelle vor `SELECT INTO`. |
| `@KeepStageTable` | `BIT` | Nein | Laesst bei `1` die erzeugte Tabelle nach der Demonstration stehen. |
| `@ShowSourcePreview` | `BIT` | Nein | Zeigt bei `1` die Quelldaten vor der Tabellenkopie zusaetzlich an. |
...
- Die eigentliche Tabellenanlage passiert innerhalb des dynamischen SQL-Blocks, weil der Zielname variabel bleiben darf.
- `PreparedSource` zeigt, dass `SELECT INTO` nicht nur kopiert, sondern zugleich eine fachlich brauchbare Staging-Projektion materialisieren kann.
- Die Summary-Ausgabe dient als schneller Kontrollpunkt direkt nach der Tabellenanlage.
- Wer das Ergebnis in `tempdb` weiter untersuchen moechte, setzt `@KeepStageTable = 1`.
...
|---|---|---|---|
| `1.0` | `2026-04-17` | `ER` | Erstversion des Labs fuer `SELECT INTO` in eine permanente `tempdb`-Staging-Tabelle |
...
```mermaid
flowchart TD
    A[Parameter validieren] --> B{Zieltabelle in tempdb vorhanden?}
    B -->|Ja und DropExisting = 1| C[Vorhandene Demo-Tabelle loeschen]
    B -->|Ja und DropExisting = 0| D[THROW Fehler]
    B -->|Nein| E[Optional Quelldaten anzeigen]
    C --> E
    E --> F[DemoOrders per VALUES aufbauen]
    F --> G[PreparedSource mit Kennzahlen und Statusfeldern ableiten]
    G --> H[SELECT INTO tempdb.dbo.StageTable ausfuehren]
    H --> I[Summary mit Zeilenanzahl und Ladezeit ausgeben]
    I --> J[Zeilen aus der erzeugten Stage-Tabelle anzeigen]
    J --> K{KeepStageTable = 1?}
    K -->|Nein| L[Demo-Tabelle wieder loeschen]
    K -->|Ja| M[Tabelle fuer Nachkontrolle stehen lassen]
```
<!-- SQLDOC:MERMAID:END -->
