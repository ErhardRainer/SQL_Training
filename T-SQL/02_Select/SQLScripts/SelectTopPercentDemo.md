# SelectTopPercentDemo.sql

Dieses Lab zeigt, wie `TOP (PERCENT)` aus einer Prozentangabe eine konkrete Zeilenzahl ableitet. Sichtbar werden sowohl die didaktische Herleitung ueber die Gesamtzeilenzahl als auch der Unterschied zwischen absteigender und aufsteigender Sortierung derselben Grundmenge.
...
| Feld | Wert |
|---|---|
| Script | [SelectTopPercentDemo.sql](SelectTopPercentDemo.sql) |
| Version | `1.0` |
| Typ | `didactic-lab` |
| Kapitel | `02_Select` |
| Sicherheit | `read-only` |
| Zweck | Demonstriert, wie `TOP (PERCENT)` seine Rueckgabemenge aus einer Prozentangabe und der aktuellen `ORDER BY`-Logik ableitet. |
<!-- SQLDOC:SUMMARY_TABLE:END -->

## Einordnung

Im Kapitel `02_Select` schliesst dieses Skript die Luecke zwischen `TOP (n)` und einer relativen Auswahl per Prozentwert. Gerade fuer Unterricht und Reviews ist hilfreich, dass dieselbe Prozentangabe je nach Groesse der Grundmenge auf ganze Zeilen abgebildet werden muss und ohne klare Sortierung keine fachlich stabile Aussage trifft.

## Annahmen

- Das Lab arbeitet ausschliesslich mit eingebetteten Demo-Auftragsdaten und greift auf keine produktiven Tabellen zu.
- Die didaktische Erwartung fuer die Zielgroesse wird ueber `CEILING(total_rows * @TopPercent / 100.0)` erklaert.
- `NetAmount` dient als leicht nachvollziehbare Sortierkennzahl fuer die Auswahl der oberen oder unteren Prozentgruppe.
- Die aufsteigende Gegenprobe ist optional, damit sichtbar wird, dass dieselbe Prozentzahl je nach `ORDER BY` eine fachlich andere Teilmenge liefert.

## Anwendungsfall

Das Skript eignet sich fuer Unterricht, Selbststudium und Query-Reviews, wenn geklaert werden soll, warum `TOP (PERCENT)` nicht mit mathematischen Bruchteilen von Zeilen arbeitet. Ebenso zeigt es, dass Prozentangaben allein keine Priorisierung definieren, solange die Sortierreihenfolge nicht explizit und nachvollziehbar festgelegt ist.
...
|---|---|---|---|
| `@TopPercent` | `DECIMAL(5,2)` | Nein | Prozentanteil der Zeilen, die ueber `TOP (PERCENT)` ausgewaehlt werden sollen. |
| `@RegionFilter` | `NVARCHAR(20)` | Nein | Optionaler Filter auf eine Region des Demo-Datensatzes. |
| `@IncludeAscendingComparison` | `BIT` | Nein | Zeigt bei `1` zusaetzlich eine Gegenprobe mit aufsteigender Sortierung. |
| `@ShowPercentCatalog` | `BIT` | Nein | Gibt bei `1` ein zusaetzliches Resultset mit Merkregeln zu `TOP (PERCENT)` aus. |
...
- `PercentCatalog` fasst die Kernbeobachtungen zu Prozent-zu-Zeilen, Aufrundung und Sortierabhaengigkeit kompakt zusammen.
- `PercentSelectionPreview` zeigt fuer jede Zeile die erwartete Zielgroesse sowie Markierungen fuer eine absteigende und optional aufsteigende Auswahl.
- Die eigentliche `TOP (PERCENT)`-Auswahl passiert in `ComparisonRows`, damit die Vorschau und die tatsaechliche Rueckgabe direkt gegeneinander gehalten werden koennen.
- `PercentSelectionSummary` verdichtet die Rueckgabemenge je Szenario und listet die ausgewaehlten `OrderID`-Werte in Sortierreihenfolge auf.
...
|---|---|---|---|
| `1.0` | `2026-04-19` | `ER` | Erstversion des Labs zu `TOP (PERCENT)` und seiner abgeleiteten Zeilenzahl |
...
```mermaid
flowchart TD
    A[Parameter vorbereiten und validieren] --> B{ShowPercentCatalog = 1?}
    B -->|Ja| C[PercentCatalog mit Merkregeln zu TOP PERCENT ausgeben]
    B -->|Nein| D[DemoOrders laden und optional nach Region filtern]
    C --> D
    D --> E[PreparedRows mit NetAmount Gesamtzeilen und zwei ROW_NUMBER-Sichten erzeugen]
    E --> F[PreviewRows mit erwarteter Zielgroesse und Auswahlmarkern ableiten]
    F --> G[TOP PERCENT Auswahl fuer DESC-Sortierung berechnen]
    G --> H{IncludeAscendingComparison = 1?}
    H -->|Ja| I[Zweite TOP PERCENT Auswahl fuer ASC-Sortierung berechnen]
    H -->|Nein| J[Direkt zur Szenario-Summary]
    I --> J
    J --> K[Preview aller Zeilen ausgeben]
    K --> L[Summary je Szenario mit Rueckgabemenge und OrderIDs ausgeben]
```
<!-- SQLDOC:MERMAID:END -->
