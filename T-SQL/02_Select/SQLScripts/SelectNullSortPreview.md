# SelectNullSortPreview.sql

Dieses Lab zeigt mit einem kleinen Backlog-Datensatz, wie SQL Server `NULL` bei Sortierungen behandelt. Sichtbar werden sowohl die Standardreihenfolge bei `ASC` und `DESC` als auch zwei explizite Alternativen, die `NULL` per `CASE` bewusst nach vorne oder hinten verschieben.
...
| Feld | Wert |
|---|---|
| Script | [SelectNullSortPreview.sql](SelectNullSortPreview.sql) |
| Version | `1.0` |
| Typ | `didactic-lab` |
| Kapitel | `02_Select` |
| Sicherheit | `read-only` |
| Zweck | Zeigt, wie SQL Server `NULL` bei Sortierungen standardmaessig und per expliziter `CASE`-Steuerung einordnet. |
<!-- SQLDOC:SUMMARY_TABLE:END -->

## Einordnung

Im Kapitel `02_Select` erweitert dieses Skript die reine `ORDER BY`-Syntax um die praktische Frage, wo fehlende Werte im Resultset landen. Gerade bei Prioritaeten, Terminen oder Kennzahlen ist die Position von `NULL` oft fachlich relevant und sollte nicht dem Zufall einer stillen Standardannahme ueberlassen werden.

## Annahmen

- Das Skript arbeitet ausschliesslich mit eingebetteten Demo-Daten und greift auf keine produktiven Tabellen zu.
- Die Beobachtung konzentriert sich auf die nullable Spalte `PriorityScore`; `ReviewDate` dient nur als weiterer Kontext im Datensatz.
- T-SQL stellt kein natives `NULLS FIRST` oder `NULLS LAST` bereit, deshalb werden explizite Varianten ueber `CASE` im `ORDER BY` gezeigt.
- Die Szenarien sollen Sortierverhalten erklaeren und nicht fachliche Priorisierungsregeln fuer ein reales Backlog festlegen.

## Anwendungsfall

Das Lab eignet sich fuer Unterricht, Query-Reviews und Datenqualitaetsgespraeche, wenn sichtbar gemacht werden soll, warum dieselbe Spalte bei `ASC` und `DESC` unterschiedlich mit `NULL` umgeht. Besonders hilfreich ist es, wenn Teams eine fachlich gewollte NULL-Platzierung konsistent und lesbar formulieren wollen.
...
|---|---|---|---|
| `@ShowSourceData` | `BIT` | Nein | Gibt bei `1` den Demo-Datensatz vor den Sortiervergleichen aus. |
| `@IncludeExplicitAlternatives` | `BIT` | Nein | Zeigt bei `1` zusaetzlich Varianten mit expliziter `CASE`-Steuerung fuer `NULL`. |
| `@ShowTeachingSummary` | `BIT` | Nein | Gibt bei `1` eine kompakte Zusammenfassung der beobachteten NULL-Position aus. |
...
- `score_asc_default` zeigt die SQL-Server-Standardeinordnung: `NULL` erscheint bei aufsteigender Sortierung zuerst.
- `score_desc_default` zeigt den Gegenpol: bei absteigender Sortierung landen `NULL`-Zeilen am Ende.
- Die expliziten Szenarien machen sichtbar, dass fachliche Anforderungen wie "fehlende Prioritaet zuletzt" oder "ungepflegte Werte zuerst" bewusst kodiert werden koennen.
- Die Summary verdichtet fuer jedes Szenario die beobachteten ersten und letzten NULL-Positionen.
...
|---|---|---|---|
| `1.0` | `2026-04-19` | `ER` | Erstversion des Labs zur `NULL`-Sortierung in SQL Server |
...
```mermaid
flowchart TD
    A[Parameter validieren] --> B[DemoBacklog mit NULL- und Nicht-NULL-PriorityScore aufbauen]
    B --> C[PreparedBacklog mit PriorityBand und ReviewStatus ableiten]
    C --> D{ShowSourceData = 1?}
    D -->|Ja| E[Quellmenge nach TaskID ausgeben]
    D -->|Nein| F[Direkt zu den Sortierszenarien]
    E --> F
    F --> G[Standard ASC mit PriorityScore ASC berechnen]
    G --> H[Standard DESC mit PriorityScore DESC berechnen]
    H --> I{IncludeExplicitAlternatives = 1?}
    I -->|Ja| J[ASC mit CASE auf NULLS LAST berechnen]
    I -->|Nein| K[Zur Summary weiter]
    J --> L[DESC mit CASE auf NULLS FIRST berechnen]
    L --> K
    K --> M[Alle Szenarien als NullSortPreview ausgeben]
    M --> N{ShowTeachingSummary = 1?}
    N -->|Ja| O[Null-Positionen je Szenario verdichten und ausgeben]
