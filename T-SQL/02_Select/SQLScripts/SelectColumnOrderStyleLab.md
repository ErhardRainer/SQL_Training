# SelectColumnOrderStyleLab.sql

Dieses Lab zeigt an einem kleinen Sales-Beispiel, wie sich die Lesbarkeit einer `SELECT`-Liste verbessert, wenn Spalten in einer stabilen Reihenfolge gruppiert werden. Dazu werden bewusst zwei Layouts gegenuebergestellt: eine ungeordnete Liste und eine fachlich gegliederte Darstellung.
...
| Feld | Wert |
|---|---|
| Script | [SelectColumnOrderStyleLab.sql](SelectColumnOrderStyleLab.sql) |
| Version | `1.0` |
| Typ | `didactic-lab` |
| Kapitel | `02_Select` |
| Sicherheit | `read-only` |
| Zweck | Demonstriert konsistente Spaltenreihenfolgen fuer lesbare SELECT-Listen anhand eines didaktischen Demo-Datensatzes. |
<!-- SQLDOC:SUMMARY_TABLE:END -->

## Einordnung

Im Kapitel `02_Select` passt dieses Skript zwischen grundlegende Projektionen und spaetere Formatierungs- oder Reporting-Fragen. Es beantwortet nicht, welche Daten fachlich wichtig sind, sondern zeigt, wie dieselben Daten mit besserer Reihenfolge leichter gelesen, erklaert und reviewed werden koennen.

## Annahmen

- Das Skript arbeitet ausschliesslich mit eingebetteten Demo-Daten und greift auf keine produktiven Tabellen zu.
- Die geordnete Liste gruppiert Spalten bewusst nach Identitaet, Kontext, Zeit, Mengen und Kennzahlen.
- Die ungeordnete Ausgabe ist absichtlich didaktisch konstruiert, um den Unterschied im Lesefluss sichtbar zu machen.
- `NeedsAttention` ist nur ein Lehrsignal, damit sich Filter und Sortierung an markanten Zeilen diskutieren lassen.

## Anwendungsfall

Das Lab eignet sich fuer Unterricht, Pair-Review und Style-Diskussionen zu `SELECT`-Listen. Lernende koennen direkt vergleichen, warum ein Identifier typischerweise frueh steht, warum Datumsfelder vor Mengen und Kennzahlen haeufig sinnvoll sind und warum Hilfsspalten wie `OwnerName` oder `DealBand` erst spaeter kommen.
...
|---|---|---|---|
| `@ShowSourceData` | `BIT` | Nein | Gibt bei `1` den Ausgangsdatensatz zusaetzlich aus. |
| `@ShowUnorderedExample` | `BIT` | Nein | Gibt bei `1` auch die bewusst unruhige Spaltenliste aus. |
| `@OnlyAttentionRows` | `BIT` | Nein | Filtert bei `1` auf Zeilen mit hohem Diskussionswert zur Spaltenordnung. |
...
- `unordered-example` und `grouped-style` werden als getrennte Resultsets ausgegeben, damit die reale Spaltenreihenfolge direkt sichtbar bleibt.
- `unordered-example` mischt Kennzahlen, Status und Stammdaten absichtlich, damit der Unterschied zur geordneten Variante sofort sichtbar wird.
- `grouped-style` fuehrt dieselben Informationen in einer stabilen Reihenfolge zusammen und erleichtert Scan, Review und spaetere Erweiterungen.
- `ReviewSequence` liefert eine reproduzierbare Sortierung fuer beide Layouts.
...
|---|---|---|---|
| `1.0` | `2026-04-17` | `ER` | Erstversion des Labs fuer konsistente Spaltenreihenfolgen |
...
```mermaid
flowchart TD
    A[Parameter validieren] --> B[Demo-Datensatz SalesInbox per VALUES aufbauen]
    B --> C[Grundfelder und Kennzahlen in PreparedData berechnen]
    C --> D[Aufmerksamkeitsmarker und Ordnungsprinzipien in AnnotatedRows ableiten]
    D --> E{ShowSourceData = 1?}
    E -->|Ja| F[Rohdaten ausgeben]
    E -->|Nein| G[Direkt zu den Layouts]
    F --> G
    G --> H{ShowUnorderedExample = 1?}
    H -->|Ja| I[Ungeordnete SELECT-Liste als Negativbeispiel ausgeben]
    H -->|Nein| J[Nur geordnete SELECT-Liste ausgeben]
    I --> J
    J --> K{OnlyAttentionRows = 1?}
    K -->|Ja| L[Nur markierte Zeilen in grouped-style zeigen]
    K -->|Nein| M[Alle Demo-Zeilen in grouped-style zeigen]
    L --> N[ChecklistSummary je Ordnungsprinzip aggregieren]
    M --> N
```
<!-- SQLDOC:MERMAID:END -->
