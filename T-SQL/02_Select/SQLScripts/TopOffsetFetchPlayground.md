# TopOffsetFetchPlayground.sql

Dieses Lab stellt `TOP`, kumulatives `TOP` und `OFFSET/FETCH` auf dieselbe eindeutige Sortierbasis. Dadurch wird sichtbar, dass `TOP (@PageSize)` nur die erste Seite beschreibt, kumulatives `TOP` zwar die Zielseite mitliefert, aber alle vorherigen Zeilen mit einschliesst, und nur `OFFSET/FETCH` exakt die angeforderte Seite adressiert.
...
| Feld | Wert |
|---|---|
| Script | [TopOffsetFetchPlayground.sql](TopOffsetFetchPlayground.sql) |
| Version | `1.0` |
| Typ | `didactic-lab` |
| Kapitel | `02_Select` |
| Sicherheit | `read-only` |
| Zweck | Vergleicht `TOP`, kumulatives `TOP` und `OFFSET/FETCH` auf einer gemeinsamen Sortierbasis fuer Paging-Szenarien. |
<!-- SQLDOC:SUMMARY_TABLE:END -->

## Einordnung

Im Kapitel `02_Select` wird haeufig zuerst `TOP` gelernt und spaeter `OFFSET/FETCH` ergaenzt. Das Playground-Skript hilft dabei, die Rollen sauber zu trennen: `TOP` begrenzt die Rueckgabemenge vom Anfang der sortierten Ergebnismenge, waehrend `OFFSET/FETCH` eine konkrete Seite innerhalb dieser sortierten Ergebnismenge adressiert.

## Annahmen

- Das Lab verwendet ausschliesslich eingebettete Demo-Daten und greift auf keine produktiven Tabellen zu.
- Die Sortierung ist absichtlich eindeutig ueber `LastContactDate DESC`, abgeleitetes Prioritaetsgewicht und `OpportunityID DESC` definiert.
- `@PageNumber` ist standardmaessig auf `2` gesetzt, damit der Unterschied zwischen erster Seite, kumulativer Auswahl und echter Zielseite direkt sichtbar wird.
- Kumulatives `TOP` wird didaktisch als `TOP (@PageNumber * @PageSize)` modelliert, um die Abgrenzung zu `OFFSET/FETCH` klar zu machen.

## Anwendungsfall

Das Skript eignet sich fuer Unterricht, Query-Reviews und API-Diskussionen rund um Paging. Besonders hilfreich ist es, wenn nachvollziehbar gezeigt werden soll, warum eine Obergrenze per `TOP` noch keine Seitennavigation darstellt und warum fuer reproduzierbares Paging eine stabile `ORDER BY`-Definition notwendig ist.
...
|---|---|---|---|
| `@PageSize` | `INT` | Nein | Anzahl der Zeilen pro Seite im Playground. |
| `@PageNumber` | `INT` | Nein | Zielseite fuer den Vergleich von kumulativem `TOP` und `OFFSET/FETCH`. |
| `@RegionFilter` | `NVARCHAR(20)` | Nein | Optionaler Filter auf eine Region der Demo-Daten. |
| `@ShowOrderingPreview` | `BIT` | Nein | Gibt bei `1` die vollstaendige Sortierbasis mit Zeilennummern und Seiteneinteilung aus. |
...
- `OrderingPreview` zeigt fuer jede Zeile ihre Sortierposition und die daraus abgeleitete Seite.
- `SelectionComparison` stellt drei Selektionsmuster gegenueber: erste Seite per `TOP (@PageSize)`, kumulativer Vorderbereich per `TOP (@PageNumber * @PageSize)` und exakte Zielseite per `OFFSET/FETCH`.
- Die Summary macht sichtbar, dass kumulatives `TOP` die Zielseite nicht isoliert, sondern immer auch die vorherigen Seiten mitliefert.
- Der optionale `@RegionFilter` reduziert die Grundmenge, ohne die didaktische Logik des Vergleichs zu veraendern.
...
|---|---|---|---|
| `1.0` | `2026-04-19` | `ER` | Erstversion des Playgrounds fuer `TOP`, kumulatives `TOP` und `OFFSET/FETCH` |
...
```mermaid
flowchart TD
    A[Parameter vorbereiten und validieren] --> B[OpportunityPipeline mit Demo-Daten fuellen]
    B --> C[FilteredPipeline optional nach Region einschraenken]
    C --> D[PreparedRows mit TotalRows und SortRowNumber aufbauen]
    D --> E{ShowOrderingPreview = 1?}
    E -->|Ja| F[OrderingPreview mit Seitennummern und Coverage-Rollen ausgeben]
    E -->|Nein| G[Direkt in die Szenariovergleiche]
    F --> G
    G --> H[TOP PageSize fuer die erste Seite berechnen]
    G --> I[Kumulatives TOP bis zum Ende der Zielseite berechnen]
    G --> J[OFFSET FETCH fuer die exakte Zielseite berechnen]
    H --> K[SelectionComparison vereinigen und kommentieren]
    I --> K
    J --> K
    K --> L[TeachingSummary mit Zeilenbereich und OpportunityIDs pro Szenario verdichten]
```
<!-- SQLDOC:MERMAID:END -->
