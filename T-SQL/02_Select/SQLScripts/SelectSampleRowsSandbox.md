# SelectSampleRowsSandbox.sql

Dieses Lab bietet eine sichere Sandbox fuer erste Stichproben mit `SELECT`. Statt produktiver Tabellen nutzt das Skript einen kleinen Demo-Auftragsdatensatz und zeigt, wie unterschiedliche Auswahlregeln die gleiche Grundmenge verschieden beleuchten.
...
| Feld | Wert |
|---|---|
| Script | [SelectSampleRowsSandbox.sql](SelectSampleRowsSandbox.sql) |
| Version | `1.0` |
| Typ | `didactic-lab` |
| Kapitel | `02_Select` |
| Sicherheit | `read-only` |
| Zweck | Bietet eine sichere Sandbox fuer Stichproben und erste Datenexploration mit `SELECT` auf einem didaktischen Auftragsdatensatz. |
<!-- SQLDOC:SUMMARY_TABLE:END -->

## Einordnung

Im Kapitel `02_Select` schliesst das Skript die Luecke zwischen einer einfachen Vollabfrage und einer gezielten Arbeitsstichprobe. Lernende sehen, dass dieselbe Datenbasis je nach Sortierung und Auswahlregel andere Fragen beantwortet: Neueste Zeilen, fachlich wichtige Zeilen oder eine regelmaessige Kontrollstichprobe.

## Annahmen

- Das Skript arbeitet ausschliesslich mit eingebetteten Demo-Auftragsdaten statt produktiver Tabellen.
- Die Stichprobe bleibt bewusst deterministisch und verwendet keine zufaellige Ziehung ueber `NEWID()` oder `TABLESAMPLE`.
- Der Modus `every_nth` nutzt die stabil sortierte Explorationsreihenfolge und ist damit gut fuer Unterricht und Reproduzierbarkeit geeignet.
- `@RegionFilter` reduziert die Grundmenge vor der eigentlichen Stichprobenlogik.
- `SampleSummary` verdichtet nur die gezogene Stichprobe und nicht den gesamten Demo-Datensatz.

## Anwendungsfall

Das Lab eignet sich fuer Unterricht, wenn `TOP`, `ORDER BY`, Filter und einfache Explorationsschritte gemeinsam erklaert werden sollen. Besonders gut laesst sich daran zeigen, dass eine Stichprobe nicht nur "weniger Zeilen" bedeutet, sondern immer auch eine Auswahlentscheidung mit fachlicher Wirkung ist.
...
|---|---|---|---|
| `@SampleSize` | `INT` | Nein | Anzahl der Zeilen fuer die Stichprobe. |
| `@SampleMode` | `NVARCHAR(20)` | Nein | Stichprobenmodus `latest`, `high_value` oder `every_nth`. |
| `@EveryNthRow` | `INT` | Nein | Intervall fuer den Modus `every_nth`. |
| `@RegionFilter` | `NVARCHAR(20)` | Nein | Optionaler Filter auf eine Region. |
| `@ShowSamplingNotes` | `BIT` | Nein | Zeigt bei `1` eine zusaetzliche Einordnung der Stichprobenmodi an. |
...
- `SamplingNotes` trennt die kurze didaktische Einordnung von der eigentlichen Arbeitsabfrage.
- `PreparedRows` baut zuerst lesbare Kennzahlen und Statuslabels auf, damit die Stichprobe nicht mit Rohwerten arbeiten muss.
- `OrderedRows` erzeugt drei verschiedene Ordnungen derselben Grundmenge, aus denen die Modi `latest`, `high_value` und `every_nth` ableiten.
- `SampleRows` begrenzt die Ausgabe mit `TOP (@SampleSize)` und macht den gewaelten Auswahlgrund pro Zeile sichtbar.
...
|---|---|---|---|
| `1.0` | `2026-04-17` | `ER` | Erstversion der Sandbox fuer Stichproben und erste Datenexploration mit `SELECT` |
...
```mermaid
flowchart TD
    A[Parameter vorbereiten und normalisieren] --> B{Parameter gueltig?}
    B -->|Nein| C[THROW Fehler]
    B -->|Ja| D{ShowSamplingNotes = 1?}
    D -->|Ja| E[SamplingNotes mit drei Stichprobenmodi ausgeben]
    D -->|Nein| F[DemoOrders aus VALUES laden]
    E --> F
    F --> G[PreparedRows mit Kennzahlen, Status und optionalem RegionFilter aufbauen]
    G --> H[OrderedRows mit drei ROW_NUMBER-Reihenfolgen erzeugen]
    H --> I[SampleRows je nach SampleMode filtern und mit TOP begrenzen]
    I --> J[SampleRows als Detailstichprobe ausgeben]
    I --> K[SampleSummary nach Region und Status verdichten]
```
<!-- SQLDOC:MERMAID:END -->
