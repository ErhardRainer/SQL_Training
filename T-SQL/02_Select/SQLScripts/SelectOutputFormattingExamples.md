# SelectOutputFormattingExamples.sql

Dieses Lab zeigt mehrere Anzeigeformate direkt in der `SELECT`-Liste. Der Schwerpunkt liegt nicht auf persistenter Datenverarbeitung, sondern auf der Frage, wie Rohwerte fuer Berichte oder Unterricht in gut lesbare Text- und Anzeigespalten ueberfuehrt werden koennen.
...
| Feld | Wert |
|---|---|
| Script | [SelectOutputFormattingExamples.sql](SelectOutputFormattingExamples.sql) |
| Version | `1.0` |
| Typ | `didactic-lab` |
| Kapitel | `02_Select` |
| Sicherheit | `read-only` |
| Zweck | Zeigt mehrere Anzeigeformate direkt in der SELECT-Liste, darunter Datumsdarstellungen, einfache Betragsformate, Prozentlabels und kompakte Statusbanner auf einem didaktischen Demo-Datensatz. |
<!-- SQLDOC:SUMMARY_TABLE:END -->

## Einordnung

Im Kapitel `02_Select` ergaenzt dieses Skript einfache Projektionen um konkrete Ausgabeformate. Sichtbar werden dabei zwei Perspektiven: Einerseits bleiben die Originalwerte erhalten, andererseits entstehen in derselben `SELECT`-Liste zusaetzliche Anzeigespalten fuer Datum, Betrag, Prozentwert, Mengenhinweis und Statusbanner.

## Annahmen

- Das Skript verwendet ausschliesslich eingebettete Demo-Auftragsdaten.
- Anzeigeformate werden bewusst mit einfachen T-SQL-Mitteln wie `CONVERT`, `STR`, `RIGHT`, `REPLICATE` und `CONCAT` aufgebaut.
- `@CurrencyPrefix` ist eine didaktische Anzeigehilfe und keine vollstaendige Waehrungs- oder Sprachlogik.
- Zeilen mit Status `B` werden als Backlog interpretiert und standardmaessig ausgeblendet, koennen aber ueber einen Parameter eingeblendet werden.
- Das optionale Resultset `FormatCatalog` dient nur der Einordnung der verwendeten Formatmuster.

## Anwendungsfall

Das Lab eignet sich fuer Unterricht, Review und Stilgespraeche zu `SELECT`-Projektionen. Lernende sehen, wie aus denselben Quellspalten mehrere Berichtssichten entstehen koennen, ohne dass dafuer produktive Tabellen oder undurchsichtige Formatfunktionen benoetigt werden.
...
|---|---|---|---|
| `@AsOfDate` | `DATE` | Nein | Stichtag fuer reproduzierbare Datums- und Restlaufzeitspalten. |
| `@CurrencyPrefix` | `NVARCHAR(10)` | Nein | Praefix fuer einfache Betragsanzeigen in der finalen Projektion. |
| `@IncludeBacklogRows` | `BIT` | Nein | Nimmt bei `1` auch Demo-Zeilen mit Status `Backlog` in die Hauptausgabe auf. |
| `@ShowFormatCatalog` | `BIT` | Nein | Zeigt bei `1` ein zusaetzliches Resultset mit den verwendeten Formatmustern an. |
...
- `FormatCatalog` zeigt vor der Hauptabfrage, welche Formatmuster spaeter in der Projektion verwendet werden.
- `PreparedOrders` trennt fachliche Vorbereitung von der eigentlichen Anzeigeformatierung.
- Die Spalten `ShipDateIso` und `ShipDateDach` stellen denselben Rohwert in zwei unterschiedlichen Lesarten gegenueber.
- `NetAmountDisplay`, `DiscountPercentLabel`, `QuantityBadge` und `ChannelStatusBanner` demonstrieren typische Berichtsspalten, die direkt in der `SELECT`-Liste entstehen.
...
|---|---|---|---|
| `1.0` | `2026-04-19` | `ER` | Erstversion des Labs fuer Anzeigeformate direkt in der `SELECT`-Liste |
...
```mermaid
flowchart TD
    A[Parameter validieren] --> B{ShowFormatCatalog = 1?}
    B -->|Ja| C[FormatCatalog mit Formatmustern ausgeben]
    B -->|Nein| D[DemoOrders per VALUES laden]
    C --> D
    D --> E[PreparedOrders mit Kennzahlen Status und Mengenband ableiten]
    E --> F{IncludeBacklogRows = 1?}
    F -->|Nein| G[Backlog-Zeilen ausfiltern]
    F -->|Ja| H[Alle Demo-Zeilen beibehalten]
    G --> I[Finale SELECT-Liste mit Datums Betrags Prozent und Bannerformaten erzeugen]
    H --> I
    I --> J[Ergebnis nach Status NetAmount und OrderID sortieren]
```
<!-- SQLDOC:MERMAID:END -->
