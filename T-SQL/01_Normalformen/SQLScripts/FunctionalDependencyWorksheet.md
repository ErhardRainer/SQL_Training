# FunctionalDependencyWorksheet.sql

Dieses Skript erzeugt ein didaktisches Arbeitsblatt fuer funktionale Abhaengigkeiten in einer breiten Einschreibungsrelation. Die Demo-Daten enthalten bewusst sowohl stabile Zuordnungen als auch ein Gegenbeispiel, bei dem `CourseCode` allein nicht ausreicht, um die Lehrperson eindeutig zu bestimmen.
...
| Feld | Wert |
|---|---|
| Script | [FunctionalDependencyWorksheet.sql](FunctionalDependencyWorksheet.sql) |
| Version | `1.0` |
| Typ | `didactic-lab` |
| Kapitel | `01_Normalformen` |
| Sicherheit | `read-only-tempdb` |
| Zweck | Liefert ein Arbeitsblatt fuer funktionale Abhaengigkeiten in Beispieldaten. |
<!-- SQLDOC:SUMMARY_TABLE:END -->

## Einordnung

Die SQL-Datei arbeitet nicht mit einer allgemeinen Attributmengen-Suche, sondern mit vorbereiteten Determinanten und abhaengigen Attributen. Dadurch koennen Lernende konkrete Zuordnungen lesen, Kennzahlen vergleichen und anschliessend diskutieren, welche Attribute eher in dieselbe Relation gehoeren und wo eine Zerlegung sinnvoll wird.

## Annahmen

- Es handelt sich um eine didaktische Erstversion mit Demo-Daten in `tempdb`.
- `StudentNo`, `ProgramCode`, `AdvisorCode`, `CampusCode` und `InstructorCode` repraesentieren stabile Stammdaten-Identifier.
- `CourseCode -> InstructorCode` ist absichtlich nicht immer stabil, weil derselbe Kurs in verschiedenen Terminen von unterschiedlichen Lehrpersonen angeboten werden kann.
- `CourseCode + TermCode` dient als didaktisches Beispiel fuer einen zusammengesetzten Determinanten eines konkreten Kursangebots.

## Anwendungsfall

Das Skript eignet sich fuer Uebungen zu funktionalen Abhaengigkeiten, Kandidatenschluesseln und ersten Normalisierungsentscheidungen. Besonders hilfreich ist es, wenn Unterricht oder Review nicht nur stabile Zuordnungen sehen sollen, sondern auch bewusst eine Abhaengigkeit betrachten moechten, die nach einem erweiterten Schluessel verlangt.
...
|---|---|---|---|
| `@ShowSourceData` | `BIT` | Nein | Gibt bei `1` die Ausgangsdaten der breiten Einschreibungsrelation aus. |
| `@OnlyStableDependencies` | `BIT` | Nein | Filtert bei `1` das Arbeitsblatt auf stabil wirkende Abhaengigkeiten. |
...
- `supports_fd` bedeutet hier, dass jeder beobachtete Determinantenwert genau einen abhaengigen Wert liefert.
- `needs_composite_key` markiert Kandidaten, bei denen mindestens ein Determinantenwert auf mehrere abhaengige Werte zeigt.
- Das dritte Resultset liefert bewusst textuelle Modellierungsfragen, damit aus den Kennzahlen direkt eine Normalisierungsdiskussion entstehen kann.
...
|---|---|---|---|
| `1.0` | `2026-04-16` | `ER` | Erstversion des didaktischen Arbeitsblatts fuer funktionale Abhaengigkeiten |
...
flowchart TD
    ...
    C -->|Nein| E[Kandidaten fuer Determinanten und Dependenzen laden]
    D --> E
    E --> F[Per CROSS APPLY Determinant- und Dependent-Werte je Zeile ableiten]
    F --> G[Distinct-Dependents je Determinantenwert zaehlen]
    G --> H[Worksheet mit stabilen und mehrdeutigen Zuordnungen fuellen]
    H --> I{OnlyStableDependencies = 1?}
    I -->|Ja| J[Nur stabile Arbeitsblattzeilen ausgeben]
    I -->|Nein| K[Komplettes Arbeitsblatt ausgeben]
    J --> L[Modellierungsfragen fuer Zerlegung ausgeben]
    K --> L
```
<!-- SQLDOC:MERMAID:END -->
