# ThirdNormalFormViolationExamples.sql

Dieses Skript zeigt drei realistisch wirkende Tabellenmodelle, in denen Nichtschluesselattribute weitere Nichtschluesselattribute bestimmen. Die Auswertung macht damit typische 3NF-Verletzungen sichtbar und leitet direkt geeignete Zielrelationen fuer eine Zerlegung ab.
...
| Feld | Wert |
|---|---|
| Script | [ThirdNormalFormViolationExamples.sql](ThirdNormalFormViolationExamples.sql) |
| Version | `1.0` |
| Typ | `didactic-lab` |
| Kapitel | `01_Normalformen` |
| Sicherheit | `read-only-tempdb` |
| Zweck | Bewertet typische transitive Abhaengigkeiten in mehreren Beispielszenarien. |
<!-- SQLDOC:SUMMARY_TABLE:END -->

## Einordnung

Im Unterschied zu einer allgemeinen Normalformdefinition arbeitet dieses Skript mit vorbereiteten Szenarien aus Auftrags-, Einschreibungs- und Asset-Kontexten. Dadurch wird konkret sichtbar, dass 3NF nicht nur von Schluesseln abhaengt, sondern auch davon, ob Nichtschluesselattribute ihrerseits weitere Beschreibungsattribute bestimmen.

## Annahmen

- Es handelt sich um eine didaktische Erstversion mit Demo-Daten in `tempdb`.
- `PostalCode`, `InstructorCode` und `LocationCode` werden bewusst als stabile Nichtschluessel-Determinanten modelliert.
- Die Primaerschluessel `OrderID`, `EnrollmentID` und `AssignmentID` repraesentieren Bewegungsdaten, waehrend Orts-, Lehrenden- und Standortinformationen als auslagerbare Stammdaten betrachtet werden.
- Die vorgeschlagenen Zielrelationen dienen als fachlich plausible Normalisierungsrichtung und nicht als einzig moegliches Produktionsmodell.

## Anwendungsfall

Das Skript eignet sich fuer Unterricht, Architektur-Reviews und Uebungen, in denen transitive Abhaengigkeiten nicht nur benannt, sondern anhand konkreter Zeilen nachvollzogen werden sollen. Besonders hilfreich ist es, wenn aus der Diagnose direkt ein moeglicher Refactoring-Schritt in Richtung 3NF abgeleitet werden soll.
...
|---|---|---|---|
| `@ShowSourceData` | `BIT` | Nein | Gibt bei `1` die didaktischen Ausgangszeilen der Beispielszenarien aus. |
| `@OnlyViolations` | `BIT` | Nein | Filtert bei `1` das Analyse-Resultset auf erkannte 3NF-Verletzungen. |
...
- `transitive_violation` bedeutet hier, dass der vorbereitete Nichtschluessel-Determinant in den Demo-Daten stabil genau einen weiteren Attributwert bestimmt.
- `review_dependency` bleibt als defensiver Status im Skript, falls ein spaeter erweitertes Beispieldaten-Set Mehrdeutigkeiten erzeugt.
- Das letzte Resultset reduziert die Diagnose auf konkrete Zielrelationen, die bei einer Zerlegung zuerst eingefuehrt werden koennen.
...
|---|---|---|---|
| `1.0` | `2026-04-16` | `ER` | Erstversion der didaktischen Beispielsammlung fuer 3NF-Verletzungen |
...
```mermaid
flowchart TD
    A[Parameter validieren] --> B[Didaktische Beispielzeilen in tempdb aufbauen]
    B --> C{ShowSourceData = 1?}
    C -->|Ja| D[Ausgangszeilen je Szenario ausgeben]
    C -->|Nein| E[Transitive Kandidaten vorbereiten]
    D --> E
    E --> F[Determinant- und Dependent-Werte per CROSS APPLY ableiten]
    F --> G[Distinct-Dependents je Determinantenwert zaehlen]
    G --> H[3NF-Risiko je Szenario bewerten]
    H --> I{OnlyViolations = 1?}
    I -->|Ja| J[Nur transitive Verletzungen ausgeben]
    I -->|Nein| K[Vollstaendige Bewertung ausgeben]
    J --> L[Zielrelationen fuer die Normalisierung ausgeben]
    K --> L
```
<!-- SQLDOC:MERMAID:END -->
