# DetectPartialDependencies.sql

Dieses Skript illustriert auf einer didaktischen Einschreibungsrelation, wie partielle Abhaengigkeiten zu 2NF-Verletzungen fuehren koennen. Die SQL-Datei verwendet einen zusammengesetzten Schluessel aus `StudentNo`, `CourseCode` und `TermCode` und prueft anschliessend, welche Attribute bereits durch echte Teilmengen dieses Schluessels bestimmt werden.
...
| Feld | Wert |
|---|---|
| Script | [DetectPartialDependencies.sql](DetectPartialDependencies.sql) |
| Version | `1.0` |
| Typ | `didactic-lab` |
| Kapitel | `01_Normalformen` |
| Sicherheit | `read-only-tempdb` |
| Zweck | Einstieg in partielle Abhaengigkeiten und 2NF-Verletzungen auf Demo-Daten. |
<!-- SQLDOC:SUMMARY_TABLE:END -->

## Einordnung

Die Ausgangsrelation mischt Lernenden-, Kurs- und Terminattribute direkt in der Einschreibung. Dadurch entstehen bewusst Attribute, die nicht vom vollstaendigen Schluessel, sondern nur von einem einzelnen Schluesselteil abhaengen. Genau diese Situation wird im Audit fuer die 2NF-Diskussion sichtbar gemacht.

## Annahmen

- Es handelt sich um eine didaktische Erstversion ohne produktive Quelltabellen.
- Der zusammengesetzte Einschreibeschluessel wird als `StudentNo + CourseCode + TermCode` modelliert.
- `StudentName` und Department-Daten haengen nur von `StudentNo` ab.
- Kursattribute haengen nur von `CourseCode` ab, waehrend `EnrollmentStatus` als Merkmal der gesamten Belegung behandelt wird.

## Anwendungsfall

Das Skript eignet sich fuer Unterricht, Review und Whiteboard-Erklaerungen rund um die zweite Normalform. Es zeigt nicht nur erkannte partielle Abhaengigkeiten, sondern liefert auch eine kleine Liste moeglicher Zerlegungsschritte fuer eine 2NF-naehere Struktur.
...
|---|---|---|---|
| `@ShowSourceData` | `BIT` | Nein | Gibt bei `1` die didaktischen Ausgangsdaten aus. |
| `@OnlyViolations` | `BIT` | Nein | Zeigt bei `1` nur erkannte partielle Abhaengigkeiten. |
...
- `partial_dependency` bedeutet hier: Die Demo-Daten tragen eine stabile Abhaengigkeit, obwohl der Determinant nur ein echter Teil des zusammengesetzten Schluessels ist.
- `full_key_dependency` markiert im Ergebnis ein Merkmal, das didaktisch an der gesamten Belegung haengt.
- Die Refactor-Hinweise sind bewusst einfach gehalten und sollen als Diskussionsgrundlage dienen, nicht als fertiges physisches Datenmodell.
...
|---|---|---|---|
| `1.0` | `2026-04-16` | `ER` | Erstversion des didaktischen Partial-Dependency-Checks |
...
flowchart TD
    ...
    C -->|Nein| E[Audit vorbereiten]
    D --> E
    E --> F[StudentNo gegen Stammdaten pruefen]
    F --> G[CourseCode gegen Kursattribute pruefen]
    G --> H[TermCode gegen Terminbezeichnung pruefen]
    H --> I[Vollen Schluessel gegen EnrollmentStatus pruefen]
    I --> J{OnlyViolations = 1?}
    J -->|Ja| K[Nur partielle Abhaengigkeiten ausgeben]
    J -->|Nein| L[Komplettes Audit ausgeben]
    K --> M[2NF-Refactor-Hinweise ausgeben]
    L --> M
```
<!-- SQLDOC:MERMAID:END -->
