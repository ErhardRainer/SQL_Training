# NormalForms1NFTo6NF.sql

Dieses Labor zeigt mit ausschließlich lokalen temporären Tabellen, wie Daten schrittweise normalisiert werden. Die ersten drei Stufen verwenden eine durchgängige Kursanmeldung; BCNF sowie 4NF bis 6NF ergänzen realistische Spezialfälle, für die das Ausgangsbeispiel allein nicht ausreicht.
...
| Feld | Wert |
|---|---|
| Script | [NormalForms1NFTo6NF.sql](NormalForms1NFTo6NF.sql) |
| Version | `1.0` |
| Typ | `didactic-lab` |
| Kapitel | `01_Normalformen` |
| Sicherheit | `read-only-tempdb` |
| Zweck | Durchgängiges Lernlabor von 1NF bis 6NF. |
<!-- SQLDOC:SUMMARY_TABLE:END -->

## Einordnung und Annahmen

- Die Tabellen beginnen mit `#` und verschwinden mit Ende der SQL-Server-Sitzung.
- 1NF ist nicht „unnormalisiert“: Die Ausgangsrelation ist bewusst atomar, aber noch redundant.
- 3NF ist für viele transaktionale Systeme das praktische Ziel. BCNF, 4NF, 5NF und 6NF lösen spezielle Abhängigkeiten und werden deshalb als eigene Fallbeispiele gezeigt.
- Die 5NF-Zerlegung ist nur korrekt, wenn die im Script kommentierte Fachregel tatsächlich gilt. Ohne diese Regel könnten durch Joins falsche Tripel entstehen.
- Das 6NF-Beispiel ist temporal: Rollen- und Kostenstellenänderungen dürfen unabhängig voneinander historisiert werden.

## Ausführung

Das Skript in SSMS oder Azure Data Studio ausführen. Mit `@ShowIntermediateData = 1` erscheinen zusätzlich die wichtigen Zwischenrelationen. Für einen kompakten Lauf den Wert auf `0` setzen.
...
|---|---|---|---|
| `@ShowIntermediateData` | `BIT` | Nein | Gibt bei `1` die Datenvorschau der einzelnen Normalformstufen aus. |
<!-- SQLDOC:PARAMETERS_TABLE:END -->

## Abhängigkeiten

<!-- SQLDOC:DEPENDENCIES_LIST:BEGIN -->
- SQL Server 2016 SP2 oder neuer für `DROP TABLE IF EXISTS`
- lokale temporäre Tabellen in `tempdb`
- `PRIMARY KEY`, `UNIQUE` und Standard-`SELECT`
...
```mermaid
flowchart TD
    A[Parameter prüfen] --> B[1NF: atomare Anmeldungsrelation erstellen]
    B --> C[2NF: Teilabhängigkeiten auslagern]
    C --> D[3NF: transitive Abhängigkeiten auslagern]
    D --> E[BCNF: Student-Dozent und Dozent-Kurs zerlegen]
    E --> F[4NF: Sprachen und Kompetenzen entkoppeln]
    F --> G[5NF: Lieferant-Teil-Projekt in Paarrelationen zerlegen]
    G --> H[6NF: zeitliche Rollen- und Kostenstellenfakten trennen]
    H --> I[Zusammenfassung ausgeben]
    I --> J{Zwischendaten anzeigen?}
    J -->|Ja| K[Relationen je Stufe ausgeben]
    J -->|Nein| L[Ende]
    K --> L
```
<!-- SQLDOC:MERMAID:END -->
...
|---|---|---|---|
| `1.0` | `2026-08-17` | `ER` | Erstversion des durchgängigen Normalformen-Labors |
