# DetectRepeatingGroups.sql

Dieses Skript zeigt an einer bewusst breiten Workshop-Registrierung, wie nummerierte Spaltenfamilien als moegliche Wiederholungsgruppen erkannt werden koennen. Die SQL-Datei sammelt Werte aus `Phone1..3`, `SkillTag1..3` und `Session1..2`, bewertet deren Mehrfachbelegung und liefert daraus einen sauberen Einstieg in die 1NF-Diskussion.
...
| Feld | Wert |
|---|---|
| Script | [DetectRepeatingGroups.sql](DetectRepeatingGroups.sql) |
| Version | `1.0` |
| Typ | `didactic-lab` |
| Kapitel | `01_Normalformen` |
| Sicherheit | `read-only-tempdb` |
| Zweck | Findet nummerierte Spaltenfamilien als moegliche Wiederholungsgruppen fuer die 1NF-Diskussion. |
<!-- SQLDOC:SUMMARY_TABLE:END -->

## Einordnung

Die Demo-Tabelle modelliert mehrere Telefonnummern, Skill-Tags und Session-Zuordnungen direkt in einer einzigen Zeile. Genau diese Struktur ist didaktisch interessant, weil sie eine tabellarische Form mit nummerierten Mehrfachspalten erzeugt und damit ein klassisches Signal fuer moegliche Wiederholungsgruppen liefert.

## Annahmen

- Es handelt sich um eine didaktische Erstversion ohne produktive Quelltabellen.
- Nummerierte Spaltenfamilien wie `Phone1..3` oder `SkillTag1..3` werden als bewusstes Gegenbeispiel zur 1NF verwendet.
- Die Normalisierungs-Vorschau zeigt nur eine plausible Zielrichtung mit Kindtabellen und keinen produktiven Migrationsablauf.

## Anwendungsfall

Das Skript eignet sich fuer Unterricht, Review und Modellierungsdiskussionen rund um die erste Normalform. Es hilft dabei, aus einer breiten Ausgangsrelation eine zeilenorientierte Sicht auf wiederholbare Werte abzuleiten und daraus moegliche Auslagerungen in eigene Relationen zu besprechen.
...
|---|---|---|---|
| `@ShowSourceData` | `BIT` | Nein | Gibt bei `1` die breite Demo-Tabelle vor der Auswertung aus. |
| `@IncludeNormalizedPreview` | `BIT` | Nein | Gibt bei `1` eine zeilenorientierte Vorschau der ausgelagerten Werte aus. |
...
- `repeating_group_likely` bedeutet hier, dass in mindestens einer Registrierung mehrere Slots derselben Spaltenfamilie belegt sind.
- Auch ein Status `single_value_per_row` entwarnt die Modellierung nicht vollstaendig, weil nummerierte Slots weiterhin eine potenziell unflexible Tabellenform anzeigen.
- Die vorgeschlagenen Kindtabellen sind absichtlich einfach gehalten, damit die 1NF-Idee im Vordergrund bleibt.
...
|---|---|---|---|
| `1.0` | `2026-04-16` | `ER` | Erstversion des didaktischen Repeating-Group-Audits |
...
```mermaid
flowchart TD
    A[Parameter validieren] --> B[Demo-Registrierungen in tempdb aufbauen]
    B --> C{ShowSourceData = 1?}
    C -->|Ja| D[Breite Ausgangsdaten ausgeben]
    C -->|Nein| E[Werte per CROSS APPLY entpivotieren]
    D --> E
    E --> F[Belegte Slots je Spaltenfamilie sammeln]
    F --> G[Audit ueber Mehrfachbelegung und Distinct-Werte berechnen]
    G --> H[RepeatingGroupAudit ausgeben]
    H --> I{IncludeNormalizedPreview = 1?}
    I -->|Ja| J[Zeilenorientierte Vorschau ausgeben]
    I -->|Nein| K[Normalisierungshinweise vorbereiten]
    J --> K
    K --> L[Didaktische Zielrelationen ausgeben]
```
<!-- SQLDOC:MERMAID:END -->
