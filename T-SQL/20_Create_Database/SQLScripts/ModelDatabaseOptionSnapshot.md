# ModelDatabaseOptionSnapshot.sql

Dieses Skript erstellt einen lesenden Schnappschuss der wichtigsten Optionen der Datenbank `model`. Damit laesst sich vor einem `CREATE DATABASE` schnell nachvollziehen, welche Baseline-Einstellungen neue Datenbanken typischerweise erben.
...
| Feld | Wert |
|---|---|
| Script | [ModelDatabaseOptionSnapshot.sql](ModelDatabaseOptionSnapshot.sql) |
| Version | `1.0` |
| Typ | `diagnostic-query` |
| Kapitel | `20_Create_Database` |
| Sicherheit | `read-only` |
| Zweck | Erstellt einen Snapshot der model-Datenbankoptionen und optional des Dateilayouts als Baseline fuer neue Datenbanken. |
<!-- SQLDOC:SUMMARY_TABLE:END -->

## Einordnung

Die Datenbank `model` liefert viele Standardwerte fuer neue SQL-Server-Datenbanken. Ein kompakter Snapshot hilft daher, geerbte Einstellungen wie Recovery Model, Statistikoptionen, Session-nahe Datenbankflags und Dateiwachstum vorab sichtbar zu machen.

## Annahmen

- Das Skript wird auf einer SQL-Server-Instanz mit lesendem Zugriff auf `sys.databases` und `sys.master_files` ausgefuehrt.
- `model` ist online und stellt die relevante Standardvorlage fuer neue Datenbanken dar.
- Das Skript dokumentiert eine Baseline und trifft keine Aussage darueber, ob einzelne Optionen in einer Zielumgebung bereits optimal sind.

## Anwendungsfall

Die erste Ausgabe zeigt eine verdichtete Optionssicht fuer die gewaehlte Datenbank. Wenn `@IncludeFileLayout = 1` gesetzt ist, folgt eine zweite Ausgabe mit Dateinamen, Groessen und Growth-Einstellungen, damit `CREATE DATABASE`-Defaults fuer Daten- und Logdateien transparent werden.
...
|---|---|---|---|
| `@DatabaseName` | `SYSNAME` | Nein | Zieldatenbank fuer den Snapshot; standardmaessig `model`. |
| `@IncludeFileLayout` | `BIT` | Nein | Zeigt bei `1` zusaetzlich Dateilayout und Growth-Infos an. |
...
- `SnapshotScope` markiert, ob wirklich die Standardvorlage `model` oder testweise eine andere Datenbank betrachtet wurde.
- `IsReadCommittedSnapshotOn`, `IsQuotedIdentifierOn` und verwandte Flags eignen sich gut als Checkliste vor standardisierten Datenbank-Neuanlagen.
- Die Dateiausgabe zeigt Growth-Konfigurationen bereits in lesbarer Form, damit MB- und Prozentwachstum ohne Zusatzrechnungen erkennbar bleiben.
...
|---|---|---|---|
| `1.0` | `2026-04-18` | `ER` | Erstversion fuer Baseline-Snapshot der model-Datenbankoptionen |
...
```mermaid
flowchart TD
    A[Parameter setzen] --> B{Datenbank vorhanden?}
    B -->|Nein| C[THROW unbekannte Datenbank]
    B -->|Ja| D{IncludeFileLayout 0 oder 1?}
    D -->|Nein| E[THROW ungueltiger Parameter]
    D -->|Ja| F{Datenbank ONLINE?}
    F -->|Nein| G[THROW Datenbank nicht online]
    F -->|Ja| H[Temp-Tabelle fuer Options-Snapshot vorbereiten]
    H --> I[Optionen aus sys.databases und DATABASEPROPERTYEX laden]
    I --> J[Resultset DatabaseOptionSnapshot ausgeben]
    J --> K{IncludeFileLayout = 1?}
    K -->|Nein| L[Ende]
    K -->|Ja| M[Dateien aus sys.master_files lesen]
    M --> N[Resultset DatabaseFileLayout ausgeben]
```
<!-- SQLDOC:MERMAID:END -->
