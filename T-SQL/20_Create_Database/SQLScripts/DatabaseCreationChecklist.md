# DatabaseCreationChecklist.sql

Dieses Skript erzeugt eine lesende Checkliste fuer `CREATE DATABASE`-Vorhaben. Es kombiniert model- und Instanzmetadaten mit einer generierten Befehlsvorlage, damit Namen, Collation, Dateipfade und Nachkontrollen vor der eigentlichen Anlage bewusst geplant werden koennen.
...
| Feld | Wert |
|---|---|
| Script | [DatabaseCreationChecklist.sql](DatabaseCreationChecklist.sql) |
| Version | `1.0` |
| Typ | `template` |
| Kapitel | `20_Create_Database` |
| Sicherheit | `read-only` |
| Zweck | Erstellt eine lesende Checkliste fuer neue Datenbanken inklusive Dateibasis und generierter CREATE-DATABASE-Vorlage. |
<!-- SQLDOC:SUMMARY_TABLE:END -->

## Einordnung

Beim Anlegen neuer Datenbanken ist nicht nur der Befehl selbst relevant, sondern auch die Vorbereitung: Namenskonventionen, Collation, Dateiverteilung, Growth-Einstellungen, Owner und die erste Verifikation direkt nach dem Anlegen. Dieses Skript verdichtet diese Punkte in einer didaktischen Checkliste und bleibt dabei rein lesend.

## Annahmen

- Das Skript wird auf einer SQL-Server-Instanz mit Zugriff auf `sys.databases`, `sys.master_files`, `sys.fn_helpcollations()` und `SERVERPROPERTY` ausgefuehrt.
- Wenn keine Zielpfade uebergeben werden, stammen die Empfehlungen aus `InstanceDefaultDataPath` beziehungsweise `InstanceDefaultLogPath` oder aus dem Dateipfad der `model`-Datenbank.
- Die generierte `CREATE DATABASE`-Vorlage ist ein konservativer Startpunkt und ersetzt keine produktive Groessen-, Sicherheits- oder Verfuegbarkeitsplanung.

## Anwendungsfall

Das erste Resultset liefert eine priorisierte Checkliste fuer den Planungs- und Review-Prozess. Optional zeigt das zweite Resultset die aus `model` abgeleitete Datei-Baseline. Das dritte Resultset stellt eine direkt lesbare `CREATE DATABASE`-Vorlage sowie eine kurze Verifikationsabfrage fuer die Nachkontrolle bereit.
...
|---|---|---|---|
| `@TargetDatabaseName` | `SYSNAME` | Nein | Name der neu anzulegenden Datenbank fuer Checkliste und Befehlsvorlage. |
| `@TargetCollation` | `SYSNAME` | Nein | Optionale Ziel-Collation; `NULL` uebernimmt die `model`-Collation. |
| `@DataFileDirectory` | `NVARCHAR(260)` | Nein | Optionales Zielverzeichnis fuer Datendateien; ohne Wert wird der Instanz- oder `model`-Fallback genutzt. |
| `@LogFileDirectory` | `NVARCHAR(260)` | Nein | Optionales Zielverzeichnis fuer Logdateien; ohne Wert wird der Instanz- oder `model`-Fallback genutzt. |
| `@IncludeModelFileBaseline` | `BIT` | Nein | Zeigt bei `1` die Datei- und Growth-Baseline aus `model` an. |
...
- Die Checkliste trennt bewusst zwischen Planungsbereichen wie Naming, Files, Options und Verification.
- Die Dateibasis orientiert sich an `model`, damit `CREATE DATABASE`-Vorhaben nicht nur an Syntax, sondern auch an Speicher- und Growth-Defaults gespiegelt werden.
- Die generierte Vorlage fuehrt keine Aktion aus; sie kann vor produktiver Nutzung gezielt um Filegroups, Groessen oder weitere Optionen erweitert werden.
...
|---|---|---|---|
| `1.0` | `2026-04-22` | `ER` | Erstversion der lesenden Checkliste fuer CREATE DATABASE inklusive Datei- und Optionsvorlage |
...
```mermaid
flowchart TD
    A[Parameter validieren] --> B[model-Collation und Instanz-Defaultpfade lesen]
    B --> C[Ziel-Collation aus Parameter oder model ableiten]
    C --> D{Collation in fn_helpcollations vorhanden?}
    D -->|Nein| E[THROW ungueltige TargetCollation]
    D -->|Ja| F[model-Dateien und Growth-Basis aus sys.master_files laden]
    F --> G{Baseline fuer model-Dateien vorhanden?}
    G -->|Nein| H[THROW fehlende model-Dateibasis]
    G -->|Ja| I[priorisierte CREATE-DATABASE-Checkliste aufbauen]
    I --> J[CREATE DATABASE Vorlage und Verifikationsabfrage generieren]
    J --> K[CreationChecklist ausgeben]
    K --> L{IncludeModelFileBaseline = 1?}
    L -->|Ja| M[ModelFileBaseline ausgeben]
    L -->|Nein| N[Datei-Baseline ueberspringen]
    M --> O[CreateDatabaseTemplate ausgeben]
    N --> O
```
<!-- SQLDOC:MERMAID:END -->
