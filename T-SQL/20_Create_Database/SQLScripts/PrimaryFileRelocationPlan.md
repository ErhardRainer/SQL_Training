# PrimaryFileRelocationPlan.sql

Dieses Skript erstellt einen lesenden Plan fuer die Verlagerung von Dateien im `PRIMARY`-Filegroup in strukturierte Pfade. Es leitet einen Zielordner aus Umgebung, Anwendung und Datenbankname ab, bewertet aktuelle oder modellierte PRIMARY-Dateien und erzeugt daraus eine Preflight-Checkliste sowie didaktische `ALTER DATABASE`-, Offline- und Verifikationsvorlagen.
...
| Feld | Wert |
|---|---|
| Script | [PrimaryFileRelocationPlan.sql](PrimaryFileRelocationPlan.sql) |
| Version | `1.0` |
| Typ | `template` |
| Kapitel | `20_Create_Database` |
| Sicherheit | `read-only` |
| Zweck | Plant die Verlagerung von PRIMARY-Dateien in strukturierte Pfade und erzeugt passende Guardrails und Befehlsvorlagen. |
<!-- SQLDOC:SUMMARY_TABLE:END -->

## Einordnung

Die physische Ablage von PRIMARY-Dateien bleibt in vielen Umgebungen historisch gewachsen. Fuer Reviews rund um `CREATE DATABASE`, Storage-Konventionen oder spaetere Reorganisationen ist es deshalb hilfreich, einen Zielpfad explizit aus wenigen stabilen Tokens abzuleiten: Root-Verzeichnis, Umgebung, Anwendung, Datenbankname und ein klarer `primary`-Unterordner.

## Annahmen

- Es handelt sich um eine didaktische Template-Erstversion fuer Planung und Review, nicht um ein Ausfuehrungsskript.
- Ohne explizites Root-Verzeichnis wird der Zielpfad aus `InstanceDefaultDataPath` oder einem `model`-Fallback abgeleitet.
- Bei `@UseDemoBaseline = 1` arbeitet das Skript mit modellierten Altpfaden, damit die Relocation-Logik ohne produktive Datenbank demonstrierbar bleibt.
- Die physische Verlagerung selbst, Betriebsfreigaben und Sonderfaelle wie Cluster, AGs oder Systemdatenbanken muessen ausserhalb dieses Skripts bewertet werden.

## Anwendungsfall

Das Skript eignet sich fuer Betriebs- und Architektur-Reviews, wenn bestehende PRIMARY-Dateien in eine sauberere Ordnerstruktur ueberfuehrt oder neue Pfadstandards dokumentiert werden sollen. Es verbindet Katalogsicht, Zielpfadlogik und Umzugs-Checkliste in einer Form, die spaeter in konkrete Betriebsrunbooks uebernommen werden kann.
...
|---|---|---|---|
| `@TargetDatabaseName` | `SYSNAME` | Nein | Datenbank, deren PRIMARY-Dateien geplant oder aus Demo-Daten modelliert werden. |
| `@EnvironmentCode` | `NVARCHAR(20)` | Nein | Kurzes Umgebungskennzeichen wie `DEV`, `TEST` oder `PROD` fuer den Zielpfad. |
| `@ApplicationCode` | `NVARCHAR(30)` | Nein | Optionales Domain- oder Anwendungstoken fuer den Zielpfad. |
| `@PrimaryRootDirectory` | `NVARCHAR(260)` | Nein | Optionales Root-Verzeichnis fuer PRIMARY-Dateien; `NULL` nutzt Instanz-Defaults oder `model`-Fallback. |
| `@UseDemoBaseline` | `BIT` | Nein | Verwendet bei `1` modellierte Demo-Dateien statt echter Katalogdaten. |
| `@IncludeSecondaryReference` | `BIT` | Nein | Zeigt bei `1` zusaetzliche Nicht-PRIMARY-Dateien als Kontext im Baseline-Aufbau. |
...
- `PrimaryFileBaseline` beschreibt die aktuellen oder modellierten PRIMARY-Dateien mit Pfadbeobachtung und Growth-Kontext.
- `RelocationPlan` leitet pro PRIMARY-Datei einen strukturierten Zielordner und einen didaktischen Dateiumzugsbefehl ab.
- `PreflightChecklist` sammelt die betriebsrelevanten Guardrails vor dem Umzug.
- `CommandTemplate` erzeugt Vorlagen fuer `ALTER DATABASE ... MODIFY FILE`, ein kontrolliertes Offline-Fenster und die Nachverifikation.
...
|---|---|---|---|
| `1.0` | `2026-04-22` | `ER` | Erstversion der lesenden Planungs- und Vorlagenlogik fuer PRIMARY-Dateiverlagerungen |
...
```mermaid
flowchart TD
    A[Parameter validieren] --> B[PrimaryRoot aus Parameter, Instanzdefault oder model ableiten]
    B --> C{UseDemoBaseline = 1?}
    C -->|Ja| D[Didaktische Demo-Dateien in CurrentFiles laden]
    C -->|Nein| E[PRIMARY-Dateien und optional Referenzen aus sys.master_files laden]
    D --> F[PrimaryFileBaseline mit Pfadbeobachtungen fuellen]
    E --> F
    F --> G[RelocationPlan mit target primary-Ordner und Zieldateinamen ableiten]
    G --> H[PreflightChecklist fuer Backup, Downtime, Berechtigungen und Nachkontrolle fuellen]
    H --> I[ALTER DATABASE-, Offline- und Verifikationsvorlagen generieren]
    I --> J[PrimaryFileBaseline ausgeben]
    J --> K[RelocationPlan ausgeben]
    K --> L[PreflightChecklist ausgeben]
