# DatabaseShrinkCandidateReview.sql

Dieses Skript hilft in einer Plattenplatz-Notlage dabei, die sinnvollsten Shrink-Kandidaten auf einem betroffenen Laufwerk oder Volume zu finden. Es priorisiert Logdateien mit realistischem Rueckgewinn vor Datenfiles, weil Datenfile-Shrinks deutlich haeufiger Folgekosten wie Fragmentierung und neues Autogrowth erzeugen.
...
| Feld | Wert |
|---|---|
| Script | [DatabaseShrinkCandidateReview.sql](DatabaseShrinkCandidateReview.sql) |
| Version | `1.1` |
| Typ | `diagnostic-query` |
| Kapitel | `20_Create_Database` |
| Sicherheit | `read-only-tempdb` |
| Zweck | Bewertet serverweit, welche Datenbankdateien auf einem betroffenen Laufwerk oder Volume realistische Shrink-Kandidaten sein koennen. Das Skript trennt Logdateien von Datenfiles, berechnet freies Potential und erzeugt vorsichtige DBCC-SHRINKFILE-Vorlagen ohne sie auszufuehren. |
<!-- SQLDOC:SUMMARY_TABLE:END -->

## Einordnung

Das Skript fuehrt einen lesenden Scan ueber alle online verfuegbaren Datenbanken aus, die mindestens eine Datei auf dem angegebenen Zielvolume besitzen. Fuer Datenfiles wird `FILEPROPERTY(..., 'SpaceUsed')` datenbanklokal ausgewertet. Fuer Logs werden `sys.dm_db_log_space_usage` und, soweit verfuegbar, `sys.dm_db_log_info` genutzt, damit nicht nur die physische Loggroesse, sondern auch der aktive VLF-Bereich beruecksichtigt wird.

## Annahmen

- Das Skript fuehrt keinen Shrink aus, sondern liefert Priorisierung und Command-Vorlagen.
- Nur Dateien, deren `physical_name` mit `@TargetVolumeRoot` beginnt, gehen in die Potentialberechnung ein.
- Logdateien sind in akuten Speicherplatzlagen oft die bessere erste Pruefung, wenn der `log_reuse_wait_desc` geloest ist.
- Datenfile-Shrinks sind als letzter Schritt gedacht und sollten gezielt pro Datei erfolgen.
- Die Zielgroesse bleibt bewusst oberhalb der genutzten Groesse und laesst einen konfigurierbaren Puffer.
- Bei mehreren Logdateien ist die Bewertung konservativ zu lesen, weil die DB-weite Lognutzung nicht exakt auf einzelne Logdateien verteilt werden kann.
...
|---|---|---|---|
| `@TargetVolumeRoot` | `NVARCHAR(260)` | Ja | Betroffenes Laufwerk oder Volume-Prefix, z. B. D:\ oder L:\MSSQLData\. |
...
| `@IncludeSystemDatabases` | `BIT` | Nein | 1 = master, model, msdb und tempdb in die Analyse einbeziehen. |
| `@OnlyShowCandidates` | `BIT` | Nein | 1 = nur Datenbanken mit Kandidaten oder Scanfehlern ausgeben. |
...
- `immediate-log-shrink-review` ist der beste Notfallpfad: viel freier Logspace und kein blockierender Reuse-Wait.
- `TargetVolumeRoot` zeigt in den Resultsets, auf welches Laufwerk oder Volume sich die Bewertung bezieht.
- `fix-log-reuse-first` bedeutet: erst Logbackup, offene Transaktion, Replikation, HA oder andere Reuse-Waits klaeren.
- `datafile-last-resort` ist bewusst defensiv; Datenfile-Shrink sollte nicht zur Dauerstrategie werden.
- Die generierten `DBCC SHRINKFILE`-Vorlagen sind Startpunkte fuer Review, keine automatische Freigabe.
...
|---|---|---|---|
| `1.1` | `2026-05-11` | `ER` | Ergaenzt Zielvolume-Parameter und filtert Daten- und Logdateien nach physical_name-Prefix |
| `1.0` | `2026-05-11` | `ER` | Erstversion des Shrink-Kandidaten-Reviews fuer Plattenplatz-Notlagen |
...
```mermaid
flowchart TD
    A[Parameter und TargetVolumeRoot validieren] --> B[Volume-Prefix normalisieren]
    B --> C[Temp-Tabellen fuer Datenfiles Logs VLFs Kandidaten und Fehler anlegen]
    C --> D[Online-Datenbanken mit Dateien auf Zielvolume per Cursor durchlaufen]
    D --> E[Je Datenbank Datenfile-Nutzung auf Zielvolume mit FILEPROPERTY lesen]
    D --> F[Je Datenbank Lognutzung mit sys.dm_db_log_space_usage lesen]
    D --> G[Je Datenbank aktives VLF-Ende mit sys.dm_db_log_info lesen]
    E --> H[Datenfile-Kandidaten auf Zielvolume berechnen]
    F --> I[Logfile-Kandidaten auf Zielvolume berechnen]
    G --> I
    H --> J[Potentiale fuer Zielvolume aggregieren]
    I --> J
    J --> K[DatabaseShrinkPriority nach sinnvollstem Notfallpfad sortieren]
    K --> L[Optionale DBCC-SHRINKFILE-Vorlagen ausgeben]
    L --> M[ScanErrors transparent ausgeben]
```
<!-- SQLDOC:MERMAID:END -->
