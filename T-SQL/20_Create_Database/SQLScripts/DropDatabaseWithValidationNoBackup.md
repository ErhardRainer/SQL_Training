# DropDatabaseWithValidationNoBackup.sql

Dieses Skript loescht eine SQL-Server-Datenbank kontrolliert ohne vorheriges Backup. Es sammelt vorab Metadaten, Dateien, aktive Sitzungen und Snapshots, fuehrt bei expliziter Freigabe `DROP DATABASE` aus und prueft danach Katalogeintraege sowie die zuvor ermittelten physischen Dateien.
...
| Feld | Wert |
|---|---|
| Script | [DropDatabaseWithValidationNoBackup.sql](DropDatabaseWithValidationNoBackup.sql) |
| Version | `1.0` |
| Typ | `admin-change` |
| Kapitel | `20_Create_Database` |
| Sicherheit | `admin-change` |
| Zweck | Kontrolliertes Loeschen einer Datenbank ohne Backup-Schritt, aber mit Preflight und Nachkontrolle. |
<!-- SQLDOC:SUMMARY_TABLE:END -->

## Einordnung

Diese Variante ist fuer Situationen gedacht, in denen bewusst kein neues Backup erstellt werden soll. Damit das nicht versehentlich passiert, blockiert das Skript beim echten Loeschen, solange `@AcknowledgeNoBackup` nicht auf `1` gesetzt wurde. Der Standardwert `@ExecuteDrop = 0` fuehrt nur die Pruefungen aus.

## Annahmen

- Das Skript wird in SQL Server beziehungsweise einer kompatiblen T-SQL-Umgebung mit ausreichenden Rechten fuer `ALTER DATABASE` und `DROP DATABASE` ausgefuehrt.
...
- Es soll bewusst kein neues Backup im Skript erstellt werden.
...
Der typische Ablauf ist zweistufig: Zuerst die Parameter setzen und mit `@ExecuteDrop = 0` die Resultsets lesen. Danach denselben Lauf mit `@ExecuteDrop = 1` und `@AcknowledgeNoBackup = 1` wiederholen, wenn Zielname, aktive Sitzungen, Snapshot-Situation und No-Backup-Entscheidung bewusst akzeptiert sind.
...
| `@ExecuteDrop` | `BIT` | Ja | `0` = nur Preflight, `1` = `DROP DATABASE` ausfuehren. |
| `@AcknowledgeNoBackup` | `BIT` | Ja | Muss beim echten Loeschen auf `1` stehen, weil dieses Skript kein Backup erstellt. |
...
- Das Skript enthaelt bewusst keinen `BACKUP DATABASE`- und keinen `RESTORE VERIFYONLY`-Schritt.
- Wenn Snapshots existieren, blockiert der Preflight, solange `@DropDatabaseSnapshots = 0` ist.
- Wenn `DROP DATABASE` nach `SINGLE_USER` fehlschlaegt und die Datenbank noch existiert, versucht der Catch-Block, `MULTI_USER` wiederherzustellen.
...
|---|---|---|---|
| `1.0` | `2026-05-12` | `ER` | Erstversion fuer kontrolliertes DROP DATABASE ohne Backup mit Preflight und Nachkontrolle |
...
```mermaid
flowchart TD
    A[Parameter vorbereiten] --> B[Ziel-DB, Dateien, Sitzungen und Snapshots erfassen]
    ...
    F -->|Nein| H[No-Backup-Warnung in PostDropChecks]
    H --> I{Snapshots loeschen?}
    I -->|Ja| J[Snapshot-Cursor: ALTER SINGLE_USER und DROP DATABASE]
    I -->|Nein| K[Snapshot-Drop ueberspringen]
    J --> L{SetSingleUserBeforeDrop = 1?}
    K --> L
    L -->|Ja| M[Ziel-DB SINGLE_USER WITH ROLLBACK IMMEDIATE]
    L -->|Nein| N[Direkt DROP vorbereiten]
    M --> O[DROP DATABASE Ziel-DB]
    N --> O
    O --> P[DB_ID, sys.master_files und Snapshot-Metadaten pruefen]
    P --> Q{CheckPhysicalFilesAfterDrop = 1?}
    Q -->|Ja| R[xp_fileexist fuer zuvor ermittelte Dateien]
    Q -->|Nein| S[Physische Pruefung als SKIPPED markieren]
    R --> T[PostDropChecks und PhysicalFileCheck ausgeben]
    S --> T
    T --> U{PostDrop-Blocker?}
    U -->|Ja| V[THROW Nachkontrolle]
    U -->|Nein| W[Fertig]
```
<!-- SQLDOC:MERMAID:END -->
...
---
sql_header: v1
script_name: "DropDatabaseWithValidationNoBackup.sql"
script_version: "1.0"
script_type: "admin-change"
chapter: "20_Create_Database"
purpose: >
  Loescht eine SQL-Server-Datenbank kontrolliert ohne vorheriges Backup,
  inklusive Preflight-Pruefungen, Metadatenkontrolle und physischer
...
parameters:
  ...
  - name: "@ExecuteDrop"
    ...
    direction: "IN"
    required: true
    description: "0 = nur Preflight ausgeben, 1 = DROP DATABASE ausfuehren"
  - name: "@AcknowledgeNoBackup"
    sql_type: "BIT"
    direction: "IN"
    required: true
    description: "Muss beim echten Loeschen auf 1 stehen, weil dieses Skript kein Backup erstellt"
...
result_sets:
  - name: "PreDropChecks"
    description: "Vorpruefungen zu Ziel-DB, Schutzschaltern, Dateien, Sitzungen, Snapshots und No-Backup-Bestaetigung"
  ...
  - name: "ExecutionPlan"
    description: "Geplante Hauptaktionen ohne Backup-Schritt"
  - name: "PostDropChecks"
    description: "Nachkontrolle von DROP DATABASE, Metadaten und Dateipruefung"
...
documentation:
  markdown_file: "T-SQL/20_Create_Database/SQLScripts/DropDatabaseWithValidationNoBackup.md"
  sync_blocks:
    - "SUMMARY_TABLE"
    - "PARAMETERS_TABLE"
    - "DEPENDENCIES_LIST"
...
version_history:
  - version: "1.0"
    date: "2026-05-12"
    user: "ER"
    description: "Erstversion fuer kontrolliertes DROP DATABASE ohne Backup mit Preflight und Nachkontrolle"

notes:
  - "Dieses Skript erstellt bewusst kein Backup vor DROP DATABASE."
...
DECLARE @AcknowledgeNoBackup BIT = 0;
...
(
    ...
    'No backup mode',
    CASE
        WHEN @ExecuteDrop = 1 AND @AcknowledgeNoBackup <> 1 THEN 'BLOCKER'
        WHEN @ExecuteDrop = 1 AND @AcknowledgeNoBackup = 1 THEN 'WARN'
        ELSE 'INFO'
    END,
    CASE
        WHEN @ExecuteDrop = 1 AND @AcknowledgeNoBackup <> 1 THEN N'Dieses Skript erstellt kein Backup. Setze @AcknowledgeNoBackup = 1, wenn das bewusst akzeptiert ist.'
        WHEN @ExecuteDrop = 1 AND @AcknowledgeNoBackup = 1 THEN N'Kein Backup-Schritt wird ausgefuehrt; die No-Backup-Variante wurde explizit bestaetigt.'
        ELSE N'Dieses Skript enthaelt bewusst keinen Backup-Schritt. Bei @ExecuteDrop = 1 muss @AcknowledgeNoBackup = 1 sein.'
    END
),
(
    90,
    ...
    CASE
        ...
        ELSE 'BLOCKER'
    END,
    N'@ExecuteDrop = 0 gibt nur die Pruefungen aus; @ExecuteDrop = 1 fuehrt DROP DATABASE aus.'
),
(
    100,
    'Boolean parameters',
    CASE
        WHEN @ExecuteDrop IS NULL
          OR @AcknowledgeNoBackup IS NULL
...
(
    ...
    'No backup step',
    0,
    N'Diese Variante fuehrt kein BACKUP DATABASE und kein RESTORE VERIFYONLY aus.'
),
(
    30,
...
BEGIN
    THROW 51100, 'Preflight hat BLOCKER gefunden. Es wurde keine Datenbank geloescht.', 1;
END;

-- 4. Optional Snapshot-Drop und DROP DATABASE ausfuehren
...
BEGIN TRY
    ...
    (
        10,
        'Backup skipped by design',
        'WARN',
        N'Diese Skriptvariante erstellt bewusst kein Backup vor DROP DATABASE.'
    );
...
END TRY
BEGIN CATCH
    IF DB_ID(@TargetDatabaseName) IS NOT NULL AND @SetSingleUserBeforeDrop = 1
    BEGIN
        BEGIN TRY
            SET @Sql =
            ...
            (
                850,
                'Multi user restored after failure',
                'WARN',
                N'Die Ziel-DB existiert noch; MULTI_USER wurde nach einem Fehler wieder gesetzt.'
            );
        END TRY
        BEGIN CATCH
            INSERT INTO #PostDropChecks
            ...
            (
                851,
                'Multi user restore failed',
                'ERROR',
                CONCAT(N'Die Ziel-DB existiert noch, aber MULTI_USER konnte nicht wieder gesetzt werden: ', ERROR_MESSAGE())
            );
        END CATCH;
    END;
...
BEGIN
    THROW 51101, 'Nachkontrolle hat BLOCKER gefunden. Bitte PostDropChecks und PhysicalFileCheck pruefen.', 1;
END;
```
<!-- SQLDOC:SQL_CODE:END -->
