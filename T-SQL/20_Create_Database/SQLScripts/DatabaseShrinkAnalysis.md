# DatabaseShrinkAnalysis.sql

Dieses Skript ist die Single-DB-Variante zu `DatabaseShrinkCandidateReview.sql`. Es analysiert genau eine Datenbank, trennt Daten- und Logdateien, bewertet realistisches Shrink-Potential und erzeugt vorsichtige `DBCC SHRINKFILE`-Vorlagen, ohne diese auszufuehren.
...
| Feld | Wert |
|---|---|
| Script | [DatabaseShrinkAnalysis.sql](DatabaseShrinkAnalysis.sql) |
| Version | `1.0` |
| Typ | `diagnostic-query` |
| Kapitel | `20_Create_Database` |
| Sicherheit | `read-only-tempdb` |
| Zweck | Analysiert fuer genau eine Datenbank, welche Daten- und Logdateien realistische Shrink-Kandidaten sein koennen. Das Skript berechnet freien Platz, vorsichtige Zielgroessen und optionale DBCC-SHRINKFILE-Vorlagen, ohne einen Shrink auszufuehren. |
<!-- SQLDOC:SUMMARY_TABLE:END -->

## Einordnung

Die Analyse arbeitet datenbanklokal mit `sys.database_files`, `FILEPROPERTY`, `sys.dm_db_log_space_usage` und `sys.dm_db_log_info`. Dadurch kann sie Datenfile-Freiraum, DB-weite Lognutzung und das Ende aktiver VLFs in einer gemeinsamen Sicht darstellen. Ein optionaler `@TargetVolumeRoot` beschraenkt die Dateiansicht auf ein betroffenes Laufwerk oder Volume, waehrend `NULL` alle Dateien der Ziel-Datenbank bewertet.

## Annahmen

- Das Skript ist eine Diagnose- und Review-Hilfe; es fuehrt keinen Shrink aus.
- `@DatabaseName` muss auf eine online verfuegbare Datenbank zeigen.
- Systemdatenbanken werden standardmaessig ausgeschlossen und nur mit `@AllowSystemDatabase = 1` analysiert.
- Fuer `FULL` und `BULK_LOGGED` wird ein frisches Logbackup als Guardrail bewertet, damit die Backup-Kette nicht aus Versehen umgangen wird.
- Datenfile-Shrinks bleiben ein letzter Notfallpfad, weil sie Fragmentierung und spaeteres Autogrowth verursachen koennen.
- Bei mehreren Logdateien bleibt die Bewertung konservativ, da die DB-weite Lognutzung nicht exakt auf einzelne Logdateien verteilt werden kann.
...
|---|---|---|---|
| `@DatabaseName` | `SYSNAME` | Ja | Name der zu analysierenden Datenbank. |
| `@TargetVolumeRoot` | `NVARCHAR(260)` | Nein | Optionales Laufwerk oder Volume-Prefix; NULL analysiert alle Dateien der Datenbank. |
| `@MinimumRecoverableMB` | `DECIMAL(18,2)` | Nein | Mindestmenge in MB, die pro Datei als potentiell rueckgewinnbar gelten muss. |
| `@MinimumRecoverablePct` | `DECIMAL(5,2)` | Nein | Mindestanteil freien Platzes in Prozent, ab dem eine Datei als Kandidat gilt. |
| `@MinimumTargetSizeMB` | `INT` | Nein | Absolute Untergrenze fuer generierte Shrink-Zielgroessen. |
| `@MinimumPostShrinkFreeMB` | `DECIMAL(18,2)` | Nein | Freier Puffer in MB, der nach einem Shrink mindestens verbleiben soll. |
| `@TargetFreePctAfterShrink` | `DECIMAL(5,2)` | Nein | Zusaetzlicher Zielpuffer in Prozent der genutzten Groesse. |
| `@MaxLogBackupAgeMinutes` | `INT` | Nein | Maximales Alter eines Logbackups, damit FULL/BULK_LOGGED als shrink-vorbereitet gilt. |
| `@AllowSystemDatabase` | `BIT` | Nein | 1 = Systemdatenbanken duerfen analysiert werden. |
| `@OnlyShowCandidates` | `BIT` | Nein | 1 = in der Dateiansicht nur Kandidaten oder problematische Dateien anzeigen. |
| `@IncludeCommandTemplates` | `BIT` | Nein | 1 = zusaetzlich DBCC SHRINKFILE Vorlagen fuer Kandidatendateien ausgeben. |
...
- `log-shrink-ready` bedeutet, dass ein Log-Kandidat nach den gesetzten Guardrails vorbereitet wirkt.
- `take-log-backup-first` weist bei `FULL` oder `BULK_LOGGED` auf fehlende oder zu alte Logbackups hin.
- `fix-log-prerequisites-first` bedeutet, dass Log-Reuse-Wait, Backup-Kette oder andere Blocker zuerst geklaert werden muessen.
- `datafile-last-resort` ist bewusst defensiv und sollte nur in einem Wartungsfenster umgesetzt werden.
- Die erzeugten Commands sind Vorlagen fuer Review und Change-Dokumentation, keine automatische Freigabe.
...
|---|---|---|---|
| `1.0` | `2026-06-01` | `ER` | Erstversion der Shrink-Analyse fuer eine einzelne Datenbank |
...
```mermaid
flowchart TD
    A[Parameter fuer Zieldatenbank und optionales Volume validieren] --> B[Metadaten und Backup-Historie der DB lesen]
    B --> C[Temp-Tabellen fuer Dateien Lognutzung VLF-Ende Analyse und Fehler anlegen]
    C --> D[Dynamisch in die Zieldatenbank wechseln]
    D --> E[Daten- und Logdateien aus sys.database_files lesen]
    D --> F[Lognutzung aus sys.dm_db_log_space_usage lesen]
    D --> G[Aktives VLF-Ende aus sys.dm_db_log_info lesen]
    E --> H[Datenfile-Potential mit FILEPROPERTY und Puffer berechnen]
    F --> I[Logfile-Potential mit Recovery-Modell Backup-Guardrails und VLF-Anker berechnen]
    G --> I
    H --> J[Summary mit ActionClass und ReviewFocus erzeugen]
    I --> J
    J --> K[Dateischarfe Analyse ausgeben]
    K --> L[Optionale DBCC-SHRINKFILE-Vorlagen ausgeben]
    L --> M[ScanErrors ausgeben]
```
<!-- SQLDOC:MERMAID:END -->
