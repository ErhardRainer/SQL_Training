# DatabaseOwnerAndOptionsBaseline.sql

Dieses Skript liefert eine lesende Basispruefung fuer Datenbank-Owner, Kompatibilitaetslevel und zentrale Datenbankoptionen. Es kombiniert den aktuellen Ist-Zustand einer Datenbank mit der `model`-Baseline und optionalen Zielvorgaben, damit Bootstrap und Review nicht nur auf Einzeloptionen reduziert bleiben.
...
| Feld | Wert |
|---|---|
| Script | [DatabaseOwnerAndOptionsBaseline.sql](DatabaseOwnerAndOptionsBaseline.sql) |
| Version | `1.0` |
| Typ | `diagnostic-query` |
| Kapitel | `20_Create_Database` |
| Sicherheit | `read-only` |
| Zweck | Prueft Owner, Kompatibilitaet und Kernoptionen fuer Bootstrap- und Review-Szenarien. |
<!-- SQLDOC:SUMMARY_TABLE:END -->

## Einordnung

Beim Anlegen oder Uebernehmen einer Datenbank bleiben Owner, Kompatibilitaetslevel und einzelne Grundoptionen oft implizit gesetzt. Das Skript macht diese Startlage sichtbar und trennt sauber zwischen Ist-Zustand, `model`-Baseline und optionalem Zielbild fuer die neue oder bestehende Datenbank.

## Annahmen

- Die Erstversion arbeitet ausschliesslich lesend gegen `sys.databases` und fuehrt keine Aenderungen aus.
- Falls die Zieldatenbank noch nicht existiert, wird `model` als konservative Bootstrap-Basis verwendet.
- Fuer den Owner wird ohne expliziten Zielparameter nur der Ist-Zustand ausgewiesen; es wird kein produktiver Sicherheitsstandard erfunden.

## Anwendungsfall

Das Skript eignet sich fuer Schulung, Bootstrap-Checklisten und Review-Runden im Kapitel `20_Create_Database`. Es hilft dabei, Owner, Compatibility Level, Recovery Model, `PAGE_VERIFY`, `AUTO_CLOSE`, `AUTO_SHRINK` und Containment nicht getrennt zu betrachten, sondern als zusammenhaengende Grundkonfiguration einer Datenbank.
...
|---|---|---|---|
| `@TargetDatabaseName` | `SYSNAME` | Nein | Name der zu pruefenden oder zu bootstrapenden Datenbank. |
| `@DesiredOwner` | `SYSNAME` | Nein | Optionaler Ziel-Owner fuer die Review; sonst nur Ist-Zustand. |
| `@DesiredCompatibilityLevel` | `SMALLINT` | Nein | Optionales Ziel fuer das Compatibility Level; sonst `model`-Fallback. |
| `@DesiredRecoveryModel` | `NVARCHAR(20)` | Nein | Optionales Ziel fuer das Recovery Model; sonst `model`-Fallback. |
| `@RequireChecksumPageVerify` | `BIT` | Nein | Bevorzugt bei `1` den Guardrail `PAGE_VERIFY CHECKSUM`. |
| `@IncludeCommandTemplates` | `BIT` | Nein | Gibt bei `1` zusaetzliche Befehlsvorlagen aus. |
...
- `DatabaseBaseline` zeigt fuer jeden Pruefbereich Ist-Zustand, `model`-Baseline, Zielwert und den Review-Status.
- `ReviewMatrix` priorisiert die Bereiche fuer Bootstrap oder Nachschaerfung.
- `CommandTemplate` erzeugt nur Vorlagen fuer spaetere `ALTER DATABASE`- bzw. `ALTER AUTHORIZATION`-Schritte.
...
|---|---|---|---|
| `1.0` | `2026-04-22` | `ER` | Erstversion der lesenden Baseline fuer Owner, Kompatibilitaet und Kernoptionen |
...
flowchart TD
    ...
    B --> C[Optional Ziel-Datenbank laden]
    C --> D[DatabaseBaseline fuer Owner, Compatibility und Optionen fuellen]
    D --> E[ReviewMatrix mit Prioritaeten ableiten]
    E --> F[CommandTemplate fuer spaetere Anpassungen erzeugen]
    F --> G[DatabaseBaseline ausgeben]
    G --> H[ReviewMatrix ausgeben]
    H --> I{IncludeCommandTemplates = 1?}
    I -->|Ja| J[CommandTemplate ausgeben]
