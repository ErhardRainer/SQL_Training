# DatabaseScopedConfigBaseline.sql

Dieses Skript leitet eine lesende Baseline fuer typische `DATABASE SCOPED CONFIGURATION`-Einstellungen neuer Datenbanken ab. Es verbindet optionale Sollwerte mit Referenzwerten der aktuellen Datenbank und erzeugt daraus Review-Hinweise sowie konkrete `USE`- plus `ALTER DATABASE SCOPED CONFIGURATION`-Vorlagen.
...
| Feld | Wert |
|---|---|
| Script | [DatabaseScopedConfigBaseline.sql](DatabaseScopedConfigBaseline.sql) |
| Version | `1.0` |
| Typ | `template` |
| Kapitel | `20_Create_Database` |
| Sicherheit | `read-only` |
| Zweck | Leitet eine konservative Baseline typischer database scoped configurations fuer neue Datenbanken ab. |
<!-- SQLDOC:SUMMARY_TABLE:END -->

## Einordnung

Beim Anlegen neuer Datenbanken werden `DATABASE SCOPED CONFIGURATION`-Schalter oft spaet oder uneinheitlich gesetzt. Dieses Skript macht eine kleine, didaktisch sinnvolle Auswahl sichtbar und trennt bewusst zwischen Referenz, Zielwert, Review-Fokus und ausfuehrbarer Befehlsvorlage.

## Annahmen

- Das Skript ist eine didaktische Erstversion fuer Bootstrap- und Review-Zwecke und fuehrt selbst keine Aenderung an Datenbanken aus.
- Wenn keine expliziten Sollwerte ueber Parameter gesetzt werden, werden Referenzwerte optional aus `sys.database_scoped_configurations` der aktuellen Datenbank gelesen.
- Ohne passende Referenzwerte verwendet die Baseline konservative Trainingsdefaults: `LEGACY_CARDINALITY_ESTIMATION = OFF`, `QUERY_OPTIMIZER_HOTFIXES = OFF`, `MAXDOP = 0` und `PARAMETER_SNIFFING = ON`.
- Die erzeugten Befehlsvorlagen wechseln zuerst in die Ziel-Datenbank und muessen vor produktiver Nutzung gegen Workload, Kompatibilitaetslevel und Query-Store-Erkenntnisse geprueft werden.

## Anwendungsfall

Das erste Resultset zeigt Referenzquelle, Referenzwert und aufgeloesten Zielwert fuer vier haeufig diskutierte Scoped Configurations. Das zweite Resultset verdichtet daraus eine Bootstrap-Checkliste. Das dritte Resultset erzeugt konkrete `USE`- plus `ALTER DATABASE SCOPED CONFIGURATION`-Vorlagen fuer eine Ziel-Datenbank, ohne diese Befehle auszufuehren.
...
|---|---|---|---|
| `@TargetDatabaseName` | `SYSNAME` | Nein | Name der Ziel-Datenbank fuer Baseline- und Befehlsvorlagen. |
| `@UseCurrentDatabaseAsReference` | `BIT` | Nein | Liest bei `1` fehlende Sollwerte aus `sys.database_scoped_configurations` der aktuellen Datenbank. |
| `@LegacyCardinalityEstimation` | `BIT` | Nein | Optionaler Sollwert fuer `LEGACY_CARDINALITY_ESTIMATION`. |
| `@QueryOptimizerHotfixes` | `BIT` | Nein | Optionaler Sollwert fuer `QUERY_OPTIMIZER_HOTFIXES`. |
| `@MaxDop` | `INT` | Nein | Optionaler Sollwert fuer `MAXDOP`; `NULL` nutzt Referenz oder Default `0`. |
| `@ParameterSniffing` | `BIT` | Nein | Optionaler Sollwert fuer `PARAMETER_SNIFFING`. |
...
- `ScopedConfigBaseline` macht sichtbar, ob ein Zielwert aus einem expliziten Parameter, aus der aktuellen Datenbank oder aus einem dokumentierten Trainingsdefault stammt.
- `BootstrapChecklist` priorisiert die Review-Reihenfolge fuer Optimizer-Schalter, Parallelism-Overrides und globale Sniffing-Entscheidungen.
- `ScopedConfigCommandTemplate` erzeugt nur Vorlagen im Kontext der Ziel-Datenbank; besonders `LEGACY_CARDINALITY_ESTIMATION`, `QUERY_OPTIMIZER_HOTFIXES` und `MAXDOP` sollten nicht ohne Testbezug uebernommen werden.
...
|---|---|---|---|
| `1.0` | `2026-04-22` | `ER` | Erstversion der lesenden Baseline fuer typische database scoped configurations |
...
```mermaid
flowchart TD
    A[Parameter validieren] --> B[Optionale Referenzwerte aus sys.database_scoped_configurations lesen]
    B --> C[ScopedConfigBaseline fuer vier Zielschalter aufbauen]
    C --> D[BootstrapChecklist mit Review-Fokus ableiten]
    D --> E[USE plus ALTER DATABASE SCOPED CONFIGURATION Vorlagen erzeugen]
    E --> F[ScopedConfigBaseline ausgeben]
    F --> G[BootstrapChecklist ausgeben]
    G --> H[ScopedConfigCommandTemplate ausgeben]
```
<!-- SQLDOC:MERMAID:END -->
