# DataFileSizeBalanceReport.sql

Dieses Skript bewertet die Groessenverteilung mehrerer Datenfiles einer Datenbank und vergleicht jeden Dateianteil mit einer einfachen Gleichverteilungsbaseline. Zusaetzlich kann es lesende `ALTER DATABASE ... MODIFY FILE`-Vorlagen fuer auffaellige Abweichungen erzeugen.
...
| Feld | Wert |
|---|---|
| Script | [DataFileSizeBalanceReport.sql](DataFileSizeBalanceReport.sql) |
| Version | `1.0` |
| Typ | `diagnostic-query` |
| Kapitel | `20_Create_Database` |
| Sicherheit | `read-only` |
| Zweck | Bewertet die Groessenverteilung mehrerer Datenfiles und erzeugt didaktische Rebalance-Hinweise. |
<!-- SQLDOC:SUMMARY_TABLE:END -->

## Einordnung

Beim Anlegen oder Nachpflegen mehrerer Datenfiles ist nicht nur die Anzahl der Dateien relevant, sondern auch deren Groessenverteilung. Das Skript macht sichtbar, ob einzelne Dateien deutlich ueber oder unter einem einfachen Gleichverteilungsziel liegen und trennt dabei zwischen reiner Metadatenanalyse und optionalen Vorlagen fuer ein spaeteres Rebalancing.

## Annahmen

- Es handelt sich um eine didaktische Diagnose fuer Dateigroessen auf Basis von `sys.master_files`, nicht um eine Laufzeit- oder I/O-Analyse.
- Standardmaessig wird `tempdb` verwendet, weil dort Mehrdatei-Konfigurationen in Trainingsumgebungen haeufig sichtbar sind.
- Die Gleichverteilung dient als konservative Baseline fuer Review-Zwecke; produktive Speziallayouts koennen bewusst davon abweichen.
- Rebalance-Vorlagen bleiben rein lesend und muessen vor realer Ausfuehrung fachlich geprueft werden.

## Anwendungsfall

Das Skript eignet sich fuer Bootstrap-Reviews, Checklisten und Unterricht im Kapitel `20_Create_Database`. Es hilft dabei, ungleiche Initialgroessen mehrerer Datenfiles frueh zu erkennen, ein einfaches Zielbild fuer gleichmaessige Verteilung abzuleiten und daraus konkrete `MODIFY FILE`-Vorlagen fuer spaetere Entscheidungen vorzubereiten.
...
|---|---|---|---|
| `@DatabaseName` | `SYSNAME` | Nein | Datenbank fuer den Dateivergleich; standardmaessig `tempdb`. |
| `@ImbalanceThresholdPct` | `DECIMAL(5,2)` | Nein | Schwelle in Prozentpunkten fuer eine auffaellige Abweichung vom Idealanteil. |
| `@ShowRebalanceTemplate` | `BIT` | Nein | Gibt bei `1` zusaetzliche didaktische `SIZE`-Vorlagen aus. |
| `@MinimumDataFileCount` | `INT` | Nein | Mindestanzahl an Datenfiles, ab der ein echter Balance-Vergleich erwartet wird. |
...
- `DataFileDistribution` zeigt pro Datenfile den aktuellen Anteil an der Gesamtdatenmenge, das Gleichverteilungsziel und die Abweichung.
- `BalanceSummary` verdichtet Datenbankname, Dateianzahl, Gesamtgroesse und die groesste beobachtete Abweichung.
- `RebalanceTemplate` erscheint nur bei `@ShowRebalanceTemplate = 1` und bleibt eine didaktische Vorlage ohne Ausfuehrung.
...
|---|---|---|---|
| `1.0` | `2026-04-22` | `ER` | Erstversion des Balance-Reports fuer mehrere Datenfiles |
...
```mermaid
flowchart TD
    A[Parameter validieren] --> B{Datenbank vorhanden und online?}
    B -->|Nein| C[THROW fuer ungueltige Datenbank]
    B -->|Ja| D[ROWS-Datenfiles aus sys.master_files lesen]
    D --> E[Gleichverteilungsziel und Dateianteile berechnen]
    E --> F[ReviewSeverity und ReviewFocus pro Datenfile ableiten]
    F --> G[BalanceSummary aus Maximalabweichung und Dateianzahl erzeugen]
    G --> H{ShowRebalanceTemplate = 1?}
    H -->|Ja| I[MODIFY FILE Vorlagen fuer auffaellige Datenfiles erzeugen]
    H -->|Nein| J[Nur Resultsets ausgeben]
    I --> J
    J --> K[DataFileDistribution ausgeben]
    K --> L[BalanceSummary ausgeben]
    L --> M{ShowRebalanceTemplate = 1?}
    M -->|Ja| N[RebalanceTemplate ausgeben]
    M -->|Nein| O[Run beenden]
    N --> O
```
<!-- SQLDOC:MERMAID:END -->
