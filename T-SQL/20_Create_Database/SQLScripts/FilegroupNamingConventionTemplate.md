# FilegroupNamingConventionTemplate.sql

Dieses Skript leitet eine didaktische Namenskonvention fuer Filegroups, logische Dateien und physische Dateinamen im Kapitel `20_Create_Database` ab. Es prueft bestehende oder modellierte Kandidaten gegen eine konsistente Baseline und erzeugt daraus wiederverwendbare `CREATE DATABASE`- sowie Review-Vorlagen, ohne selbst Umbenennungen oder DDL auszufuehren.
...
| Feld | Wert |
|---|---|
| Script | [FilegroupNamingConventionTemplate.sql](FilegroupNamingConventionTemplate.sql) |
| Version | `1.0` |
| Typ | `template` |
| Kapitel | `20_Create_Database` |
| Sicherheit | `read-only` |
| Zweck | Definiert eine stabile Benennung fuer Filegroups und Dateien und erzeugt dazu passende Review- und DDL-Vorlagen. |
<!-- SQLDOC:SUMMARY_TABLE:END -->

## Einordnung

Beim Aufbau neuer Datenbanken werden Filegroups und Dateien oft funktional richtig, aber sprachlich uneinheitlich benannt. Das Skript trennt deshalb bewusst zwischen Rollenlogik, Sequenzierung und Dateiendungen. So wird sichtbar, welche Namen fuer `PRIMARY`, `DATA`, `ARCHIVE` und `LOG` konsistent bleiben und welche Muster spaeter Reviews oder Automatisierung erschweren.

## Annahmen

- Es handelt sich um eine didaktische Template-Erstversion fuer Bootstrap-, Review- und Standardisierungszwecke.
- Ohne weitere Fachvorgaben nutzt die Baseline Grossbuchstaben fuer Rollen-Tokens sowie Unterstriche als einziges Trennzeichen.
- Mehrere Datenfiles werden mit zweistelligen Suffixen wie `01` und `02` modelliert.
- Falls keine Referenzdatenbank ueber `@SampleDatabaseName` gelesen wird, arbeitet die Review-Ausgabe mit absichtlich uneinheitlichen Demo-Kandidaten.

## Anwendungsfall

Das erste Resultset beschreibt die eigentlichen Benennungsregeln. Das zweite Resultset zeigt einen konkreten Namensplan fuer PRIMARY-, DATA-, optionale ARCHIVE- und LOG-Dateien. Das dritte Resultset bewertet Katalog- oder Demo-Kandidaten gegen die Zielkonvention. Das vierte Resultset liefert sofort nutzbare Vorlagen fuer `CREATE DATABASE` und eine anschliessende Rename-Review.
...
|---|---|---|---|
| `@TargetDatabaseName` | `SYSNAME` | Nein | Zieldatenbankname fuer die Namensvorlage. |
| `@PrimaryPrefix` | `NVARCHAR(30)` | Nein | Rollenkuerzel fuer primaere Datendatei und PRIMARY-Bezug. |
| `@DataPrefix` | `NVARCHAR(30)` | Nein | Rollenkuerzel fuer weitere Datenfiles in der DATA-Filegroup. |
| `@ArchivePrefix` | `NVARCHAR(30)` | Nein | Rollenkuerzel fuer optionale Archiv-Filegroups und Dateien. |
| `@LogPrefix` | `NVARCHAR(30)` | Nein | Rollenkuerzel fuer Logdateien. |
| `@DataFileCount` | `INT` | Nein | Anzahl geplanter Datenfiles fuer die DATA-Filegroup. |
| `@IncludeArchiveGroup` | `BIT` | Nein | Aktiviert bei `1` zusaetzliche ARCHIVE-Namen und Vorlagen. |
| `@SampleDatabaseName` | `SYSNAME` | Nein | Optionale Referenzdatenbank fuer bestehende Dateinamen; `NULL` nutzt nur Demo-Kandidaten. |
...
- `NamingConventionChecklist` fasst die eigentlichen Guardrails fuer Datenbankstamm, Rollen-Tokens, Sequenzen und Dateiendungen zusammen.
- `NamingPlan` zeigt die Zielnamen fuer die Baseline inklusive Beispielklauseln fuer DDL-Vorlagen.
- `NamingReview` markiert problematische Trennzeichen, fehlende Datenbankstaemme oder uneinheitliche Sequenzen.
- `CommandTemplate` bleibt bewusst lesend und liefert nur Vorlagen fuer spaetere Review- oder Bootstrap-Schritte.
...
|---|---|---|---|
| `1.0` | `2026-04-22` | `ER` | Erstversion der didaktischen Benennungsvorlage fuer Filegroups und Dateien |
...
```mermaid
flowchart TD
    A[Parameter validieren] --> B[NamingConventionChecklist mit Rollen-, Sequenz- und Dateiendungsregeln fuellen]
    B --> C[NamingPlan fuer PRIMARY, DATA, optional ARCHIVE und LOG erzeugen]
    C --> D{SampleDatabaseName gesetzt?}
    D -->|Ja| E[Katalognamen aus sys.master_files als Kandidaten laden]
    D -->|Nein| F[Didaktische Demo-Kandidaten mit typischen Benennungsfehlern laden]
    E --> G[NamingReview gegen Zielmuster ableiten]
    F --> G
    G --> H[CREATE DATABASE- und Rename-Vorlagen generieren]
    H --> I[NamingConventionChecklist ausgeben]
    I --> J[NamingPlan ausgeben]
    J --> K[NamingReview ausgeben]
    K --> L[CommandTemplate ausgeben]
```
<!-- SQLDOC:MERMAID:END -->
