# FileLayoutTemplate.sql

Dieses Skript erzeugt eine didaktische Vorlage fuer saubere Datei-, Filegroup- und Growth-Konfigurationen beim Anlegen neuer Datenbanken. Es leitet Verzeichnis-Fallbacks aus Instanzdefaults oder `model` ab, baut daraus einen klaren Dateiplan auf und generiert anschliessend lesbare `CREATE DATABASE`-Bausteine.
...
| Feld | Wert |
|---|---|
| Script | [FileLayoutTemplate.sql](FileLayoutTemplate.sql) |
| Version | `1.0` |
| Typ | `template` |
| Kapitel | `20_Create_Database` |
| Sicherheit | `read-only` |
| Zweck | Liefert eine saubere Vorlage fuer Datei-, Filegroup- und feste Growth-Konfigurationen rund um `CREATE DATABASE`. |
<!-- SQLDOC:SUMMARY_TABLE:END -->

## Einordnung

Im Kapitel `20_Create_Database` entsteht ein sauberes Layout nicht nur aus Dateinamen, sondern aus dem Zusammenspiel von Pfaden, Filegroup-Rollen, Dateianzahl und festen Growth-Werten. Das Skript trennt diese Entscheidungen bewusst in eine Checkliste, einen konkreten Dateiplan und ausfuehrbare T-SQL-Bausteine, ohne produktive DDL direkt abzusetzen.

## Annahmen

- Es handelt sich um eine didaktische Erstversion fuer Bootstrap-, Review- und Template-Zwecke.
- Verzeichnisse werden aus Parametern, `SERVERPROPERTY('InstanceDefaultDataPath')`, `SERVERPROPERTY('InstanceDefaultLogPath')` oder `model` abgeleitet.
- Growth-Werte bleiben bewusst fix in MB, damit die Vorlage planbar und mit den uebrigen Baseline-Skripten des Kapitels konsistent bleibt.
- Die optionale `REPORTING`-Filegroup dient als Beispiel fuer getrennte leseorientierte Lasten und ist kein produktiver Muss-Bestandteil.

## Anwendungsfall

Das erste Resultset formuliert Review-Leitplanken fuer Pfade, Namenskonventionen, Dateianzahl und Growth. Das zweite Resultset beschreibt den konkreten Dateiplan fuer `PRIMARY`, `DATA`, optional `REPORTING` und `LOG`. Das dritte Resultset erzeugt eine direkt lesbare `CREATE DATABASE`-Vorlage plus Nachlauf zur Verifikation der Datei- und Filegroup-Konfiguration.
...
|---|---|---|---|
| `@TargetDatabaseName` | `SYSNAME` | Nein | Name der geplanten Zieldatenbank fuer die Vorlage. |
| `@PrimaryDataDirectory` | `NVARCHAR(260)` | Nein | Optionales Verzeichnis fuer die PRIMARY-Datei. |
| `@SecondaryDataDirectory` | `NVARCHAR(260)` | Nein | Optionales Verzeichnis fuer weitere Datenfiles und optionale Reporting-Dateien. |
| `@LogDirectory` | `NVARCHAR(260)` | Nein | Optionales Verzeichnis fuer die Logdatei. |
| `@PrimaryDataSizeMB` | `INT` | Nein | Didaktische Startgroesse der PRIMARY-Datei in MB. |
| `@SecondaryDataFileCount` | `INT` | Nein | Anzahl zusaetzlicher Datenfiles in der DATA-Filegroup. |
| `@SecondaryDataFileSizeMB` | `INT` | Nein | Didaktische Startgroesse je Datei in der DATA-Filegroup. |
| `@PrimaryGrowthMB` | `INT` | Nein | Fixer Growth-Wert fuer die PRIMARY-Datei in MB. |
| `@SecondaryGrowthMB` | `INT` | Nein | Fixer Growth-Wert je Datei in der DATA-Filegroup in MB. |
| `@LogSizeMB` | `INT` | Nein | Didaktische Startgroesse der Logdatei in MB. |
| `@LogGrowthMB` | `INT` | Nein | Fixer Growth-Wert der Logdatei in MB. |
| `@CreateReportingFilegroup` | `BIT` | Nein | Aktiviert bei `1` eine zusaetzliche `REPORTING`-Filegroup mit einer NDF-Datei. |
...
- `LayoutChecklist` buendelt die wichtigsten Guardrails fuer Pfadtrennung, Growth-Konventionen, Namensgebung und Filegroup-Rollen.
- `FileLayoutPlan` zeigt pro Datei den logischen Namen, den physischen Pfad, Startgroesse und Growth-Wert.
- `CreateDatabaseTemplate` erzeugt sowohl das eigentliche `CREATE DATABASE`-Statement als auch einen kompakten Nachlauf fuer Default-Filegroup und Verifikation.
...
|---|---|---|---|
| `1.0` | `2026-04-22` | `ER` | Erstversion der didaktischen Datei- und Growth-Vorlage fuer CREATE DATABASE |
...
flowchart TD
    ...
    B --> C{Alle benoetigten Verzeichnisse ableitbar?}
    C -->|Nein| D[THROW fuer fehlende Verzeichnisbasis]
    C -->|Ja| E[LayoutChecklist mit Pfad-, Growth-, Naming- und Filegroup-Regeln fuellen]
    E --> F[PRIMARY-Datei in FileLayoutPlan eintragen]
    F --> G[DATA-Dateien per Zahlen-CTE erzeugen]
    G --> H{CreateReportingFilegroup = 1?}
    H -->|Ja| I[REPORTING-Datei in den Plan einfuegen]
    H -->|Nein| J[REPORTING ueberspringen]
    I --> K[LOG-Datei in den Plan einfuegen]
    J --> K
    K --> L[CREATE DATABASE- und Verifikationsbausteine aus dem Plan generieren]
    L --> M[LayoutChecklist ausgeben]
    M --> N[FileLayoutPlan ausgeben]
    N --> O[CreateDatabaseTemplate ausgeben]
```
<!-- SQLDOC:MERMAID:END -->
