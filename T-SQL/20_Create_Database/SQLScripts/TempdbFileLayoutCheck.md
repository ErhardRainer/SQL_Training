# TempdbFileLayoutCheck.sql

Dieses Skript prueft die aktuelle TempDB-Dateikonfiguration einer SQL-Server-Instanz. Es liest die TempDB-Dateien aus `sys.master_files`, verdichtet Groessen-, Growth- und Verzeichnismerkmale und markiert daraus moegliche Ungleichgewichte fuer ein anschliessendes Betriebsreview.
...
| Feld | Wert |
|---|---|
| Script | [TempdbFileLayoutCheck.sql](TempdbFileLayoutCheck.sql) |
| Version | `1.0` |
| Typ | `diagnostic-query` |
| Kapitel | `20_Create_Database` |
| Sicherheit | `read-only` |
| Zweck | Inventarisiert TempDB-Dateien und bewertet moegliche Groessen-, Growth- und Layout-Ungleichgewichte. |
<!-- SQLDOC:SUMMARY_TABLE:END -->

## Einordnung

TempDB wird oft historisch erweitert: eine zusaetzliche Data-Datei hier, ein anderes Growth dort, ein zweites Laufwerk spaeter. Fuer Unterricht und Betriebsreviews ist deshalb nicht nur die reine Dateiliste relevant, sondern die Frage, ob Data-Dateien in Groesse, Growth und Platzierung noch konsistent genug fuer eine saubere Baseline sind.

## Annahmen

- Das Skript ist bewusst rein lesend und aendert weder TempDB-Dateien noch Serverkonfiguration.
- Die Diagnose basiert auf `sys.master_files`; Laufzeitmetriken wie Waits oder echte I/O-Verteilung werden nicht ausgewertet.
- Eine abweichende Dateianzahl ist kein automatischer Fehler, sondern ein Review-Signal im Zusammenspiel mit CPU-Zahl, Lastprofil und Betriebsstandard.
- Unterschiedliche Verzeichnisse koennen fachlich gewollt sein; das Skript markiert sie nur als Layout-Thema, wenn `@FlagMixedDirectories = 1`.

## Anwendungsfall

Das Skript eignet sich fuer Baseline-Checks nach Neuinstallation, Storage-Aenderungen, TempDB-Tuning-Workshops oder Review-Runbooks im Kapitel `20_Create_Database`. Es hilft dabei, aus der reinen Dateikonfiguration unmittelbar eine kleine Liste priorisierter TempDB-Fragen abzuleiten.
...
|---|---|---|---|
| `@SizeVarianceTolerancePct` | `DECIMAL(5,2)` | Nein | Zulaessige prozentuale Groessenabweichung einer Data-Datei vom Durchschnitt. |
| `@GrowthDeltaToleranceMB` | `DECIMAL(10,2)` | Nein | Zulaessige absolute Growth-Abweichung in MB fuer MB-basierte Growth-Werte. |
| `@ExpectedDataFileCount` | `INT` | Nein | Optionaler Sollwert fuer die Anzahl von TempDB-Data-Dateien. |
| `@FlagMixedDirectories` | `BIT` | Nein | Markiert bei `1` unterschiedliche Dateiverzeichnisse als Review-Thema. |
...
- `TempdbFileInventory` zeigt alle TempDB-Dateien inklusive Pfad, Groesse, Growth und Balance-Flag.
- `DataFileBalanceSummary` verdichtet die wichtigsten Beobachtungen fuer Dateianzahl, Groessenspreizung, Growth-Muster und Verzeichnisse.
- `DirectoryLayout` gruppiert Dateipfade nach Data- und Log-Dateien.
- `Findings` priorisiert die Review-Punkte als kompakte Arbeitsliste.
...
|---|---|---|---|
| `1.0` | `2026-04-22` | `ER` | Erstversion des lesenden TempDB-Datei-Checks |
...
```mermaid
flowchart TD
    A[Parameter validieren] --> B[TempDB-Dateien aus sys.master_files laden]
    B --> C[Data-Datei-Metriken fuer Anzahl, Groesse, Growth und Verzeichnisse berechnen]
    C --> D[DataFileBalanceSummary mit Bewertungen und Empfehlungen fuellen]
    D --> E[DirectoryLayout nach Verzeichnis und Dateityp aggregieren]
    E --> F[Findings fuer Dateianzahl, Groessenspreizung, Growth und Pfade ableiten]
    F --> G[Per-File-Size-Findings fuer jede Data-Datei ergaenzen]
    G --> H[TempdbFileInventory ausgeben]
    H --> I[DataFileBalanceSummary ausgeben]
    I --> J[DirectoryLayout ausgeben]
    J --> K[Findings ausgeben]
```
<!-- SQLDOC:MERMAID:END -->
