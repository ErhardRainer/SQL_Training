# ViewDependencyTree.sql

Dieses Skript baut einen lesbaren Abhaengigkeitsbaum fuer Views in der aktuellen Datenbank. Es eignet sich fuer Trainings- und Review-Szenarien, in denen die Verkettung von Views, Tabellen, Synonymen und externen Referenzen nicht nur als Kantenliste, sondern als nachvollziehbare Baumstruktur sichtbar werden soll.
...
| Feld | Wert |
|---|---|
| Script | [ViewDependencyTree.sql](ViewDependencyTree.sql) |
| Version | `1.0` |
| Typ | `diagnostic-query` |
| Kapitel | `22_Views_Schemata` |
| Sicherheit | `read-only` |
| Zweck | Baut einen lesbaren Abhaengigkeitsbaum fuer Views, Tabellen und weitere Module. |
<!-- SQLDOC:SUMMARY_TABLE:END -->

## Einordnung

Im Unterschied zu einer reinen Graph- oder Kantenliste hebt dieses Skript die Parent-Child-Struktur pro Root-View hervor. Dadurch laesst sich schneller erkennen, welche View weitere Views aufspannt, wo die Baumtiefe zunimmt und an welchen Stellen Tabellen, Synonyme oder externe Referenzen als Blattknoten enden.

## Annahmen

- Die Analyse bleibt rein lesend und nutzt nur Metadaten der aktuellen Datenbank.
- Die Rekursion verfolgt nur weitere Views; Tabellen, Synonyme, externe Ziele und unaufgeloeste Referenzen werden als Blattknoten behandelt.
- Externe oder unvollstaendig aufgeloeste Referenzen erscheinen als technische Knoten im Baum, auch wenn kein lokales Objekt aufgeloest werden kann.
- Die Baumdarstellung ist didaktisch kompakt gehalten und ersetzt keine vollstaendige Dependency-Visualisierung ueber alle Objektarten.

## Anwendungsfall

Das Skript eignet sich fuer Architektur-Reviews, View-Refactorings, Release-Vorbereitungen und Schulungsunterlagen. Besonders nuetzlich ist es, wenn eine View-Landschaft schrittweise verstanden oder vor Schemaaenderungen auf tiefe Verkettungen und kritische Blattknoten untersucht werden soll.
...
|---|---|---|---|
| `@RootViewLike` | `SYSNAME` | Nein | Optionales LIKE-Muster fuer die Auswahl der Root-Views. |
| `@MaxDepth` | `INT` | Nein | Maximale Tiefe fuer die rekursive Baumdarstellung. |
| `@IncludeSummary` | `BIT` | Nein | Steuert, ob ein zusaetzliches Summary-Resultset pro Root-View ausgegeben wird. |
...
- Das erste Resultset liefert die Baumansicht mit Einrueckung, Knotentyp, Scope und vollem Pfad.
- Das zweite Resultset zeigt die direkten Parent-Child-Kanten, falls eine flachere technische Sicht benoetigt wird.
- Das Summary markiert tiefe Baeume, unaufgeloeste Knoten und potenzielle Zyklen pro Root-View.
...
|---|---|---|---|
| `1.0` | `2026-04-22` | `ER` | Erstversion des lesbaren View-Abhaengigkeitbaums |
...
```mermaid
flowchart TD
    A[Parameter validieren] --> B[Root-Views aus sys.views und sys.schemas laden]
    B --> C[Direkte Parent-Child-Kanten aus sys.sql_expression_dependencies ableiten]
    C --> D[Knotentyp, Scope und Traversierbarkeit mit sys.objects und sys.synonyms anreichern]
    D --> E[Rekursive Baumstruktur nur ueber weitere Views bis MaxDepth aufbauen]
    E --> F[Besuchte Objekt-IDs pruefen und Zyklen markieren]
    F --> G[Baumlabels mit Einrueckung und Pfadtext materialisieren]
    G --> H[Summary pro Root-View mit Tiefe, Blattknoten und unresolved Referenzen aggregieren]
    H --> I[Baumresultset ausgeben]
    I --> J[Direkte Kanten ausgeben]
    J --> K{IncludeSummary = 1?}
    K -->|Ja| L[Summary pro Root-View ausgeben]
    K -->|Nein| M[Run ohne Summary beenden]
    L --> M
```
<!-- SQLDOC:MERMAID:END -->
