# ViewSchemaBindingCandidates.sql

Dieses Skript bewertet Views der aktuellen Datenbank als moegliche Kandidaten fuer `SCHEMABINDING`. Es bleibt rein lesend, sammelt typische Ausschlussgruende aus den Systemkatalogen und erzeugt optional Review-Templates fuer eine spaetere manuelle Umstellung.
...
| Feld | Wert |
|---|---|
| Script | [ViewSchemaBindingCandidates.sql](ViewSchemaBindingCandidates.sql) |
| Version | `1.0` |
| Typ | `diagnostic-query` |
| Kapitel | `22_Views_Schemata` |
| Sicherheit | `read-only` |
| Zweck | Markiert Views, die auf Basis von Metadaten als SCHEMABINDING-Kandidaten in Frage kommen koennten. |
<!-- SQLDOC:SUMMARY_TABLE:END -->

## Einordnung

`SCHEMABINDING` ist in Trainings- und Review-Szenarien besonders interessant, weil es Objektvertraege fuer Views schaerfer macht. Das Skript liefert deshalb keine automatische Aenderung, sondern eine konservative Audit-Sicht mit Ausschlussgruenden, Review-Prioritaet und optionalen `CREATE OR ALTER`-Templates.

## Annahmen

- Die Bewertung nutzt nur Metadaten der aktuellen Datenbank und den View-Text aus `sys.sql_modules`.
- Das Resultat ist eine Heuristik fuer Reviews und kein vollstaendiger Nachweis, dass eine View ohne weitere Anpassung mit `SCHEMABINDING` erstellt werden kann.
- Referenzen ohne explizites Schema, nicht aufloesbare Abhaengigkeiten und externe Objekte werden als Blocker oder Review-Faelle behandelt.
- Das optionale Template ersetzt nur den offensichtlichen Einstiegspunkt fuer `WITH SCHEMABINDING` und muss vor produktiver Nutzung manuell geprueft werden.

## Anwendungsfall

Das Skript eignet sich fuer Architektur-Reviews, Refactorings und Unterrichtssituationen, in denen sichtbar werden soll, welche Views bereits nahe an einem stabilen Objektvertrag liegen und wo noch Anpassungen an der Definition noetig sind.
...
|---|---|---|---|
| `@SchemaNameLike` | `SYSNAME` | Nein | Optionales LIKE-Muster, um nur Views passender Schemata zu bewerten. |
| `@OnlyCandidates` | `BIT` | Nein | Zeigt bei `1` nur Views mit Status `candidate`. |
| `@IncludeAlterTemplate` | `BIT` | Nein | Steuert, ob ein drittes Resultset mit Review-Templates ausgegeben wird. |
...
- `SELECT *`, externe Referenzen und nicht aufloesbare Abhaengigkeiten werden als harte Warnsignale behandelt.
- Fehlende Schemaqualifikation wird bewusst als `review-needed` eingestuft, weil die Metadaten hier nur einen starken Hinweis liefern.
- Views mit bereits gesetztem `SCHEMABINDING` bleiben in der Gesamtsicht sichtbar und koennen als Vergleichsobjekte fuer Konventionen dienen.
...
|---|---|---|---|
| `1.0` | `2026-04-22` | `ER` | Erstversion des SCHEMABINDING-Kandidaten-Audits fuer Views |
...
flowchart TD
    ...
    B --> C[Heuristiken im View-Text ableiten]
    C --> C1[SELECT Stern, UNION und TOP markieren]
    C1 --> D[Abhaengigkeiten aus sys.sql_expression_dependencies aggregieren]
    D --> D1[Nicht aufgeloeste, externe und caller-dependent Referenzen zaehlen]
    D1 --> D2[Fehlende Schemaqualifikation und Systemobjekte als Review-Signale markieren]
    D2 --> E[CandidateStatus, ReviewPriority und Aktion je View ableiten]
    E --> F{OnlyCandidates = 1?}
    F -->|Ja| G[Nur Status candidate im Detailresultset ausgeben]
    F -->|Nein| H[Alle geprueften Views im Detailresultset ausgeben]
    G --> I[Summary nach CandidateStatus aggregieren]
    H --> I
    I --> J{IncludeAlterTemplate = 1?}
    J -->|Ja| K[Review-Templates fuer CREATE OR ALTER VIEW WITH SCHEMABINDING ausgeben]
    J -->|Nein| L[Run ohne Template-Resultset beenden]
    K --> L
```
<!-- SQLDOC:MERMAID:END -->
