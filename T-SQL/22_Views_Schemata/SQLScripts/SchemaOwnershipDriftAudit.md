# SchemaOwnershipDriftAudit.sql

Dieses Skript prueft die Ownership-Verteilung auf Schemaebene und markiert Abweichungen zwischen Schema-Ownern und explizit gesetzten Objekt-Ownern. Die Analyse bleibt rein lesend und eignet sich fuer Reviews nach Deployment-Aenderungen, Berechtigungsanpassungen oder Ownership-Refactorings.
...
| Feld | Wert |
|---|---|
| Script | [SchemaOwnershipDriftAudit.sql](SchemaOwnershipDriftAudit.sql) |
| Version | `1.0` |
| Typ | `diagnostic-query` |
| Kapitel | `22_Views_Schemata` |
| Sicherheit | `read-only` |
| Zweck | Audit ueber Drift zwischen Schema-Ownern und expliziten Objekt-Ownern. |
<!-- SQLDOC:SUMMARY_TABLE:END -->

## Einordnung

Eigentuemerschaft ist in SQL Server oft unauffaellig, solange Objekte den Owner ihres Schemas erben. Sichtbare Drift entsteht vor allem dann, wenn einzelne Objekte einen expliziten Owner erhalten oder wenn sich Schema-Owner bewusst von `dbo` unterscheiden. Das Skript fuehrt diese Signale in einer gemeinsamen Review-Sicht zusammen.

## Annahmen

- Ownership-Drift wird als Review-Signal modelliert und nicht automatisch als Fehler oder Sicherheitsvorfall.
- Das Skript betrachtet explizite Objekt-Owner ueber `sys.objects.principal_id`; indirekte Effekte ueber Rollen, Berechtigungsketten oder Server-Logins werden nicht vollstaendig aufgeloest.
- Ein Schema-Owner ungleich `dbo` ist hier ein dokumentationspflichtiger Hinweis, aber kein pauschaler Mangel.
- Leere Schemas bleiben im Audit sichtbar, damit auch ungenutzte oder vorbereitete Schemata bewusst beurteilt werden koennen.

## Anwendungsfall

Das Skript eignet sich fuer Berechtigungsreviews, Ownership-Chaining-Checks und Refactorings, bei denen nachvollzogen werden soll, ob Objekte noch dem Ownership-Modell ihres Schemas folgen oder bereits abweichende Verantwortlichkeiten tragen.
...
| `@OnlyOwnershipDrift` | `BIT` | Nein | Zeigt bei `1` nur Schemas und Befunde mit Risiko `Medium` oder `High`. |
| `@IncludeRecommendations` | `BIT` | Nein | Steuert, ob ein drittes Resultset mit Review-Empfehlungen ausgegeben wird. |
...
- Das Detailresultset trennt Schema-Hinweise wie nicht-`dbo`-Owner von Objektbefunden wie fehlenden oder abweichenden expliziten Ownern.
- Die Summary aggregiert pro Schema Objektanzahl, explizite Owner, Mismatches und den hoechsten Risikowert.
- Das optionale dritte Resultset liefert nur Review-Hinweise und fuehrt keine Ownership-Aenderungen aus.
...
|---|---|---|---|
| `1.0` | `2026-04-22` | `ER` | Erstversion fuer das Audit ueber Ownership-Drift auf Schemaebene |
...
```mermaid
flowchart TD
    A[Parameter validieren und leere Filter bereinigen] --> B[Schema-Inventar aus sys.schemas und sys.database_principals laden]
    B --> C[Schemaobjekte mit expliziten Objekt-Ownern aus sys.objects anreichern]
    C --> D[Ownership-Signale je Objekt ableiten]
    D --> E[Schema- und Objektbefunde fuer nicht dbo Owner fehlende explizite Owner und Owner-Mismatches erzeugen]
    E --> F[Summary pro Schema mit Objektzahl Mismatches und hoechstem Risikowert aggregieren]
    F --> G{OnlyOwnershipDrift = 1?}
    G -->|Ja| H[Nur Medium und High in Detail- und Summary-Resultset zeigen]
    G -->|Nein| I[Alle Befunde und Schemas ausgeben]
    H --> J{IncludeRecommendations = 1?}
    I --> J
    J -->|Ja| K[Review-Empfehlungen pro Schema ausgeben]
    J -->|Nein| L[Run ohne Empfehlungen beenden]
    K --> L
```
<!-- SQLDOC:MERMAID:END -->
