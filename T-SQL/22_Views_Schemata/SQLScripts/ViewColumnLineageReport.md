# ViewColumnLineageReport.sql

Dieses Skript erstellt einen lesenden Bericht zur Spaltenherkunft in Views der aktuellen Datenbank. Der Fokus liegt auf einem didaktisch brauchbaren Lineage-Report, der View-Spalten, bekannte Abhaengigkeiten und nachvollziehbare Namensheuristiken kombiniert, ohne produktive Objekte zu veraendern.
...
| Feld | Wert |
|---|---|
| Script | [ViewColumnLineageReport.sql](ViewColumnLineageReport.sql) |
| Version | `1.0` |
| Typ | `diagnostic-query` |
| Kapitel | `22_Views_Schemata` |
| Sicherheit | `read-only` |
| Zweck | Bericht zur moeglichen Spaltenherkunft in Views fuer Dokumentation und Analyse. |
<!-- SQLDOC:SUMMARY_TABLE:END -->

## Einordnung

Lineage fuer View-Spalten ist in T-SQL-Metadaten nicht immer direkt und eindeutig sichtbar. Das Skript nutzt deshalb drei Stufen: ein Inventar aller View-Spalten, direkte Metadatenhinweise ueber bekannte Abhaengigkeiten und eine bewusst konservative Namensheuristik fuer Faelle, in denen nur moegliche Quellspalten abgeleitet werden koennen.

## Annahmen

- Der Bericht bleibt rein lesend und analysiert nur Metadaten der aktuellen Datenbank.
- Direkte Spaltenzuordnungen aus `sys.sql_expression_dependencies` haben Vorrang vor heuristischen Namensmatches.
- Gleichnamige Spalten in mehreren abhaengigen Objekten werden bewusst als mehrdeutig markiert.
- Ein einzelnes abhaengiges Objekt ohne klare Spaltenzuordnung wird nur als Review-Hinweis und nicht als belastbarer Beweis fuer echte Lineage behandelt.

## Anwendungsfall

Das Skript eignet sich fuer Dokumentation, Review-Sessions und Schulungsumgebungen, in denen nachvollzogen werden soll, aus welchen Basisobjekten und Spalten eine View ihre Ausgabespalten wahrscheinlich ableitet. Besonders nuetzlich ist es vor Refactorings, bei Schemaaenderungen oder beim Onboarding in gewachsene Datenbankschemata.
...
|---|---|---|---|
| `@SchemaNameLike` | `SYSNAME` | Nein | Optionales LIKE-Muster, um nur Views aus passenden Schemata auszuwerten. |
| `@OnlyResolvable` | `BIT` | Nein | Zeigt bei `1` nur View-Spalten mit mindestens einem gefundenen Lineage-Kandidaten. |
| `@IncludeSummary` | `BIT` | Nein | Steuert, ob ein zusaetzliches Summary-Resultset pro View ausgegeben wird. |
...
- Das erste Resultset zeigt das reine Spalteninventar der Views inklusive Datentyp-Metadaten.
- Das zweite Resultset listet moegliche Herkunftskandidaten je View-Spalte mit Vertrauensniveau und Review-Hinweis.
- Das Summary-Resultset verdichtet den Lineage-Abdeckungsgrad pro View und hebt Views mit manuellem Analysebedarf hervor.
...
|---|---|---|---|
| `1.0` | `2026-04-22` | `ER` | Erstversion des Berichts zur Spaltenherkunft in Views |
...
```mermaid
flowchart TD
    A[Parameter validieren] --> B[View-Inventar aus sys.views und sys.schemas laden]
    B --> C[View-Spalten und Datentypen aus sys.columns und sys.types erfassen]
    C --> D[Objekt- und Spaltenabhaengigkeiten aus sys.sql_expression_dependencies anreichern]
    D --> E[Moegliche Quellspalten pro abhaengigem Objekt vorbereiten]
    E --> F[Direkte Spaltenmatches aus Metadaten suchen]
    F --> G[Fallback ueber Namensheuristik und Single-Dependency-Pruefung bilden]
    G --> H[Lineage-Kandidaten nach Vertrauen und Review-Bedarf einstufen]
    H --> I[View-Spalteninventar ausgeben]
    I --> J{OnlyResolvable = 1?}
    J -->|Ja| K[Nur Spalten mit Lineage-Kandidaten im Detailreport zeigen]
    J -->|Nein| L[Alle View-Spalten inklusive unresolved zeigen]
    K --> M{IncludeSummary = 1?}
    L --> M
    M -->|Ja| N[Abdeckungsgrad und Review-Empfehlung pro View ausgeben]
    M -->|Nein| O[Run ohne Summary beenden]
    N --> O
```
<!-- SQLDOC:MERMAID:END -->
