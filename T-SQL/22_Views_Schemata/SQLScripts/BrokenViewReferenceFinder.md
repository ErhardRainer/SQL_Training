# BrokenViewReferenceFinder.sql

Dieses Skript prueft Views in der aktuellen Datenbank auf Warnsignale fuer gebrochene Objekt- oder Spaltenreferenzen. Der Fokus liegt auf einem lesenden Metadaten-Audit ueber `sys.sql_expression_dependencies`, damit Schema-Drift, nicht aufloesbare Abhaengigkeiten und pflegekritische `SELECT *`-Definitionen frueh sichtbar werden.
...
| Feld | Wert |
|---|---|
| Script | [BrokenViewReferenceFinder.sql](BrokenViewReferenceFinder.sql) |
| Version | `1.0` |
| Typ | `diagnostic-query` |
| Kapitel | `22_Views_Schemata` |
| Sicherheit | `read-only` |
| Zweck | Audit fuer Views mit potenziell gebrochenen Objekt- oder Spaltenreferenzen. |
<!-- SQLDOC:SUMMARY_TABLE:END -->

## Einordnung

Views kapseln Abfragen, haengen aber von stabilen Basistabellen, Spaltennamen und konsistenten Deployments ab. Das Skript kombiniert deshalb drei Perspektiven: ein Detailaudit pro Referenz, eine verdichtete Risikosicht pro View und ein optionales Refresh-Template fuer auffaellige Definitionen.

## Annahmen

- Das Skript bewertet nicht aufloesbare Metadaten als potenzielles Risiko und nicht automatisch als sicher bestaetigten Defekt.
- Die Analyse bleibt auf die aktuelle Datenbank beschraenkt; externe Datenbanken oder Server werden nur als Referenztyp markiert.
- `SELECT *` ohne `SCHEMABINDING` wird als Wartungsrisiko behandelt, weil verdeckte Schemaaenderungen dadurch spaeter sichtbar werden koennen.
- Das Resultset mit `sp_refreshview` ist ein Runbook-Startpunkt und fuehrt keine Aenderung selbst aus.

## Anwendungsfall

Das Skript eignet sich fuer Trainingsumgebungen, Datenbank-Reviews und Release-Checks vor Schemaaenderungen. Es hilft besonders dann, wenn Views laenger bestehen, mehrere Teams Basistabellen aendern oder Cross-Database-Referenzen in Deployments abgesichert werden muessen.
...
|---|---|---|---|
| `@SchemaNameLike` | `SYSNAME` | Nein | Optionales LIKE-Muster, um nur Views aus passenden Schemata zu pruefen. |
| `@OnlyPotentialBreaks` | `BIT` | Nein | Zeigt bei `1` nur Views oder Referenzen mit Risiko `High` oder `Medium`. |
| `@IncludeRefreshCommands` | `BIT` | Nein | Steuert, ob ein drittes Resultset mit `sp_refreshview`-Kommandos ausgegeben wird. |
...
- Das Detailresultset markiert nicht aufloesbare Referenzen, fehlende Spaltenauflosung sowie externe Abhaengigkeiten mit gestufter Risikobewertung.
- Die Summary-Sicht verdichtet diese Hinweise pro View und hebt problematische Definitionen zuerst hervor.
- Das Refresh-Template ist bewusst optional, damit lesende Audits ohne Folgeaktionen moeglich bleiben.
...
|---|---|---|---|
| `1.0` | `2026-04-22` | `ER` | Erstversion des View-Referenz-Audits |
...
```mermaid
flowchart TD
    A[Parameter validieren] --> B[Views und SQL-Definitionen aus sys.views und sys.sql_modules laden]
    B --> C[Abhaengigkeiten aus sys.sql_expression_dependencies mit sys.columns abgleichen]
    C --> D[Warnsignale fuer nicht aufloesbare Referenzen und fehlende Spalten markieren]
    D --> E[Zusatzcheck fuer SELECT Stern ohne SCHEMABINDING einfuegen]
    E --> F{OnlyPotentialBreaks = 1?}
    F -->|Ja| G[Nur High oder Medium im Detailresultset ausgeben]
    F -->|Nein| H[Alle Detailpruefungen ausgeben]
    G --> I[Risikosummary pro View aggregieren]
    H --> I
    I --> J[Summary-Resultset ausgeben]
    J --> K{IncludeRefreshCommands = 1?}
    K -->|Ja| L[sp_refreshview-Template fuer auffaellige Views ausgeben]
    K -->|Nein| M[Run ohne Refresh-Template beenden]
    L --> M
```
<!-- SQLDOC:MERMAID:END -->
