# ViewDependencyGraph.sql

Dieses Skript erstellt einen lesenden Abhaengigkeitsgraphen fuer Views der aktuellen Datenbank. Der Fokus liegt auf einer didaktisch gut nachvollziehbaren Darstellung von direkten und transitiven Abhaengigkeiten zwischen Views, Tabellen, Synonymen und nicht vollstaendig aufgeloesten Referenzen.
...
| Feld | Wert |
|---|---|
| Script | [ViewDependencyGraph.sql](ViewDependencyGraph.sql) |
| Version | `1.0` |
| Typ | `diagnostic-query` |
| Kapitel | `22_Views_Schemata` |
| Sicherheit | `read-only` |
| Zweck | Aufbau eines einfachen Abhaengigkeitsgraphen fuer Views und ihre Folgeobjekte. |
<!-- SQLDOC:SUMMARY_TABLE:END -->

## Einordnung

Views liegen oft nicht isoliert vor, sondern bilden Ketten aus weiteren Views, Tabellen, Synonymen oder externen Objekten. Das Skript kombiniert deshalb eine direkte Kantensicht mit einer rekursiven Pfadsicht, damit sowohl lokale Abhaengigkeiten als auch tiefere Verkettungen sichtbar werden.

## Annahmen

- Die Analyse bleibt rein lesend und nutzt nur Metadaten der aktuellen Datenbank.
- `sys.sql_expression_dependencies` bildet die Ausgangsbasis; fehlende oder externe Aufloesung wird als eigener Graph-Knoten dokumentiert.
- Die Rekursion verfolgt nur weitere Views transitiv, waehrend Tabellen, Synonyme und andere Objekttypen als Blattknoten im Graph verbleiben.
- Die maximale Tiefe ist bewusst begrenzt, damit Schulungs- und Review-Szenarien uebersichtlich bleiben.

## Anwendungsfall

Das Skript eignet sich fuer Architektur-Reviews, Dokumentation von View-Landschaften, Refactoring-Vorhaben und Schulungsumgebungen. Besonders hilfreich ist es vor Schemaaenderungen oder bei Deployments, wenn sichtbar werden soll, welche Views voneinander abhaengen und wo nicht aufgeloeste Referenzen eine manuelle Pruefung erfordern.
...
|---|---|---|---|
| `@RootViewLike` | `SYSNAME` | Nein | Optionales LIKE-Muster fuer den Startpunkt der View-Auswahl. |
| `@MaxDepth` | `INT` | Nein | Begrenzt die maximale Rekursionstiefe fuer transitive Abhaengigkeitspfade. |
...
- Das erste Resultset zeigt direkte Graph-Kanten von der Root-View zu ihren referenzierten Objekten.
- Das zweite Resultset verfolgt rekursiv weitere View-zu-View-Pfade und markiert moegliche Zyklen.
- Das optionale Summary-Resultset verdichtet Tiefe, Blattknoten und offene Risiken pro Root-View.
...
|---|---|---|---|
| `1.0` | `2026-04-22` | `ER` | Erstversion des View-Abhaengigkeitsgraphen |
...
flowchart TD
    ...
    B --> C[Direkte Abhaengigkeitskanten aus sys.sql_expression_dependencies aufbauen]
    C --> D[Referenztypen, Scope und Aufloesungsstatus mit sys.objects und sys.synonyms anreichern]
    D --> E[Rekursive Pfade nur ueber weitere Views bis MaxDepth verfolgen]
    E --> F[Zyklen ueber besuchte Objekt-IDs markieren]
    F --> G[Graph-Pfade und Blattknoten in einer Pfadtabelle materialisieren]
    G --> H[Summary mit Tiefe, Nested Views und unresolved Referenzen aggregieren]
    H --> I[Direkte Kanten ausgeben]
    I --> J[Transitive Pfade ausgeben]
