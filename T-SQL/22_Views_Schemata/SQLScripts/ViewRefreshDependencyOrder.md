# ViewRefreshDependencyOrder.sql

Dieses Skript berechnet eine nachvollziehbare Refresh-Reihenfolge fuer Views innerhalb der aktuellen Datenbank. Es eignet sich fuer Releases, Refactorings und Schemaaenderungen, bei denen zuerst tiefer liegende Basis-Views und danach darauf aufbauende Views aktualisiert werden sollen.
...
| Feld | Wert |
|---|---|
| Script | [ViewRefreshDependencyOrder.sql](ViewRefreshDependencyOrder.sql) |
| Version | `1.0` |
| Typ | `diagnostic-query` |
| Kapitel | `22_Views_Schemata` |
| Sicherheit | `read-only` |
| Zweck | Ermittelt eine sinnvolle Reihenfolge fuer View-Refreshes nach Aenderungen. |
<!-- SQLDOC:SUMMARY_TABLE:END -->

## Einordnung

Das Skript leitet aus Metadaten eine pragmatische Refresh-Stufung ab. Views ohne weitere View-Abhaengigkeiten erscheinen in fruehen Stufen, waehrend darauf aufbauende Views spaeter einsortiert werden. Zyklen, externe Referenzen und unaufgeloeste Metadateneintraege werden bewusst nicht automatisiert aufgeloest, sondern als Review-Hinweis markiert.

## Annahmen

- Die Analyse bleibt auf Metadaten der aktuellen Datenbank beschraenkt.
- Die berechnete Reihenfolge ist eine didaktische und operative Hilfe, fuehrt aber selbst keinen Refresh aus.
- Nur View-zu-View-Abhaengigkeiten beeinflussen die Refresh-Stufe direkt; andere Objektarten erscheinen als Hinweise.
- Cross-Database-, Cross-Server- und unaufgeloeste Referenzen gelten als Review-Blocker fuer eine vollautomatische Abarbeitung.

## Anwendungsfall

Das Skript hilft bei manuellen Deployment-Checks, bei der Planung von `sp_refreshview`-Runs und beim Review tiefer View-Ketten. Besonders nuetzlich ist es, wenn nach Aenderungen an Tabellen, Views oder Definitionen eine belastbare Reihenfolge fuer Refresh-Schritte benoetigt wird.
...
|---|---|---|---|
| `@SchemaNameLike` | `SYSNAME` | Nein | Optionales LIKE-Muster fuer das Schema der betrachteten Views. |
| `@RootViewLike` | `SYSNAME` | Nein | Optionales LIKE-Muster fuer einzelne Views oder vollqualifizierte View-Namen. |
| `@IncludeCommands` | `BIT` | Nein | 1 = zusaetzliches Resultset mit empfohlenen Refresh-Kommandos ausgeben. |
...
- `RefreshLevel = 1` bedeutet, dass fuer diese View keine weitere lokale View-Basis vorab refreshed werden muss.
- Views mit `Readiness = review` sind nicht zwingend unbenutzbar, brauchen aber vor einem operativen Refresh eine manuelle Pruefung.
- Das vierte Resultset liefert direkt ausfuehrbare Refresh-Kommandos in der berechneten Reihenfolge.
...
|---|---|---|---|
| `1.0` | `2026-04-22` | `ER` | Erstversion der diagnostischen Refresh-Reihenfolge fuer Views |
...
```mermaid
flowchart TD
    A[Parameter validieren] --> B[View-Inventar mit Refresh-Kommando aufbauen]
    B --> C[Referenzen aus sys.sql_expression_dependencies laden]
    C --> D[View-Kanten von sonstigen oder unaufgeloesten Referenzen trennen]
    D --> E[Rekursive View-zu-View-Pfade bis zur Basis verfolgen]
    E --> F[Besuchte View-IDs pruefen und Zyklen markieren]
    F --> G[Maximale Tiefe zur Basis je View berechnen]
    G --> H[Refresh-Stufe, Bereitschaft und Empfehlung materialisieren]
    H --> I[Zusammenfassung nach ready versus review aggregieren]
    I --> J[Refresh-Plan ausgeben]
    J --> K[Abhaengigkeiten und Summary ausgeben]
    K --> L{IncludeCommands = 1?}
    L -->|Ja| M[Refresh-Kommandos in Stufenreihenfolge ausgeben]
    L -->|Nein| N[Run ohne Kommandoliste beenden]
    M --> N
```
<!-- SQLDOC:MERMAID:END -->
