# ViewRefreshCandidateList.sql

Dieses Skript listet Views, deren Metadaten nach Aenderungen an Basisobjekten oder an der Aufloesung von Abhaengigkeiten geprueft und bei Bedarf refreshed werden sollten. Es bleibt rein lesend und erzeugt nur Review-Hinweise samt optionalen Refresh-Kommandos.
...
| Feld | Wert |
|---|---|
| Script | [ViewRefreshCandidateList.sql](ViewRefreshCandidateList.sql) |
| Version | `1.0` |
| Typ | `diagnostic-query` |
| Kapitel | `22_Views_Schemata` |
| Sicherheit | `read-only` |
| Zweck | Liste von Views, die nach Metadatenaenderungen refreshed werden sollten. |
<!-- SQLDOC:SUMMARY_TABLE:END -->

## Einordnung

Views koennen nach Schemaaenderungen, geaenderten Basisobjekten oder uneindeutigen Referenzen veraltete Metadaten mit sich tragen. Das Skript kombiniert deshalb mehrere Signale zu einer kompakten Kandidatenliste, ohne bereits automatisch `sp_refreshview` oder `sp_refreshsqlmodule` auszufuehren.

## Annahmen

- Die Analyse beschraenkt sich auf die aktuelle Datenbank und nutzt Metadaten aus den Systemkatalogen.
- Ein spaeteres `modify_date` eines referenzierten Objekts ist ein Refresh-Signal, aber kein harter Nachweis fuer eine fehlerhafte View.
- Bei schemagebundenen Views wird ein `sp_refreshsqlmodule`-Review vorgeschlagen, bei nicht schemagebundenen Views `sp_refreshview`.
- Externe oder caller-dependent Referenzen werden als vorsichtige Review-Signale behandelt, weil ihre Aufloesung nicht immer vollstaendig lokal sichtbar ist.

## Anwendungsfall

Das Skript eignet sich fuer Release-Checks, Schema-Reviews und Trainingsumgebungen, in denen sichtbar werden soll, welche Views nach Metadatenaenderungen bevorzugt erneut validiert oder refreshed werden sollten.
...
|---|---|---|---|
| `@SchemaNameLike` | `SYSNAME` | Nein | Optionales LIKE-Muster, um nur Views passender Schemata zu betrachten. |
| `@OnlyCandidates` | `BIT` | Nein | Zeigt bei `1` nur Views mit mindestens einem Refresh-Signal. |
| `@IncludeRefreshCommands` | `BIT` | Nein | Steuert, ob ein drittes Resultset mit empfohlenen Refresh-Kommandos ausgegeben wird. |
...
- Die Detailsicht priorisiert Views mit nicht aufgeloesten Referenzen oder fehlenden Spalten am hoechsten.
- `SELECT *` ohne `SCHEMABINDING` wird als mittleres Refresh-Signal modelliert, weil spaetere Spaltenaenderungen verdeckt bleiben koennen.
- Das dritte Resultset liefert nur Runbook-Kommandos fuer eine manuelle Pruefung im passenden Ausfuehrungskontext.
...
|---|---|---|---|
| `1.0` | `2026-04-22` | `ER` | Erstversion der Refresh-Kandidatenliste fuer Views |
...
flowchart TD
    ...
    C --> D[Refresh-Signale pro View ableiten]
    D --> D1[Nicht aufgeloeste Referenzen und fehlende Spalten markieren]
    D1 --> D2[Geaenderte Basisobjekte sowie externe und caller-dependent Referenzen bewerten]
    D2 --> D3[SELECT Stern ohne SCHEMABINDING als Zusatzsignal aufnehmen]
    D3 --> E[Empfohlenen Refresh-Befehl je View bestimmen]
    E --> F{OnlyCandidates = 1?}
    F -->|Ja| G[Nur Views mit Refresh-Signal im Detailresultset ausgeben]
    F -->|Nein| H[Alle geprueften Views im Detailresultset ausgeben]
    G --> I[Summary nach Signalstufe aggregieren]
    H --> I
    I --> J{IncludeRefreshCommands = 1?}
    J -->|Ja| K[Refresh-Kommandos fuer Kandidaten ausgeben]
    J -->|Nein| L[Run ohne Kommandoliste beenden]
    K --> L
```
<!-- SQLDOC:MERMAID:END -->
