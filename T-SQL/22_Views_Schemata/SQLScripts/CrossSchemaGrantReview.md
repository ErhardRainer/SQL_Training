# CrossSchemaGrantReview.sql

Dieses Skript prueft, welche Views, Prozeduren und Funktionen der aktuellen Datenbank schemauebergreifend auf andere Objekte verweisen und zugleich explizite Objektberechtigungen tragen. Ziel ist ein lesendes Review, das Schemagrenzen, Freigaben an Principals und moegliche Weitergabe von Rechten in einem gemeinsamen Blick sichtbar macht.
...
| Feld | Wert |
|---|---|
| Script | [CrossSchemaGrantReview.sql](CrossSchemaGrantReview.sql) |
| Version | `1.0` |
| Typ | `diagnostic-query` |
| Kapitel | `22_Views_Schemata` |
| Sicherheit | `read-only` |
| Zweck | Review ueber explizite Objektberechtigungen auf schemauebergreifende Objekte. |
<!-- SQLDOC:SUMMARY_TABLE:END -->

## Einordnung

Schema-Rechte und Ownership Chaining sind in SQL Server oft gewollt, koennen aber die sichtbare Freigabeebene von der realen Datenherkunft entkoppeln. Das Skript markiert deshalb Objekte, die ueber eine Schemagrenze hinweg auf andere Objekte zugreifen, und stellt daneben dar, ob dafuer explizite Objektberechtigungen an Datenbank-Principals vergeben wurden.

## Annahmen

- Das Skript betrachtet nur explizite Objektberechtigungen aus `sys.database_permissions`; Rechte ueber Rollenmitgliedschaften, Schema-Grants oder Ownership Chaining werden nicht vollstaendig aufgeloest.
- Eine schemauebergreifende Referenz wird hier als Review-Signal verstanden und nicht pauschal als Designfehler.
- Fehlt eine aufloesbare `referenced_id`, bleibt die Referenz dennoch als Metadatenhinweis im Audit sichtbar.
- `public`, `guest` und `GRANT WITH GRANT OPTION` werden bewusst strenger bewertet, weil sie Reichweite oder Delegierbarkeit vergroessern.

## Anwendungsfall

Das Skript eignet sich fuer Security-Reviews, Schema-Refactorings und Schulungsumgebungen, in denen Views oder Prozeduren Daten aus anderen Schemata kapseln. Es hilft besonders dann, wenn Berechtigungen knapp gehalten werden sollen und nachvollziehbar bleiben muss, ueber welche Objektgrants eine schemauebergreifende Abhaengigkeit indirekt erreichbar wird.
...
|---|---|---|---|
| `@PrincipalNameLike` | `SYSNAME` | Nein | Optionales LIKE-Muster fuer Datenbank-Principals. |
| `@SourceSchemaLike` | `SYSNAME` | Nein | Optionales LIKE-Muster fuer das Schema des freigegebenen Objekts. |
| `@TargetSchemaLike` | `SYSNAME` | Nein | Optionales LIKE-Muster fuer referenzierte Zielschemata. |
| `@OnlyWithExplicitGrant` | `BIT` | Nein | Zeigt bei `1` nur Objekte mit expliziter Objektberechtigung. |
...
- Das erste Resultset verbindet Objektgrants mit der Anzahl schemauebergreifender Referenzen und einer kompakten Risikoeinordnung.
- Das Summary-Resultset hilft bei der Priorisierung, wenn mehrere Objekte in unterschiedliche Zielschemata verzweigen.
- Das dritte Resultset zeigt die eigentlichen schemauebergreifenden Abhaengigkeiten und ist damit die technische Grundlage fuer das Berechtigungsreview.
...
|---|---|---|---|
| `1.0` | `2026-04-22` | `ER` | Erstversion fuer das Review schemauebergreifender Objektberechtigungen |
...
```mermaid
flowchart TD
    A[Parameter validieren und leere Filter bereinigen] --> B[Schemauebergreifende Abhaengigkeiten aus sys.objects sys.schemas und sys.sql_expression_dependencies laden]
    B --> C[Objekte mit Anzahl der Zielschemata und Referenzen verdichten]
    C --> D[Explizite Objektberechtigungen aus sys.database_permissions und sys.database_principals laden]
    D --> E[Review-Zeilen mit Risikohinweis fuer public guest Grant Option oder fehlende Objektgrants erzeugen]
    E --> F[Summary pro Objekt mit Anzahl Grants und hoechstem Review-Risiko aggregieren]
    F --> G[Detailresultset fuer Grants ausgeben]
    G --> H[Summary-Resultset pro Objekt ausgeben]
    H --> I[Abhaengigkeitsdetail pro referenziertem Zielschema ausgeben]
```
<!-- SQLDOC:MERMAID:END -->
