# ViewInvalidObjectProbe.sql

Dieses Skript prueft Views in der aktuellen Datenbank auf ungueltige, nicht aufloesbare oder potenziell veraltete Objekt- und Spaltenreferenzen. Der Fokus liegt auf einer rein lesenden Metadatenanalyse, damit defekte Views oder Drift-Risiken frueh sichtbar werden.
...
| Feld | Wert |
|---|---|
| Script | [ViewInvalidObjectProbe.sql](ViewInvalidObjectProbe.sql) |
| Version | `1.0` |
| Typ | `diagnostic-query` |
| Kapitel | `22_Views_Schemata` |
| Sicherheit | `read-only` |
| Zweck | Probe auf ungueltige, nicht aufloesbare oder veraltete View-Referenzen. |
<!-- SQLDOC:SUMMARY_TABLE:END -->

## Einordnung

Views bleiben nur stabil, wenn ihre Basistabellen, Spalten und externen Abhaengigkeiten mit der Definition uebereinstimmen. Das Skript kombiniert daher eine Detailsicht auf einzelne Referenzen mit einer verdichteten Risikosicht je View und optionalen `sp_refreshview`-Kommandos fuer den naechsten Wartungsschritt.

## Annahmen

- Die Analyse bewertet Metadatenluecken als Warnsignal und nicht automatisch als garantiert reproduzierten Laufzeitfehler.
- Externe Referenzen auf andere Datenbanken oder Server werden als Wartungsrisiko markiert, auch wenn sie technisch erreichbar sein koennen.
- Das Skript fuehrt keine Korrektur aus und erzeugt nur vorbereitende `sp_refreshview`-Kommandos.
- Die Pruefung bezieht sich auf die aktuell erreichbaren Metadaten der verbundenen Datenbank.

## Anwendungsfall

Die Probe eignet sich fuer Release-Checks, View-Reviews nach Schemaaenderungen und Trainingsumgebungen, in denen kaputte Referenzen gezielt sichtbar gemacht werden sollen. Besonders nuetzlich ist sie nach `ALTER TABLE`, Schema-Verschiebungen oder Refactorings an Basistabellen.
...
|---|---|---|---|
| `@SchemaNameLike` | `SYSNAME` | Nein | Optionales LIKE-Muster fuer das Schema der zu pruefenden Views. |
| `@ViewNameLike` | `SYSNAME` | Nein | Optionales LIKE-Muster fuer die zu pruefenden View-Namen. |
| `@OnlyProblems` | `BIT` | Nein | Zeigt bei `1` nur Referenzen und Views mit Warnsignal oder kritischem Fund. |
| `@IncludeRefreshCommands` | `BIT` | Nein | Steuert ein optionales Resultset mit `sp_refreshview`-Kommandos. |
...
- `MissingObject` markiert Referenzen, deren Zielobjekt in den Metadaten nicht mehr aufloesbar ist.
- `MissingColumn` zeigt Spalten, die in der View noch referenziert werden, aber in der aktuellen Objektstruktur fehlen.
- `UnresolvedName` deckt Faelle ab, in denen keine eindeutige Objektauflosung vorliegt oder gar keine Abhaengigkeiten geliefert werden.
- Externe Referenzen werden als Warnsignal fuer Deployments und Cross-Database-Abhaengigkeiten behandelt.
...
|---|---|---|---|
| `1.0` | `2026-04-22` | `ER` | Erstversion der Probe auf ungueltige View-Referenzen |
...
```mermaid
flowchart TD
    A[Parameter validieren und leere Filter auf NULL setzen] --> B[Views und Definitionen aus sys.views, sys.schemas und sys.sql_modules laden]
    B --> C[Abhaengigkeiten ueber sys.sql_expression_dependencies und sys.columns pruefen]
    C --> D[Referenzen als MissingObject, MissingColumn, UnresolvedName, ExternalReference oder Resolved klassifizieren]
    D --> E[Views ohne Dependency-Zeilen als metadata-gap ergaenzen]
    E --> F[Problemsummary pro View aggregieren]
    F --> G{OnlyProblems = 1?}
    G -->|Ja| H[Nur Warning und Critical im Detail- und Summary-Resultset ausgeben]
    G -->|Nein| I[Alle ermittelten Referenzen und Views ausgeben]
    H --> J{IncludeRefreshCommands = 1?}
    I --> J
    J -->|Ja| K[sp_refreshview-Kommandos fuer betroffene Views ausgeben]
    J -->|Nein| L[Ohne Refresh-Resultset beenden]
    K --> L
```
<!-- SQLDOC:MERMAID:END -->
