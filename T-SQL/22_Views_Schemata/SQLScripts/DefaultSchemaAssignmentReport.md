# DefaultSchemaAssignmentReport.sql

Dieses Skript analysiert Default-Schema-Zuordnungen von Datenbank-Principals und stellt ihnen typische Review-Heuristiken gegenueber: existiert das referenzierte Schema, gibt es ein gleichnamiges Schema, besitzt der Principal andere Schemata und wie stark wird das eingetragene Default-Schema bereits genutzt.
...
| Feld | Wert |
|---|---|
| Script | [DefaultSchemaAssignmentReport.sql](DefaultSchemaAssignmentReport.sql) |
| Version | `1.0` |
| Typ | `diagnostic-query` |
| Kapitel | `22_Views_Schemata` |
| Sicherheit | `read-only` |
| Zweck | Default-Schema-Zuordnungen von Principals und moegliche Abweichungssignale sichtbar machen. |
<!-- SQLDOC:SUMMARY_TABLE:END -->

## Einordnung

Default-Schemas werden in SQL Server oft stillschweigend mitgefuehrt, obwohl sie fuer Namensauflosung, Objektanlage und Rollenbilder relevant sind. Das Skript baut deshalb keine starre Regelpruefung, sondern eine didaktische Reviewsicht auf: gemeinsame `dbo`-Nutzung, gleichnamige Schemakandidaten und Besitz anderer Schemata werden als Signale sichtbar gemacht, ohne sie pauschal als Fehler zu bewerten.

## Annahmen

- Die Auswertung arbeitet nur auf Metadaten der aktuellen Datenbank und betrachtet keine serverweiten Logins.
- Ein gleichnamiges Schema zum Principal ist nur eine Heuristik fuer moegliche beabsichtigte Nutzung, kein harter Soll-Zustand.
- Besitz eines anderen Schemas als des Default-Schemas wird bewusst als Review-Signal markiert, weil dies auf abweichende Zustandsbilder oder historisch gewachsene Konfigurationen hindeuten kann.
- Rollen koennen optional einbezogen werden, sind aber standardmaessig ausgeblendet, damit die Sicht auf klassische Benutzer- und Gruppen-Principals fokussiert bleibt.

## Anwendungsfall

Das Skript eignet sich fuer Security- und Betriebsreviews, fuer Unterricht zu Schemata sowie fuer Refactorings, bei denen Benutzerobjekte aus `dbo` in fachliche Schemata ueberfuehrt werden sollen. Es ist besonders hilfreich, wenn nachvollziehbar sein muss, welche Principals ihre Namens- oder Besitzstruktur bereits widerspruchsfrei nutzen und wo eine gezielte Rueckfrage sinnvoll ist.
...
| `@DefaultSchemaLike` | `SYSNAME` | Nein | Optionales LIKE-Muster fuer Default-Schemanamen. |
| `@IncludeRoles` | `BIT` | Nein | Bei `1` werden auch Rollen in die Grundmenge aufgenommen. |
| `@OnlyDeviationSignals` | `BIT` | Nein | Bei `1` werden nur Principals oder Schemata mit Abweichungssignalen gezeigt. |
...
- `aligned` bedeutet hier nur, dass aus Default-Schema, Namensschema und Besitzverhaeltnissen kein direktes Warnsignal entsteht.
- `shared_default_schema` ist fuer `dbo` absichtlich kein Fehler, sondern ein Hinweis auf gemeinsame Objektanlage trotz vorhandenem Namensschema.
- Das Summary-Resultset hilft, stark gemeinsam genutzte Default-Schemata und Haefungen von Abweichungssignalen schnell zu erkennen.
...
|---|---|---|---|
| `1.0` | `2026-04-22` | `ER` | Erstversion des Reports fuer Default-Schema-Zuordnungen |
...
```mermaid
flowchart TD
    A[Parameter validieren und leere Filter bereinigen] --> B[Principal-Basis aus sys.database_principals aufbauen]
    B --> C[Existenz von Default-Schema und gleichnamigem Schema pruefen]
    C --> D[Besessene Schemata je Principal aus sys.schemas und sys.objects aggregieren]
    D --> E[Objektzahlen pro Schema verdichten]
    E --> F[Review-Signal pro Principal mit Besitz Namensschema und Default-Schema ableiten]
    F --> G[Summary je Default-Schema aggregieren]
    G --> H{OnlyDeviationSignals = 1?}
    H -->|Ja| I[Nur Abweichungssignale ausgeben]
    H -->|Nein| J[Komplette Reviewsicht ausgeben]
    I --> K[OwnedSchemaDetail fuer Rueckfragen ausgeben]
    J --> K
```
<!-- SQLDOC:MERMAID:END -->
