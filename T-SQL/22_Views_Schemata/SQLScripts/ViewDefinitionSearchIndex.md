# ViewDefinitionSearchIndex.sql

Dieses Skript baut einen einfachen Suchindex ueber die Definitionen aller Views in der aktuellen Datenbank. Der Fokus liegt auf einer rein lesenden Metadatenanalyse, damit haeufige SQL-Muster, auffaellige Begriffe und Review-Schwerpunkte je View schnell gefiltert und fuer Refactoring oder Architektur-Reviews genutzt werden koennen.
...
| Feld | Wert |
|---|---|
| Script | [ViewDefinitionSearchIndex.sql](ViewDefinitionSearchIndex.sql) |
| Version | `1.0` |
| Typ | `diagnostic-query` |
| Kapitel | `22_Views_Schemata` |
| Sicherheit | `read-only` |
| Zweck | Baut einen Suchindex ueber normalisierte View-Definitionen fuer Analyse- und Review-Zwecke. |
<!-- SQLDOC:SUMMARY_TABLE:END -->

## Einordnung

View-Definitionen enthalten oft wiederkehrende Muster wie `JOIN`, `GROUP BY`, Window Functions oder externe Zugriffe. Das Skript zerlegt diese Definitionen in suchbare Tokens, verdichtet deren Haeufigkeit je View und liefert dadurch einen pragmatischen Einstieg fuer Code-Reviews, Drift-Analysen und thematische Inventuren.

## Annahmen

- Die Tokenisierung arbeitet bewusst heuristisch ueber normalisierte Textfragmente und ersetzt keinen vollstaendigen SQL-Parser.
- Analysiert werden nur View-Definitionen der aktuellen Datenbank, die ueber `sys.views` und `sys.sql_modules` erreichbar sind.
- Der Suchindex ist fuer Diagnose und Unterricht gedacht; er fuehrt keine semantische Korrektheitspruefung der View-Logik durch.
- `SELECT *`, externe Zugriffsmuster und Isolation-Hinweise werden als Review-Signale markiert, nicht automatisch als Fehler.

## Anwendungsfall

Das Skript eignet sich fuer Architektur-Reviews, Lernumgebungen und technische Bestandsaufnahmen. Es hilft besonders dann, wenn ein Team wissen will, welche Views bestimmte SQL-Muster verwenden, wo moegliche Drift-Risiken liegen oder welche Definitionen fuer ein Refactoring zuerst betrachtet werden sollten.
...
|---|---|---|---|
| `@SchemaNameLike` | `SYSNAME` | Nein | Optionales LIKE-Muster, um nur Views aus passenden Schemata einzubeziehen. |
| `@SearchTermLike` | `NVARCHAR(200)` | Nein | Optionales LIKE-Muster fuer normalisierte Suchbegriffe im Index. |
| `@MinimumTokenLength` | `TINYINT` | Nein | Mindestlaenge eines Suchbegriffs; erlaubt sind Werte zwischen `2` und `30`. |
...
- Das Detailresultset liefert pro View Suchbegriffe, deren Haeufigkeit, einen Rang innerhalb der View und einen kompakten Review-Hinweis.
- Die Summary verdichtet Token-Anzahl, markante Top-Begriffe und typische Review-Signale wie `SELECT *` oder externe Zugriffsmuster.
- Die Definitionsvorschau bleibt absichtlich kurz, damit der Index als Navigationshilfe dient und nicht die eigentliche Code-Review ersetzt.
...
|---|---|---|---|
| `1.0` | `2026-04-22` | `ER` | Erstversion des Suchindex fuer View-Definitionen |
...
```mermaid
flowchart TD
    A[Parameter validieren und leere Filter bereinigen] --> B[Views und Definitionen aus sys.views sys.schemas und sys.sql_modules laden]
    B --> C[Definitionen normalisieren und Sonderzeichen in Leerzeichen ueberfuehren]
    C --> D[Tokens per STRING_SPLIT extrahieren und nach Mindestlaenge filtern]
    D --> E[Suchbegriff-Haeufigkeit je View aggregieren]
    E --> F[Pattern-Hinweise und Review-Fokus pro Suchbegriff ableiten]
    F --> G[Summary je View mit Top-Begriffen und Review-Prioritaet bilden]
    G --> H[Detailresultset des Suchindex ausgeben]
    H --> I[Summary-Resultset ausgeben]
    I --> J{IncludeDefinitionPreview = 1?}
    J -->|Ja| K[Gekuerzte Definitionen der betroffenen Views ausgeben]
