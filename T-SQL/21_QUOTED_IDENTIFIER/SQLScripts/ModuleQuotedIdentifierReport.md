# ModuleQuotedIdentifierReport.sql

Dieses Diagnose-Skript erstellt einen Report ueber `QUOTED_IDENTIFIER` je SQL-Modul. Die Umsetzung bleibt rein lesend, nutzt `sys.sql_modules` als technische Quelle und kombiniert eine Detailsicht pro Modul mit einer Schema-Summary und kompakten Review-Hinweisen.
...
| Feld | Wert |
|---|---|
| Script | [ModuleQuotedIdentifierReport.sql](ModuleQuotedIdentifierReport.sql) |
| Version | `1.0` |
| Typ | `diagnostic-query` |
| Kapitel | `21_QUOTED_IDENTIFIER` |
| Sicherheit | `read-only` |
| Zweck | Report ueber QUOTED_IDENTIFIER-Captures je Modul und je Schema. |
<!-- SQLDOC:SUMMARY_TABLE:END -->

## Einordnung

Im Kapitelkontext `QUOTED_IDENTIFIER` ist haeufig nicht nur interessant, ob einzelne Module problematisch sind, sondern auch, wie gleichmaessig eine Datenbank oder ein Schema mit diesem Capture-Wert arbeitet. Das Skript verdichtet deshalb den Metadatenbestand so, dass sowohl konkrete Nacharbeitskandidaten als auch gemischte Schemastrukturen sichtbar werden.

## Annahmen

- Die Kurzbeschreibung liefert keine produktive Zielarchitektur, daher arbeitet das Skript ausschliesslich mit Katalogmetadaten.
- Die erwartete Baseline ist standardmaessig `QUOTED_IDENTIFIER ON`, weil dies fuer viele moderne Deployment-Muster die naheliegende Ausgangslage ist.
- Verschluesselte Module bleiben optional im Report sichtbar, obwohl nur der Capture-Wert und nicht die komplette Definition ausgewertet werden kann.
- Das Guidance-Resultset erzeugt nur Header- und Review-Hinweise, aber keine automatische Neuerstellung von Modulen.

## Anwendungsfall

Das Skript eignet sich fuer Bestandsaufnahmen, Modernisierungsprojekte und technische Schulungen rund um Modul-Header und gespeicherte Session-Settings. Es ist besonders hilfreich, wenn zunaechst ein sauberer Ueberblick ueber `QUOTED_IDENTIFIER` pro Modul und pro Schema benoetigt wird.
...
| `@SchemaName` | `SYSNAME` | Nein | Grenzt den Report optional auf ein einzelnes Schema ein. |
| `@OnlyMismatches` | `BIT` | Nein | Zeigt bei `1` nur Module mit Abweichung gegen die erwartete Baseline. |
...
- Das erste Resultset priorisiert Module mit `QUOTED_IDENTIFIER`-Abweichung und haelt gleichzeitig hilfreiche Metadaten wie `ANSI_NULLS`, Schema-Bindung und Review-Kommandos bereit.
- Das zweite Resultset zeigt pro Schema, ob der Capture-Wert konsistent ausgerichtet ist, gemischt vorliegt oder einheitlich von der Erwartung abweicht.
- Das dritte Resultset verdichtet den erwarteten Header und einen Review-Batch fuer die manuelle Nacharbeit.
...
|---|---|---|---|
| `1.0` | `2026-04-22` | `ER` | Erstversion des Modul-Reports fuer QUOTED_IDENTIFIER |
...
flowchart TD
    ...
    B --> C[Schemafilter und Encrypted-Filter anwenden]
    C --> D[QUOTED_IDENTIFIER-Status und Review-Prioritaet je Modul ableiten]
    D --> E{OnlyMismatches = 1?}
    E -->|Ja| F[Nur abweichende Module im Detailreport ausgeben]
    E -->|Nein| G[Alle Module im Detailreport ausgeben]
    F --> H[QUOTED_IDENTIFIER-Verteilung je Schema aggregieren]
    G --> H
    H --> I[Schema-Status Aligned, Mixed oder UniformMismatch bestimmen]
    I --> J[Header-Guidance und Review-Batch ausgeben]
```
<!-- SQLDOC:MERMAID:END -->
