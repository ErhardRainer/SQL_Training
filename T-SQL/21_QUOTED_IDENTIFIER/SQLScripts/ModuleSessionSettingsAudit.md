# ModuleSessionSettingsAudit.sql

Dieses Diagnose-Skript auditiert die persistierten Session-Settings von SQL-Modulen ueber `sys.sql_modules`. Der Fokus liegt auf einer rein lesenden Bestandsaufnahme, die `ANSI_NULLS` und `QUOTED_IDENTIFIER` pro Modul sichtbar macht, typische Compile-Kontexte klassifiziert und pro Schema eine verdichtete Audit-Sicht erzeugt.
...
| Feld | Wert |
|---|---|
| Script | [ModuleSessionSettingsAudit.sql](ModuleSessionSettingsAudit.sql) |
| Version | `1.0` |
| Typ | `diagnostic-query` |
| Kapitel | `21_QUOTED_IDENTIFIER` |
| Sicherheit | `read-only` |
| Zweck | Audit ueber persistierte Modul-Session-Settings und moegliche Legacy-Capture-Kontexte. |
<!-- SQLDOC:SUMMARY_TABLE:END -->

## Einordnung

Im Kapitel `21_QUOTED_IDENTIFIER` ist nicht nur wichtig, welche Einstellung ein einzelnes Modul gespeichert hat. Ebenso relevant ist, ob ein Schema als Ganzes modern ausgerichtet ist oder noch aus Legacy-Batches mit alten Session-Settings stammt. Das Skript konzentriert sich deshalb auf die gespeicherten Capture-Werte im Katalog und leitet daraus eine nachvollziehbare Audit-Sicht ab.

## Annahmen

- Die Kurzbeschreibung liefert keinen produktiven Re-Deployment-Prozess, daher arbeitet die Erstversion ausschliesslich lesend auf Katalogmetadaten.
- Fuer SQL-Module persistiert SQL Server im Katalog vor allem `ANSI_NULLS` und `QUOTED_IDENTIFIER`; weitere Session-Settings werden hier nicht als gleichwertige Modul-Capture-Werte modelliert.
- Die Baseline fuer eine moderne Neuerstellung wird als `SET ANSI_NULLS ON;` plus `SET QUOTED_IDENTIFIER ON;` dargestellt.
- Das Skript generiert Review-Hinweise und `sp_helptext`-Kommandos, fuehrt aber keine automatische Neuerstellung oder Neukompilierung von Modulen aus.

## Anwendungsfall

Das Skript eignet sich fuer technische Bestandsaufnahmen, Migrationsprojekte und Trainingsumgebungen mit gewachsenen Modulbestaenden. Es ist besonders hilfreich, wenn vor einer Vereinheitlichung von Modul-Headern zunaechst sichtbar werden soll, welche Capture-Kontexte aktuell gespeichert sind.
...
| `@OnlySettingMismatches` | `BIT` | Nein | Zeigt bei `1` nur Module mit `QUOTED_IDENTIFIER OFF`, `ANSI_NULLS OFF` oder gemischten Capture-Kontexten. |
| `@IncludeEncryptedModules` | `BIT` | Nein | Bezieht bei `1` auch verschluesselte Module in die Audit-Sicht ein. |
...
- Das erste Resultset ordnet Module in `ModernBaseline`, `LegacyBatch` oder `MixedCapture` ein und priorisiert `QUOTED_IDENTIFIER OFF` als hoehere Audit-Stufe.
- Das zweite Resultset zeigt pro Schema, ob echte Legacy-Risiken vorliegen oder nur gemischte Capture-Kontexte auf eine uneinheitliche Neuerstellung hindeuten.
- Das dritte Resultset verdichtet eine empfohlene Header-Baseline und erzeugt einen Review-Batch fuer manuelle Nacharbeiten.
...
|---|---|---|---|
| `1.0` | `2026-04-22` | `ER` | Erstversion des Audits fuer persistierte Modul-Session-Settings |
...
```mermaid
flowchart TD
    A[Parameter validieren] --> B[Module und persistierte Capture-Werte aus sys.sql_modules laden]
    B --> C[Setting-Paar und Compile-Kontext je Modul ableiten]
    C --> D[Audit-Risiko, Hinweistext und Review-Kommando erzeugen]
    D --> E{OnlySettingMismatches = 1?}
    E -->|Ja| F[Nur auditrelevante Module im Detailreport ausgeben]
    E -->|Nein| G[Alle Module im Detailreport ausgeben]
    F --> H[Capture-Kontexte pro Schema aggregieren]
    G --> H
    H --> I[Schema-Status LegacyRisk, Mixed oder Aligned bestimmen]
    I --> J[Header-Baseline und Review-Batch fuer Nacharbeit ausgeben]
```
<!-- SQLDOC:MERMAID:END -->
