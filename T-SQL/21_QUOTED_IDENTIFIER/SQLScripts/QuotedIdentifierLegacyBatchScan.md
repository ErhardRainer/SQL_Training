# QuotedIdentifierLegacyBatchScan.sql

Dieses Diagnose-Skript durchsucht SQL-Module nach aelteren Batch-Mustern, die im Umfeld von `QUOTED_IDENTIFIER` haeufig Review-Bedarf erzeugen. Die Umsetzung bleibt rein lesend und kombiniert Katalogmetadaten mit klar dokumentierten Heuristiken fuer Session-Header, Legacy-Alter und doppelte Anfuehrungszeichen in Definitionsausschnitten.
...
| Feld | Wert |
|---|---|
| Script | [QuotedIdentifierLegacyBatchScan.sql](QuotedIdentifierLegacyBatchScan.sql) |
| Version | `1.0` |
| Typ | `diagnostic-query` |
| Kapitel | `21_QUOTED_IDENTIFIER` |
| Sicherheit | `read-only` |
| Zweck | Scan fuer aeltere Batch-Muster mit problematischem `QUOTED_IDENTIFIER`. |
<!-- SQLDOC:SUMMARY_TABLE:END -->

## Einordnung

Bei gewachsenen Datenbanken ist selten sofort sichtbar, welche Module ihre Session-Settings noch aus aelteren Deployment-Mustern geerbt haben. Der Scan konzentriert sich deshalb nicht nur auf den gespeicherten Capture-Wert aus `sys.sql_modules`, sondern auch auf textuelle Hinweise in der Moduldefinition, die bei `QUOTED_IDENTIFIER` typischerweise genauer geprueft werden sollten.

## Annahmen

- Das Skript bewertet doppelte Anfuehrungszeichen bewusst nur als Review-Signal, nicht als automatischen Syntaxfehler.
- Legacy wird in dieser Erstversion ueber den frei waehlbaren Parameter `@LegacyCutoffDate` modelliert.
- Verschluesselte Module bleiben sichtbar, koennen aber nur ueber Metadaten und nicht ueber die vollstaendige Definition beurteilt werden.
- Die Heuristiken ersetzen keinen vollstaendigen SQL-Parser, liefern aber eine didaktisch brauchbare Priorisierung fuer Header- und Deployment-Reviews.

## Anwendungsfall

Das Skript eignet sich fuer technische Bestandsaufnahmen, wenn Teams alte Module vor Refactorings, Index-Features oder Standardisierungsarbeiten nach Session-Headern und kritischen Batch-Mustern vorsortieren moechten. Es ist auch als Trainingsskript geeignet, um die Unterschiede zwischen gespeichertem `QUOTED_IDENTIFIER`-Capture und textuellen Batch-Hinweisen sichtbar zu machen.
...
|---|---|---|---|
| `@LegacyCutoffDate` | `DATE` | Nein | Definiert den Stichtag fuer legacy-nahe Module. |
| `@SchemaName` | `SYSNAME` | Nein | Grenzt den Scan optional auf ein einzelnes Schema ein. |
| `@OnlyFlaggedBatches` | `BIT` | Nein | Zeigt bei `1` nur Module mit mindestens einem Flag an. |
| `@PreviewLength` | `INT` | Nein | Steuert die Laenge des Definitionsauszugs im Detailreport. |
...
- Das erste Resultset priorisiert Module mit `QUOTED_IDENTIFIER OFF`, expliziten `SET QUOTED_IDENTIFIER OFF`-Headern und weiteren Legacy-Heuristiken.
- Das zweite Resultset fasst die Fundstellen nach Flag-Typ zusammen, damit Review-Wellen einfacher geplant werden koennen.
- Das dritte Resultset erzeugt eine kompakte Remediation-Warteschlange mit vorgeschlagenem Session-Header fuer die manuelle Nacharbeit.
...
|---|---|---|---|
| `1.0` | `2026-04-22` | `ER` | Erstversion des Legacy-Batch-Scans fuer QUOTED_IDENTIFIER |
...
```mermaid
flowchart TD
    A[Parameter validieren] --> B[Module und Metadaten aus sys.sql_modules, sys.objects und sys.schemas laden]
    B --> C[Definition fuer heuristische Suche normalisieren und Preview erzeugen]
    C --> D[Legacy-, Header- und Double-Quote-Flags je Modul ableiten]
    D --> E{OnlyFlaggedBatches = 1?}
    E -->|Ja| F[Nur Module mit mindestens einem Flag im Detailreport ausgeben]
    E -->|Nein| G[Alle gescannten Module im Detailreport ausgeben]
    F --> H[FlagSummary je Fundstellen-Typ aggregieren]
    G --> H
    H --> I[RemediationQueue nach Risiko, Flag-Anzahl und Alter aufbauen]
```
<!-- SQLDOC:MERMAID:END -->
