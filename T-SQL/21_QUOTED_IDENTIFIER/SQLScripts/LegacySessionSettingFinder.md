# LegacySessionSettingFinder.sql

Dieses Diagnose-Skript durchsucht gespeicherte SQL-Module nach Legacy-Session-Settings. Im Fokus stehen Module, die mit `QUOTED_IDENTIFIER OFF` oder `ANSI_NULLS OFF` abgelegt wurden und deshalb bei Reviews, Modernisierungen oder beim Einsatz indexnaher Features gesondert geprueft werden sollten.
...
| Feld | Wert |
|---|---|
| Script | [LegacySessionSettingFinder.sql](LegacySessionSettingFinder.sql) |
| Version | `1.0` |
| Typ | `diagnostic-query` |
| Kapitel | `21_QUOTED_IDENTIFIER` |
| Sicherheit | `read-only` |
| Zweck | Findet Altmodule mit problematischen Session-Settings. |
<!-- SQLDOC:SUMMARY_TABLE:END -->

## Einordnung

Das Skript bildet eine Bestandsaufnahme fuer vorhandene Prozeduren, Views, Funktionen und Trigger. Es veraendert keine Objekte, sondern liefert eine priorisierte Inventarliste mit kurzen Review-Hinweisen, damit problematische Altmodule kontrolliert nachgezogen werden koennen.

## Annahmen

- Die Kurzbeschreibung legt keinen produktiven Zielbestand fest, daher arbeitet das Skript rein lesend gegen Systemkataloge.
- Als problematisch gelten in dieser Erstversion vor allem `QUOTED_IDENTIFIER OFF` und `ANSI_NULLS OFF`, weil diese Einstellungen in Legacy-Modulen haeufig Review- und Kompatibilitaetsarbeit ausloesen.
- Die Ausgabe erzeugt bewusst nur Review-Kommandos und Header-Vorlagen statt automatischer `ALTER`-Statements, damit bestehende Definitionen vor einer Nachbearbeitung manuell geprueft werden.

## Anwendungsfall

Das Skript eignet sich fuer technische Schulungen, Migrations-Checks und Datenbank-Hygiene-Reviews. Typisch ist der Einsatz vor Modernisierungen, beim Vereinheitlichen von DDL-Standards oder vor Features, die saubere Session-Settings erwarten.
...
|---|---|---|---|
| `@SchemaName` | `SYSNAME` | Nein | Optionaler Schemaname fuer die Eingrenzung auf ein bestimmtes Schema. |
| `@OnlyProblematic` | `BIT` | Nein | `1` zeigt nur Module mit problematischen Settings; `0` liefert das gesamte Inventar. |
| `@IncludeMsShipped` | `BIT` | Nein | `1` bezieht systemnahe Objekte ein; `0` beschraenkt die Pruefung auf eigene Module. |
...
- Das erste Resultset fasst Umfang und Problemanteile des gescannten Modulbestands zusammen.
- Das zweite Resultset priorisiert problematische Module nach Risikostufe und altem Bearbeitungsstand.
- Das dritte Resultset verdichtet die Nachbearbeitung auf kurze Review-Kommandos und eine Session-Header-Vorlage fuer ein spaeteres `ALTER`.
...
|---|---|---|---|
| `1.0` | `2026-04-22` | `ER` | Erstversion des Diagnose-Skripts fuer Legacy-Session-Settings |
...
```mermaid
flowchart TD
    A[Parameter validieren] --> B[SQL-Module aus sys.objects und sys.sql_modules laden]
    B --> C[Schema- und MS-Shipped-Filter anwenden]
    C --> D[Risikostufe aus QUOTED_IDENTIFIER und ANSI_NULLS ableiten]
    D --> E[Review-Hinweise und Header-Vorlage erzeugen]
    E --> F[Summenresultat ausgeben]
    F --> G{OnlyProblematic = 1?}
    G -->|Ja| H[Nur problematische Module listen]
    G -->|Nein| I[Komplettes Inventar listen]
    H --> J[ReviewCommands fuer betroffene Module ausgeben]
    I --> J
```
<!-- SQLDOC:MERMAID:END -->
