# LegacyQuotedIdentifierModules.sql

Dieses Skript untersucht vorhandene SQL-Module im Kapitelkontext `QUOTED_IDENTIFIER` und markiert Kandidaten, die wegen ihres Alters oder wegen eines mit `OFF` gespeicherten Session-Captures bevorzugt geprueft werden sollten. Die Umsetzung bleibt rein lesend und erzeugt zusaetzlich eine kompakte Review-Vorlage fuer die spaetere Neuerstellung.
...
| Feld | Wert |
|---|---|
| Script | [LegacyQuotedIdentifierModules.sql](LegacyQuotedIdentifierModules.sql) |
| Version | `1.0` |
| Typ | `diagnostic-query` |
| Kapitel | `21_QUOTED_IDENTIFIER` |
| Sicherheit | `read-only` |
| Zweck | Findet aeltere Module mit problematischen `QUOTED_IDENTIFIER`-Settings. |
<!-- SQLDOC:SUMMARY_TABLE:END -->

## Einordnung

`QUOTED_IDENTIFIER` wird beim Erstellen oder Aendern eines Moduls in dessen Metadaten festgehalten. In gewachsenen Datenbanken lohnt es sich deshalb, aeltere Objekte gezielt zu sichten, statt nur die aktuelle Session zu pruefen. Das Skript verbindet Objektalter, die in `sys.sql_modules` gespeicherten Flags und eine priorisierte Handlungsempfehlung zu einer Audit-Sicht.

## Annahmen

- Legacy wird in dieser Erstversion didaktisch ueber den Parameter `@LegacyCutoffDate` modelliert.
- Module mit `uses_quoted_identifier = 0` gelten als primaere Review-Kandidaten, weil sie haeufig auf alte Deployment-Muster oder uneinheitliche Session-Optionen hinweisen.
- Verschluesselte Module koennen ohne zusaetzliche Source-of-Truth nicht vollstaendig verifiziert werden und werden deshalb gesondert markiert.
- Das Remediation-Template dient als Review-Checkliste und fuehrt selbst keine Neuerstellung aus.

## Anwendungsfall

Das Skript eignet sich fuer Bestandsaufnahmen in Trainings- oder Migrationsumgebungen, wenn ein Team vor Refactorings, Index-Features oder schemagebundenen Objekten zuerst das Modul-Portfolio nach problematischen Session-Captures sortieren moechte. Es hilft auch dabei, alte Deployments mit unbekanntem SET-Kontext systematisch nachzuarbeiten.
...
|---|---|---|---|
| `@LegacyCutoffDate` | `DATE` | Nein | Definiert den Stichtag, ab dem Module als legacy-nah gelten. |
| `@OnlyFlaggedModules` | `BIT` | Nein | Zeigt bei `1` nur Module mit Review-Bedarf an. |
| `@IncludeEncryptedModules` | `BIT` | Nein | Bezieht bei `1` auch verschluesselte Module in die Sicht ein. |
...
- Das Detailresultset priorisiert Module mit `QUOTED_IDENTIFIER OFF` vor rein altersbedingten Legacy-Kandidaten.
- Die Summary verdichtet Fundstellen nach Risikoklasse, damit Review-Wellen geplant werden koennen.
- `definition_preview` bleibt bei verschluesselten Modulen leer, damit klar bleibt, wo zusaetzliche Deployment-Quellen noetig sind.
...
|---|---|---|---|
| `1.0` | `2026-04-22` | `ER` | Erstversion des Legacy-Audits fuer Module mit QUOTED_IDENTIFIER-Fokus |
...
flowchart TD
    ...
    B --> C[Metadaten zu QUOTED_IDENTIFIER, ANSI_NULLS, Alter und Verschluesselung erfassen]
    C --> D[LegacyAssessment mit Review-Status, Risikoklasse und Handlungsempfehlung ableiten]
    D --> E{OnlyFlaggedModules = 1?}
    E -->|Ja| F[Nur ReviewNeeded-Module ausgeben]
    E -->|Nein| G[Alle inventarisierten Module ausgeben]
    F --> H[RiskSummary je Risikoklasse aggregieren]
    G --> H
    H --> I[Review- und Recreate-Template ausgeben]
```
<!-- SQLDOC:MERMAID:END -->
