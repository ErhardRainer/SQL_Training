# QuotedIdentifierSessionMismatch.sql

Dieses Diagnose-Skript vergleicht die Capture-Werte aus `sys.sql_modules` mit den aktuellen Session-Einstellungen der laufenden Verbindung. Der Fokus liegt darauf, sichtbar zu machen, welche Module mit einer anderen `QUOTED_IDENTIFIER`- oder `ANSI_NULLS`-Baseline erstellt wurden als die Session, in der gerade gearbeitet wird.
...
| Feld | Wert |
|---|---|
| Script | [QuotedIdentifierSessionMismatch.sql](QuotedIdentifierSessionMismatch.sql) |
| Version | `1.0` |
| Typ | `diagnostic-query` |
| Kapitel | `21_QUOTED_IDENTIFIER` |
| Sicherheit | `read-only` |
| Zweck | Vergleicht Modul-Metadaten mit den aktuellen Session-Einstellungen fuer `ANSI_NULLS` und `QUOTED_IDENTIFIER`. |
<!-- SQLDOC:SUMMARY_TABLE:END -->

## Einordnung

Im Kapitel `21_QUOTED_IDENTIFIER` ist nicht nur relevant, welche Werte ein Modul historisch gespeichert hat. Ebenso wichtig ist, ob die aktuelle Session, in der DDL oder Review-Schritte vorbereitet werden, dieselbe Baseline verwendet. Genau diese Luecke schliesst das Skript mit einem direkten Vergleich zwischen Session und Objektmetadaten.

## Annahmen

- Die Erstversion arbeitet rein lesend auf `sys.sql_modules`, `sys.objects` und `sys.schemas`.
- Als aktuelle Vergleichsbasis gelten die Rueckgaben von `SESSIONPROPERTY('ANSI_NULLS')` und `SESSIONPROPERTY('QUOTED_IDENTIFIER')`.
- Ein Mismatch ist didaktisch relevant, weil ein spaeteres `CREATE OR ALTER` unter anderer Session-Baseline neue Capture-Werte speichern kann.
- Verschluesselte Module koennen im Report enthalten sein, auch wenn ihre Definition nicht lesbar ist.

## Anwendungsfall

Das Skript eignet sich fuer Reviews vor Modul-Neuerstellungen, Migrationen und Trainingslabore zu Session-Settings. Es ist besonders hilfreich, wenn vor einem Recreate sichtbar werden soll, welche Module nicht mit der aktuellen Arbeits-Session ausgerichtet sind.
...
|---|---|---|---|
| `@SchemaName` | `SYSNAME` | Nein | Grenzt den Vergleich optional auf ein einzelnes Schema ein. |
| `@ModuleNamePattern` | `NVARCHAR(128)` | Nein | Wendet optional ein `LIKE`-Muster auf Modulnamen an. |
| `@OnlyMismatches` | `BIT` | Nein | Zeigt bei `1` nur Module mit Abweichung gegen die aktuelle Session. |
| `@IncludeEncryptedModules` | `BIT` | Nein | Bezieht bei `1` auch verschluesselte Module in den Report ein. |
...
- Das erste Resultset priorisiert Abweichungen bei `QUOTED_IDENTIFIER`, weil diese im Kapitelkontext meist die praktisch kritischeren DDL-Unterschiede markieren.
- Das zweite Resultset verdichtet die Lage pro Schema und trennt `QuotedRisk`, `AnsiRisk` und voll ausgerichtete Schemas.
- Das dritte Resultset zeigt die aktuelle Session-Baseline sowie einen Review-Batch fuer die betroffenen Module.
...
|---|---|---|---|
| `1.0` | `2026-04-22` | `ER` | Erstversion des Session-vs-Metadaten-Vergleichs fuer QUOTED_IDENTIFIER |
...
```mermaid
flowchart TD
    A[Parameter und SESSIONPROPERTY validieren] --> B[Module aus sys.sql_modules und sys.objects laden]
    B --> C[Session-Paar und Modul-Paar fuer ANSI_NULLS und QUOTED_IDENTIFIER ableiten]
    C --> D[Mismatch-Scope, Prioritaet und Review-Hinweise je Modul erzeugen]
    D --> E{OnlyMismatches = 1?}
    E -->|Ja| F[Nur abweichende Module im Detailreport ausgeben]
    E -->|Nein| G[Alle Module im Detailreport ausgeben]
    F --> H[Abweichungen pro Schema aggregieren]
    G --> H
    H --> I[Schema-Status QuotedRisk, AnsiRisk oder Aligned bestimmen]
    I --> J[Aktuellen Session-Header und Review-Batch fuer Nacharbeit ausgeben]
```
<!-- SQLDOC:MERMAID:END -->
