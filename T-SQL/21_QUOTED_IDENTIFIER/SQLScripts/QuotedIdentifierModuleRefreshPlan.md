# QuotedIdentifierModuleRefreshPlan.sql

Dieses Skript erstellt einen kontrollierbaren Refresh-Plan fuer SQL-Module im Kapitel `21_QUOTED_IDENTIFIER`. Statt sofort Recreate-Batches auszufuehren, priorisiert es betroffene Module, ordnet sie in Wellen ein und liefert je Objekt die notwendige Header-Baseline sowie Review-Hinweise.
...
| Feld | Wert |
|---|---|
| Script | [QuotedIdentifierModuleRefreshPlan.sql](QuotedIdentifierModuleRefreshPlan.sql) |
| Version | `1.0` |
| Typ | `diagnostic-query` |
| Kapitel | `21_QUOTED_IDENTIFIER` |
| Sicherheit | `read-only` |
| Zweck | Plant das kontrollierte Neu-Erstellen betroffener Module mit priorisierten Refresh-Wellen und Header-Baseline. |
<!-- SQLDOC:SUMMARY_TABLE:END -->

## Einordnung

Im Themenfeld `QUOTED_IDENTIFIER` reicht ein reiner Ist-Bericht oft nicht aus. Fuer Nacharbeiten ist ebenso wichtig, in welcher Reihenfolge Module angefasst werden sollten, welche Faelle zunaechst externe Definitionen brauchen und wo ein moderner Session-Header vor dem Recreate gesetzt werden muss. Dieses Skript liefert genau diese Planungsansicht.

## Annahmen

- Die Erstversion modelliert den Refresh bewusst als read-only Plan und fuehrt keine DDL aus.
- Eine moderne Ziel-Baseline wird standardmaessig als `SET ANSI_NULLS ON;` plus `SET QUOTED_IDENTIFIER ON;` angenommen.
- Verschluesselte Module werden nicht erraten, sondern als externe Beschaffungsfaelle markiert.
- Die Wellenplanung steuert nur die technische Reihenfolge fuer Reviews und Recreates, nicht fachliche Freigaben oder Rollback-Prozesse.

## Anwendungsfall

Das Skript eignet sich fuer Bestandsaufnahmen vor Bereinigungen von Modul-Headern, fuer Review-Sessions mit gewachsenen Deployments und fuer Trainingsumgebungen, in denen QUOTED_IDENTIFIER-bezogene Recreate-Risiken sauber portioniert werden sollen.
...
|---|---|---|---|
| `@SchemaName` | `SYSNAME` | Nein | Filtert den Plan optional auf ein einzelnes Schema. |
| `@OnlyActionable` | `BIT` | Nein | Zeigt bei `1` nur Module mit echtem Refresh- oder Review-Bedarf. |
| `@ExpectedAnsiNulls` | `BIT` | Nein | Zielwert fuer `SET ANSI_NULLS` im geplanten Refresh-Batch. |
| `@ExpectedQuotedIdentifier` | `BIT` | Nein | Zielwert fuer `SET QUOTED_IDENTIFIER` im geplanten Refresh-Batch. |
| `@MaxModulesPerWave` | `INT` | Nein | Legt die maximale Modulanzahl je geplanter Refresh-Welle fest. |
| `@IncludeEncryptedModules` | `BIT` | Nein | Bezieht bei `1` verschluesselte Module als externe Beschaffungsfaelle ein. |
...
- `refresh_action = RecreateWithModernHeader` markiert Module, deren persistierte Session-Settings nicht zur Ziel-Baseline passen.
- `refresh_action = ExternalDefinition` deckt verschluesselte Module ab, fuer die zuerst eine vertrauenswuerdige Quelle benoetigt wird.
- Die Wellen werden nach Prioritaet, Aenderungsdatum und Objektname aufgebaut, damit sensible Legacy-Faelle zuerst sichtbar werden.
- Das Batch-Skelett ist absichtlich knapp und verweist auf `sp_helptext` oder externe Quellen statt automatische DDL umzuschreiben.
...
|---|---|---|---|
| `1.0` | `2026-04-22` | `ER` | Erstversion des Refresh-Plans fuer betroffene Module mit QUOTED_IDENTIFIER-Bezug |
...
```mermaid
flowchart TD
    A[Parameter validieren] --> B[Module und persistierte Session-Settings aus sys.sql_modules laden]
    B --> C[Refresh-Aktion, Prioritaet und Header-Baseline je Modul ableiten]
    C --> D[Module per Row_Number in Refresh-Wellen einteilen]
    D --> E{OnlyActionable = 1?}
    E -->|Ja| F[Nur Module mit Refresh- oder Review-Bedarf im Detailplan zeigen]
    E -->|Nein| G[Auch bereits ausgerichtete Referenzfaelle zeigen]
    F --> H[Wellenzusammenfassung nach Aktion und Prioritaet bilden]
    G --> H
    H --> I[Execution-Checklist fuer Definition, Header, Tests und Deployment ausgeben]
```
<!-- SQLDOC:MERMAID:END -->
