---
task_id: MSSQL_TASK_initial_0201
repository: SQL_Training
domain: MSSQL
title: "Script MergeActionAudit.sql fuer 13_Merge erstellen"
status: done
priority: 800
priority_class: high
created_at: "2026-04-18T22:47:52Z"
updated_at: "2026-04-18T17:21:59Z"
...
source:
  type: repository
  reference: "T-SQL/Script.md#upcomming-scripts; queue: sql-0201"
  url: null
  author: null
  published_at: null

affected_paths:
  - path: "T-SQL/13_Merge/SQLScripts/MergeActionAudit.sql"
    change: "SQL-Skript erstellen oder pruefen"
  - path: "T-SQL/13_Merge/SQLScripts/MergeActionAudit.md"
...
# MSSQL_TASK_initial_0201 - Script MergeActionAudit.sql fuer 13_Merge erstellen

## Anlass und Relevanz

Protokolliert `INSERT`-, `UPDATE`- und `DELETE`-Aktionen eines `MERGE` ueber `OUTPUT`.
...
- T-SQL/13_Merge/SQLScripts/MergeActionAudit.sql
- T-SQL/13_Merge/SQLScripts/MergeActionAudit.md

## Umsetzungsauftrag

Erzeuge `MergeActionAudit.sql` und die zugehoerige Markdown-Dokumentation gemaess `_internal/SQL-Script/instruction_SQL_Script.md`. Verwende Kapitel `13_Merge` und diese fachliche Kurzbeschreibung als Rahmen: Protokolliert `INSERT`-, `UPDATE`- und `DELETE`-Aktionen eines `MERGE` ueber `OUTPUT`. Lege fehlende Ordner an, dokumentiere Annahmen neutral und halte YAML-Header, SQLDOC-Marker sowie Mermaid-Block ein.
