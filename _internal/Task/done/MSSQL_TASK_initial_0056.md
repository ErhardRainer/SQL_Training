---
task_id: MSSQL_TASK_initial_0056
repository: SQL_Training
domain: MSSQL
title: "Script OptionalFilterBranching.sql fuer 04_Where erstellen"
status: done
priority: 945
priority_class: high
created_at: "2026-04-18T22:47:52Z"
updated_at: "2026-04-17T08:22:38Z"
...
source:
  type: repository
  reference: "T-SQL/Script.md#upcomming-scripts; queue: sql-0056"
  url: null
  author: null
  published_at: null

affected_paths:
  - path: "T-SQL/04_Where/SQLScripts/OptionalFilterBranching.sql"
    change: "SQL-Skript erstellen oder pruefen"
  - path: "T-SQL/04_Where/SQLScripts/OptionalFilterBranching.md"
...
# MSSQL_TASK_initial_0056 - Script OptionalFilterBranching.sql fuer 04_Where erstellen

## Anlass und Relevanz

Vergleicht optionale Filter mit `OR`, `IF`-Branches und `UNION ALL`.
...
- T-SQL/04_Where/SQLScripts/OptionalFilterBranching.sql
- T-SQL/04_Where/SQLScripts/OptionalFilterBranching.md

## Umsetzungsauftrag

Erzeuge `OptionalFilterBranching.sql` und die zugehoerige Markdown-Dokumentation gemaess `_internal/SQL-Script/instruction_SQL_Script.md`. Verwende Kapitel `04_Where` und diese fachliche Kurzbeschreibung als Rahmen: Vergleicht optionale Filter mit `OR`, `IF`-Branches und `UNION ALL`. Lege fehlende Ordner an, dokumentiere Annahmen neutral und halte YAML-Header, SQLDOC-Marker sowie Mermaid-Block ein.
