---
task_id: MSSQL_TASK_initial_0696
repository: SQL_Training
domain: MSSQL
title: "Script CdcCleanupJobStatus.sql fuer 68_TemporalTables_CDC_CT erstellen"
status: open
priority: 305
...
source:
  type: repository
  reference: "T-SQL/Script.md#upcomming-scripts; queue: sql-0696"
  url: null
  author: null
  published_at: null

affected_paths:
  - path: "T-SQL/68_TemporalTables_CDC_CT/SQLScript/CdcCleanupJobStatus.sql"
    change: "SQL-Skript erstellen oder pruefen"
  - path: "T-SQL/68_TemporalTables_CDC_CT/SQLScript/CdcCleanupJobStatus.md"
...
# MSSQL_TASK_initial_0696 - Script CdcCleanupJobStatus.sql fuer 68_TemporalTables_CDC_CT erstellen

## Anlass und Relevanz

Zeigt den Zustand der CDC-Cleanup-Jobs und moegliche Rueckstaende.
...
- T-SQL/68_TemporalTables_CDC_CT/SQLScript/CdcCleanupJobStatus.sql
- T-SQL/68_TemporalTables_CDC_CT/SQLScript/CdcCleanupJobStatus.md

## Umsetzungsauftrag

Erzeuge `CdcCleanupJobStatus.sql` und die zugehoerige Markdown-Dokumentation gemaess `_internal/SQL-Script/instruction_SQL_Script.md`. Verwende Kapitel `68_TemporalTables_CDC_CT` und diese fachliche Kurzbeschreibung als Rahmen: Zeigt den Zustand der CDC-Cleanup-Jobs und moegliche Rueckstaende. Lege fehlende Ordner an, dokumentiere Annahmen neutral und halte YAML-Header, SQLDOC-Marker sowie Mermaid-Block ein.
