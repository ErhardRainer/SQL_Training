---
task_id: MSSQL_TASK_initial_0973
repository: SQL_Training
domain: MSSQL
title: "Script NativelyCompiledProcAudit.sql fuer 81_InMemory_OLTP erstellen"
status: open
priority: 28
priority_class: low
...
source:
  type: repository
  reference: "T-SQL/Script.md#upcomming-scripts; queue: sql-0973"
  url: null
  author: null
  published_at: null

affected_paths:
  - path: "T-SQL/81_InMemory_OLTP/SQLScripts/NativelyCompiledProcAudit.sql"
    change: "SQL-Skript erstellen oder pruefen"
  - path: "T-SQL/81_InMemory_OLTP/SQLScripts/NativelyCompiledProcAudit.md"
...
# MSSQL_TASK_initial_0973 - Script NativelyCompiledProcAudit.sql fuer 81_InMemory_OLTP erstellen

## Anlass und Relevanz

Audit fuer nativ kompilierte Prozeduren und deren Eigenschaften.
...
- T-SQL/81_InMemory_OLTP/SQLScripts/NativelyCompiledProcAudit.sql
- T-SQL/81_InMemory_OLTP/SQLScripts/NativelyCompiledProcAudit.md

## Umsetzungsauftrag

Erzeuge `NativelyCompiledProcAudit.sql` und die zugehoerige Markdown-Dokumentation gemaess `_internal/SQL-Script/instruction_SQL_Script.md`. Verwende Kapitel `81_InMemory_OLTP` und diese fachliche Kurzbeschreibung als Rahmen: Audit fuer nativ kompilierte Prozeduren und deren Eigenschaften. Lege fehlende Ordner an, dokumentiere Annahmen neutral und halte YAML-Header, SQLDOC-Marker sowie Mermaid-Block ein.
