---
task_id: MSSQL_TASK_initial_0681
repository: SQL_Training
domain: MSSQL
title: "Script ListenerPortReview.sql fuer 67_HA_DR erstellen"
status: open
priority: 320
...
source:
  type: repository
  reference: "T-SQL/Script.md#upcomming-scripts; queue: sql-0681"
  url: null
  author: null
  published_at: null

affected_paths:
  - path: "T-SQL/67_HA_DR/SQLScripts/ListenerPortReview.sql"
    change: "SQL-Skript erstellen oder pruefen"
  - path: "T-SQL/67_HA_DR/SQLScripts/ListenerPortReview.md"
...
# MSSQL_TASK_initial_0681 - Script ListenerPortReview.sql fuer 67_HA_DR erstellen

## Anlass und Relevanz

Review fuer Listener-Ports und deren Konfiguration.
...
- T-SQL/67_HA_DR/SQLScripts/ListenerPortReview.sql
- T-SQL/67_HA_DR/SQLScripts/ListenerPortReview.md

## Umsetzungsauftrag

Erzeuge `ListenerPortReview.sql` und die zugehoerige Markdown-Dokumentation gemaess `_internal/SQL-Script/instruction_SQL_Script.md`. Verwende Kapitel `67_HA_DR` und diese fachliche Kurzbeschreibung als Rahmen: Review fuer Listener-Ports und deren Konfiguration. Lege fehlende Ordner an, dokumentiere Annahmen neutral und halte YAML-Header, SQLDOC-Marker sowie Mermaid-Block ein.
