---
task_id: MSSQL_TASK_initial_0978
repository: SQL_Training
domain: MSSQL
title: "Script XtpFilegroupCapacityCheck.sql fuer 81_InMemory_OLTP erstellen"
status: open
priority: 23
priority_class: low
...
source:
  type: repository
  reference: "T-SQL/Script.md#upcomming-scripts; queue: sql-0978"
  url: null
  author: null
  published_at: null

affected_paths:
  - path: "T-SQL/81_InMemory_OLTP/SQLScripts/XtpFilegroupCapacityCheck.sql"
    change: "SQL-Skript erstellen oder pruefen"
  - path: "T-SQL/81_InMemory_OLTP/SQLScripts/XtpFilegroupCapacityCheck.md"
...
# MSSQL_TASK_initial_0978 - Script XtpFilegroupCapacityCheck.sql fuer 81_InMemory_OLTP erstellen

## Anlass und Relevanz

Prueft Kapazitaet und Reserven der XTP-Filegroup.
...
- T-SQL/81_InMemory_OLTP/SQLScripts/XtpFilegroupCapacityCheck.sql
- T-SQL/81_InMemory_OLTP/SQLScripts/XtpFilegroupCapacityCheck.md

## Umsetzungsauftrag

Erzeuge `XtpFilegroupCapacityCheck.sql` und die zugehoerige Markdown-Dokumentation gemaess `_internal/SQL-Script/instruction_SQL_Script.md`. Verwende Kapitel `81_InMemory_OLTP` und diese fachliche Kurzbeschreibung als Rahmen: Prueft Kapazitaet und Reserven der XTP-Filegroup. Lege fehlende Ordner an, dokumentiere Annahmen neutral und halte YAML-Header, SQLDOC-Marker sowie Mermaid-Block ein.
