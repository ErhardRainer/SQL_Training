---
task_id: MSSQL_TASK_initial_0643
repository: SQL_Training
domain: MSSQL
title: "Script PartitionFunctionDriftCheck.sql fuer 65_Partitioning erstellen"
status: open
priority: 358
...
source:
  type: repository
  reference: "T-SQL/Script.md#upcomming-scripts; queue: sql-0643"
  url: null
  author: null
  published_at: null

affected_paths:
  - path: "T-SQL/65_Partitioning/SQLScripts/PartitionFunctionDriftCheck.sql"
    change: "SQL-Skript erstellen oder pruefen"
  - path: "T-SQL/65_Partitioning/SQLScripts/PartitionFunctionDriftCheck.md"
...
# MSSQL_TASK_initial_0643 - Script PartitionFunctionDriftCheck.sql fuer 65_Partitioning erstellen

## Anlass und Relevanz

Check auf Drift zwischen erwartetem und realem Boundary-Set.
...
- T-SQL/65_Partitioning/SQLScripts/PartitionFunctionDriftCheck.sql
- T-SQL/65_Partitioning/SQLScripts/PartitionFunctionDriftCheck.md

## Umsetzungsauftrag

Erzeuge `PartitionFunctionDriftCheck.sql` und die zugehoerige Markdown-Dokumentation gemaess `_internal/SQL-Script/instruction_SQL_Script.md`. Verwende Kapitel `65_Partitioning` und diese fachliche Kurzbeschreibung als Rahmen: Check auf Drift zwischen erwartetem und realem Boundary-Set. Lege fehlende Ordner an, dokumentiere Annahmen neutral und halte YAML-Header, SQLDOC-Marker sowie Mermaid-Block ein.
